...
import { symbolPfad } from './symbole.js';
...
export function personAvatar(person, options = {}) {
  ...
  // Runde 9 (Jonathan): Der pulsierende grüne Ring „Meet ist aktiv" ist weg — an JEDEM Bild.
  // options.active wird noch angenommen (viele Aufrufer geben es mit), zeichnet aber nichts mehr.
  void active;
  const activeFade = '';
...
<div class="ui-sheet-card" data-scroll-keep="sheet-${esc(options.scrollKey || options.closeAct || 'sheet')}" style="position:relative;width:100%;max-height:86%;overflow-y:auto;overscroll-behavior:contain;padding:14px 20px 28px;border-radius:26px 26px 0 0;background:var(--surface);box-shadow:0 -10px 30px var(--shadow-15)">
...
<div style="display:flex;align-items:center;gap:8px;flex:none;min-width:0">${right}</div></div>`;
}

// --- Das Zeichen „neues Meet“ (Runde 10) ---------------------------------------------------
// Jonathan: Ein nacktes „+“ sagt nicht, WAS entsteht. Den runden Knopf gibt es an zwei Orten — in
// der FREE-Reihe des Crew-Tabs und unten rechts im Meet-Tab. Beide holen ihr Zeichen HIER: den
// Kalender des Symbolsatzes mit einem kleinen Plus im Blatt. Ein Ort, ein Zeichen, zwei Knoepfe.
export function meetPlusZeichen(farbe = 'var(--green-dark)', groesse = 20) {
  return `<svg width="${groesse}" height="${groesse}" viewBox="0 0 24 24" fill="none" style="flex:none;pointer-events:none">${symbolPfad('kalender', farbe)}<path d="M12 12.6v4.8M9.6 15h4.8" stroke="${farbe}" stroke-width="1.8" stroke-linecap="round"></path></svg>`;
}

// --- Hinweis-Pille (Runde 10) ----------------------------------------------------------------
// Ein STICHWORT, kein Satz: ein "!" davor, warmer Ton statt Rot-Alarm. Sie steht IN der Zeile
// (nie als Overlay) und bleibt immer einzeilig — reicht der Platz nicht, kuerzt der Text mit "...".
// Mit `act` ist sie ein Knopf, ohne `act` nur ein Zeichen. `klein` fuer Kopfzeilen und Leisten.
export function hinweisPille({ text, act = '', ton = 'warnung', klein = false }) {
  if (!text) return '';
  const warm = ton !== 'ruhig';
  const vorn = warm ? 'var(--orange-dark)' : 'var(--ink-soft)';
  const grund = warm ? 'var(--orange-tint)' : 'var(--field)';
  const rand = warm ? 'var(--orange-a28)' : 'var(--ink-a10)';
  const hoehe = klein ? 28 : 32;
  const ruf = klein ? 14 : 16;
  const zeichen = `<span aria-hidden="true" style="flex:none;width:${ruf}px;height:${ruf}px;border-radius:50%;background:${vorn};color:var(--surface);display:flex;align-items:center;justify-content:center;font:700 ${klein ? 10 : 11}px/1 'Instrument Sans',sans-serif;pointer-events:none">!</span>`;
  const wort = `<span style="pointer-events:none;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(text)}</span>`;
  const stil = `display:inline-flex;align-items:center;gap:${klein ? 5 : 7}px;max-width:100%;min-width:0;height:${hoehe}px;box-sizing:border-box;padding:0 ${klein ? 9 : 12}px;border-radius:999px;border:1px solid ${rand};background:${grund};color:${vorn};font:650 ${klein ? 12 : 13}px/1 'Instrument Sans',sans-serif;white-space:nowrap;overflow:hidden;text-align:left`;
  if (!act) return `<span data-role="hinweis-pille" style="${stil}">${zeichen}${wort}</span>`;
  return `<button data-act="${esc(act)}" data-role="hinweis-pille" data-treffer aria-label="${esc(text)}" style="${stil};cursor:pointer;appearance:none">${zeichen}${wort}</button>`;
}
