...
//
// Was man sieht: Das Bild wächst aus der angetippten Scheibe in die Mitte, darunter Name und eine
// ruhige Zeile (frei, im Meet, Mitgliederzahl). Echte Fotos stehen in voller Größe im <img> — der
...
//
// Schließen: Tipp irgendwohin · nach unten wischen (das Bild folgt dem Finger) · Zurück (Knopf oben
// links an der Stelle des Zurück-Pfeils der Seite, Escape, Zurück-Geste des Browsers bzw. Android).
//
...
export function bildGrossOeffnen(ctx, element, { art = 'person', id, zeichnen = null, knopf = null } = {}) {
  ...
  // Groß, aber mit Luft zum Rand; auf flachen Schirmen bleibt Platz für Name und Zeile darunter.
  const groesse = Math.round(Math.max(160, Math.min(breite - 64, 360, hoehe * 0.5)));
  ctx.ui.bildGross = { art: art === 'gruppe' ? 'gruppe' : 'person', id, von: scheibeLage(element), groesse, zeichnen, knopf, knopfLage: null };
  rueckmeldung('tipp');
  verlaufMerken();
  ctx.render();
}
...
function inhaltVon(ctx, zustand) {
  ...
  const meet = person.activeMeetId ? ctx.repo.getMeet?.(person.activeMeetId) : null;
  let unter = '';
  if (meet) {
    unter = `<span data-role="bild-gross-zustand" data-art="meet" style="font-size:13.5px;font-weight:650;color:var(--green-dark)">${esc(t('Im Meet · {titel}', { titel: meet.title }))}</span>`;
  } else if (person.free?.active) {
...
export function bildGrossHtml(ctx) {
  ...
  const knopf = zustand.knopf
    ? `<button data-act="${esc(zustand.knopf.act)}" data-role="bild-gross-knopf" style="pointer-events:auto;margin-top:6px;min-height:44px;padding:0 18px;border-radius:999px;border:1.5px solid var(--ink-a14);background:var(--surface);color:var(--ink);font:650 14px 'Instrument Sans',sans-serif;cursor:pointer;appearance:none;box-shadow:0 2px 8px var(--shadow-08)"><span style="pointer-events:none">${esc(zustand.knopf.text)}</span></button>`
    : '';
  return `<div data-role="bild-gross" data-art="${zustand.art}" data-sheet-kein-zug role="dialog" aria-modal="true" aria-label="${esc(inhalt.label)}" style="position:absolute;inset:0;z-index:40;display:flex;align-items:center;justify-content:center;touch-action:none;user-select:none;-webkit-user-select:none;-webkit-touch-callout:none;cursor:zoom-out;font-family:'Instrument Sans',sans-serif">
<div data-role="bild-gross-grund" aria-hidden="true" style="position:absolute;inset:0;background:var(--bild-gross-grund);-webkit-backdrop-filter:blur(20px) saturate(1.05);backdrop-filter:blur(20px) saturate(1.05)"></div>
...
<div data-role="bild-gross-buehne" style="position:relative;z-index:1;display:flex;flex-direction:column;align-items:center;gap:16px;max-width:calc(100% - 40px);margin-top:-36px;pointer-events:none">
<div data-role="bild-gross-scheibe" style="position:relative;width:${d}px;height:${d}px;border-radius:50%;overflow:hidden;flex:none;background:var(--field);box-shadow:0 0 0 1px var(--ink-a07),0 22px 54px var(--shadow-22);transform-origin:50% 50%">${inhalt.flaeche}</div>
<div data-role="bild-gross-text" style="display:flex;flex-direction:column;align-items:center;gap:5px;max-width:100%;text-align:center">
<span style="display:flex;align-items:center;gap:8px;max-width:100%;font-family:'Bricolage Grotesque',sans-serif;font-size:24px;font-weight:650;color:var(--ink);letter-spacing:-.01em"><span style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(inhalt.name)}</span>${inhalt.zusatz}</span>
${inhalt.unter}
${knopf}
</div>
</div>
...
export function bildGrossBinden(root, ctx) {
  ...
  if (typeof overlay.animate === 'function') {
    if (reduziert()) {
      ...
      teil('bild-gross-text')?.animate([{ opacity: 0, transform: 'translateY(10px)' }, { opacity: 1, transform: 'none' }], { duration: 260, delay: 90, easing: 'ease-out', fill: 'backwards' });
  ...
  overlay.__schliessen = (art = 'tipp') => {
    ...
    if (art === 'wisch') {
      ...
      teil('bild-gross-text')?.animate([{ opacity: 1 }, { opacity: 0 }], { duration: 120, fill: 'forwards' });
    }
  ...
  overlay.addEventListener('pointerdown', (event) => {
    if (!event.isPrimary || overlay.__schliesst || event.target.closest('[data-act]')) return;
