...
export function leererZustand() {
  return { m: '', a: '', e: '', g: PB_FARBEN[0].key, v: '', r: 'u', p: '', i: '' };
}
...
export function istSchlicht(z) {
  return Boolean(z) && !z.m && !z.e && !z.v && !z.p;
}
...
export function mitInitialen(wert, initialen) {
  ...
  if (!z || z.m || z.e || (!z.v && !z.p)) return wert;
...
export function profilbildSvg(z) {
  ...
  const inhalt = motiv
    ...
    : emoji
      ...
      : (z.i ? `<text x="48" y="48" dy=".355em" text-anchor="middle" font-family="${SYSTEMSCHRIFT}" font-weight="600" font-size="33" letter-spacing="-.3" fill="#fff">${xml(z.i)}</text>` : '');
...
export function profilbildAdresse(z) {
  const schluessel = `${profilbildToken(z)}|${z.i || ''}`;
