...
export async function fotoQuelleAusDatei(datei) {
  ...
  return quelleVorbereiten(URL.createObjectURL(datei));
}

// Runde 6 (P3, A2): Dasselbe für eine fertige Adresse — so liefert das Capacitor-Camera-Plugin
// in der nativen Hülle (dataUrl bzw. webPath) dieselbe Quelle wie ein Dateifeld.
export async function fotoQuelleAusAdresse(src) {
  if (!src) return null;
  return quelleVorbereiten(src);
}

// Nur selbst erzeugte blob:-Adressen dürfen zurückgegeben werden; eine data:-Adresse oder ein
// Pfad des Camera-Plugins gehört uns nicht.
const aufraeumen = (adresse) => { if (typeof adresse === 'string' && adresse.startsWith('blob:')) URL.revokeObjectURL(adresse); };

async function quelleVorbereiten(roh) {
  try {
...
// Runde 6 (P3, A2, Jonathan: „in der app war nur schwarz"). WebKit auf dem iPhone wirft keinen
// Fehler, wenn ihm für ein großes Foto der Bildspeicher ausgeht — drawImage() malt dann einfach
// nichts, und aus der Fläche würde ein schwarzes JPEG (leer + JPEG ohne Alpha = schwarz), das
// still als Profilbild gespeichert wird. Deshalb wird die fertige Fläche abgetastet: Ist an
// 25 Stellen ÜBERALL nichts angekommen (Alpha 0), gilt sie als leer und wir geben null zurück —
// der Aufrufer sagt es dann ehrlich, statt ein schwarzes Bild zu speichern.
// getImageData kann an einer fremden Quelle scheitern (tainted); dann gilt die Fläche als gut.
function flaecheLeer(g, kante) {
  try {
    const schritt = kante / 6;
    for (let i = 1; i <= 5; i += 1) {
      for (let j = 1; j <= 5; j += 1) {
        if (g.getImageData(Math.round(i * schritt), Math.round(j * schritt), 1, 1).data[3] !== 0) return false;
      }
    }
    return true;
  } catch {
...
// z. B. ein fremdes Bild ohne Freigabe oder eine leer gebliebene Fläche; dann bleibt das bisherige Foto.
export function zuschnittRendern(zs, kante = ZUSCHNITT_KANTE) {
  ...
  try {
    ...
    if (flaecheLeer(g, kante)) {
      zuschnittRendern.letzte = { leer: true, at: Date.now() };
      return null;
    }
    zuschnittRendern.letzte = { leer: false, at: Date.now() };
