...
//   · Routen rechnet web/ui/route.js: mit OpenRouteService-Schlüssel echte Straßen, sonst ehrlich
//     geschätzt (gestrichelt, und es steht „geschätzt" daneben).
...
import {
  UMWEG_FAKTOR, KMH, routeBerechnen, routeSchaetzen, routeGemerkt, routeMitSchluessel, googleMapsUrl,
} from './route.js';
...
function namenListe(repo, ids) {
  const namen = ids.map((id) => nameVon(repo, id));
...
export function anreisePlan(repo, meet, optionen = {}) {
  ...
  const mitLage = mitfahrer.map((id) => ({ id, lage: lageVon(repo, id) })).filter((stopp) => stopp.lage);
...
function wahlAnsicht(ctx, meet, stand) {
  const repo = ctx.repo;
  const andereFahrer = stand.fahrer.filter((auto) => auto.id !== ME);
  const suchende = stand.suchen.filter((id) => id !== ME);
  const zeilen = [];
...
function kartenFlaeche(meet, plan) {
  const punkte = [plan.start, plan.ziel, ...plan.kandidaten.map((k) => k.lage), ...plan.stopps.map((s) => s.lage)].filter(hatLage);
...
function routeKarte(ctx, meet, plan) {
  const repo = ctx.repo;
  ...
  plan.stopps.forEach((stopp, nr) => {
    ...
    zeilen.push(zeile(`<span style="position:relative;display:flex;flex:none">${gesicht(repo, ctx.repo.getSettings(), stopp.id, 28)}<span aria-hidden="true" style="position:absolute;right:-5px;top:-5px;min-width:17px;height:17px;border-radius:9px;background:var(--green);color:var(--on-accent);font:700 10px/17px ${FONT};text-align:center;box-shadow:0 0 0 2px var(--surface)">${nr + 1}</span></span><span style="flex:1;min-width:0;font-size:14px;font-weight:650;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(name)}</span>`, ` data-role="anreise-stopp" data-art="abholung" data-person="${esc(stopp.id)}"`, index, uhrzeit(index), knopf));
    index += 1;
  });
  if (plan.ziel) {
...
function fahrerAnsicht(ctx, meet, plan) {
  return `${kartenFlaeche(meet, plan)}
${routeKarte(ctx, meet, plan)}
...
// Wann holt mich mein Fahrer ab? Dieselbe Rechnung wie auf seiner Seite — nur von außen.
...
function mitfahrerAnsicht(ctx, meet, stand) {
  const repo = ctx.repo;
...
${gesicht(repo, settings, auto.id, 34)}
<span style="flex:1;min-width:0;display:flex;flex-direction:column;gap:1px"><span style="font-size:14px;font-weight:650;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(t('{name} fährt', { name }))}</span><span style="font-size:12px;color:var(--muted)">${esc(unterZeile)}</span></span>
${rechts}
</div>`;
  }).join('');
...
export function renderAnreise(ctx) {
  ...
  } else if (modus === 'fahrer') inhalt = fahrerAnsicht(ctx, meet, plan);
  else if (modus === 'mitfahrer') inhalt = mitfahrerAnsicht(ctx, meet, stand);
  else inhalt = wahlAnsicht(ctx, meet, stand);
  ...
  return { html, bind: (root) => bindActions(root, bindAnreise(root, ctx, meet, { modus, plan, stand })) };
}
...
function linieSetzen(karte, route) {
  if (!karte || !route) return;
  const daten = { type: 'Feature', properties: {}, geometry: { type: 'LineString', coordinates: route.linie || [] } };
  const zeichnen = () => {
    const quelle = karte.getSource?.(LINIE_QUELLE);
    if (quelle) quelle.setData(daten);
    else {
      karte.addSource(LINIE_QUELLE, { type: 'geojson', data: daten });
      karte.addLayer({
        id: LINIE_EBENE,
        type: 'line',
        source: LINIE_QUELLE,
        layout: { 'line-cap': 'round', 'line-join': 'round' },
        paint: { 'line-color': farbeAus('--green', '#1B9E5F'), 'line-width': 4, 'line-opacity': 0.9 },
      });
    }
    // Geschätzt ist gestrichelt — eine gerade Linie ist keine Straße und soll auch nicht so aussehen.
    karte.setPaintProperty(LINIE_EBENE, 'line-dasharray', route.geschaetzt ? [1.2, 1.6] : [1, 0]);
  };
  try {
    if (karte.isStyleLoaded?.()) zeichnen();
    else karte.once('load', () => { try { zeichnen(); } catch { /* Stil fehlt: dann ohne Linie */ } });
  } catch { /* ohne Linie bleibt die Karte bedienbar */ }
}

async function karteBinden(root, ctx, meet, plan) {
  const knoten = root.querySelector('[data-role="anreise-karte"]');
  if (!knoten) return;
  const repo = ctx.repo;
  const settings = repo.getSettings();
  const schluessel = `anreise-${meet.id}`;
  const alle = [plan.start, plan.ziel, ...plan.kandidaten.map((k) => k.lage), ...plan.stopps.map((s) => s.lage)].filter(hatLage);
  const mitte = plan.ziel || plan.start || alle[0];
  ...
  if (plan.ziel) {
    setzen('ziel', plan.ziel, `<span data-role="anreise-ziel-marke" style="display:flex;border-radius:10px;${ring('var(--surface)')}">${meetKachel(meet, 32)}</span>`, { 'data-role': 'anreise-ziel' }, 3);
  }
  if (plan.start) {
  ...
  plan.stopps.forEach((stopp, nr) => {
    setzen(`p:${stopp.id}`, stopp.lage, `<span style="position:relative;display:flex;border-radius:50%;${ring('var(--green)')}">${gesicht(repo, settings, stopp.id, 36)}<span style="position:absolute;right:-7px;top:-7px;min-width:19px;height:19px;border-radius:10px;background:var(--green);color:var(--on-accent);font:700 11px/19px ${FONT};text-align:center;box-shadow:0 0 0 2px var(--surface)">${nr + 1}</span></span>`, {
  ...
  plan.kandidaten.filter((k) => k.lage).forEach((kandidat) => {
  ...
  if (plan.route && plan.ziel) linieSetzen(karte, plan.route);
  ...
  if (knoten.__eingepasst !== bild && (!knoten.__eingepasst || !karte.__anreiseVonHand)) {
    ...
    passeAufPunkte(karte, alle, { padding: 46, maxZoom: 14.5 });
  }
}

// --- Handler der Seite ----------------------------------------------------------------------------

function bindAnreise(root, ctx, meet, { modus, plan, stand }) {
  const { repo, ui } = ctx;
  ...
  // Eine Route mit echtem Straßenverlauf kommt nach — dann wird einmal neu gezeichnet.
  if (plan?.punkte?.length >= 2 && routeMitSchluessel() && !routeGemerkt(plan.punkte)) {
    const punkte = plan.punkte;
    routeBerechnen(punkte).then((route) => { if (route && !route.geschaetzt && root.isConnected) ctx.render(); }).catch(() => {});
  }
  if (modus === 'mitfahrer' && stand.meine.fahrerId && routeMitSchluessel()) {
    const abholung = abholZeit(repo, meet, stand);
    if (abholung && !routeGemerkt(abholung.punkte)) {
      routeBerechnen(abholung.punkte).then((route) => { if (route && !route.geschaetzt && root.isConnected) ctx.render(); }).catch(() => {});
    }
  }
  if (modus === 'fahrer' && plan) karteBinden(root, ctx, meet, plan).catch(() => {});
  ...
  return {
    ...
    'anreise-standort': (_daten, _knopf, ereignis) => {
      ...
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ui.anreiseStart = { lat: position.coords.latitude, lon: position.coords.longitude };
          rueckmeldung('erfolg');
          ctx.render();
        },
