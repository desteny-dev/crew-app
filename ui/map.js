...
export function spalteRechnen(breite, hoehe, hindernisse = [], mitRegler = true) {
  ...
  const untenGrenze = info.o - S.abstand;
  const platz = untenGrenze - obenGrenze;
  const hoeheRegler = Math.min(S.regler, platz);
  if (hoeheRegler < S.reglerMin) return { info, regler: null };
  ...
  return { info, regler: { l, r, o, u: o + Math.round(hoeheRegler) } };
}
...
function ichStellen(karte) {
  ...
  // R8-32: Die Genauigkeit steht als klarer Kreis darunter — pulsierend, solange die Lage frisch
  // ist (dann ist sie „live"), still, wenn sie alt ist. Unter 6 m und ohne Angabe: kein Kreis.
  const r = genauKreis(karte, 'ich', { lat: lageStand.lat, lon: lageStand.lon, meter: lageStand.genauigkeitM }, { puls: lageStand.genau, unterPx: ICH_PUNKT / 2 });
  const meter = r ? String(Math.round(lageStand.genauigkeitM)) : '';
  if ((el.dataset.genauM || '') !== meter) { if (meter) el.dataset.genauM = meter; else delete el.dataset.genauM; }
}

// --- Genauigkeit: EIN klarer Kreis für jede Lage (Runde 8, R8-32) ----------------------------
// Jonathan: „Genauigkeit: ein klarer, durchsichtig grüner Kreis und darin Ringe, die pulsierend von
// innen nach außen laufen und verblassen, im Takt neu; der größte Ring ist die Genauigkeit. Unter 6 m
// kein Kreis." Bis Runde 7 standen hier zwei verschiedene Dinge: um meinen Punkt ein FESTER 40-px-Hof,
// der mit der Genauigkeit nichts zu tun hatte, und um Gesichter ein weicher Hof ohne Kante. Jetzt gilt
// EINE Form auf jeder Karte:
//   · ein klarer Kreis mit Rand, Halbmesser = die gemeldete Genauigkeit in Metern, auf den Maßstab
//     gerechnet — er wächst und schrumpft beim Zoomen wie die Wirklichkeit;
//   · darin laufen drei Ringe versetzt von der Mitte nach außen und verblassen; der größte ist der
//     Kreis selbst. Pulsieren heißt „das ist live": Es läuft an MEINER Lage, solange sie frisch ist.
//     Die Lage eines Freundes ist eine Momentaufnahme („vor 12 Min") — ein pulsierender Kreis täte
//     dort so, als wäre sie live. Ihr Kreis steht still.
//   · unter 6 m kein Kreis (der Punkt IST dann die Antwort); ohne gemeldete Zahl auch keiner —
//     erfunden wird nichts (projections.js › genauigkeitVon).
// Alle Kreise liegen auf EINER Ebene unter allen Markierungen: Ein großer Kreis legt sich nie über ein
// Gesicht oder ein Meet. Ein Kreis, größer als das ganze Bild, pulsiert nicht (man sähe keinen Ring,
// und das Gerät zahlte für eine riesige bewegte Fläche) und zeigt keinen Rand, den es nicht gibt.
export const GENAU_MIN_M = 6;
const GENAU_PULS_MAX_PX = 260;
// Durchmesser meines Punktes (.crew-ich-punkt) — darunter liegt ein kleinerer Kreis unsichtbar.
const ICH_PUNKT = 18;

// Meter je Pixel: EINE Rechnung für die ganze App (core/entfernung.js). MapLibre zeichnet 512-px-Kacheln
// — bei Zoom z ist die Welt 512 · 2^z Pixel breit. Bis Runde 7 stand hier die Formel für 256er-Kacheln:
// Gemessen (unabhängig, mit karte.project) waren 30 m bei Zoom 16 wirklich 37,2 px, gezeichnet 18,6 px —
// jeder Kreis war halb so groß, wie die Genauigkeit ist.
export function meterJePixel(lat, zoom) {
  return meterProPixel(Number(lat), Number(zoom));
}

function genauEbene(karte) {
  const basis = kartenEbene(karte);
  if (!basis) return null;
  const da = karte.__crewGenauEbene;
  if (da && da.parentNode === basis) return da;
  const ebene = document.createElement('div');
  ebene.className = 'crew-genau-ebene';
  ebene.dataset.role = 'genau-ebene';
  basis.insertBefore(ebene, basis.firstChild);
  karte.__crewGenauEbene = ebene;
  return ebene;
}

// Stellt den Kreis zu `schluessel` — oder nimmt ihn weg: ohne Lage, unter 6 m, oder solange er ganz
// unter dem Bild läge (`unterPx` = Halbmesser des Bildes bzw. Punktes — ein Kreis, den niemand sieht,
// sagt nichts). Liefert den Halbmesser in Pixeln (0 = kein Kreis).
function genauKreis(karte, schluessel, lage, { puls = false, unterPx = 0 } = {}) {
  const alle = karte.__crewGenau || (karte.__crewGenau = new Map());
  let el = alle.get(schluessel);
  const meter = Number(lage?.meter) || 0;
  const lat = Number(lage?.lat);
  const lon = Number(lage?.lon);
  let r = 0;
  let weit = false;
  if (lage && meter >= GENAU_MIN_M && Number.isFinite(lat) && Number.isFinite(lon)) {
    const roh = meter / meterJePixel(lat, karte.getZoom());
    const c = karte.getContainer?.();
    const deckel = Math.max(240, Math.hypot(c?.clientWidth || 0, c?.clientHeight || 0));
    weit = roh > deckel;
    r = Math.round(Math.min(roh, deckel));
  }
  const ebene = r >= 2 && r > unterPx ? genauEbene(karte) : null;
  if (!ebene) {
    if (el) { el.remove(); alle.delete(schluessel); }
    return 0;
  }
  if (!el || el.parentNode !== ebene) {
    el?.remove();
    el = document.createElement('div');
    el.className = 'crew-genau';
    el.dataset.fuer = String(schluessel);
    el.setAttribute('aria-hidden', 'true');
    ebene.appendChild(el);
    alle.set(schluessel, el);
  }
  const pulsiert = Boolean(puls) && !weit && r <= GENAU_PULS_MAX_PX && !bewegungGedrosselt();
  if ((el.dataset.puls === '1') !== pulsiert) {
    if (pulsiert) { el.dataset.puls = '1'; el.innerHTML = '<i></i><i></i><i></i>'; } else { delete el.dataset.puls; el.textContent = ''; }
  }
  if (weit) el.dataset.weit = '1'; else delete el.dataset.weit;
  const groesse = `${r * 2}px`;
  if (el.style.width !== groesse) {
    el.style.width = groesse;
    el.style.height = groesse;
    el.style.left = `${-r}px`;
    el.style.top = `${-r}px`;
  }
  const p = karte.project([lon, lat]);
  const stelle = `translate(${aufGeraetepixel(p.x)}px,${aufGeraetepixel(p.y)}px)`;
  if (el.style.transform !== stelle) el.style.transform = stelle;
  el.dataset.meter = String(Math.round(meter));
  el.dataset.px = String(r);
  return r;
}
...
function kartenRand(karte) {
  ...
  return { top: deckel(72, hoehe), bottom: deckel(unten, hoehe), left: deckel(52, breite), right: deckel(52, breite) };
}
...
// zudeckt; der Punkt landet in der Mitte dessen, was darüber frei bleibt. Die feste Höhe ist die des
// Vogels auf dem Regler (BLATT_ZOOM): nah genug, um zu sehen, in welchem Lokal jemand ist.
...
// Flug hängen bleibt.
export const BLATT_ZOOM = 16;

export function punktUeberBlatt(karte, punkt, { zoom = BLATT_ZOOM, verdeckt = 0 } = {}) {
  if (!karte?.easeTo || !hatKoordinaten(punkt)) return false;
  const hoehe = karte.getContainer?.()?.clientHeight || 0;
  const unten = Math.max(0, Math.min(Number(verdeckt) || 0, hoehe - 80));
  const ziel = {
    center: [punkt.lon, punkt.lat],
    zoom: Math.max(ZOOM_OBEN, Math.min(ZOOM_UNTEN, Number(zoom) || BLATT_ZOOM)),
...
/* Runde 8 (R8-32): Genauigkeit als klarer, durchsichtig gruener Kreis — auf einer eigenen Ebene unter
   allen Markierungen. Darin laufen drei Ringe versetzt von innen nach aussen und verblassen; der
   groesste ist der Kreis selbst. Gepulst wird nur, was live ist (die eigene, frische Lage). */
.crew-genau-ebene{position:absolute;left:0;top:0;width:0;height:0;pointer-events:none;z-index:5}
.crew-genau{position:absolute;left:0;top:0;box-sizing:border-box;border-radius:50%;pointer-events:none;background:var(--karte-genau-flaeche);border:1.5px solid var(--karte-genau-rand)}
.crew-genau[data-weit="1"]{border-color:transparent}
.crew-genau>i{position:absolute;left:-1.5px;top:-1.5px;right:-1.5px;bottom:-1.5px;box-sizing:border-box;border-radius:50%;border:1.5px solid var(--karte-genau-ring);opacity:0;transform:scale(.06);animation:crew-genau-puls 2.4s cubic-bezier(.22,.61,.36,1) infinite}
.crew-genau>i:nth-child(2){animation-delay:.8s}
.crew-genau>i:nth-child(3){animation-delay:1.6s}
@keyframes crew-genau-puls{0%{opacity:0;transform:scale(.06)}14%{opacity:1}100%{opacity:0;transform:scale(1)}}
:root{--karte-genau-flaeche:var(--green-a12);--karte-genau-rand:var(--green-a45);--karte-genau-ring:var(--green-a60)}
:root[data-theme="dark"]{--karte-genau-flaeche:var(--green-a16);--karte-genau-rand:var(--green-a55);--karte-genau-ring:var(--green-a65)}
...
export function ortMarken(karte, optionen = {}) {
  ...
  function buendeln(liste) {
    ...
    for (let i = 0; i < liste.length; i += 1) {
      for (let j = i + 1; j < liste.length; j += 1) {
        ...
        const grenze = zusammen ? MARKE.buendelnAuf : MARKE.buendeln;
  ...
  function anordnen(optionen = {}) {
    ...
    for (const e of liste) { e.cluster = [e.key]; e.buendel = false; e.kopf = true; e.unten = false; e.zahl = e.anzahl; }
    ...
    for (const e of alle) {
      ...
      const kopfKey = e.buendel ? String(e.cluster?.[0] ?? '') : '';
...
const PERSON = Object.freeze({ bild: 36, nameLinks: 4, nameAb: 9.5, stapel: 20, box: 40, zeile: 46, luft: 10, rand: 8, trennen: 46 });
const FRISCHE_TAKT_MS = 30000;
// Solange ist MEINE Lage „live" (dieselbe halbe Stunde wie repo.getMyLocation › genau).
const LAGE_LIVE_MS = 30 * 60000;
...
export function personenMarken(karte, optionen = {}) {
  ...
  const st = { leute: new Map(), ebene: null, faecher: null, faecherBreite: 0, offen: null, gerueckt: false, weg: false, takt: 0 };
  // R8-32: Der Kreis der Genauigkeit liegt auf der gemeinsamen Genauigkeits-Ebene der Karte, unter
  // allen Gesichtern — je Person ein Schlüssel.
  const kreisSchluessel = (id) => `${schluessel}:${id}`;
  ...
  function setzen(liste) {
    ...
    for (const [id, p] of st.leute) if (!neu.has(id)) { p.el.remove(); genauKreis(karte, kreisSchluessel(id), null); }
    st.leute = neu;
  ...
  function anordnen() {
    ...
    liste.forEach((p, index) => {
      ...
      if (p.beschriftung !== beschriftung) {
        ...
        const grund = stapel ? tn(anzahl - 1, '{name} und {n} weitere Person', '{name} und {n} weitere Personen', { name }) : name;
      ...
      // R8-32: die Genauigkeit als klarer Kreis. Ein Stapelkopf zeigt keinen — er gehört mehreren
      // Lagen. Gepulst wird nur MEINE Lage, solange sie frisch ist (dann ist sie live).
      const kreis = p.oben && !stapel && p.genau ? { lat: p.lat, lon: p.lon, meter: p.genau } : null;
      const live = Boolean(p.ich) && p.at > 0 && jetzt - p.at < LAGE_LIVE_MS;
      const radius = genauKreis(karte, kreisSchluessel(p.id), kreis, { puls: live, unterPx: R });
      const genauM = radius ? String(Math.round(p.genau)) : '';
  ...
  function tipp(ereignis) {
    ...
    const punkte = p.gruppe.map((m) => ({ lat: m.lat, lon: m.lon }));
  ...
  const steuerung = Object.assign(steuer, {
    ...
    entfernen() {
      ...
      for (const id of st.leute.keys()) genauKreis(karte, kreisSchluessel(id), null);
      st.ebene?.remove();
      st.ebene = null;
      st.faecher = null;
      st.offen = null;
...
function karteTot(karte) {
  return !karte || !karte.style || !karte.getContainer?.();
}
