...
// 'vorgemerkt' (Runde 11, A5: gewünscht, die Systemfrage kommt beim ersten Tipp auf FREE).
//
// Runde 11 (A5, Jonathan): „‚Mitteilen auf diesem Gerät' standardmäßig an." Ehrlich umgesetzt:
//   · Der WUNSCH steht ab Werk auf an (crew.push.wunsch fehlt = an). Die Einstellung zeigt den
//     Schalter an und sagt dazu, dass die Erlaubnis des Systems noch fehlt ('vorgemerkt').
//   · Die Systemfrage kommt NICHT ungefragt beim Start (Safari verweigert sie ohne Berührung, und
//     ein Fenster beim ersten Start ist die sicherste Art, ein Nein zu bekommen), sondern EINMAL im
//     ersten Tipp auf FREE — dem Moment, in dem man wissen will, wer antwortet
//     (mitteilungenImTipp, gerufen aus screens/crew.js). In der App-Hülle fragt dort das
//     Betriebssystem über core/native.js (data/push.js › nativAnschalten → erlaubnisFragen).
//   · Sagt das System Nein, steht die Einstellung auf „Aus · Vom System blockiert" mit dem Weg,
//     es zurückzunehmen. Wer den Schalter selbst ausschaltet, bleibt aus (crew.push.wunsch = 'aus').
//   · Gefragt wird höchstens einmal von selbst (crew.push.gefragt) — jede echte Systemfrage zählt,
//     auch die aus dem Guide, der Karte oder dem Schalter.
...
import { widgetStandAnHuelle, widgetStandGrund, erlaubnisStandSofort } from '../core/native.js';
...
// Runde 11 (A5): der Wunsch dieses Geräts ('an' | 'aus'; fehlt = an) und ob die Systemfrage schon lief.
const SPEICHER_WUNSCH = 'crew.push.wunsch';
const SPEICHER_GEFRAGT = 'crew.push.gefragt';
...
// Runde 11 (A5): Will dieses Gerät Mitteilungen? Ab Werk ja. Nein nur, wenn jemand den Schalter
// ausgeschaltet hat — oder (vor Runde 11) die Erlaubnis gegeben und Mitteilungen danach selbst
// ausgeschaltet hatte: nichts wird wieder angeschaltet, was ein Mensch ausgeschaltet hat.
export function pushGewuenscht() {
  const wunsch = lesen(SPEICHER_WUNSCH);
  if (wunsch === 'an' || wunsch === 'aus') return wunsch === 'an';
  return !(erlaubnisStandSofort('mitteilungen') === 'erteilt' && lesen(SPEICHER_LAGE) === 'bereit');
}

function wunschSetzen(an) {
  schreiben(SPEICHER_WUNSCH, an ? 'an' : 'aus');
}

// 'bereit' heißt technisch: könnte an sein, ist es nicht. Ist es gewünscht und wurde das System noch
// nie gefragt, ist das kein „Aus", sondern „vorgemerkt" — die Frage kommt beim ersten Tipp auf FREE.
function mitWunsch(wert) {
  return wert === 'bereit' && pushGewuenscht() && !lesen(SPEICHER_GEFRAGT) ? 'vorgemerkt' : wert;
}
...
export function pushLage(repo) {
  ...
  if (lage.wert) return mitWunsch(lage.wert);
  const sofort = pushZustandSofort();
  if (sofort === 'erlaubt') return lesen(SPEICHER_LAGE) === 'bereit' ? mitWunsch('bereit') : 'an';
  return mitWunsch(sofort);
}

// Runde 11 (A5): Der eine Tipp, in dem von selbst gefragt wird. MUSS synchron im Tipp laufen (wie
// pushEinschalten). Fragt nur, wenn Mitteilungen gewünscht sind, noch nicht laufen und das System
// noch nie gefragt wurde — also höchstens einmal je Gerät. → true, wenn gefragt wurde.
export function mitteilungenImTipp(ctx) {
  if (pushLage(ctx?.repo) !== 'vorgemerkt') return false;
  pushEinschalten(ctx, { leise: true });
  return true;
}
...
// Runde 11 (A5): { leise } — aus dem Tipp auf FREE. Dort gibt FREE selbst Rückmeldung; hier
// kommen weder Ton noch Stoß dazu, und gesagt wird nur, was sich entschieden hat.
export function pushEinschalten(ctx, { ausKarte = false, leise = false } = {}) {
  ...
  // Jede echte Systemfrage zählt als die eine (A5) — und wer einschaltet, will es.
  schreiben(SPEICHER_GEFRAGT, Date.now());
  wunschSetzen(true);
  lage.wert = 'laeuft';
  ...
  return versprechen.then((ergebnis) => {
    lage.wert = ergebnis;
    lage.ausKarte = false;
    schreiben(SPEICHER_LAGE, ergebnis === 'fehler' ? (vorher === 'vorgemerkt' ? 'bereit' : vorher) : ergebnis);
    if (ergebnis === 'an') {
      if (!leise) {
        rueckmeldung('erfolg');
...
export function pushAusschaltenFuer(ctx) {
  const client = ctx?.repo?.client;
  if (!client) return Promise.resolve('konto');
  // Runde 11 (A5): Ausgeschaltet ist ausgeschaltet — auch der erste Tipp auf FREE fragt dann nicht.
  wunschSetzen(false);
  lage.wert = 'laeuft';
  ctx.render?.();
...
<span style="font-size:12px;color:var(--ink-soft);line-height:1.35">${tx('Nur vom Home-Bildschirm')}</span></span></div>
<div style="padding-left:2px">${homescreenAnleitung()}</div>
  ...
  const unterzeile = wert === 'laeuft'
    ...
    : wert === 'fehler'
      ...
      : `${tx('Nichts verpassen')} · <b style="font-weight:650;color:var(--green-dark)">${tx('Einschalten')}</b>`;
...
function seitenTexte(wert) {
  ...
  switch (wert) {
    ...
    // Runde 11 (A5): ehrlich „Aus" — mit dem Grund und dem Weg zurück darunter.
    case 'abgelehnt': return {
      status: `${tx('Aus')} · ${tx('Vom System blockiert')}`, farbe: 'var(--orange-dark)', grund: 'var(--orange-tint)',
      hinweis: apple
    ...
    // Runde 11 (A5): gewünscht (Schalter an), die Erlaubnis des Systems fehlt noch. Kein „An" —
    // es käme noch nichts an; aber auch kein „Aus", denn es schaltet sich beim ersten FREE ein.
    case 'vorgemerkt': return {
      status: tx('An, sobald du erlaubst'), farbe: 'var(--green-dark)', grund: 'var(--green-tint)',
      hinweis: tx('Beim nächsten Tipp auf FREE fragt dein Gerät einmal nach der Erlaubnis.'),
    };
...
export function pushSeitenKarte(ctx, { schalter, tonProbe: mitTonProbe = true } = {}) {
  ...
  const schaltbar = wert === 'an' || wert === 'bereit' || wert === 'fehler' || wert === 'server' || wert === 'vorgemerkt';
  const rechts = schaltbar && typeof schalter === 'function'
    ? schalter(wert === 'an' || wert === 'server' || wert === 'vorgemerkt')
    : '';
  ...
  const zusatz = wert === 'homescreen'
    ...
    : wert === 'vorgemerkt'
      // Wer nicht bis zum nächsten FREE warten will: derselbe Weg, jetzt, in diesem Tipp.
      ? `<button data-act="push-jetzt" data-role="push-jetzt" data-treffer style="margin-top:9px;align-self:flex-start;border:1px solid var(--ink-a12);background:var(--surface);color:var(--ink);font:650 12.5px/1 ${FONT};padding:9px 13px;border-radius:999px;cursor:pointer;appearance:none;display:flex;align-items:center;gap:6px"><span style="pointer-events:none;display:flex">${symbol('glocke', 'var(--ink-soft)', 14)}</span><span style="pointer-events:none">${tx('Jetzt erlauben')}</span></button>`
...
export function pushSeitenAktionen(ctx) {
  return {
    ...
    'push-toggle': () => {
      ...
      // Runde 11 (A5): 'vorgemerkt' steht am Schalter als AN (gewünscht) — ein Tipp nimmt nur den
      // Wunsch zurück. Angemeldet ist noch nichts, also gibt es nichts abzumelden.
      if (wert === 'vorgemerkt') {
        wunschSetzen(false);
        rueckmeldung('tipp');
        ctx.render?.();
    ...
    'push-jetzt': () => {
      if (pushLage(ctx.repo) !== 'vorgemerkt') return;
      rueckmeldung('tipp');
      pushEinschalten(ctx);
    },
    'push-probe': () => {
