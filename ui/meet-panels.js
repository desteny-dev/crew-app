...
//     v3.1 §7: Dabei | Offen | Abgesagt ist ZUGLEICH die direkt antippbare Dreifach-Control
//     (data-act="set-part", data-state). Der eigene Zustand liegt als ruhige getönte Spalte
//     sichtbar an; die Zahlen/Avatare bleiben wie in der Referenz.
...
//   bewertungsHinweis(ctx, options?)       → html ('' wenn nichts zu bewerten ist).
//     Das jüngste vorbeie Meet der letzten 7 Tage, bei dem ich dabei war, das nicht abgesagt,
//     nicht ausgeblendet, noch nicht bewertet und nicht weggeklickt ist. options.margin (CSS).
//   bewertungsHinweisAktionen(ctx)         → Handler-Karte für bindActions:
//     'bewertung-hinweis' (öffnet meet.review), 'bewertung-hinweis-weg' (blendet ihn aus).
//   bewertungsHinweisMeet(repo)            → das Meet des Hinweises oder null.
//
...
//     R5 (P4, E1): Darüber steht die Leiste „Mitfahren" (ui/mitfahren.js mitfahrenBereich) mit
//     eigenem Auf/Zu (ctx.ui.mitfahren = meetId). Sie klappt Mitbringen/Umfragen nicht zu — so
//     bewegt sich beim Öffnen weder sie noch die Kachelreihe. Ihre Handler stecken in bindMeetPanels.
...
// R5 (P4, E1): Mitfahren — wer fährt, wer mit wem, wer wen abholt (API in ui/mitfahren.js).
import { mitfahrenBereich, mitfahrenAktionen } from './mitfahren.js';
...
export function meetOrtKarte(ctx, meet, options = {}) {
  ...
  const echteKarte = hatEchteLage(place);
  if (!echteKarte && !place.address) return '';
...
<button data-act="route-open" data-provider="google" style="display:flex;align-items:center;gap:9px;min-height:44px;padding:0 14px;width:100%;border:0;background:transparent;cursor:pointer;appearance:none;font:650 13px ${FONT};color:var(--ink);text-align:left">Google Maps</button>
  ...
  // Scrollen nicht weg. Ein Tipp darauf öffnet dieselbe Routenwahl wie „Route". Die
  // Herkunftsangabe (ⓘ der Karte) bleibt bedienbar: map.js hält ihre Klicks bei sich.
  ...
  const karte = echteKarte
    ? `<div data-act="route-menu" data-role="meet-map-flaeche" role="button" aria-label="${esc(t('Route'))}" style="position:relative;height:${hoehe}px;background:var(--map-tint);overflow:hidden;border-radius:20px 20px 0 0;cursor:pointer">
...
// Markierung (Zeichen der Aktivität, Ortsname, grobe Zeit). Ein Tipp darauf öffnet die Routenwahl.
...
function avatarStack(people, size = 26, options = {}) {
  const overlap = Math.round(size * 0.27);
  if (!people.length) return `<span style="height:${size}px;display:flex;align-items:center;font-size:13px;color:var(--line-solid);pointer-events:none">—</span>`;
  const separator = options.separator || 'var(--surface)';
...
// „Offen" ist der unentschiedene Zustand und bekommt deshalb eine neutrale Auswahlfläche —
// Orange bleibt laut Abnahme §10 der Benachrichtigung vorbehalten.
const PART_STATES = [
  { state: 'yes', key: 'yes', label: t('Dabei'), color: 'var(--green-dark)', tint: 'var(--green-tint)', ring: 'var(--green-a40)' },
  { state: 'open', key: 'open', label: t('Offen'), color: 'var(--orange-dark)', tint: 'var(--field)', ring: 'var(--ink-a14)' },
...
export function participationSection(ctx, meet, options = {}) {
  ...
  // Trenner nur dort, wo keine getönte Spalte anschließt (sonst entsteht eine harte Kante).
  const divider = (left, right) => `<span style="width:1px;flex:none;align-self:stretch;margin:${compact ? 4 : 6}px 0;background:${left || right ? 'transparent' : 'var(--ink-a08)'}"></span>`;
  const row = `${cells[0].html}${divider(cells[0].selected, cells[1].selected)}${cells[1].html}${divider(cells[1].selected, cells[2].selected)}${cells[2].html}`;

  if (options.bare) {
    return `<div style="display:flex;border-top:1px solid var(--ink-a07);padding-top:6px">${row}</div>`;
  }
  const pad = compact ? 4 : 5;
  return `<div style="background:var(--surface);border:1px solid var(--ink-a10);border-radius:20px;padding:${pad}px;display:flex;flex:none">${row}</div>`;
}
...
export function meetPanelReihe(ctx, meet) {
  ...
  const items = meet.bring || [];
  const polls = meet.polls || [];
  const reihe = `<div data-role="panel-reihe" style="display:flex;gap:9px;flex:none">
${panelKachel('bring', circleCheckIcon(), `${t('Mitbringen')} · ${items.length}`, offen === 'bring', items.length === 0)}
${panelKachel('polls', barsIcon(), `${t('Umfragen')} · ${polls.length}`, offen === 'polls', polls.length === 0)}
</div>`;
  const inhalt = offen === 'bring' ? bringInhalt(ctx, meet) : (offen === 'polls' ? pollInhalt(ctx, meet) : '');
  ...
  return `<div data-role="panel-bereich" data-meet="${esc(meet.id)}" style="display:flex;flex-direction:column;gap:9px;flex:none">${mitfahrenBereich(ctx, meet)}${reihe}${inhalt}</div>`;
}
...
export function meetVorbei(meet, jetzt = now()) {
  ...
  return Boolean(meet.date) && meet.date < toISODate(jetzt);
}
...
function hinweisWegklicken(meetId) {
  const liste = weggeklickteHinweise().filter((id) => id !== meetId);
  liste.push(meetId);
  try {
    globalThis.localStorage?.setItem(HINWEIS_MERKER, JSON.stringify(liste.slice(-60)));
  } catch { /* privates Fenster: gilt dann bis zum Neuladen nicht */ }
}

export function bewertungsHinweisMeet(repo, jetzt = now()) {
  if (!repo?.getMeets) return null;
  const grenze = toISODate(new Date(jetzt.getFullYear(), jetzt.getMonth(), jetzt.getDate() - HINWEIS_TAGE));
  const weg = new Set(weggeklickteHinweise());
  let meets = [];
  try {
    meets = repo.getMeets({ context: { mine: true }, direction: 'history' }) || [];
  } catch {
    meets = [];
  }
  const offen = meets.filter((meet) => meetVorbei(meet, jetzt)
    && !meet.cancelled
    && !meet.hiddenFromHistory
    && !meet.review?.verdict
    && meet.participation?.[ME] === 'yes'
    && (meet.date || '') >= grenze
    && !weg.has(meet.id));
...
function kleinesKreuz(color = 'var(--muted)', size = 14) {
  return `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none"><path d="M7 7l10 10M17 7 7 17" stroke="${color}" stroke-width="2" stroke-linecap="round"></path></svg>`;
}

export function bewertungsHinweis(ctx, options = {}) {
  const meet = bewertungsHinweisMeet(ctx.repo);
  if (!meet) return '';
  const titel = t('Wie war {titel}?', { titel: meet.title });
  // Die ganze Karte ist die Aktion; das Kreuz ist ein eigenes, kleineres Ziel an ihrem Rand.
  // bindActions findet über closest('[data-act]') zuerst das Kreuz — kein verschachtelter Knopf.
  return `<div data-role="bewertungs-hinweis" data-meet="${esc(meet.id)}" style="position:relative;flex:none;margin:${options.margin ?? '0'}">
<div role="button" tabindex="0" data-act="bewertung-hinweis" data-meet="${esc(meet.id)}" aria-label="${esc(t('{titel} bewerten', { titel: meet.title }))}" style="display:flex;align-items:center;gap:12px;padding:12px 48px 12px 12px;background:var(--surface);border:1px solid var(--ink-a10);border-radius:18px;box-shadow:0 1px 2px var(--shadow-05);cursor:pointer;font-family:${FONT};color:var(--ink);text-align:left">
<span style="position:relative;width:42px;height:42px;border-radius:13px;background:var(--orange-tint);display:flex;align-items:center;justify-content:center;flex:none;pointer-events:none">${activityIconSvg(meet, 'var(--orange-dark)', 20)}<span style="position:absolute;right:-2px;top:-2px;width:10px;height:10px;border-radius:50%;background:var(--orange);box-shadow:0 0 0 2px var(--surface)"></span></span>
<span style="display:flex;flex-direction:column;gap:3px;flex:1;min-width:0;pointer-events:none">
<span style="font-size:14px;font-weight:650;letter-spacing:-.005em;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(titel)}</span>
<span style="font-size:12px;color:var(--ink-soft);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(meetDateLabel(meet.date))} · <b style="font-weight:650;color:var(--orange-dark)">${t('Jetzt bewerten')}</b></span>
</span>
</div>
<button data-act="bewertung-hinweis-weg" data-treffer data-meet="${esc(meet.id)}" aria-label="${esc(t('Hinweis ausblenden'))}" style="position:absolute;right:7px;top:50%;transform:translateY(-50%);width:34px;height:34px;border-radius:50%;border:0;background:transparent;display:flex;align-items:center;justify-content:center;cursor:pointer;appearance:none;padding:0"><span style="pointer-events:none;display:flex">${kleinesKreuz('var(--muted)', 15)}</span></button>
</div>`;
}

export function bewertungsHinweisAktionen(ctx) {
  return {
    'bewertung-hinweis': (data) => ctx.nav.go('meet.review', { meetId: data.meet }),
    'bewertung-hinweis-weg': (data) => {
      hinweisWegklicken(data.meet);
      ctx.render();
    },
  };
}
...
export function bindMeetPanels(root, ctx, meet) {
  ...
  const handlers = {
    ...
    // R5 (P4, E1): Mitfahren — Rolle, Plätze, Mitnehmen, Einsteigen, Vorschlag, Route.
    ...mitfahrenAktionen(ctx, meet),
    ...
    // v3.1 §7: die Dreifach-Control setzt den eigenen Zustand direkt.
    // v5 A18: Jede Zustandsaenderung bestaetigt sich kurz und nicht blockierend.
    'set-part': (data) => {
      rueckmeldung('auswahl');
      repo.setParticipation(meet.id, data.state);
      ctx.toast?.(PART_TOAST[data.state] || t('Zusage geändert'));
    },
