...
//   R6 D6 — Der Hinweis lebt am MEET (nicht mehr als Kachel auf der Crew-Startseite):
//   bewertungOffen(meet)                   → boolean. Die EINE Regel: vorbei, nicht abgesagt,
//     nicht ausgeblendet, noch nicht bewertet, ich war dabei, höchstens 7 Tage her, nicht weggetippt.
//   bewertungsMeetIn(repo, context)        → das jüngste wartende Meet eines Kontexts (Raum/Crew/Person).
//   bewertungsZeile(ctx, meet, options?)   → html ('' wenn nichts zu fragen ist). Eine Zeile mit
//     der Frage und den drei Stufen als 44-px-Knöpfe; ✕ nimmt sie dauerhaft weg.
//     options.variante: 'karte' (Rückblick, Raum-Panel) · 'liste' (leise Zeile unter der Meet-Karte)
//     options.titel: false → nur „Wie war's?" statt „Wie war <Titel>?"
//   bewertungsZeileAktionen(ctx)           → Handler: 'bew-stufe', 'bew-mehr', 'bew-weg'.
...
//
//   Runde 8 (R8-24): „Ich bin da" ist weg — Knopf, Zeile und Handler. (Jonathan: „‚Ich bin da'
...
import { anreiseKnopf, anreiseAktionen } from './mitfahren.js';
...
${panelKachel('polls', barsIcon(), `${t('Umfragen')} · ${polls.length}`, offen === 'polls', polls.length === 0)}
  ...
  return `<div data-role="panel-bereich" data-meet="${esc(meet.id)}" style="display:flex;flex-direction:column;gap:9px;flex:none">${anreiseKnopf(ctx, meet)}${reihe}${inhalt}</div>`;
}
...
//
// Sie mahnt nicht: kein Zähler, keine Farbfläche, keine zweite Erinnerung. Ein Tipp auf ✕
// nimmt sie für dieses Meet für immer weg (derselbe Merker wie beim alten Hinweis).
//
//   bewertungOffen(meet)              → boolean: wartet dieses Meet auf meine Bewertung?
//   bewertungsMeetIn(repo, context)   → das jüngste wartende Meet EINES Kontexts (Raum, Crew, Person)
//   bewertungsZeile(ctx, meet, opts)  → html ('' wenn nichts zu fragen ist)
//     opts.variante: 'karte' (Rückblick, Raum-Panel — eigene Karte) · 'liste' (unter der
//     Meet-Karte in der Liste — kein Rahmen, kleinere Schrift)
//   bewertungsZeileAktionen(ctx)      → Handler: 'bew-stufe', 'bew-mehr', 'bew-weg'
...
export function bewertungsZeile(ctx, meet, options = {}) {
  ...
  const leise = options.variante === 'liste';
  const ohneRahmen = leise || options.variante === 'blank';
  const frage = options.titel === false ? t("Wie war's?") : t('Wie war {titel}?', { titel: meet.title });
  // Dieselben Knöpfe wie auf der Bewertungsseite (themenBlock): runde Fläche, Gesicht in der
  // Textfarbe. Am Meet sind sie 44 px (Mitte zu Mitte 50), in der Liste bewusst kleiner und
  // leiser — die unsichtbare Trefferfläche (data-treffer) hält dort trotzdem 46 px, und mit
  // 8 px Abstand liegen die Mitten 46 px auseinander. Kein Finger trifft zwei Gesichter.
  const rund = leise ? 38 : 44;
  const abstand = leise ? 8 : 6;
  const knopf = (stufe) => `<button data-act="bew-stufe" data-meet="${esc(meet.id)}" data-stufe="${stufe.id}"${leise ? ' data-treffer' : ''} aria-label="${esc(t('Bewerten: {stufe}', { stufe: stufe.label }))}" style="width:${rund}px;height:${rund}px;border-radius:50%;background:var(--surface);border:1.5px solid ${leise ? 'var(--ink-a12)' : 'var(--ink-a16)'};box-sizing:border-box;display:flex;align-items:center;justify-content:center;flex:none;cursor:pointer;appearance:none;padding:0"><span style="pointer-events:none;display:flex">${gesichtSvg(stufe.gesicht, leise ? 'var(--muted)' : 'var(--ink-soft)', leise ? 28 : 34, false)}</span></button>`;
  const knoepfe = `<div role="group" aria-label="${esc(t("Wie war's?"))}" style="display:flex;gap:${abstand}px;flex:none">${BEWERTUNG_STUFEN.map(knopf).join('')}</div>`;
  const text = `<button data-act="bew-mehr" data-meet="${esc(meet.id)}" style="flex:1;min-width:0;display:flex;align-items:center;gap:4px;min-height:${rund}px;border:0;background:transparent;padding:0;cursor:pointer;appearance:none;font-family:${FONT};text-align:left">
<span style="font-size:${leise ? 12.5 : 14}px;font-weight:650;color:${leise ? 'var(--ink-soft)' : 'var(--ink)'};white-space:nowrap;overflow:hidden;text-overflow:ellipsis;pointer-events:none">${esc(frage)}</span>
<span style="pointer-events:none;display:flex;flex:none">${chevronRight(leise ? 'var(--line-solid)' : 'var(--line-strong)')}</span>
</button>`;
  const weg = `<button data-act="bew-weg" data-meet="${esc(meet.id)}" data-treffer aria-label="${esc(t('Hinweis ausblenden'))}" style="width:30px;height:30px;border-radius:50%;border:0;background:transparent;display:flex;align-items:center;justify-content:center;cursor:pointer;appearance:none;padding:0;flex:none"><span style="pointer-events:none;display:flex">${kleinesKreuz('var(--muted-light)', 14)}</span></button>`;
  const rahmen = ohneRahmen
    ? (leise ? 'padding:2px 6px 0 2px' : 'padding:0')
    : 'background:var(--surface);border:1px solid var(--ink-a10);border-radius:18px;padding:6px 8px 6px 14px';
  return `<div data-role="bewertungs-zeile" data-variante="${esc(options.variante || 'karte')}" data-meet="${esc(meet.id)}" style="display:flex;align-items:center;gap:8px;flex:0 1 auto;width:100%;box-sizing:border-box;min-width:0;${rahmen}">${text}${knoepfe}${weg}</div>`;
}

export function bewertungsZeileAktionen(ctx) {
  return {
    // Ein Tipp genügt: die Stufe wird gespeichert, alles andere bleibt, wie es war.
    'bew-stufe': (data) => {
      const meet = ctx.repo.getMeet?.(data.meet);
      const alt = meet?.review || {};
      const topics = alt.topics && typeof alt.topics === 'object' && !Array.isArray(alt.topics) ? { ...alt.topics } : {};
      rueckmeldung('auswahl');
      ctx.repo.submitReview?.(data.meet, {
        verdict: data.stufe,
        topics,
        reasons: Object.keys(topics).filter((id) => topics[id] === 'nicht-gut'),
        aspects: Object.entries(topics).map(([id, wert]) => `${id}:${wert}`),
        note: alt.note || '',
      });
      // Derselbe Wortlaut, den die Bewertungsseite seit R3 meldet — eine Sache, ein Satz.
      ctx.toast?.(t('Bewertung gespeichert'));
    },
    'bew-mehr': (data) => ctx.nav?.go('meet.review', { meetId: data.meet }),
    // Einmal wegtippen heißt weg: der Merker gilt auf diesem Gerät dauerhaft.
    'bew-weg': (data) => {
...
export function bindMeetPanels(root, ctx, meet) {
  ...
  const handlers = {
    ...
    'set-part': (data) => {
      ...
      const naechster = jetzt === data.state ? 'open' : data.state;
      repo.setParticipation(meet.id, naechster);
      ctx.toast?.(PART_TOAST[naechster] || t('Zusage geändert'));
    },
