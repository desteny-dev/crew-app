...
// Runde 11 (A10): Die markierten Leute stehen DIREKT ÜBER der Suchleiste.
// R8-5  (Runde 11, A11: nur noch nach Zeit — Frei-Leute stehen nicht mehr oben.) Nie unter dem Finger:
...
// R11-B6 …und links ein kleiner runder Knopf: die Liste der gerade freien Leute (mit dem Schalter
//        „Auf Karte zeigen"). Der Meet-Tab hat seit Runde 8c keine FREE-Reihe mehr — die Liste gibt es dort nicht.
...
import { search, groupIcon } from '../ui/icons.js';
// Runde 11 (B6): „frei" und der Schalter „Auf Karte zeigen" gehören der Karte (ui/map.js).
import { istGeradeFrei, nurFreieAufKarte, nurFreieAufKarteSetzen } from '../ui/map.js';
...
// Runde 11 (B10, D10): Die Frage „Wie war's?" steckt in der Zeile eines vergangenen Meets.
import { bewertungOffen, bewertungsPille, meetDateLabel } from '../ui/meet-panels.js';
...
import { mitteilungsKarte, mitteilungsKarteAktionen, pushLage, pushLageAuffrischen, mitteilungenBeobachten, mitteilungenImTipp } from '../ui/mitteilungen.js';
...
// Runde 11 (C1, D3): Zeilen wischen = Anheften · Ausblenden · Löschen (ui/chat-wisch.js).
import { chatZeilenWischBinden, chatAusgeblendet } from '../ui/chat-wisch.js';
...
function sucheAnwenden(seite, roh) {
  ...
  seite.querySelectorAll('[data-role="chat-liste"] > [data-role="chat-zeile"]').forEach((zeile) => {
    // Runde 11 (C1): Ein ausgeblendeter Chat steht nicht in der Liste — die Suche findet ihn weiter.
    const ja = passt(zeile.dataset.suche, begriff) && (Boolean(begriff) || !zeile.hasAttribute('data-ausgeblendet'));
    zeile.hidden = !ja;
    if (ja) treffer += 1;
  });
...
//
// Die Reihenfolge, die man SIEHT (Runde 11, A11): NUR nach Zeit — die letzte Nachricht, jüngste zuerst.
// Wer frei ist, steht nicht mehr oben (das zeigt der grüne Punkt am Bild). Wo die Zeit nichts sagt,
// bleibt die Ordnung der Datenschicht stehen (stabil sortiert).
function anzeigeOrdnung(eintraege) {
  return eintraege
    .map((eintrag, index) => [eintrag, index])
    .sort(([a, ia], [b, ib]) => ((Number(b.zeit) || 0) - (Number(a.zeit) || 0)) || (ia - ib))
    .map(([eintrag]) => eintrag);
}
...
// --- Die Zeile (R8-2/R8-3) -----------------------------------------------------------------
function chatZeile(ctx, eintrag, settings, { langDruck = true, begriff = '', bewertung = false } = {}) {
  const art = zeilenArt(eintrag);
  const name = eintragName(ctx, eintrag);
  const text = vorschau(ctx, eintrag);
  // Runde 11 (B10, D10): Unter „Vergangene Meets" fragt die Zeile selbst — eine leise Pille rechts,
  // und die zweite Zeile sagt, WORUM es ging (Tag · Ort) statt der letzten Nachricht. Gefragt wird
  // nur, wo bewertungOffen es erlaubt (ich war dabei, klarer Inhalt, noch nicht bewertet).
  const frage = bewertung && eintrag.meet && bewertungOffen(eintrag.meet) ? eintrag.meet : null;
  ...
  const frageMeta = frage ? [meetDateLabel(frage.date), frage.placePending ? '' : frage.place?.name].filter(Boolean).join(' · ') : '';
  const zweite = frage
    ? `<span data-role="chat-zustand" style="display:flex;align-items:center;gap:8px;min-width:0;padding-right:96px">
<span data-role="chat-nachricht" style="flex:1;min-width:0;font-size:13.5px;line-height:18px;font-weight:450;color:var(--muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(frageMeta)}</span>${unread ? orangeZahl(unread) : ''}</span>`
    : text || unread
...
${unread ? orangeZahl(unread) : ''}</span>`
    : '';
  // Runde 11 (C1): per Wischen ausgeblendet (nur in der Liste, nicht unter „Vergangene Meets").
  const weg = !bewertung && chatAusgeblendet(settings, eintrag);
  const zeigen = passt(suchText, begriff) && (Boolean(begriff) || !weg);
  return `<div data-role="chat-zeile" class="r8a-zeile" data-art="${art}" data-id="${esc(eintrag.id)}" data-suche="${esc(suchText)}"${weg ? ' data-ausgeblendet' : ''}${druck}${zeigen ? '' : ' hidden'}>
...
${zweite}
</span></button>${frage ? `<span data-role="bewertungs-ort" style="position:absolute;right:${RAND}px;bottom:8px;z-index:1;display:flex">${bewertungsPille(frage)}</span>` : ''}</div>`;
}
...
// Runde 9: EINE Bauform für die Anfrage-Zeile — die aus dem Profil (profile.js › anfrageZeile).
const anfrageZeile = (anfrage, index) => profil.anfrageZeile(anfrage, index);
...
function vergangeneKnopf(anzahl) {
  if (!anzahl) return '';
  return `<button data-act="vergangene-oeffnen" data-role="vergangene-knopf" data-treffer style="display:flex;align-items:center;justify-content:center;gap:6px;width:fit-content;margin:18px auto 6px;height:34px;padding:0 14px 0 12px;box-sizing:border-box;border:1px solid var(--ink-a12);border-radius:999px;background:transparent;color:var(--muted);font:600 13px/1 'Instrument Sans',sans-serif;cursor:pointer;appearance:none"><span style="pointer-events:none;display:flex">${symbol('verlauf', 'var(--muted)', 14)}</span><span style="pointer-events:none">${esc(tx('Vergangene Meets'))}</span></button>`;
}
...
function vergangeneBlattHtml(ctx) {
  ...
  const inhalt = liste.length
    ? `<div class="r8a-chats" data-role="vergangene-liste" style="margin:6px -20px 0">${liste.map((eintrag) => chatZeile(ctx, eintrag, settings, { langDruck: false, bewertung: true })).join('')}</div>`
...
// Runde 10 (Jonathan): Sind Mitteilungen aus, sagt es ein Stichwort im Kopf — zwischen „crew.“
// und dem „+“, in der Zeile, ohne etwas zu verdecken. Ein Tipp geht denselben Weg wie die grosse
// Karte ('push-karte-an'). Steht die Karte gerade oben, schweigt die Pille: einmal sagen genuegt.
// Geprueft wird NICHT neu — pushLage(repo) ist dieselbe Quelle, aus der die Karte lebt.
function kopfPille(ctx) {
  const wert = pushLage(ctx.repo);
  if (wert !== 'bereit' && wert !== 'fehler') return '';
  if (mitteilungsKarte(ctx)) return '';
  // Obergrenze, damit auch eine lange Sprache den Schriftzug links nie drueckt: die Pille kuerzt.
  return `<span style="display:flex;min-width:0;max-width:178px">${bausteine.hinweisPille({ text: tx('Mitteilungen aktivieren'), act: 'push-karte-an', klein: true })}</span>`;
}
...
function header(ctx, hinweisLaeuft, kopfHinweis = '') {
  ...
  const knopf = `<button data-act="crew-plus" data-role="crew-plus" data-treffer data-anfragen="${offen}" aria-label="${esc(label)}" style="position:relative;flex:none;width:${PLUS_KNOPF}px;height:${PLUS_KNOPF}px;border:0;border-radius:50%;background:transparent;display:flex;align-items:center;justify-content:center;padding:0;cursor:pointer;appearance:none"><span data-role="crew-plus-kreis" aria-hidden="true" style="position:absolute;inset:0;border-radius:50%;border:1px solid var(--ink-a12);background:var(--surface);box-sizing:border-box;pointer-events:none;${loch}"></span><span style="position:relative;pointer-events:none;display:flex">${symbol('personPlus', 'var(--ink)', 19)}</span>${punkt}</button>`;
  // Die Anfrage-Karte liegt (ohne Freunde) genau hier — dann tritt die Pille zurueck.
  const pille = kopfHinweis ? '' : kopfPille(ctx);
  return tabHeader(
...
<p data-role="crew-suche-leer"${begriff && !treffer ? '' : ' hidden'} style="margin:18px ${RAND}px 0;text-align:center;font:500 14px/1.4 'Instrument Sans',sans-serif;color:var(--muted)">${esc(tx('Keine Treffer'))}</p>`
  ...
  // Rollen). Steht die Reihe da (seit Runde 11, A10, ÜBER der Suche), reicht sie von der Oberkante der
  // Reihe bis zur Unterkante der Suche ('hoch'); sonst ist sie genau so hoch wie die Suchleiste ('flach').
  // Es rückt nichts.
  ...
  const hinweisLage = inListe
    ? ` style="position:absolute;z-index:4;left:50%;right:${RAND}px;top:${SUCHE_OBEN}px;${form === 'hoch' ? 'bottom:6px' : `height:${SUCHE_HOEHE}px`};pointer-events:none"`
    : '';
...
// Reihe und ist damit vom Knopf oben rechts (Freund/Gruppe) zu unterscheiden. Seit Runde 10 trägt
// er nicht mehr ein nacktes „+", sondern den Kalender mit Plus (components.js › meetPlusZeichen) —
// dasselbe Zeichen wie der Knopf unten rechts im Meet-Tab. Er sitzt außerhalb
...
// in Folge überlebt.
// Runde 9 (Jonathan): der „+" etwas weiter nach außen — seine linke Kante 80 px rechts der Mitte
// (vorher 62). Das Meet-Paket stellt seinen „+" an genau dieselbe Stelle.
const FREI_PLUS_ABSTAND = 80;

// --- Runde 11 (B6): die Liste der gerade freien Leute ---------------------------------------------------
// Jonathan: „Knopf links neben FREE: Liste der gerade freien Leute, mit kleinem Schalter ‚auf Karte zeigen'."
// Der Knopf sitzt spiegelbildlich zum „+" (dieselbe Größe, derselbe Ring, dieselbe Achse), trägt das Zeichen
// der Leute und — solange jemand frei ist — ihre Zahl. Ein Tipp öffnet über der Reihe eine ruhige Karte:
// links „n frei", rechts der kleine Schalter, darunter die Freien mit Bild und Name; ein Tipp auf eine Zeile
// öffnet den Chat mit ihr/ihm (die Absicht hinter „wer ist frei" ist: etwas zusammen machen).
// Warum eine Karte IN der Reihe und kein Blatt über dem Rahmen: Die Crew-Liste dahinter bleibt sichtbar
// und bedienbar (wer frei ist, steht ohnehin darin), nichts wird abgedunkelt. Zu geht sie mit einem Tipp
// daneben oder auf den Knopf.
// Der Schalter wirkt wirklich: An = die Karte zeigt von den Leuten nur die Freien (ui/map.js ›
// nurFreieAufKarte, screens/karte.js › sichtbareLeute). „Frei" heißt wie der grüne Punkt am Profilbild:
// frei gemeldet und nicht gerade in einem Meet — auch „frei ab 17:30" (dann steht die Zeit dabei).
function freieLeute(repo) {
  const spaeter = (person) => (person.free?.pending && Number(person.free.fromAt) > Date.now() ? String(person.free.from || '') : '');
  return (repo.getPeople?.() || [])
    .filter(istGeradeFrei)
    .map((person) => ({ person, spaeter: spaeter(person) }))
    .sort((a, b) => Number(Boolean(a.spaeter)) - Number(Boolean(b.spaeter)) || String(a.person.name).localeCompare(String(b.person.name)));
}

function freiListeHtml(ctx, verbergen) {
  const leute = freieLeute(ctx.repo);
  const zeigen = nurFreieAufKarte();
  const schrift = "'Instrument Sans',sans-serif";
  const schalter = leute.length || zeigen
    ? `<button data-act="frei-liste-karte" data-role="frei-liste-karte" data-treffer role="switch" aria-checked="${zeigen ? 'true' : 'false'}" style="display:flex;align-items:center;gap:8px;min-height:32px;padding:0 0 0 10px;border:0;background:transparent;cursor:pointer;appearance:none;font:600 12.5px/1 ${schrift};color:var(--ink-soft);-webkit-tap-highlight-color:transparent"><span style="pointer-events:none">${esc(tx('Auf Karte zeigen'))}</span><span aria-hidden="true" style="pointer-events:none;position:relative;width:34px;height:20px;border-radius:999px;flex:none;background:${zeigen ? 'var(--green)' : 'var(--field)'};box-shadow:inset 0 0 0 1px var(--ink-a08);transition:background .16s ease"><span style="position:absolute;top:2px;left:${zeigen ? 16 : 2}px;width:16px;height:16px;border-radius:50%;background:var(--surface);box-shadow:0 1px 3px var(--shadow-20);transition:left .16s ease"></span></span></button>`
    : '';
  const kopf = leute.length || schalter
    ? `<div style="display:flex;align-items:center;justify-content:space-between;gap:12px;padding:8px 8px 4px 18px"><span data-role="frei-liste-titel" style="font:650 15px/1.2 ${schrift};color:var(--ink)">${leute.length ? esc(tx('{n} frei', { n: leute.length })) : ''}</span>${schalter}</div>`
    : '';
  const zeilen = leute.map(({ person, spaeter }) => `<button data-act="frei-liste-person" data-person="${esc(person.id)}" data-treffer style="display:flex;align-items:center;gap:12px;width:100%;min-height:54px;padding:6px 10px;border:0;border-radius:14px;background:transparent;text-align:left;cursor:pointer;appearance:none;font-family:${schrift};-webkit-tap-highlight-color:transparent"><span style="pointer-events:none;display:flex;flex:none">${personAvatar(person, { size: 38, fontSize: 14, free: false, active: false })}</span><span style="pointer-events:none;flex:1;min-width:0;font:600 15px/1.25 ${schrift};color:var(--ink);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(person.name)}</span>${spaeter ? `<span style="pointer-events:none;flex:none;font:600 12.5px/1 ${schrift};color:var(--green-dark)">${esc(tx('Frei ab {zeit}', { zeit: spaeter }))}</span>` : ''}</button>`).join('');
  const leer = leute.length ? '' : `<div data-role="frei-liste-leer" style="padding:18px 18px 20px;font:600 14px/1.35 ${schrift};color:var(--muted)">${esc(tx('Gerade ist niemand frei'))}</div>`;
  return `<div data-role="frei-liste" role="dialog" aria-label="${esc(tx('Wer ist frei'))}" style="position:absolute;left:16px;right:16px;bottom:calc(100% + 2px);z-index:2;pointer-events:auto;background:var(--surface);border-radius:20px;box-shadow:0 8px 28px var(--shadow-18),0 0 0 1px var(--ink-a08);overflow:hidden;${verbergen ? 'visibility:hidden;' : ''}">${kopf}<div data-role="frei-liste-zeilen" style="max-height:240px;overflow-y:auto;overscroll-behavior:contain;touch-action:pan-y;padding:0 8px ${leute.length ? 8 : 0}px">${zeilen}${leer}</div></div>`;
}

// Ein Tipp daneben schließt die Liste. Der Tipp selbst geht weiter an das, worauf er fiel (eine Zeile der
// Liste dahinter, FREE …) — die Liste hält niemanden fest; deshalb hört ein Lauscher auf dem Dokument zu und
// schließt erst NACH dem Tipp. Ein Zug (Rollen der Liste dahinter) ist kein Tipp und schließt nichts.
let freiListeRahmen = null;
let freiListeLauscher = null;
function freiListeLauschen(ctx) {
  freiListeRahmen = ctx;
  if (freiListeLauscher) return;
  freiListeLauscher = (ereignis) => {
    const rahmen = freiListeRahmen;
    if (!rahmen?.ui?.freiListe) {
      document.removeEventListener('click', freiListeLauscher, true);
      freiListeLauscher = null;
      return;
    }
    if (ereignis.target?.closest?.('[data-role="frei-liste"], [data-act="frei-liste"]')) return;
    window.setTimeout(() => {
      if (!rahmen.ui.freiListe) return;
      rahmen.ui.freiListe = false;
      rahmen.render();
    }, 0);
  };
  document.addEventListener('click', freiListeLauscher, true);
}
...
export function freiReiheHtml(ctx) {
  ...
  const neu = `<button data-act="frei-neues-meet" data-role="frei-neues-meet" data-treffer aria-label="${esc(tx('Neues Meet'))}" style="position:absolute;left:calc(50% + ${FREI_PLUS_ABSTAND}px);top:50%;margin-top:-19px;width:38px;height:38px;box-sizing:border-box;border-radius:50%;border:1.5px solid var(--green-a42);background:var(--surface);box-shadow:0 1px 4px var(--shadow-07);display:flex;align-items:center;justify-content:center;padding:0;cursor:pointer;appearance:none;pointer-events:auto;${holding ? 'visibility:hidden;' : ''}"><span style="pointer-events:none;display:flex">${bausteine.meetPlusZeichen()}</span></button>`;
  // Runde 11 (B6): der Knopf links (Spiegel des „+") und die Liste darüber. Beide stehen HINTER FREE und dem
  // „+" im Baum, damit deren Knoten beim Halten nie ihren Platz wechseln (Runde 6, A3); beim Halten sind sie
  // nur unsichtbar. Die Liste steht nur, solange sie offen ist.
  const anzahl = freieLeute(repo).length;
  const offen = Boolean(ui.freiListe);
  const zahl = anzahl
    ? `<span data-role="frei-liste-zahl" aria-hidden="true" style="position:absolute;top:-6px;right:-6px;min-width:18px;height:18px;padding:0 5px;box-sizing:border-box;border-radius:9px;background:var(--green);color:var(--on-accent);font:700 11px/18px 'Instrument Sans',sans-serif;text-align:center;box-shadow:0 0 0 2px var(--paper);pointer-events:none">${anzahl}</span>`
    : '';
  const liste = `<button data-act="frei-liste" data-role="frei-liste-knopf" data-treffer aria-expanded="${offen ? 'true' : 'false'}" aria-label="${esc(tx('Wer ist frei'))}" style="position:absolute;right:calc(50% + ${FREI_PLUS_ABSTAND}px);top:50%;margin-top:-19px;width:38px;height:38px;box-sizing:border-box;border-radius:50%;border:1.5px solid var(--green-a42);background:${offen ? 'var(--green-tint)' : 'var(--surface)'};box-shadow:0 1px 4px var(--shadow-07);display:flex;align-items:center;justify-content:center;padding:0;cursor:pointer;appearance:none;pointer-events:auto;${holding ? 'visibility:hidden;' : ''}"><span style="pointer-events:none;display:flex">${groupIcon('var(--green-dark)', 19)}</span>${zahl}</button>`;
  return `<div data-role="frei-reihe" style="pointer-events:none;position:relative;display:flex;align-items:center;justify-content:center;height:112px">${freiKreis(free, { hidden: holding, anim, caption })}${neu}${liste}${offen ? freiListeHtml(ctx, holding) : ''}</div>`;
}
...
function freiUmschalten(ctx) {
  ...
  if (free?.active) {
    ...
    rueckmeldung('erfolg');
    // Runde 11 (A5): Mitteilungen sind ab Werk gewünscht; die Systemfrage kommt EINMAL, hier, in
    // der Berührung (iOS verlangt sie) — bevor irgendetwas wartet.
    mitteilungenImTipp(ctx);
    repo.setFree();
  }
}
...
function bindCrewHome(root, ctx) {
  ...
  const aktionen = {
    ...
    // Runde 11 (B10): die Frage in einer vergangenen Meet-Zeile öffnet die Bewertungsseite.
    'bew-mehr': (data) => { rueckmeldung('tipp'); ui.vergangeneOffen = false; nav.go('meet.review', { meetId: data.meet }); },
  ...
  bindListe(seite, ctx);
  chatZeilenWischBinden(seite, ctx);
  zeilenGleiten(seite);
...
export function bindFreiControl(root, ctx, handlers = {}) {
  bindActions(root, {
    ...
    // Runde 11 (B6): die Liste der Freien.
    'frei-liste': () => { rueckmeldung('tipp'); ctx.ui.freiListe = !ctx.ui.freiListe; ctx.render(); },
    'frei-liste-karte': () => { rueckmeldung('tipp'); nurFreieAufKarteSetzen(!nurFreieAufKarte()); ctx.render(); },
    'frei-liste-person': (data) => {
      rueckmeldung('tipp');
      ctx.ui.freiListe = false;
      ctx.nav.go('room.view', { roomId: roomIdForPerson(data.person) });
    },
  });
  if (ctx.ui.freiListe) freiListeLauschen(ctx);
