...
import {
  ...
  meetPanelReihe, vorschlagTitel,
  BEWERTUNG_STUFEN, BEWERTUNG_TON, gesichtSvg, bewertungsZeile, bewertungsZeileAktionen,
...
function renderMeetDetails(ctx) {
  ...
  // Die Zeile „Teilnehmende bearbeiten" steht direkt unter der Teilnahme-Auswahl:
...
<div style="display:flex;flex-direction:column;gap:9px;padding:12px 20px 0">${sections}</div>
  ...
  const html = screenScaffold({
    ...
    // Routenwechsel aus.
    overlays: `${ui.menu ? detailsMenu(ctx, meet) : ''}${ui.pencilSheet ? pencilSheet() : ''}${ui.loopSheet ? loopSheet(ctx, meet) : ''}${participantsSheet(ctx, meet)}${variantPickSheet(ctx, meet)}${ui.confirm ? confirmSheet(ui.confirm) : ''}${discardSheet(ctx)}${chatWahlHtml(ctx)}`,
...
function renderRueckblick(ctx, meet) {
  ...
  const abgesagtHinweis = abgesagt
    ...
    : '';
  // R6 D6: Ist das Meet noch unbewertet, steht die Frage GANZ OBEN am Meet — ein Tipp auf
  // eines der drei Gesichter genügt. Wer mehr sagen will, tippt die Frage an oder nimmt
  // „Bewerten" unten; wer nichts sagen will, tippt ✕ und wird nie wieder gefragt.
  const sections = [
    abgesagtHinweis,
    abgesagt ? '' : bewertungsZeile(ctx, meet, { titel: false }),
  ...
  return {
    ...
    bind: (root) => {
      ...
      bindActions(root, {
        ...
        // R6 D6: die drei Gesichter, „mehr" und das Wegtippen der Frage.
