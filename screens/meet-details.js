...
import {
  ...
  meetPanelReihe, vorschlagTitel, zusageReihe, teilnehmendeKarte, modulSheet,
  BEWERTUNG_STUFEN, BEWERTUNG_TON, gesichtSvg, bewertungMoeglich,
} from '../ui/meet-panels.js';
// Runde 11 (B10): Unter welchem Namen nach einem Meet gefragt wird (Titel, sonst Ort) — eine Regel der Daten.
import { meetBewertungsTitel } from '../data/projections.js';
// Runde 8 (R8-27): Die Seite „Anreise" (Route 'meet.anreise') — gebaut in ui/mitfahren.js.
import { renderAnreise } from '../ui/mitfahren.js';
// R6 D6: BEWERTUNG_STUFEN/BEWERTUNG_TON/gesichtSvg kommen aus dem gemeinsamen Baustein —
// die Bewertungsseite und die neue Bewertungs-Zeile am Meet sprechen dieselbe Sprache.
import { renderDiscoverCore, bindDiscoverCore, chooserScaffold, fitChooserInsets } from './new-meet.js';
// Runde 8 (R8-6): Nach „Meet senden" landet man HIER (new-meet.js publishDraftAction →
// resetTo('meet.details')). Waren genau die Leute einer Gruppe eingeladen, steht hier einmal die
// Frage „Welcher Chat?" — dieselbe Bauform wie auf der Crew-Seite (crew.js chatWahl*).
import { chatWahlHtml, chatWahlAktionen } from './crew.js';
import {
...
// R6 D6: Stufen, Farbtöne und Gesichter stehen jetzt im gemeinsamen Baustein
// (ui/meet-panels.js) — die Bewertungsseite UND die neue Bewertungs-Zeile am Meet
// sprechen damit nachweislich dieselbe Sprache.
const STUFEN = BEWERTUNG_STUFEN;
...
const REVIEW_TONE = BEWERTUNG_TON;
...
<span style="font-size:12.5px;font-weight:650;color:var(--green-dark);font-variant-numeric:tabular-nums;min-width:14px;text-align:right;pointer-events:none">${originalVotes}</span>
  ...
  for (const variant of variants) {
    ...
    if (kind === 'time') {
      ...
      // Runde 8 (R8-26): Ein Vorschlag mit eigenem Ort nennt ihn („Kochabend · Strandbad Lochau").
      const titel = esc(vorschlagTitel(variant, meet));
      label = variant.votes.length > 1
        ? `${titel} · ${variant.votes.length}×`
        : (own
          ? t('{titel} · von dir', { titel })
          : t('{titel} · von {name}', { titel, name: esc(authorName) }));
    }
    ...
    // Runde 8 (gemessen, seit Vorschläge aus dem Chat oft EIGENE sind): Der Stift stand als Knopf
    // IM Abstimm-Knopf — ungültiges HTML. Der Browser schloss den äußeren Knopf vor dem inneren,
    // die Stimmenzahl rutschte unter den Balken. Jetzt liegt er daneben in derselben Zeile, genau
    // über dem rechten Ende des Balkens (Zahl 14 px + Abstand 9 px + 5 px Rand = 28 px).
    const pencilButton = own
      ? `<button data-act="edit-variant" data-variant="${esc(variant.id)}" data-kind="${kind}" aria-label="${esc(t('Bearbeiten'))}" style="position:absolute;right:28px;top:5px;width:24px;height:24px;border-radius:50%;background:var(--surface);border:1px solid var(--ink-a12);display:flex;align-items:center;justify-content:center;cursor:pointer;appearance:none;padding:0;z-index:5"><span style="pointer-events:none;display:flex">${pencilIcon('var(--ink)', 11)}</span></button>`
      : '';
...
//
// Runde 7 (E7), Welle 2: Hier stand der Beweis, dass der neue Weg niemandem nützt — dieser
// Bildschirm kannte createDraft UND updateDraft UND die innere Form eines Entwurfs (idea.source,
// place.mode), nur um eine Vorlage zu öffnen. Drei Wahrheiten über den Aufbau eines Entwurfs,
// gepflegt in einer Datei, die davon nichts wissen müsste. Jetzt ist es EIN nav.go: Der
// Zusammenhang steht in der Route, den Rest baut der Erstellen-Bereich (new-meet.js
// › kontextAusRoute). Das ist zugleich der Weg, den Karte und Kalender künftig gehen.
//
// Eine Vorlage nimmt ALLES mit, was wieder gilt — Idee, ZEICHEN, Ort UND die Notiz („Turnbeutel
// nicht vergessen"). Datum und Uhrzeit bleiben bewusst draußen: sie sind das einzige, was beim
// nächsten Mal sicher anders ist.
function alsVorlageNutzen(ctx, meet) {
  const { nav, ui } = ctx;
  ui.menu = false;
  const mit = meet.crewId
    ? { crewId: meet.crewId }
    : { personIds: (meet.personIds || []).filter((id) => id !== ME) };
  const ort = meet.place?.name
    ? {
      placeName: meet.place.name,
      placeAddress: meet.place.address || '',
      ...(typeof meet.place.lat === 'number' ? { lat: meet.place.lat, lon: meet.place.lon } : {}),
    }
    : {};
  nav.go('newMeet.discover', {
    ...mit,
    ...ort,
    ideaTitle: meet.title,
    ...(meet.category ? { ideaCategory: meet.category } : {}),
    // Runde 7, Welle 3 (Prüferfund): Das Zeichen des alten Meets ging verloren — eine Vorlage,
    // die das Zeichen verliert, ist keine Vorlage. Es kommt mit — und zwar in BEIDEN Formen,
    // getrennt, so wie das Meet sie hält: `icon` das Sinnbild, `iconKey` der Katalogschlüssel.
    // (Die erste Fassung schickte `meet.iconKey || meet.icon` in EINEM Feld; damit stand im
    // Entwurf „boots", wo am Vorbild „⛰️" steht — eine Vorlage, die ein anderes Zeichen erzeugt
    // als ihr Vorbild, ist keine Vorlage. Siehe zeichenAusRoute in screens/new-meet.js.)
    ...(meet.icon ? { ideaIcon: meet.icon } : {}),
    ...(meet.iconKey ? { ideaIconKey: meet.iconKey } : {}),
    ...(meet.note ? { note: meet.note } : {}),
  });
}
...
function detailsMenu(ctx, meet) {
  ...
  // Runde 11 (A8): „Teilnehmende bearbeiten" steht im „···", nicht mehr als Zeile unter der Teilnahme.
  if (darfTeilnehmerBearbeiten(meet)) items.push({ act: 'menu-participants', icon: peopleIcon('var(--ink)', 16), label: t('Teilnehmende bearbeiten') });
...
function renderMeetDetails(ctx) {
  ...
  // Runde 11 (B2, D2): EIN System — Kopf (mit Foto ODER Kartenvorschau, nie beides) → Zusage-Reihe →
  // Teilnehmende → nur die hinzugefügten Module → „+ Hinzufügen" (alles in meetPanelReihe).
  const map = meetOrtKarte(ctx, meet, { kartenHoehe: hasVariants ? 132 : 150, ohneKarte: Boolean(meetFoto(ctx.repo, meet)) });
  const zusage = zusageReihe(ctx, meet);
  const teilnehmende = teilnehmendeKarte(ctx, meet);
  // Runde 8 (R8-24): „Ich bin da" ist weg (Jonathan: „‚Ich bin da' weg.").
  const sections = `${activityCard}${timeCard}${map}${zusage}${teilnehmende}${panels}`;
...
<div style="display:flex;flex-direction:column;gap:9px;padding:12px 20px 0">${sections}</div>
  ...
  const html = screenScaffold({
    ...
    body,
    // Runde 11: Der Chat eines ABGESAGTEN Meets wird nicht mehr geführt (er wäre leer) — dann
    // gibt es auch den Einstieg „Chat öffnen" nicht.
    bottom: meet.cancelled ? '' : detailBottom(),
    bottomInset: meet.cancelled ? 24 : CTA_INSET,
    bottomFadeHeight: meet.cancelled ? 22 : 118,
    ...
    // Routenwechsel aus.
    overlays: `${ui.menu ? detailsMenu(ctx, meet) : ''}${ui.pencilSheet ? pencilSheet() : ''}${ui.loopSheet ? loopSheet(ctx, meet) : ''}${participantsSheet(ctx, meet)}${modulSheet(ctx, meet)}${variantPickSheet(ctx, meet)}${ui.confirm ? confirmSheet(ui.confirm) : ''}${discardSheet(ctx)}${chatWahlHtml(ctx)}`,
...
//   2. Nicht gut · Mittel · Gut als drei gleich große Kacheln, darunter „Hat nicht stattgefunden".
//   3. Erst NACH dem Tipp auf eine Stufe: „Woran lag’s genau?" — Aktivität, Ort, Kosten, je
//      dreifach, freiwillig. Bei „Hat nicht stattgefunden" steht an GENAU dieser Stelle und in
//      derselben Höhe die Nachfrage. Vorher ist die Stelle leer (Runde 11, D10 e).
//   Unten „Fertig" — erst nach der Wahl da. Eine Notiz gibt es nicht mehr (Runde 11, D10 e).
// Nichts klappt mehr zu: In R3 schrumpfte die Skala nach der Wahl zu einer Zeile, und alles
// darunter rückte nach oben. Jetzt bleibt jede Fläche an ihrem Platz — auch die Stelle der
// freiwilligen Angaben ist von Anfang an reserviert; die Wahl erkennt man an Fläche und Gesicht.
// Die Abstände wachsen auf großen Geräten mit (luft), auf kleinen gehen sie bis auf ihr Mindestmaß
// zurück. Jede Wahl ist sofort gespeichert; „Fertig" schließt nur.
...
// Ein Gesicht je Stufe (froh · neutral · traurig) kommt aus dem gemeinsamen Baustein
// (ui/meet-panels.js gesichtSvg) — dieselben Züge auf der Bewertungsseite und in der
// Bewertungs-Zeile am Meet.
...
<span data-role="review-titel" style="font-size:15px;font-weight:650;line-height:19px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(meetBewertungsTitel(meet))}</span>
<span data-role="review-meta" style="font-size:12px;line-height:16px;color:var(--ink-soft);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-variant-numeric:tabular-nums">${esc([wann, ort].filter(Boolean).join(' · '))}</span>
</span>
...
// Ein ECHTER Knopf, grün. Runde 11 (D10 e): Er steht erst da, wenn eine Stufe gewählt ist — vorher
// gäbe es nichts abzuschließen, und ein graues Feld wäre nur Lärm.
function fertigKnopf() {
  const haken = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" style="pointer-events:none;flex:none">${reviewCheckPath('var(--on-accent)', 2.6)}</svg>`;
  return `<button data-act="review-fertig" style="pointer-events:auto;width:100%;display:flex;align-items:center;justify-content:center;gap:8px;background:var(--green);color:var(--on-accent);border:1.5px solid var(--green-dark);box-shadow:0 6px 18px var(--green-a26);font:650 15px/1 ${FONT};padding:16px 0;border-radius:999px;box-sizing:border-box;cursor:pointer;appearance:none">${haken}<span style="pointer-events:none">${t('Fertig')}</span></button>`;
}
...
function renderMeetReview(ctx) {
  ...
  // Runde 11 (D10 a/b): Bewertet wird nur, wo ich dabei war (Meet vorbei, nicht abgesagt, klarer
  // Inhalt). Eine schon vorhandene Bewertung bleibt les- und änderbar.
  if (!stand.verdict && !bewertungMoeglich(meet)) return missingScreen();
  const gewaehlt = Boolean(stand.verdict);
  // Runde 11 (D10 e): Zuerst nur die drei Stufen; erst nach dem Tipp erscheinen die freiwilligen
  // Angaben — an einer Stelle, die von Anfang an reserviert ist, damit nichts springt.
  const weiter = !gewaehlt ? '' : (stand.verdict === NICHT_STATT ? nichtStattBlock(stand) : themenBlock(stand.themen));
  ...
  // und Nachfrage tauschen sich darin, ohne dass etwas darunter oder darüber wandert.
...
<div data-role="review-weiter" style="flex:2 1 0;min-height:${WEITER_MIN}px;max-height:${WEITER_MAX}px;display:flex;flex-direction:column">${weiter}</div>
  ...
  const html = screenScaffold({
    ...
    body,
    bottom: gewaehlt ? bottomLayer(fertigKnopf()) : '',
...
function bindMeetReview(root, ctx, meet) {
  ...
  // Jede Wahl ist sofort gespeichert. Eine ältere Notiz (bis Runde 10 gab es ein Feld dafür) geht
  // unverändert mit — nur neu schreiben lässt sie sich nicht mehr.
  const speichern = (stand) => repo.submitReview(meet.id, bewertungSchreiben(stand));
  ...
  bindActions(root, {
    ...
    'review-thema': (data) => {
      const stand = aktuell();
      if (!stand.verdict || stand.verdict === NICHT_STATT) return;
      rueckmeldung('auswahl');
    ...
    'review-fertig': () => {
      // Der Knopf steht erst nach der Wahl da; gespeichert ist längst alles.
...
function renderRueckblick(ctx, meet) {
  ...
  const abgesagtHinweis = abgesagt
    ...
    : '';
  // Runde 11 (B10, D10): Die kleine Gesichter-Zeile über der Karte ist weg. Bewertet wird nur, wo ich
  // dabei war (bewertungMoeglich); dann führt EIN Knopf unten zur Bewertung (drei große Stufen).
  const moeglich = bewertungMoeglich(meet);
  const sections = [
    abgesagtHinweis,
...
<div style="display:flex;flex-direction:column;gap:9px;padding:12px 20px 0">${sections}</div>
  ...
  const knopf = abgesagt || !(moeglich || bewertet) ? '' : rueckblickKnopf(bewertet);
  const html = screenScaffold({
    header: headerRow(Boolean(ui.menu)),
    body,
...
export const meetDetailScreens = {
  ...
  // Runde 8 (R8-27): „Anreise planen" — aus den Meet-Details und aus dem Raum.
  'meet.anreise': renderAnreise,
};
