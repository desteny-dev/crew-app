...
// Runde 6 (B1): der kurze Einstieg, hier von Hand wiederholbar.
import { guideStarten } from '../ui/guide.js';
// N3/N5: Teilen nur über die ehrliche Tabelle der Hülle (kann('teilen')) — nie direkt navigator.share.
import { istHuelle, kann, huellePlugin, erlaubnisFragen, huelleLage, wacheAnfragen } from '../core/native.js';
import {
  ...
  specialAvatarMark, avatarFlaeche, MASS, avatarMasse,
  dirtyGuard, discardSheet, discardActions, leerZeile,
  radSpalte, bindRad, RAD_ZEILE,
  tabHeader, MARKER_COLORS,
} from '../ui/components.js';
// Runde 8 (R8-61): kreisPunkt kommt aus dem Paket form — gelesen über den Namensraum. Ein benannter
// Import, den es dort (noch) nicht gibt, bräche das ganze Modul; so lädt es in jedem Stand.
import * as bausteine from '../ui/components.js';
import { backArrow, plus, pencil, chevronRight } from '../ui/icons.js';
// Runde 7 (C3): Das Bild groß ansehen ist EIN Baustein fuer die ganze App — der Raum benutzt
// ihn seit Runde 5, die Freundesliste jetzt auch. Keine zweite Groß-Ansicht im Profil.
import { bildGrossHtml, bildGrossBinden, bildGrossAktionen, bildGrossInZeilen } from '../ui/bild-gross.js';
// Runde 8 (R8-11): Eine neue Anfrage erscheint EINMAL groß oben und schrumpft dann zum orangen
// Punkt am Freunde-Knopf — derselbe Baustein wie in der Crew-Liste, kein eigener Nachbau.
import { anfrageHinweisHtml, anfrageHinweisGesehen, anfrageHinweisIds, anfrageHinweisBinden, anfrageHinweisBeenden } from '../ui/anfragen-hinweis.js';
...
// Runde 4: Zeichen nach Kategorien (B7) · Keine Zeit, seit Runde 8 „Busy" (B5) · ein Zeichen für alle besten Freunde (B4).
import { zeichenAuswahlHtml, zeichenAuswahlBinden } from '../ui/zeichen-auswahl.js';
import { belegtBereinigen, tageText, tagKurz, woche, WOCHE_AB_MONTAG } from '../core/belegt.js';
// Runde 7 (H1): Die Regel, WANN „frei“ zurueckgesetzt wird, steht genau einmal — in
// data/projections.js. Der Bildschirm liest sie, um die Vorschau zu zeigen, und rechnet sie nicht nach.
import { naechsteFreiGrenze, meetSpanne, MEET_OHNE_ENDE_MIN, ZUSAMMEN_TAGE } from '../data/projections.js';
...
// Runde 7, Welle 5 (Regel 3): der abgelegte Einladungscode — EINE Lesart, dieselbe wie in app.js.
import { einladungLesen, einladungWeg, einladungCodesFuer, einladungKonto } from '../core/einladung.js';
import { weekdayShort, fromISODate, dayOffset, formatMeetDate } from '../core/dates.js';
...
// Runde 6 (A5, gemessen): qrMatrix baut ACHT vollständige Matrizen und bewertet jede einzeln.
// Das kostete hier 0,6 ms, mit sechsfacher Bremse 4,6 ms — und lief bei JEDEM Neuzeichnen des
// Sheets neu, obwohl Text und Größe dieselben blieben. Der Code ist eine reine Funktion seiner
// zwei Eingaben; gemerkt wird deshalb genau das letzte Ergebnis.
let qrGemerkt = { text: null, size: 0, svg: '' };

function qrSvg(text, sizePx) {
  if (qrGemerkt.text === text && qrGemerkt.size === sizePx) return qrGemerkt.svg;
  ...
  const svg = `<svg data-qr="1" width="${sizePx}" height="${sizePx}" viewBox="${-QR_QUIET} ${-QR_QUIET} ${span} ${span}" fill="${QR_TINTE}" shape-rendering="geometricPrecision">`
    + QR_MARKE_DUNKEL
    + `<rect x="${-QR_QUIET}" y="${-QR_QUIET}" width="${span}" height="${span}" rx="2.6" fill="${QR_GRUND}"></rect>`
    + `<g>${modules}</g><g>${finders}</g></svg>`;
  qrGemerkt = { text, size: sizePx, svg };
  return svg;
}
...
const svgStarGold = (size = 16) => `<svg width="${size}" height="${size}" viewBox="0 0 24 24" style="flex:none;pointer-events:none"><path d="m12 3.5 2.6 5.3 5.9.9-4.3 4.1 1 5.9L12 16.9l-5.2 2.8 1-5.9-4.3-4.1 5.9-.9L12 3.5Z" fill="#E1B72E"></path></svg>`;
...
function v7DefaultsSchreiben(repo) {
  ...
  // Hier stand `location: { …, use: false, … }`. Diese Migration läuft bei JEDEM Konto, das
  // noch kein `v7Defaults` trägt — also bei jedem NEUEN —, und zwar beim ersten Öffnen des
  // Profils. Sie schaltete damit „Standort verwenden" still wieder aus, nachdem Runde 10 es
  // gerade auf an gestellt hatte (app.js › standardEinstellungen): Die gemerkte Lage wurde
  // gelöscht, die Wache beendet, am Server sogar die Zeile in `standorte`. Danach stand auf der
  // Karte das Zuhause und bewegte sich nie mehr — und weil `standardAn` schon gesetzt war,
  // stellte es auch kein zweiter Start wieder her.
  // Über `use` entscheidet jetzt genau EINE Stelle: der Mensch, sonst der Standard aus Runde 10.
  // Diese Migration ergänzt nur noch, was ganz FEHLT — und schreibt `location` gar nicht, wenn
  // nichts fehlt (ein Schreiben wäre nicht folgenlos: es weckt die Standort-Wache).
  const lage = settings.location || {};
  const fehlt = {};
  if (lage.use === undefined) fehlt.use = false;
  if (lage.shareMode === undefined) fehlt.shareMode = 'alle';
  if (!Array.isArray(lage.shareIds)) fehlt.shareIds = [];
  projiziereFreigaben(repo, {
    v7Defaults: true,
    activityStatus: 'zeigen',
    visibility: { ...(settings.visibility || {}), unknownProfile: 'profil' },
    ...(Object.keys(fehlt).length ? { location: { ...lage, ...fehlt } } : {}),
    interests,
...
function stackAvatar(person, options = {}) {
  ...
  // Runde 9: kein grüner Trenner mehr für „im Meet" — dieselbe Scheibe für alle.
  const separator = '<span style="position:absolute;inset:-2px;border-radius:50%;background:var(--surface);z-index:0"></span>';
...
// --- Runde 4 (B5) → Runde 8 (R8-39): Busy ------------------------------------------------
// Jonathan (Runde 4): „Montags bis donnerstags von x bis y und am Freitag x bis y … diese Uhrzeiten
// hab ich Arbeit." Mehrere Zeitfenster, jede Woche wieder (settings.belegt, web/core/belegt.js).
//
// Runde 8 (R8-39, Jonathan): „Busy: Titel ‚Busy', rechts ‚+', nur der Kalender 0–24 Uhr
// (Beschriftung alle 6 h, etwas höher), keine Liste, keine Sichtbarkeits-Einstellung. Freunde sehen
// die Titel; Schloss für private Einträge (wie bei Ressourcen) = nur ‚busy'. Eine Farbe. Busy sperrt
// nichts. Dasselbe in den Profilen anderer."
//   · Die Karte IST der Kalender: keine Zeilen darunter, kein „bis 17:00", kein Satz, wer ihn
//     sieht, kein Weg zu einer Sammelseite. Die Wochen der Freunde stehen in IHREM Profil
//     (busyKarte — dieselbe Bauform, nur lesend). Die Sammelseite „Wann wer keine Zeit hat" ist
//     damit entfallen: Sie war der einzige Weg zu fremden Wochen, und genau den gibt es jetzt dort,
//     wo man nach einer Person sieht.
//   · Jeder Eintrag darf einen Titel tragen („Arbeit"), und den sehen Freunde. Das Schloss macht ihn
//     privat: Freunde sehen dann nur „Busy". Eine Sichtbarkeits-Einstellung gibt es nicht mehr —
//     Busy ist für Freunde immer sichtbar (Datenvertrag des Pakets daten: getBelegtWoche liefert
//     jeden Block mit titel und privat).
//   · EINE Farbe für jeden Block — auch für private und für heute. Grün heißt in dieser App „frei"
//     und kommt im Kalender gar nicht vor; heute erkennt man an der kräftigen Tagesbeschriftung.
//   · Busy sperrt nichts: Wer Busy hat, kann trotzdem FREE drücken, eingeladen werden und selbst
//     einladen. Es ist eine Auskunft, kein Riegel (nachgewiesen in scratch/r8b-profil.mjs).
...
// Der Kalender: 0–24 Uhr, beschriftet alle 6 Stunden. „Etwas höher als heute": 56 → 132 px — hoch
// genug, dass in einem Arbeitstag ein Titel lesbar steht, und immer noch eine Karte, keine Seite.
const BUSY_HOEHE = 132;
const BUSY_MARKEN = [0, 6, 12, 18, 24];
// Die Tagesbeschriftung über jeder Spalte (Zeile + Abstand) — die Stundenachse beginnt darunter.
const BUSY_KOPF = 11;
const BUSY_KOPF_ABSTAND = 6;
// EINE Farbe für jeden Block: ruhige Tinte. Der Titel steht in var(--ink) darauf — in beiden
// Themen deutlich über 4,5 : 1 (hell ≈ 10 : 1, dunkel ≈ 5,6 : 1).
// Runde 9 (Jonathan: „bessere Füllfarbe"): ein ruhiges Schieferblau statt Grau — weder Grün (frei)
// noch Orange (Mitteilung). Der Titel in var(--ink) bleibt in beiden Themen deutlich lesbar.
const BUSY_TON = '#5b6fd6';
const BUSY_FARBE = 'rgba(91, 111, 214, .36)';
// Dieselbe Farbe deckend gemischt — sonst schienen die Stundenlinien durch den Block. Browser ohne
// color-mix nehmen die Zeile davor (durchscheinend, aber dieselbe Farbe).
const BUSY_FARBE_DECKEND = `color-mix(in srgb, ${BUSY_TON} 36%, var(--field))`;

// Runde 9: Die Jetzt-Linie wie in der Meet-Woche — grüne Linie mit Punkt in der Spalte von heute.
// Sie wandert live mit (einmal pro Minute), ohne dass die Seite neu gezeichnet wird.
const jetztProzent = () => { const d = new Date(); return ((d.getHours() * 60 + d.getMinutes()) / 1440) * 100; };
function busyJetztHtml() {
  return `<span data-role="busy-jetzt" aria-hidden="true" style="position:absolute;left:0;right:0;top:${jetztProzent().toFixed(2)}%;height:0;border-top:2px solid var(--green);margin-top:-1px;z-index:3;pointer-events:none"><span style="position:absolute;left:0;top:-5px;width:8px;height:8px;border-radius:50%;background:var(--green)"></span></span>`;
}
if (typeof document !== 'undefined') {
  setInterval(() => {
    const oben = `${jetztProzent().toFixed(2)}%`;
    document.querySelectorAll('[data-role="busy-jetzt"]').forEach((el) => { el.style.top = oben; });
  }, 60000);
}
// Höhe einer Titelzeile im Block — darunter passt kein Wort, dann bleibt der Block ohne Text.
const BUSY_ZEILE = 11;

const svgBusySchloss = `<svg width="8" height="8" viewBox="0 0 24 24" fill="none" aria-hidden="true" style="display:block;pointer-events:none"><rect x="4.5" y="10.5" width="15" height="10" rx="2.6" fill="currentColor"></rect><path d="M8 10.5V8a4 4 0 0 1 8 0v2.5" stroke="currentColor" stroke-width="2.6"></path></svg>`;

function busyZeit(min) {
  const m = Math.max(0, Math.min(1440, Math.round(min)));
  return `${String(Math.floor(m / 60) % 24).padStart(2, '0')}:${String(m % 60).padStart(2, '0')}`;
}

// Was in einem Block steht. In der eigenen Woche: der Titel (ohne Titel bleibt der Block still —
// man weiß selbst, was er ist). Bei anderen: der Titel — oder „Busy", wenn der Eintrag privat ist
// oder keinen Titel hat.
function busyText(block, eigen) {
  const titel = String(block?.titel || '').trim();
  if (eigen) return titel;
  return block?.privat || !titel ? tx('Busy') : titel;
}

// Ein Tag als Satz — für Vorlesestimme, Titel und Prüflauf.
function busyTagSatz(eintrag, eigen) {
  const bloecke = (eintrag.streifen || []).map((block) => {
    const text = busyText(block, eigen) || tx('Busy');
    return `${text} ${busyZeit(block.vonMin)}–${busyZeit(block.bisMin)}`;
  });
  return bloecke.length ? `${tagKurz(eintrag.tag)} · ${bloecke.join(', ')}` : `${tagKurz(eintrag.tag)} · ${tx('frei')}`;
}

function busyBlockHtml(block, eigen, hoehe) {
  const text = busyText(block, eigen);
  const privatEigen = eigen && Boolean(block.privat);
  // „Freunde sehen die Titel" — auch bei einem kurzen Eintrag. Zwei Stunden Training wären 11 px
  // hoch, zu flach für ein Wort. Ein Block mit Text ist deshalb mindestens eine Zeile hoch (wie in
  // jedem Kalender: der kurze Termin wird etwas länger GEZEICHNET, damit sein Titel lesbar ist). Sein
  // Anfang bleibt dabei IMMER genau seine Uhrzeit — „wann“ ist die eigentliche Auskunft. Am Tagesende
  // wächst er nur bis Mitternacht; passt dort keine Zeile mehr hin (22–24 Uhr), bleibt er ohne Wort —
  // eine Nachtschicht trägt ihren Titel dann in der Spalte des nächsten Morgens.
  const obenPx = (block.vonMin / 1440) * hoehe;
  const echt = ((block.bisMin - block.vonMin) / 1440) * hoehe;
  const mindest = text || privatEigen ? BUSY_ZEILE + 4 : hoehe * 0.025;
  const hochPx = Math.min(hoehe - obenPx, Math.max(echt, mindest));
  const hoch = (hochPx / hoehe) * 100;
  const oben = (obenPx / hoehe) * 100;
  const platz = hochPx - 4;
  const passt = platz >= BUSY_ZEILE - 1;
  // Das Schloss (nur in der EIGENEN Woche: welche Einträge sehen Freunde nur als „Busy"?) steht als
  // eigene, schmale Zeile über dem Titel — so bleibt dem Wort die ganze Spaltenbreite. Ist dafür
  // kein Platz, steht es vorne in der Titelzeile.
  const schlossZeile = privatEigen && platz >= 9 + BUSY_ZEILE ? 9 : 0;
  const zeilen = passt ? Math.max(1, Math.min(3, Math.floor((platz - schlossZeile) / BUSY_ZEILE))) : 0;
  const schloss = schlossZeile ? `<span style="display:flex;justify-content:center;height:${schlossZeile}px;color:var(--ink);pointer-events:none">${svgBusySchloss}</span>` : '';
  const schlossVorne = privatEigen && !schlossZeile ? `<span style="display:inline-block;vertical-align:-1px;margin-right:2px">${svgBusySchloss}</span>` : '';
  const titel = zeilen >= 1 && (text || schlossVorne)
    ? `<span style="display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:${zeilen};overflow:hidden;hyphens:auto;-webkit-hyphens:auto;overflow-wrap:break-word;text-align:center;font:650 9.5px/${BUSY_ZEILE}px ${FONT};letter-spacing:-.02em;color:var(--ink);pointer-events:none">${schlossVorne}${esc(text)}</span>`
    : '';
  const inhalt = schloss + titel;
  const index = Number.isInteger(block.indizes?.[0]) ? ` data-index="${block.indizes[0]}"` : '';
  // Der feine Rand in der Farbe des Grundes trennt zwei aneinanderstoßende Blöcke mit
  // verschiedenem Titel — ohne zweite Farbe.
  return `<span data-role="streifen-block" data-privat="${Boolean(block.privat)}"${index} style="position:absolute;left:0;right:0;top:${oben.toFixed(2)}%;height:${hoch.toFixed(2)}%;border-radius:5px;background:${BUSY_FARBE};background:${BUSY_FARBE_DECKEND};box-shadow:0 0 0 1px var(--field);overflow:hidden;display:flex;flex-direction:column;align-items:stretch;padding:${inhalt ? 2 : 0}px 1px 0;box-sizing:border-box;pointer-events:none">${inhalt}</span>`;
}

// tage: [{ tag, streifen:[{ vonMin, bisMin, titel?, privat?, indizes? }] }] — die Form aus
// repo.getBelegtWoche(). Ohne `act` ist der Kalender rein lesend: an der Woche einer Freundin gibt
// es nichts zu drehen. EINE Bauform für eigene und fremde Woche.
export function busyKalender(tage, optionen = {}) {
  const { act = null, eigen = false, heute = new Date().getDay(), rolle = 'wochenstreifen-lesend', hoehe = BUSY_HOEHE, blockHtml = busyBlockHtml, tagSatz = busyTagSatz } = optionen;
  const achse = `<div data-role="busy-achse" aria-hidden="true" style="position:relative;flex:none;width:17px;margin-top:${BUSY_KOPF + BUSY_KOPF_ABSTAND}px;height:${hoehe}px">${BUSY_MARKEN.map((stunde) => `<span data-role="busy-stunde" style="position:absolute;right:1px;top:${((stunde / 24) * 100).toFixed(2)}%;transform:translateY(-50%);font:600 9.5px/1 ${FONT};color:var(--muted-light);font-variant-numeric:tabular-nums;pointer-events:none">${stunde}</span>`).join('')}</div>`;
  const linien = [6, 12, 18].map((stunde) => `<span aria-hidden="true" style="position:absolute;left:0;right:0;top:${((stunde / 24) * 100).toFixed(2)}%;height:1px;background:var(--ink-a08)"></span>`).join('');
  const spalten = (tage || []).map((eintrag, index) => {
    const istHeute = eintrag.tag === heute;
    const bloecke = (eintrag.streifen || []).map((block) => blockHtml(block, eigen, hoehe)).join('');
    const satz = tagSatz(eintrag, eigen);
    const inneres = `<span data-role="streifen-kopf" style="display:block;height:${BUSY_KOPF}px;font:${istHeute ? 750 : 650} 10.5px/${BUSY_KOPF}px ${FONT};color:${istHeute ? 'var(--ink)' : 'var(--muted-light)'};text-align:center;pointer-events:none">${esc(tagKurz(eintrag.tag))}</span>
<span data-role="streifen-flaeche" style="position:relative;display:block;height:${hoehe}px;margin-top:${BUSY_KOPF_ABSTAND}px;border-radius:7px;background:var(--field);overflow:hidden;pointer-events:none">${linien}${bloecke}${istHeute ? busyJetztHtml() : ''}</span>`;
    const merkmale = `data-role="streifen-tag" data-tag="${eintrag.tag}" data-index="${index}" data-bloecke="${(eintrag.streifen || []).length}"${istHeute ? ' data-heute="true"' : ''}`;
    if (!act) return `<div ${merkmale} aria-label="${esc(satz)}" title="${esc(satz)}" style="flex:1 1 0;min-width:0">${inneres}</div>`;
    return `<button data-act="${act}" ${merkmale} aria-label="${esc(satz)}" title="${esc(satz)}" style="flex:1 1 0;min-width:0;border:0;background:transparent;padding:0;cursor:pointer;appearance:none;font-family:${FONT};touch-action:manipulation">${inneres}</button>`;
  }).join('');
  return `<div data-role="${esc(rolle)}" style="display:flex;gap:3px;align-items:flex-start;padding-bottom:7px">${achse}${spalten}</div>`;
}

// Die eigenen Einträge — so, wie der Vertrag sie liefert (repo.getBelegt, mit titel und privat).
function eigeneBusyListe(repo) {
  if (typeof repo.getBelegt === 'function') return repo.getBelegt() || [];
  return belegtBereinigen(repo.getSettings().belegt);
}

// Die eigene Woche kommt aus dem VERTRAG: repo.getBelegtWoche() liefert jeden Block mit Titel,
// Schloss und `indizes` (welche Fenster in ihm stecken — projections.js busyWoche, für beide
// Gateways gleich). Aneinanderstoßende Blöcke mit gleichem Titel sind dort schon EIN Block (08–12
// und 12–17 „Arbeit"), verschiedene Titel bleiben zwei. Nur eine ältere Datenschicht ohne `indizes`
// wird hier nachgebaut — mit derselben Rechnung (core/belegt.js woche, Fenster für Fenster).
function eigeneBusyTage(repo) {
  let sicht = null;
  try { sicht = repo.getBelegtWoche?.() || null; } catch { sicht = null; }
  const tageVertrag = sicht?.tage || [];
  if (tageVertrag.length === 7 && tageVertrag.every((tag) => (tag.streifen || []).every((block) => Array.isArray(block.indizes)))) return tageVertrag;
  const liste = eigeneBusyListe(repo);
  const tage = WOCHE_AB_MONTAG.map((tag) => ({ tag, streifen: [] }));
  liste.forEach((fenster, index) => {
    woche([fenster]).forEach((tagEintrag, i) => {
      for (const block of tagEintrag.streifen || []) {
        tage[i].streifen.push({ vonMin: block.vonMin, bisMin: block.bisMin, titel: String(fenster.titel || '').trim() || null, privat: Boolean(fenster.privat), indizes: [index] });
      }
    });
  });
  for (const tag of tage) {
    tag.streifen.sort((a, b) => a.vonMin - b.vonMin);
    const zusammen = [];
    for (const block of tag.streifen) {
      ...
      if (letzter && block.vonMin <= letzter.bisMin && letzter.titel === block.titel && letzter.privat === block.privat) {
        ...
        letzter.indizes.push(...block.indizes);
      } else {
        zusammen.push({ ...block, indizes: [...block.indizes] });
      }
    }
    tag.streifen = zusammen;
  }
  return tage;
}

// Welcher Eintrag liegt an diesem Tag um diese Minute? Beantwortet aus DENSELBEN Blöcken, die der
// Kalender zeichnet. Kleine Blöcke (eine Stunde sind 5,5 px) trifft kein Daumen genau — deshalb
// zählt auch ein Block bis 45 Minuten daneben. Steckt in einem Block mehr als ein Fenster (08–12 und
// 12–17 „Arbeit"), öffnet sich das, das die getippte Minute wirklich deckt. `minute` null: der
// erste Block des Tages.
function busyEintragBei(repo, tagIndex, minute) {
  const tag = eigeneBusyTage(repo)[tagIndex];
  let treffer = null;
  let abstand = Infinity;
  for (const block of tag?.streifen || []) {
    const weg = minute === null ? 0 : (minute < block.vonMin ? block.vonMin - minute : (minute > block.bisMin ? minute - block.bisMin : 0));
    if (weg < abstand) { abstand = weg; treffer = block; }
  }
  if (!treffer || abstand > 45) return -1;
  const indizes = treffer.indizes || [];
  if (indizes.length > 1 && minute !== null) {
    const liste = eigeneBusyListe(repo);
    const deckt = indizes.find((index) => (woche([liste[index]])[tagIndex]?.streifen || []).some((s) => s.vonMin <= minute && minute < s.bisMin));
    if (deckt !== undefined) return deckt;
  }
  return Number.isInteger(indizes[0]) ? indizes[0] : -1;
}

// Wer sieht was — EIN Satz im Blatt, der sich beim Tippen und beim Schloss mitändert.
function busySichtSatz(entwurf) {
  const titel = String(entwurf?.titel || '').trim();
  if (entwurf?.privat) return tx('Jede Woche. Freunde sehen nur „Busy“.');
  if (!titel) return tx('Jede Woche. Freunde sehen „Busy“.');
  return tx('Jede Woche. Freunde sehen „{titel}“.', { titel });
}

// Die eigene Karte im Profil: „Busy", rechts das „+", darunter NUR der Kalender. Auch leer steht er
// da — ein Tipp in eine Spalte trägt genau dort ein.
function belegtBox(ctx) {
  const tage = eigeneBusyTage(ctx.repo);
  return `<div data-role="belegt-box" style="flex:none;background:var(--surface);border:1.5px solid var(--ink-a09);border-radius:20px;padding:2px 16px 6px;box-shadow:0 1px 2px var(--shadow-05)">
<div style="display:flex;align-items:center;gap:8px;padding:8px 0 6px;min-height:30px;box-sizing:border-box"><span data-role="busy-titel" style="font-size:11px;font-weight:650;letter-spacing:.09em;text-transform:uppercase;color:var(--muted-light);flex:1;min-width:0">${tx('Busy')}</span>${addRoundButton('belegt-add', tx('Zeiten hinzufügen'))}</div>
<div data-role="belegt-woche" style="padding-top:8px;border-top:1px solid var(--ink-a05)">${busyKalender(tage, { act: 'belegt-tag-open', eigen: true, rolle: 'wochenstreifen-eigen' })}</div>
</div>`;
}

// --- Runde 11 (B4, D5): das Profil einer ANDEREN Person — die Karte „Zusammen" ------------------
// Jonathan (D5): „Die Karte heißt ‚Zusammen' (nicht Busy), Schalter Kalender | Liste. Gemeinsame Meets
// farbig mit Titel; fremde Meets und Busy der Person immer als graue Blöcke ohne Titel; leer ist leer."
// Das ändert R8-39 („Freunde sehen die Titel"): Der Titel eines Busy-Eintrags erscheint hier gar nicht
// mehr, und ein Meet zeigt seinen Titel nur, wenn ICH selbst darin bin. Das eigene Profil bleibt „Busy"
// (belegtBox) — die Regel gilt nur für den Blick auf andere.
//   · Sieben Tage ab heute (nicht Mo–So): wer eine Person sieht, will wissen, wann sie in nächster Zeit
//     kann. Das wöchentliche Busy-Muster (getBelegtWoche) wird dafür einfach auf die Tage gelegt.
//   · GEMEINSAM = die Person hat zugesagt UND ich bin im Meet (zugesagt oder unentschieden). Farbig, mit
//     Titel, antippbar (öffnet das Meet).
//   · GRAU = alles andere, was die Person bindet: ihr Busy, und Meets, in denen sie zugesagt hat, ich aber
//     nicht dabei bin. Kein Titel, kein Ort — nur „beschäftigt".
//   · Die Karte steht immer da, auch leer: leer ist leer, kein Platzhalter-Satz im Kalender.
//   · Meets kommen aus dem Vertrag (getMeets/getLoops mit context.personId) — also so, wie sie dem Gerät
//     vorliegen: im Server-Modus sind das die Meets, die MEINE Zugriffsregeln zeigen.
//   · Meets der Person, die ICH nicht sehen darf, liegen im Server-Modus gar nicht auf dem Gerät. Ihre
//     Zeit (nur Datum, Beginn, Ende) liefert getBelegtWoche(personId).meetZeiten (0041,
//     meet_zeiten_sicht) — sie werden grau. Eine Zeit, die schon als sichtbares Meet der Person im
//     Kalender liegt, kommt nicht ein zweites Mal (dieselbe meetSpanne, also auf die Minute gleich).
// Wie im Meet-Kalender: ohne Ende läuft ein Meet 4 h aus (eine Stunde fest, dann linear weg).
const ZUSAMMEN_OHNE_ENDE_MIN = MEET_OHNE_ENDE_MIN;
const ZUSAMMEN_OHNE_ENDE_KERN = 60;
// Ein Grau für alles Fremde. Deckend gemischt (die Stundenlinien scheinen nicht durch); Browser ohne
// color-mix nehmen die Zeile davor.
const ZUSAMMEN_GRAU = 'background:var(--ink-a14);background:color-mix(in srgb, var(--ink) 16%, var(--field))';
const ZUSAMMEN_GRUEN = 'background:var(--green-a16);background:color-mix(in srgb, var(--green) 26%, var(--field))';

// Beginn/Ende in Minuten — dieselbe Rechnung wie für meetZeiten (projections.js meetSpanne).
const zusammenSpanne = meetSpanne;

const zusammenPersonDabei = (antwort, meet, personId) => antwort === 'yes' || (antwort === undefined && meet.creatorId === personId);
const zusammenIchDabei = (antwort, meet) => antwort === 'yes' || antwort === 'open' || (antwort === undefined && meet.creatorId === ME);

// → [{ tag, iso, streifen:[{ art:'grau'|'meet', vonMin, bisMin, offen, titel?, meetId? }] }] — sieben Tage
// ab heute, in dieser Reihenfolge. Graue Blöcke stehen vor den farbigen (die farbigen liegen oben).
export function zusammenTage(repo, personId) {
  const tage = Array.from({ length: ZUSAMMEN_TAGE }, (_, i) => {
    const iso = dayOffset(i);
    return { tag: fromISODate(iso).getDay(), iso, streifen: [] };
  });
  let sicht = null;
  try { sicht = repo?.getBelegtWoche?.(personId) || null; } catch { sicht = null; }
  if (sicht && sicht.sichtbar !== false) {
    for (const eintrag of tage) {
      const tagVertrag = (sicht.tage || []).find((entry) => entry.tag === eintrag.tag);
      for (const block of tagVertrag?.streifen || []) eintrag.streifen.push({ art: 'grau', vonMin: block.vonMin, bisMin: block.bisMin, offen: false });
    }
  }
  // Welche Meet-Zeiten schon als Meet im Kalender liegen (Tag|Beginn|Ende) — die Liste aus dem Vertrag
  // unten legt dort keinen zweiten Block hin.
  const meetZeitGesehen = new Set();
  let meets = [];
  try { meets = repo?.getMeets?.({ context: { personId }, direction: 'upcoming' }) || []; } catch { meets = []; }
  for (const meet of meets) {
    if (meet.status === 'draft' || meet.cancelled) continue;
    if (!zusammenPersonDabei(meet.participation?.[personId], meet, personId)) continue;
    const eintrag = tage.find((entry) => entry.iso === meet.date);
    const spanne = eintrag ? zusammenSpanne(meet.time, meet.endTime) : null;
    if (!spanne) continue;
    const gemeinsam = zusammenIchDabei(meet.participation?.[ME], meet);
    meetZeitGesehen.add(`${eintrag.iso}|${spanne.vonMin}|${spanne.bisMin}`);
    eintrag.streifen.push(gemeinsam ? { art: 'meet', ...spanne, titel: meet.title, meetId: meet.id } : { art: 'grau', ...spanne });
  }
  // Ein Loop zählt an genau den Tagen, für die die Person „dabei" gesagt hat.
  let loops = [];
  try { loops = repo?.getLoops?.({ context: { personId } }) || []; } catch { loops = []; }
  for (const meet of loops) {
    for (const eintrag of tage) {
      if (meet.loop?.weekday !== eintrag.tag) continue;
      const antworten = meet.loop.responses?.[eintrag.iso] || {};
      if (antworten[personId] !== 'yes') continue;
      const spanne = zusammenSpanne(meet.loop.time || meet.time, meet.loop.endTime || meet.endTime);
      if (!spanne) continue;
      meetZeitGesehen.add(`${eintrag.iso}|${spanne.vonMin}|${spanne.bisMin}`);
      eintrag.streifen.push(antworten[ME] === 'yes' ? { art: 'meet', ...spanne, titel: meet.title, meetId: meet.id } : { art: 'grau', ...spanne });
    }
  }
  // Zeitfenster ihrer zugesagten Meets aus dem Vertrag — auch solcher, die ich nicht sehen darf. Grau,
  // ohne Titel: mehr als die Zeit kommt dafür gar nicht erst auf das Gerät.
  if (sicht && sicht.sichtbar !== false) {
    for (const zeit of sicht.meetZeiten || []) {
      const eintrag = tage.find((entry) => entry.iso === zeit.datum);
      if (!eintrag || meetZeitGesehen.has(`${zeit.datum}|${zeit.vonMin}|${zeit.bisMin}`)) continue;
      meetZeitGesehen.add(`${zeit.datum}|${zeit.vonMin}|${zeit.bisMin}`);
      eintrag.streifen.push({ art: 'grau', vonMin: zeit.vonMin, bisMin: zeit.bisMin, offen: Boolean(zeit.offen) });
    }
  }
  for (const eintrag of tage) {
    eintrag.streifen.sort((a, b) => ((a.art === 'meet' ? 1 : 0) - (b.art === 'meet' ? 1 : 0)) || (a.vonMin - b.vonMin));
  }
  return tage;
}

// Ein Block im Kalender: grau ohne Wort, oder farbig mit Titel (antippbar). Der Grund liegt auf einer
// eigenen Schicht, damit das Auslaufen eines Meets ohne Ende nur ihn ausblendet und nicht den Titel.
function zusammenBlockHtml(block, eigen, hoehe) {
  const meet = block.art === 'meet';
  const obenPx = (block.vonMin / 1440) * hoehe;
  const echt = ((block.bisMin - block.vonMin) / 1440) * hoehe;
  // Ein gemeinsames Meet ist mindestens zwei Zeilen hoch, damit sein Titel lesbar bleibt („Strand-bad").
  const mindest = meet ? BUSY_ZEILE * 2 + 4 : hoehe * 0.025;
  const hochPx = Math.min(hoehe - obenPx, Math.max(echt, mindest));
  const platz = hochPx - 4;
  const zeilen = meet && platz >= BUSY_ZEILE - 1 ? Math.max(1, Math.min(3, Math.floor(platz / BUSY_ZEILE))) : 0;
  const maske = block.offen ? `-webkit-mask-image:linear-gradient(180deg,#000 0,#000 ${((ZUSAMMEN_OHNE_ENDE_KERN / ZUSAMMEN_OHNE_ENDE_MIN) * 100).toFixed(0)}%,transparent 100%);mask-image:linear-gradient(180deg,#000 0,#000 ${((ZUSAMMEN_OHNE_ENDE_KERN / ZUSAMMEN_OHNE_ENDE_MIN) * 100).toFixed(0)}%,transparent 100%);` : '';
  const grund = `<span aria-hidden="true" style="position:absolute;left:0;right:0;top:0;bottom:0;${meet ? ZUSAMMEN_GRUEN : ZUSAMMEN_GRAU};${maske}pointer-events:none"></span>`;
  const titel = zeilen >= 1
    ? `<span style="position:relative;display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:${zeilen};overflow:hidden;hyphens:auto;-webkit-hyphens:auto;overflow-wrap:break-word;text-align:center;font:650 9.5px/${BUSY_ZEILE}px ${FONT};letter-spacing:-.02em;color:var(--green-dark);pointer-events:none">${esc(block.titel)}</span>`
    : '';
  const lage = `position:absolute;left:0;right:0;top:${((obenPx / hoehe) * 100).toFixed(2)}%;height:${((hochPx / hoehe) * 100).toFixed(2)}%;border-radius:5px;box-shadow:0 0 0 1px var(--field);overflow:hidden;box-sizing:border-box`;
  if (!meet) return `<span data-role="streifen-block" data-art="grau" style="${lage};pointer-events:none">${grund}</span>`;
  return `<button data-act="dt-meet" data-meet="${esc(block.meetId)}" data-role="streifen-block" data-art="meet" aria-label="${esc(`${block.titel} ${busyZeit(block.vonMin)}`)}" style="${lage};z-index:2;display:flex;flex-direction:column;align-items:stretch;padding:2px 1px 0;margin:0;border:0;background:transparent;cursor:pointer;appearance:none;font-family:${FONT};pointer-events:auto;touch-action:manipulation">${grund}${titel}</button>`;
}

function zusammenZeitText(block) {
  return block.offen ? busyZeit(block.vonMin) : `${busyZeit(block.vonMin)}–${busyZeit(block.bisMin)}`;
}

// Der Tag als Satz — für Vorlesestimme und Titel.
function zusammenTagSatz(eintrag) {
  const teile = (eintrag.streifen || []).map((block) => `${block.art === 'meet' ? block.titel : tx('beschäftigt')} ${zusammenZeitText(block)}`);
  return teile.length ? `${tagKurz(eintrag.tag)} · ${teile.join(', ')}` : `${tagKurz(eintrag.tag)} · ${tx('frei')}`;
}

// Die Liste: nur Tage, an denen etwas steht. Gemeinsame Meets mit Titel (antippbar), alles Fremde als
// „beschäftigt" mit der Zeit.
function zusammenListeHtml(tage) {
  const mitEintrag = tage.filter((eintrag) => eintrag.streifen.length);
  if (!mitEintrag.length) {
    return `<div data-role="zusammen-leer" style="padding:12px 0 10px;font-size:12.5px;color:var(--muted-light)">${tx('Keine Einträge in den nächsten 7 Tagen')}</div>`;
  }
  return mitEintrag.map((eintrag, i) => {
    const zeilen = [...eintrag.streifen].sort((a, b) => a.vonMin - b.vonMin).map((block) => {
      const zeit = `<span style="font-size:12px;font-weight:600;color:var(--muted);font-variant-numeric:tabular-nums;flex:none;pointer-events:none">${esc(zusammenZeitText(block))}</span>`;
      if (block.art === 'meet') {
        return `<button data-act="dt-meet" data-meet="${esc(block.meetId)}" data-role="zusammen-zeile" data-art="meet" style="display:flex;align-items:center;gap:10px;width:100%;min-height:44px;padding:0;margin:0;border:0;background:transparent;cursor:pointer;appearance:none;text-align:left;font-family:${FONT};color:var(--ink)"><span style="width:9px;height:9px;border-radius:3px;background:var(--green);flex:none;pointer-events:none"></span><span style="flex:1;min-width:0;font-size:14px;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;pointer-events:none">${esc(block.titel)}</span>${zeit}</button>`;
      }
      return `<div data-role="zusammen-zeile" data-art="grau" style="display:flex;align-items:center;gap:10px;min-height:32px"><span style="width:9px;height:9px;border-radius:3px;${ZUSAMMEN_GRAU};flex:none"></span><span style="flex:1;min-width:0;font-size:13.5px;color:var(--muted)">${tx('beschäftigt')}</span>${zeit}</div>`;
    }).join('');
    return `<div data-role="zusammen-tag" data-tag="${eintrag.tag}" style="padding:8px 0 4px;border-top:${i ? '1px solid var(--ink-a05)' : '0'}"><div style="font-size:11.5px;font-weight:650;color:var(--muted-light);padding-bottom:2px">${esc(formatMeetDate(eintrag.iso))}</div>${zeilen}</div>`;
  }).join('');
}

// Welche der beiden Ansichten die Karte zeigt, merkt sich das Gerät bis zum Schließen der App (nicht
// gespeichert: eine Sicht ist keine Einstellung).
let zusammenAnsicht = 'kalender';
let zusammenKontext = null;

function zusammenSchalter(aktiv) {
  const feld = (ansicht, wort) => `<button data-act="zusammen-ansicht" data-view="${ansicht}" data-role="zusammen-ansicht-${ansicht}" data-treffer aria-pressed="${aktiv === ansicht}" style="height:28px;min-width:28px;padding:0 11px;box-sizing:border-box;border:0;border-radius:999px;cursor:pointer;appearance:none;font:650 12.5px/1 ${FONT};white-space:nowrap;${aktiv === ansicht ? 'background:var(--green-tint);color:var(--green-dark)' : 'background:transparent;color:var(--muted)'}"><span style="pointer-events:none">${esc(wort)}</span></button>`;
  return `<div role="group" aria-label="${esc(tx('Ansicht'))}" data-role="zusammen-ansicht" style="display:flex;align-items:center;flex:none;height:32px;box-sizing:border-box;background:var(--surface);border:1.5px solid var(--green-a42);border-radius:999px;padding:1px">${feld('kalender', tx('Kalender'))}${feld('liste', tx('Liste'))}</div>`;
}

function zusammenKarteHtml(ctx, personId) {
  const tage = zusammenTage(ctx?.repo, personId);
  const inhalt = zusammenAnsicht === 'liste'
    ? zusammenListeHtml(tage)
    : busyKalender(tage, { eigen: false, rolle: 'wochenstreifen-lesend', blockHtml: zusammenBlockHtml, tagSatz: zusammenTagSatz });
  return `<div data-role="zusammen-karte" data-person="${esc(personId)}" data-ansicht="${zusammenAnsicht}" style="flex:none;background:var(--surface);border-radius:20px;border:1px solid var(--ink-a07);padding:2px 16px 6px">
<div style="display:flex;align-items:center;gap:8px;padding:8px 0 6px;min-height:30px;box-sizing:border-box"><span data-role="zusammen-titel" style="font-size:11px;font-weight:650;letter-spacing:.09em;text-transform:uppercase;color:var(--muted-light);flex:1;min-width:0">${tx('Zusammen')}</span>${zusammenSchalter(zusammenAnsicht)}</div>
<div data-role="zusammen-inhalt" style="padding-top:8px;border-top:1px solid var(--ink-a05)">${inhalt}</div>
</div>`;
}

// Einbau (room.js › renderPersonDetails, fremde Datei, Name blieb): `${busyKarte(ctx, person.id)}`.
// Die Karte steht immer da — auch ohne jeden Eintrag.
export function busyKarte(ctx, personId) {
  if (!personId) return '';
  zusammenKontext = ctx;
  return zusammenKarteHtml(ctx, personId);
}
export { busyKarte as zusammenKarte };

// Der Schalter Kalender | Liste. Das Personenprofil (room.js) kennt diese Handlung nicht — sie liegt hier,
// bei der Karte, und zeichnet nur die Karte neu.
if (typeof document !== 'undefined') {
  document.addEventListener('click', (event) => {
    const knopf = event.target?.closest?.('[data-act="zusammen-ansicht"]');
    const karte = knopf?.closest('[data-role="zusammen-karte"]');
    if (!knopf || !karte || !zusammenKontext) return;
    zusammenAnsicht = knopf.dataset.view === 'liste' ? 'liste' : 'kalender';
    karte.outerHTML = zusammenKarteHtml(zusammenKontext, karte.dataset.person);
  });
}
...
// danach nur der Punkt."): Der Anfragen-Balken aus Runde 7 stand bei JEDEM Öffnen oben im Profil,
// bis man antwortete. Er ist ersatzlos entfallen. An seiner Stelle steht der gemeinsame Hinweis
// (web/ui/anfragen-hinweis.js): einmal groß, ein Tipp führt zu „Freunde" (dort stehen alle
// erhaltenen Anfragen mit Annehmen/Ablehnen), danach nur noch der Punkt am Knopf oben rechts.

// Runde 8 (R8-61, Schnittstelle des Pakets form): JEDER Punkt an einem Kreis — Frei, Beste,
// Besondere, Mitteilung — kommt aus kreisPunkt(durchmesser, ecke, stil) in ui/components.js. Solange
// der Export (noch) fehlt, zeichnet dieser Rückfall nach DERSELBEN Regel: Mittelpunkt auf dem
// Kreisrand unter 45°, Größe = MASS.punkt × Durchmesser (derselbe Anteil wie der Frei-Punkt).
function punktAmKreis(durchmesser, ecke, stil = {}) {
  if (typeof bausteine.kreisPunkt === 'function') return bausteine.kreisPunkt(durchmesser, ecke, stil);
  const d = Number(durchmesser) || 0;
  const g = Math.round(d * MASS.punkt * 100) / 100;
  const r = d / 2;
  const weg = r * Math.SQRT1_2;
  const x = r + (ecke === 'or' || ecke === 'ur' ? weg : -weg) - g / 2;
  const y = r + (ecke === 'ur' || ecke === 'ul' ? weg : -weg) - g / 2;
  return `<span ${stil.attribute || 'data-role="kreis-punkt"'} data-ecke="${esc(ecke)}" aria-hidden="true" style="position:absolute;left:${x.toFixed(2)}px;top:${y.toFixed(2)}px;width:${g}px;height:${g}px;border-radius:50%;background:${stil.farbe || 'var(--orange)'};display:flex;align-items:center;justify-content:center;pointer-events:none;z-index:5">${stil.inhalt || ''}</span>`;
}

// Die runden Knöpfe oben rechts: dieselbe Größe und derselbe Rand wie das „+" in Crew (38 px, feine
// Linie) — der Rand als innerer Schatten, damit der Punkt am Kreis genau auf dem Rand sitzt.
const KOPF_KNOPF = 38;

// v6 A20a: „Freund hinzufügen" sitzt OBEN RECHTS neben den Einstellungen. Runde 8 (R8-36): Er öffnet
// „Freunde" (mein Code · Scannen · Anfragen); eine offene Anfrage zeigt der orange Punkt an seinem
// Kreis — nach der einen Regel für Punkte an Kreisen (R8-61), wie am „+" in Crew.
// Runde 9 (Jonathan): Die neue Anfrage erscheint im Profil GENAUSO wie in Crew — als Karte ÜBER dem
// Inhalt (ui/anfragen-hinweis.js, Form „flach"), links neben dem Freunde-Knopf, und schrumpft nach
// ihrer Zeit in EINER Bewegung zu seinem Punkt. Sie hält Abstand zum Titel links (KOPF_FREI).
const KOPF_FREI = 96;
function profilKopfHinweis(hinweis) {
  if (!hinweis) return '';
  const rechts = bausteine.TAB_HEADER_PAD_X + KOPF_KNOPF * 2 + 8 + 8;
  return `<div data-role="anfrage-hinweis-kopf" style="position:absolute;z-index:6;top:${6 + (bausteine.TAB_HEADER_HEIGHT - KOPF_KNOPF) / 2}px;height:${KOPF_KNOPF}px;right:${rechts}px;width:min(220px, calc(100% - ${rechts + KOPF_FREI}px));pointer-events:none">${hinweis}</div>`;
}

function homeHeader(ctx, kopfHinweis = '') {
  const openRequests = (ctx.repo.getFriendRequests() || []).length;
  // Wie am „+" in Crew (Anfragen-Muster, R8-36): Steht die große Karte gerade oben, erscheinen der
  // Punkt UND die Lücke um ihn erst, wenn sie zu ihm hin geschrumpft ist — danach bleibt nur er.
  const hinweisLaeuft = Boolean(ctx.ui?.anfrageHinweisDa);
  // Dieselbe Bauform wie das „+" in Crew: ein durchsichtiger Knopf, darin die runde Fläche mit Rand
  // als EIGENES Element — sie bekommt das Loch für die Lücke um den Punkt (kreisMaske). Läge die
  // Maske am Knopf selbst, schnitte sie auch den Punkt weg.
  const rundKnopf = (act, aria, inner, punkt = false) => {
    const loch = punkt && !hinweisLaeuft && typeof bausteine.kreisMaske === 'function' ? bausteine.kreisMaske(KOPF_KNOPF, ['or']) : '';
    const dot = punkt
      ? `<span data-role="kopf-punkt" aria-hidden="true" style="position:absolute;inset:0;pointer-events:none;z-index:2;opacity:${hinweisLaeuft ? 0 : 1}">${punktAmKreis(KOPF_KNOPF, 'or', { farbe: 'var(--orange)', luecke: true, attribute: 'data-punkt="profil-punkt" aria-hidden="true"' })}</span>`
      : '';
    return `<button data-act="${act}" data-treffer aria-label="${aria}" style="position:relative;flex:none;width:${KOPF_KNOPF}px;height:${KOPF_KNOPF}px;border:0;border-radius:50%;background:transparent;display:flex;align-items:center;justify-content:center;padding:0;cursor:pointer;appearance:none"><span data-role="kopf-kreis" aria-hidden="true" style="position:absolute;inset:0;border-radius:50%;border:1px solid var(--ink-a12);background:var(--surface);box-sizing:border-box;pointer-events:none;${loch}"></span><span style="position:relative;pointer-events:none;display:flex">${inner}</span>${dot}</button>`;
  };
  const freundeLabel = openRequests
    ? `${tx('Freund hinzufügen')}, ${tnx(openRequests, '{n} neue Anfrage', '{n} neue Anfragen')}`
    : tx('Freund hinzufügen');
  ...
  return tabHeader(
    `<span style="font-family:'Bricolage Grotesque',sans-serif;font-size:22px;font-weight:650">${tx('Profil')}</span><style>@keyframes profilPunktEin{from{opacity:0;transform:scale(.2)}}@keyframes profilLochEin{from{-webkit-mask-image:none;mask-image:none}}@media (prefers-reduced-motion:reduce){[data-role="kopf-punkt"]{animation-duration:1ms!important}}</style>`,
    `${rundKnopf('go-friend-add', esc(freundeLabel), svgPersonPlus, openRequests > 0)}${rundKnopf('go-settings', esc(tx('Einstellungen')), svgGear)}`,
  ) + profilKopfHinweis(kopfHinweis);
}

// R8-36: Das eigene Bild steht VOLL da — ohne Frei-Punkt (den zeigt FREE selbst), ohne Aktiv-Fade,
// ohne Marke; nur der Stift sagt „antippen zum Bearbeiten". Er sitzt nach der Regel für Punkte an
// Kreisen (kreisPunktLage: Mitte auf dem Rand unter 45°, unten rechts; Größe = MASS.punkt ×
// Durchmesser), sieht aber wie ein Knopf aus (Fläche, Haarlinie, weicher Schatten) — er ist keine
// Zustandsanzeige, deshalb auch keine Lücke durch das Bild: das Bild bleibt ganz.
const EIGENES_BILD = 92;

function stiftAmBild(durchmesser) {
  const lage = typeof bausteine.kreisPunktLage === 'function'
    ? bausteine.kreisPunktLage(durchmesser, 'ur')
    : { x: durchmesser / 2 * (1 + Math.SQRT1_2), y: durchmesser / 2 * (1 + Math.SQRT1_2), groesse: Math.round(durchmesser * MASS.punkt * 100) / 100 };
  const g = lage.groesse;
  return `<span data-role="stift" aria-hidden="true" style="position:absolute;left:${(lage.x - g / 2).toFixed(2)}px;top:${(lage.y - g / 2).toFixed(2)}px;width:${g}px;height:${g}px;border-radius:50%;background:var(--surface);box-shadow:0 0 0 1px var(--ink-a10),0 2px 6px var(--shadow-14);display:flex;align-items:center;justify-content:center;pointer-events:none;z-index:6">${svgPencilSmall(Math.round(g * 0.47))}</span>`;
}
...
function homeBody(ctx, { hinweisHtml = '' } = {}) {
  const { repo, ui } = ctx;
  const settings = repo.getSettings();
  ...
  return `${hinweisHtml}<div style="display:flex;flex-direction:column;align-items:center;gap:12px;padding:10px 24px 18px">
<button data-act="edit-profile" aria-label="${esc(tx('Profil bearbeiten'))}" style="position:relative;display:block;width:${EIGENES_BILD}px;height:${EIGENES_BILD}px;border:0;background:transparent;padding:0;cursor:pointer;appearance:none;border-radius:50%;flex:none"><span style="pointer-events:none;display:contents">${personAvatar(me, { size: EIGENES_BILD, free: false, active: false })}</span>${stiftAmBild(EIGENES_BILD)}</button>
...
function homeSheetValue(ctx, key) {
  ...
  // Runde 7 (Jonathan, J3): „Die ‚nicht gespeichert'-Warnung fehlt an mehreren Stellen,
  // z. B. beim Profilbild, wenn man ins Leere tippt. Das gehört EINMAL zentral gelöst."
  // Der Baukasten ist ein Blatt mit Entwurf wie jedes andere — sein Vergleichswert ist das,
  // was „Übernehmen" speichern würde. Damit greift für ihn dieselbe Ü4-Entscheidung
  // (dirtyGuard + discardSheet), die Interessen, Ressourcen und Zeiten schon schützt.
  if (key === 'foto') {
    return ui.pb ? pbErgebnis(ui.pb) : null;
  }
  return null;
}
...
function restoreHomeSheet(ctx, key, snapshot) {
  ...
  // J3: „Verwerfen" wirft den Bildentwurf weg — das Gespeicherte war nie angetastet.
  if (key === 'foto') {
    ui.pb = null;
    return;
  }
...
function backHomeSheet(ctx, options = {}) {
  ...
  const finish = () => {
    ...
    // Der Bildentwurf gehört zum Baukasten-Blatt; verlässt man es, ist er verbraucht.
    if (key === 'foto') ui.pb = null;
...
function closeHomeSheet(ctx, options = {}) {
  const { ui } = ctx;
  // J3: Der Fotoschritt hatte hier den Schlüssel des Profil-Sheets geliehen — und dessen
  // Vergleichswert kennt nur den Namen. Ein geändertes Bild fiel deshalb still auf den Boden,
  // sobald man daneben tippte. Er ist jetzt ein eigenes Blatt mit eigenem Entwurf.
  const key = ui.sheet;
  const finish = () => {
    ui.sheet = null;
    ui.sheetStack = [];
...
${bearbeitet ? '' : `<div style="max-height:330px;overflow-y:auto;scrollbar-width:none;margin:0 -2px" data-scroll-keep="int-katalog">${interestCatalogHtml({
      act: 'int-pick',
      istGewaehlt: (label) => label === (ui.intNameDraft || '').trim(),
    })}</div>`}
${bearbeitet ? `<div style="display:flex;flex-direction:column;gap:6px;padding:0 0 6px">
${groupLabel(tx('Interesse'))}
...
${zeichenAuswahlHtml({ act: 'res-icon-set', gewaehlt, worte: RESSOURCEN_WORTE })}`, { closeAct: 'close-home-sheet', scrollKey: 'res-icon' });
  ...
  // Runde 4 (B5) → Runde 8 (R8-39): EIN Busy-Eintrag — Titel, Schloss, Tage antippen, von/bis
  // drehen. Das Schloss oben rechts ist dasselbe wie bei den Ressourcen: an = privat, Freunde sehen
  // nur „Busy". Der Satz unter dem Titel sagt genau das in Worten und ändert sich beim Tippen mit.
  // Die frühere zweite Frage „Wer sieht deine Zeiten?" ist entfallen (R8-39: keine Sichtbarkeits-
  // Einstellung — Busy sehen Freunde immer).
  if (ui.sheet === 'belegt' && ui.belegtEntwurf) {
    const entwurf = ui.belegtEntwurf;
    const neu = entwurf.index === null;
    const privat = Boolean(entwurf.privat);
...
${wheelColumn(`belegt-${feld}-m`, BELEGT_MINUTEN, m, { runden: 3, width: 44 })}
    ...
    const schloss = `<button data-act="busy-privat" data-role="busy-schloss" aria-pressed="${privat}" aria-label="${esc(tx('Privat'))}" title="${esc(tx('Privat'))}" style="width:30px;height:30px;border-radius:50%;border:1.5px solid ${privat ? 'var(--green-a50)' : 'var(--ink-a14)'};background:${privat ? 'var(--green-tint)' : 'var(--surface)'};display:flex;align-items:center;justify-content:center;flex:none;cursor:pointer;appearance:none;padding:0"><span style="pointer-events:none;display:flex">${svgLockSmall(privat ? 'var(--green-dark)' : 'var(--muted)')}</span></button>`;
    return sheet(`<div style="display:flex;align-items:center;gap:10px"><span style="font-family:${BRICOLAGE};font-size:19px;font-weight:650;flex:1;min-width:0">${neu ? tx('Busy eintragen') : tx('Busy bearbeiten')}</span>${schloss}</div>
<input id="busy-titel" value="${esc(entwurf.titel || '')}" maxlength="30" autocomplete="off" enterkeyhint="done" placeholder="${esc(tx('Titel, z. B. Arbeit'))}" aria-label="${esc(tx('Titel'))}" style="${INPUT_STYLE};margin-top:12px">
<span data-role="busy-sicht" style="display:block;font-size:12px;color:var(--muted);line-height:1.45;padding:7px 2px 0">${esc(busySichtSatz(entwurf))}</span>
...
function renderProfileHome(ctx) {
  ...
  // Runde 8 (R8-11/R8-60): die Karte „neue Anfrage" (einmal groß) — und der Kopf weiß davon.
  const hinweis = anfrageHinweisHtml(ctx, { act: 'go-anfragen', form: 'flach' });
  ctx.ui.anfrageHinweisDa = Boolean(hinweis);
  const html = screenScaffold({
    page: true,
    header: homeHeader(ctx, hinweis),
    body: homeBody(ctx),
...
function bindProfileHome(root, ctx) {
  ...
  const settings = repo.getSettings();
  // Runde 8 (R8-11): Die Karte ist gezeigt, sobald die Seite wirklich steht (gebunden wird nur die
  // aktive Seite der Bahn, nie die Vorschau daneben).
  // Runde 9: wie in Crew — gezeigt gilt als gesehen, dann Eintritt und Flug in den Punkt.
  const seite = root.querySelector('.tab-page[data-role="active"]') || root;
  const gezeigt = anfrageHinweisIds(seite);
  if (gezeigt.length) anfrageHinweisGesehen(ctx, gezeigt);
  anfrageHinweisBinden(seite, ctx, { ziel: '[data-role="kopf-punkt"] > [data-punkt]' });
  ...
  // R8-39: Der Titel eines Busy-Eintrags lebt als Entwurf (Ü4 schützt ihn); der Satz darunter
  // („Freunde sehen …") ändert sich beim Tippen mit — gezielt an seinem Knoten, ohne Neuzeichnen.
  const busyTitel = root.querySelector('#busy-titel');
  if (busyTitel) {
    busyTitel.addEventListener('input', () => {
      if (!ui.belegtEntwurf) return;
      ui.belegtEntwurf.titel = busyTitel.value;
      const satz = root.querySelector('[data-role="busy-sicht"]');
      if (satz) satz.textContent = busySichtSatz(ui.belegtEntwurf);
    });
    busyTitel.addEventListener('keydown', (event) => { if (event.key === 'Enter') busyTitel.blur(); });
  }
  ...
  // Runde 4 (B5): Die Uhrzeit-Räder im Blatt „Busy" schreiben in den Entwurf.
  ...
  bindActions(root, {
    ...
    // Runde 8 (R8-11): der Tipp auf den Anfrage-Hinweis — „Freunde", ganz offen.
    'go-anfragen': () => nav.go('profile.friendAdd', { anfragen: 1 }),
    ...
    // J3: Der Baukasten liegt jetzt als eigenes Blatt AUF dem Profil-Sheet (stack) — damit
    // gilt für ihn dieselbe Verwerfen-Entscheidung wie für jedes andere Blatt mit Entwurf,
    // und „Abbrechen" führt sauber eine Stufe zurück statt den Zustand von Hand zu setzen.
    'edit-photo': () => openHomeSheet(ctx, 'foto', () => {
      const aktuell = repo.getSettings();
    ...
    // Abbrechen verwirft NUR den Entwurf; das gespeicherte Bild bleibt unberührt. Wurde
    // wirklich etwas verändert, fragt die App vorher nach (Ü4).
    'foto-cancel': () => backHomeSheet(ctx),
    'foto-apply': () => {
      if (!ui.pb) return;
    ...
    // Runde 7 (E1) → Runde 8 (R8-39): Tipp in den Kalender. Die Spalte ist das Ziel (ein Block von
    // einer Stunde wäre 5,5 px hoch), die Höhe des Tipps sagt die Uhrzeit: Liegt dort ein Eintrag,
    // öffnet sich genau er; ist dort nichts, entsteht ein neuer an genau diesem Tag ab genau dieser
    // Stunde. Welcher Eintrag hinter einem Block steckt, beantwortet dieselbe Funktion, die den
    // Kalender zeichnet (core/belegt.js woche) — keine zweite Rechnung über Mitternacht.
    'belegt-tag-open': (data, el, event) => {
      const liste = eigeneBusyListe(repo);
      const flaeche = el?.querySelector?.('[data-role="streifen-flaeche"]');
      let minute = null;
      if (flaeche && event && Number.isFinite(event.clientY) && event.clientY > 0) {
        const r = flaeche.getBoundingClientRect();
        if (r.height > 0 && event.clientY >= r.top - 4 && event.clientY <= r.bottom + 4) {
          minute = Math.max(0, Math.min(1439, ((event.clientY - r.top) / r.height) * 1440));
        }
      }
      const treffer = busyEintragBei(repo, Number(data.index), minute);
      if (treffer >= 0) {
        const fenster = liste[treffer];
        openHomeSheet(ctx, 'belegt', () => {
          ui.belegtEntwurf = { index: treffer, tage: [...fenster.tage], von: aufFuenfMinuten(fenster.von), bis: aufFuenfMinuten(fenster.bis), titel: String(fenster.titel || ''), privat: Boolean(fenster.privat) };
        });
        return;
      }
      const tag = Number(data.tag);
      const stunde = minute === null ? 8 : Math.floor(minute / 60);
      const bisStunde = minute === null ? 17 : Math.min(24, stunde + 2);
      const hh = (h) => `${String(h % 24).padStart(2, '0')}:00`;
      openHomeSheet(ctx, 'belegt', () => {
        ui.belegtEntwurf = { index: null, tage: [tag], von: hh(stunde), bis: hh(bisStunde), titel: '', privat: false };
      });
    },
    // Runde 4 (B5) → Runde 8 (R8-39): Busy — anlegen, ändern, entfernen.
    'belegt-add': () => openHomeSheet(ctx, 'belegt', () => {
      ui.belegtEntwurf = { index: null, tage: [1, 2, 3, 4, 5], von: '08:00', bis: '17:00', titel: '', privat: false };
    }),
    // Das Schloss (wie bei den Ressourcen): an = privat, Freunde sehen nur „Busy".
    'busy-privat': () => {
      const entwurf = ui.belegtEntwurf;
      if (!entwurf) return;
      const feld = root.querySelector('#busy-titel');
      if (feld) entwurf.titel = feld.value;
      entwurf.privat = !entwurf.privat;
      haptik('tipp');
      ctx.render();
    },
    'belegt-tag': (data) => {
    ...
    'belegt-vorlage': (data) => {
      const entwurf = ui.belegtEntwurf;
      if (!entwurf) return;
      const tage = String(data.tage || '').split(',').map(Number).filter((tag) => tag >= 0 && tag <= 6);
      // Runde 9: Ein Tipp auf die schon aktive Vorwahl wählt sie wieder ab.
      const schonAn = entwurf.tage.length === tage.length && entwurf.tage.every((tag) => tage.includes(tag));
      entwurf.tage = schonAn ? [] : tage;
      haptik('tipp');
      ctx.render();
    },
    // Geschrieben wird über den Vertrag (addBelegt/updateBelegt nehmen titel und privat an) — nie an
    // der Datenschicht vorbei. Beim Anlegen gehen nur gesetzte Felder mit (ein Eintrag ohne Titel
    // bleibt { tage, von, bis }); beim Ändern beide ausdrücklich, damit ein gelöschter Titel und ein
    // geöffnetes Schloss auch wirklich weg sind.
    'belegt-save': () => {
      const entwurf = ui.belegtEntwurf;
      if (!entwurf) return;
      const feld = root.querySelector('#busy-titel');
      ...
      const titel = String(entwurf.titel || '').trim().slice(0, 30);
      const privat = Boolean(entwurf.privat);
      const tage = [...entwurf.tage].sort((a, b) => a - b);
      const neu = entwurf.index === null;
      const ergebnis = neu
        ? repo.addBelegt({ tage, von: entwurf.von, bis: entwurf.bis, ...(titel ? { titel } : {}), ...(privat ? { privat: true } : {}) })
        : repo.updateBelegt(entwurf.index, { tage, von: entwurf.von, bis: entwurf.bis, titel, privat });
      if (ergebnis && ergebnis.ok === false) { ctx.toast(tx('Das hat nicht geklappt')); return; }
      haptik('erfolg');
      closeHomeSheet(ctx, { commit: true });
      ctx.toast(neu ? tx('Busy eingetragen') : tx('Busy gespeichert'));
    },
    'belegt-remove': () => {
      const entwurf = ui.belegtEntwurf;
      if (!entwurf || entwurf.index === null) return;
      const ergebnis = repo.removeBelegt(entwurf.index);
      if (ergebnis && ergebnis.ok === false) { ctx.toast(tx('Das hat nicht geklappt')); return; }
      closeHomeSheet(ctx, { commit: true });
      ctx.toast(tx('Busy entfernt'));
    },
...
function inNativerApp() {
  if (istHuelle()) return true;
  const ort = globalThis.location;
  return Boolean(ort && ort.hostname === 'localhost' && !ort.port);
}

// =====================================================================================
// Runde 7 (N1/N2) — der abgelegte Einladungscode
// -------------------------------------------------------------------------------------
// app.js nimmt `?einladung=CODE` aus der Adresse und legt den Code ab; eingelöst wird er erst,
// wenn jemand angemeldet UND eingerichtet ist. Der Einrichten-Weg LIEST denselben Eintrag und
// zeigt damit 'onboarding.invite' (N2). Welle 5: Beide Hälften benutzen nur noch
// web/core/einladung.js — die eigene Kopie hier (und die in app.js) ist weg.
// Gezeigt wird genau das, was app.js nach „Einladung annehmen" einlöst (einladungCodesFuer):
// jeder liegende Code dieses Kontos, keiner, den ein anderes Konto liegen ließ.
// =====================================================================================
function sichtbareEinladungen(repo) {
  return einladungCodesFuer(einladungLesen(), einladungKonto(repo));
}
...
// =====================================================================================
// Runde 8 (R8-36, Jonathan): „‚Freunde': links mein Code, rechts Scannen, darunter erhaltene
// Anfragen. Anfragen-Muster wie in Crew."
// -------------------------------------------------------------------------------------
// Bis hier war das Blatt ein Umschalter „Scannen | Mein Code": Man sah immer nur EINE Richtung,
// die andere stand als Wort in einem Schalter, und für die Anfragen musste man das Blatt erst
// hochziehen. Jetzt stehen beide Wege NEBENEINANDER, gleich groß, ohne Schalter:
//   links   mein Code (der QR) und darunter der Kurzcode zum Kopieren,
//   rechts  Scannen (die Kachel IST der Knopf, die Kamera läuft in ihr) und darunter das Feld für
//           einen Code, den man vorgelesen oder geschickt bekommen hat,
//   darunter „Teilen" (für alle, die nicht danebenstehen — N5) und die erhaltenen Anfragen — im
//           Muster aus Crew: Bild, Name, darunter „Annehmen" und „Ignorieren", Überschrift
//           „Anfragen" mit oranger Zahl. Keine offenen Anfragen: dann steht dort nichts.
// Weg sind: die Kopfzeile mit eigenem Bild, Namen und Erklärsatz (das Blatt ist aus sich heraus
// klar), der Umschalter und die zweite Hauptaktion „Anfrage senden" (das Feld schickt selbst).
// Warum nebeneinander statt eines umgedrehten Schalters: Ein Schalter zeigt immer nur eine Seite —
// Runde 7 (K1) wollte „zwei Richtungen, gleich sichtbar"; nebeneinander IST das, ohne einen Tipp.
//
// Die Kachel „Scannen" trägt weiter data-act="qr-toggle" data-richtung="scan": Drei fremde
// Prüfläufe tippen den ersten [data-act="qr-toggle"] an und meinen damit „Scannen" — das stimmt
// jetzt wörtlich, es gibt nur noch diesen einen.
// Runde 9 (Jonathan): „QR und Scannen nicht nebeneinander, sondern wie früher EIN Umschalter."
// Oben die Wahl „Scannen | Mein Code" (die Form aus Runde 7, K1: jeder Knopf SETZT seine Richtung),
// darunter GROSS genau eine Kachel: mein Code samt Kurzcode — oder Scannen samt Codefeld.
// Der erste [data-act="qr-toggle"] im Blatt ist weiter „Scannen".
const QR_KACHEL = 232;       // Kantenlänge der einen Kachel (höchstens)
function qrRichtung(scan) {
  const feld = (an, richtung, icon, label) => `<button data-act="qr-toggle" data-richtung="${richtung}" role="radio" aria-checked="${an}" aria-label="${esc(label)}" style="flex:1 1 0;min-width:0;display:flex;align-items:center;justify-content:center;gap:6px;height:36px;border-radius:12px;border:0;padding:0;background:${an ? 'var(--surface)' : 'transparent'};box-shadow:${an ? '0 1px 3px var(--shadow-08)' : 'none'};cursor:pointer;appearance:none;font-family:${FONT}"><span style="display:flex;opacity:${an ? 1 : 0.45};pointer-events:none">${icon}</span><span style="font-size:12.5px;font-weight:${an ? 650 : 600};color:${an ? 'var(--green-dark)' : 'var(--muted)'};white-space:nowrap;pointer-events:none">${esc(label)}</span></button>`;
  return `<div data-role="qr-richtung" role="radiogroup" aria-label="${esc(tx('Richtung wählen'))}" style="display:flex;align-items:center;gap:3px;width:100%;padding:3px;border-radius:15px;background:var(--field);box-sizing:border-box;flex:none;font-family:${FONT}">
${feld(scan, 'scan', svgPersonPlusGreen, tx('Scannen'))}
${feld(!scan, 'code', svgQrMini, tx('Mein Code'))}</div>`;
}
const QR_ZEILE_H = 44;       // Kurzcode und Codefeld: dieselbe Höhe, dieselbe Linie
const QR_AKTION_H = 46;      // „Teilen"
// Im kompakten Zustand zeigt die Liste die Überschrift und die erste Anfrage ganz — mehr bringt
// das Aufziehen (dieselbe Geste wie bisher, bindQrZug).
const LISTE_KOMPAKT = 148;
// Luft unter der Liste — Teil ihrer Höhe (box-sizing), damit die letzte Anfrage nie am Rand klebt.
const QR_LISTE_UNTEN = 24;

// Runde 3 (D4) → Runde 8 (R8-38): Was das Kamerafeld sagt, wenn kein Bild läuft. Kurz, weil es in
// einer halben Breite steht — und ohne „gib den Code unten ein": das Feld dafür steht jetzt immer
// direkt darunter. Wo ein neuer Versuch helfen kann, sagt eine zweite Zeile, dass die Kachel das tut.
const KAMERA_HINWEIS = {
  verweigert: tx('Kamera nicht freigegeben'),
  'keine-kamera': tx('Keine Kamera gefunden'),
  'nicht-moeglich': tx('Scannen geht hier nicht'),
  belegt: tx('Die Kamera benutzt gerade eine andere App'),
  schwarz: tx('Die Kamera zeigt kein Bild'),
  unbekannt: tx('Die Kamera startet nicht'),
};
const KAMERA_NOCHMAL = new Set(['verweigert', 'belegt', 'schwarz', 'unbekannt']);

// Das Kamerafeld (Ebenen wie seit Runde 3):
//   0  Grund: Kamerazeichen mit „Scannen" — oder ein kurzer Hinweis, wenn die Kamera nicht geht
//   1  [data-qr-kamera] mit data-fremd: Hier liegt das Video. Der Abgleich fasst den Inhalt nicht
//      an, deshalb übersteht das laufende Bild jedes Neuzeichnen (Toast, Anfragen …).
//   2  die Sucherecken
//   3  „Tippen, um die Kamera zu starten" (R8-38) bzw. „Code erkannt" — über dem Bild
function scanSurface(kamera = '') {
  const rand = '2.5px solid var(--green)';
  const ecke = (pos, radius) => `<span style="position:absolute;${pos};width:22px;height:22px;border-radius:${radius};pointer-events:none;z-index:2"></span>`;
  const hinweis = KAMERA_HINWEIS[kamera];
  const grund = hinweis
    ? `<span data-role="kamera-hinweis" data-grund="${esc(kamera)}" style="position:relative;display:flex;flex-direction:column;align-items:center;gap:6px;padding:0 14px;text-align:center;pointer-events:none">${svgCameraLens(22, 'var(--muted)')}<span style="font-size:11.5px;font-weight:600;line-height:1.35;color:var(--ink-soft)">${esc(hinweis)}</span>${KAMERA_NOCHMAL.has(kamera) ? `<span style="font-size:11px;font-weight:650;line-height:1.3;color:var(--green-dark)">${tx('Nochmal versuchen')}</span>` : ''}</span>`
    : `<span data-role="kamera-bereit" style="position:relative;display:flex;flex-direction:column;align-items:center;gap:7px;pointer-events:none">${svgCameraLens(30, 'var(--green-dark)')}<span style="font-size:13px;font-weight:650;color:var(--green-dark)">${tx('Scannen')}</span></span>`;
  const oben = kamera === 'erkannt'
    ? `<span data-role="kamera-erkannt" style="position:absolute;inset:0;z-index:3;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;background:var(--green);border-radius:inherit;pointer-events:none"><svg width="30" height="30" viewBox="0 0 24 24" fill="none"><path d="m5.5 12.5 4.2 4.2L18.5 7.8" stroke="var(--on-accent)" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round"></path></svg><span style="font-size:12.5px;font-weight:650;color:var(--on-accent)">${tx('Code erkannt')}</span></span>`
    : kamera === 'antippen'
      ? `<span data-role="kamera-antippen" style="position:absolute;inset:0;z-index:3;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:7px;padding:0 14px;text-align:center;background:var(--scrim);border-radius:inherit;pointer-events:none">${svgCameraLens(24, 'var(--on-accent)')}<span style="font-size:12px;font-weight:650;line-height:1.35;color:var(--on-accent)">${tx('Tippen, um die Kamera zu starten')}</span></span>`
      : '';
  // Nur <span>: die Fläche liegt in einem Knopf (scanKachel), und in einen Knopf gehört kein <div>.
  return `<span data-qr-scan data-kamera="${esc(kamera || 'an')}" style="position:absolute;inset:0;border-radius:inherit;background:var(--field);overflow:hidden;display:flex;align-items:center;justify-content:center;box-sizing:border-box">
...
${grund}
<span data-qr-kamera data-fremd style="position:absolute;inset:0;display:block;border-radius:inherit;z-index:1;pointer-events:none"></span>
${ecke(`left:14px;top:14px;border-left:${rand};border-top:${rand}`, '7px 0 0 0')}
${ecke(`right:14px;top:14px;border-right:${rand};border-top:${rand}`, '0 7px 0 0')}
${ecke(`left:14px;bottom:14px;border-left:${rand};border-bottom:${rand}`, '0 0 0 7px')}
${ecke(`right:14px;bottom:14px;border-right:${rand};border-bottom:${rand}`, '0 0 7px 0')}
${oben}
</span>`;
}

// Links: mein Code. Der Grund bleibt in JEDEM Thema hell (--qr-grund) — ein umgedrehter Code wäre
// für manche Scanner unlesbar. Die Kachel ist quadratisch und füllt ihre Spalte.
function codeKachel(code) {
  return `<div data-role="code-kachel" style="position:relative;width:100%;aspect-ratio:1/1;border-radius:22px;background:var(--qr-grund);box-shadow:inset 0 0 0 1px var(--ink-a06);display:flex;align-items:center;justify-content:center;padding:7px;box-sizing:border-box">${qrSvg(inviteUrl(code), '100%')}</div>`;
}

// Rechts: Scannen. Die ganze Kachel ist der Knopf — sie startet die Kamera IN sich.
function scanKachel(scan, kamera) {
  return `<button data-act="qr-toggle" data-richtung="scan" data-role="scan-kachel" aria-pressed="${scan}" aria-label="${esc(tx('Scannen'))}" style="position:relative;display:block;width:100%;aspect-ratio:1/1;border-radius:22px;border:0;padding:0;margin:0;background:var(--field);cursor:pointer;appearance:none;overflow:hidden;font-family:${FONT}">${scanSurface(kamera)}</button>`;
}

// Kurzcode (reference 06.3): tippen = kopieren, danach kurz ein grüner Haken.
function inviteCodeRow(code, copied) {
  return `<button data-act="qr-copy" data-role="code-kopieren" aria-label="${esc(tx('Code {code} kopieren', { code }))}" style="display:flex;align-items:center;justify-content:center;gap:7px;background:var(--paper);border-radius:14px;height:${QR_ZEILE_H}px;padding:0 8px;border:0;cursor:pointer;appearance:none;width:100%;min-width:0;box-sizing:border-box">
<span style="font:650 14px/1 ui-monospace,monospace;letter-spacing:.05em;color:var(--ink);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;min-width:0;pointer-events:none">${esc(code)}</span>
<span style="pointer-events:none;display:flex;flex:none">${copied ? `<svg width="15" height="15" viewBox="0 0 24 24" fill="none"><path d="m5 12.5 4.5 4.5L19 7.5" stroke="var(--green-dark)" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"></path></svg>` : svgCopy}</span></button>`;
}

// Unter „Scannen": der Code von jemand anderem, von Hand. Das Feld schickt selbst — mit dem runden
// Pfeil darin oder mit Enter. Auch ein leeres Feld bekommt eine Antwort („Code eingeben").
function codeEingabe() {
  return `<div data-role="code-eingabe" style="position:relative;height:${QR_ZEILE_H}px;min-width:0">
<style>#qr-code-input::placeholder{text-transform:none;letter-spacing:0;font:600 13.5px ${FONT};color:var(--muted-light)}</style>
<input id="qr-code-input" placeholder="${esc(tx('Code eingeben'))}" aria-label="${esc(tx('Code von Freund:in eingeben'))}" autocomplete="off" autocapitalize="characters" enterkeyhint="send" spellcheck="false" style="width:100%;height:${QR_ZEILE_H}px;box-sizing:border-box;padding:0 42px 0 12px;border:1.5px solid var(--ink-a12);border-radius:14px;background:var(--surface);outline:none;font:650 14px ui-monospace,monospace;letter-spacing:.04em;color:var(--ink);caret-color:var(--green);text-transform:uppercase">
<button data-act="qr-code-submit" data-treffer aria-label="${esc(tx('Anfrage senden'))}" style="position:absolute;right:5px;top:50%;margin-top:-17px;width:34px;height:34px;border-radius:50%;border:0;background:var(--green);display:flex;align-items:center;justify-content:center;padding:0;cursor:pointer;appearance:none"><span style="pointer-events:none;display:flex">${svgSendWhite}</span></button>
</div>`;
}

// „Teilen" — die Einladung für alle, die NICHT danebenstehen.
function teilenKnopf() {
  return `<button data-act="qr-share" data-role="qr-teilen" style="width:100%;height:${QR_AKTION_H}px;box-sizing:border-box;background:var(--green);border:0;display:flex;align-items:center;justify-content:center;gap:8px;border-radius:999px;font:650 14px ${FONT};color:var(--on-accent);cursor:pointer;appearance:none;flex:none"><span style="pointer-events:none;display:flex">${svgShareWhite}</span><span style="pointer-events:none">${tx('Teilen')}</span></button>`;
}

// Erhaltene Anfrage auf der alten Seite „Anfragen" (profile.friendRequests): runde Controls in
// RSVP-Sprache (reference 06.3b/06.12). Die Reihenfolge im DOM ist Ablehnen · Annehmen — die
// zustimmende Handlung steht rechts, dort, wo der Daumen am Rand ohnehin liegt.
function requestControls(requestId, size = 30) {
  const haken = Math.max(14, Math.round(size * 0.42));
  const kreuz = Math.max(12, Math.round(size * 0.36));
  return `<div style="display:flex;gap:6px;flex:none">
<button data-act="req-decline" data-request="${esc(requestId)}" data-treffer aria-label="${esc(tx('Ablehnen'))}" style="width:${size}px;height:${size}px;border-radius:50%;border:1.5px solid var(--ink-a14);background:transparent;box-sizing:border-box;display:flex;align-items:center;justify-content:center;cursor:pointer;appearance:none;padding:0"><svg width="${kreuz}" height="${kreuz}" viewBox="0 0 24 24" fill="none" style="pointer-events:none"><path d="M7 7l10 10M17 7 7 17" stroke="var(--muted)" stroke-width="2.4" stroke-linecap="round"></path></svg></button>
<button data-act="req-accept" data-request="${esc(requestId)}" data-treffer aria-label="${esc(tx('Annehmen'))}" style="width:${size}px;height:${size}px;border-radius:50%;background:var(--green);border:0;display:flex;align-items:center;justify-content:center;cursor:pointer;appearance:none;padding:0"><svg width="${haken}" height="${haken}" viewBox="0 0 24 24" fill="none" style="pointer-events:none"><path d="m7.5 12.5 3 3 6-6.5" stroke="var(--on-accent)" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"></path></svg></button>
</div>`;
}

// R8-36 „Anfragen-Muster wie in Crew": dieselbe Zeile wie im Blatt hinter dem „+" in Crew
// (screens/crew.js › anfrageZeile) — Bild 44 px, Name, darunter die Herkunft, darunter „Annehmen"
// und „Ignorieren" (34 px hoch, Trefferfläche ≥ 46 px über data-treffer). Dieselben Acts
// (anfrage-annehmen / anfrage-ignorieren), damit dieselbe Zeile überall dasselbe tut.
// Exportiert, damit Crew sie übernehmen kann und es nur EINE Bauform gibt.
// Runde 9 (Jonathan): EINE Zeile je Anfrage — Bild, Name (darunter klein die Herkunft), rechts
// daneben „Annehmen" und „Ablehnen". Crew benutzt genau diese Zeile (crew.js › plusBlattHtml).
const ANFRAGE_PILLE = `flex:none;height:32px;padding:0 12px;border-radius:999px;font:650 12.5px/1 ${FONT};cursor:pointer;appearance:none;white-space:nowrap`;

export function anfrageZeile(anfrage, index = 0) {
  ...
  return `<div data-role="anfrage" data-anfrage="${id}" style="display:flex;align-items:center;gap:10px;padding:10px 2px;${index ? 'border-top:1px solid var(--ink-a06);' : ''}">
<span style="display:flex;flex:none">${gesicht}</span>
<span style="flex:1;min-width:0;display:flex;flex-direction:column;gap:2px">
<span data-role="anfrage-name" style="font-size:15px;font-weight:650;letter-spacing:-.01em;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(anfrage.name)}</span>
${anfrage.meta ? `<span data-role="anfrage-herkunft" style="font-size:12px;color:var(--muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(anfrage.meta)}</span>` : ''}
</span>
<span data-role="anfrage-knoepfe" style="display:flex;gap:6px;flex:none">
<button data-act="anfrage-annehmen" data-anfrage="${id}" data-treffer style="${ANFRAGE_PILLE};border:0;background:var(--green);color:var(--on-accent)"><span style="pointer-events:none">${esc(tx('Annehmen'))}</span></button>
<button data-act="anfrage-ignorieren" data-anfrage="${id}" data-treffer style="${ANFRAGE_PILLE};border:1px solid var(--ink-a14);background:var(--surface);color:var(--ink-soft)"><span style="pointer-events:none">${esc(tx('Ablehnen'))}</span></button>
</span></div>`;
}

const orangeZahl = (zahl) => `<span data-role="anfragen-zahl" style="flex:none;min-width:20px;height:20px;border-radius:10px;background:var(--orange);color:var(--on-accent);font:650 11px/20px ${FONT};text-align:center;padding:0 6px;box-sizing:border-box;pointer-events:none">${zahl}</span>`;

function anfragenAbschnitt(anfragen) {
  if (!anfragen.length) return '';
  return `<div data-role="freunde-anfragen" style="padding-top:12px;border-top:1px solid var(--ink-a07)">
<div style="display:flex;align-items:center;gap:8px;padding:0 2px 2px"><span style="font:650 13px/1 ${FONT};color:var(--muted)">${esc(tx('Anfragen'))}</span>${orangeZahl(anfragen.length)}</div>
...
// Runde 6 (A5): EINE Quelle für die Höhe der beiden Zustände — das Markup schreibt sie, und die
// Geste schreibt nach dem Einrasten genau denselben Wert zurück. R8-36: Kompakt ist das Blatt so
// hoch wie sein Inhalt (die Liste zeigt dabei höchstens LISTE_KOMPAKT) — ohne Anfragen gibt es
// keine leere Fläche unter „Teilen" mehr.
// Runde 9 (Jonathan): Auch offen nur so hoch, wie der Inhalt es braucht (max-height begrenzt).
const qrHoehe = () => 'auto';
const qrListeHoehe = (offen) => (offen ? 'none' : `${LISTE_KOMPAKT}px`);
...
<div data-qr-card class="ui-sheet-card" data-qr-state="${offen ? 'offen' : 'kompakt'}" style="position:absolute;left:0;right:0;bottom:0;height:${qrHoehe(offen)};max-height:calc(100% - 48px);box-sizing:border-box;background:var(--surface);border-radius:28px 28px 0 0;padding:14px 22px 0;display:flex;flex-direction:column;gap:14px;box-shadow:0 -8px 24px var(--shadow-18);overflow:hidden">
${sheetDecor}
${grabber(offen ? 'qr-collapse' : 'qr-expand')}
<div data-role="freunde-wege" style="position:relative;flex:none;display:flex;flex-direction:column;gap:12px">
${qrRichtung(scan)}
${scan
    ? `<div data-role="weg-scan" style="display:flex;flex-direction:column;gap:8px;width:min(100%, ${QR_KACHEL}px);margin:0 auto">${scanKachel(scan, ui.qrKamera || '')}${codeEingabe()}</div>`
    : `<div data-role="weg-code" style="display:flex;flex-direction:column;gap:8px;width:min(100%, ${QR_KACHEL}px);margin:0 auto">${codeKachel(code)}${inviteCodeRow(code, ui.copied)}</div>`}
${teilenKnopf()}
</div>
<div data-scroll-keep="invite-requests" style="position:relative;flex:1 1 auto;min-height:0;max-height:${qrListeHoehe(offen)};box-sizing:border-box;overflow-y:${offen ? 'auto' : 'hidden'};scrollbar-width:none;padding-bottom:${QR_LISTE_UNTEN}px">
${anfragenAbschnitt(requests)}
</div>
</div>
</div>`;
}

// N3/N5 (R8-36: „Teilen" im Blatt „Freunde"): Bis hier hing der Knopf direkt an navigator.share. In
// der Android-Hülle gibt es das nicht — dort kopierte er STILL die Adresse, und der kleine Haken
// stand am Kurzcode, als wäre der kopiert worden. Jetzt derselbe Weg wie in Crew: natives Teilen
// über die ehrliche Tabelle (core/native.js kann('teilen')), sonst das Teilen-Blatt des Browsers,
// sonst die Zwischenablage — mit einem Satz, der sagt, was passiert ist. Ein abgebrochenes
// Teilen-Blatt ist kein Fehler: da hat der Mensch selbst „nein" gesagt.
async function einladungTeilen(ctx, code) {
  ...
  let kopiert = false;
  try {
    if (globalThis.navigator?.clipboard?.writeText) { await globalThis.navigator.clipboard.writeText(`${text} ${adresse}`); kopiert = true; }
  } catch { kopiert = false; }
  ctx.toast(kopiert ? tx('Link kopiert') : tx('Link konnte nicht kopiert werden'));
}
...
function qrCoreActs(ctx) {
  ...
  return {
    // Runde 7, Welle 2 (K1): Zwei Knöpfe, jeder SETZT seine Richtung. Wer auf die Richtung
    // tippt, die schon gilt, bleibt dort — kein Zurückkippen (das war der gemessene Fehler).
    // Die Freundesanfragen bleiben davon unberührt — sie hängen allein am Zieh-/Aufziehzustand
    // (ui.qrExpanded).
    // Runde 3 (D4): Jeder Wechsel beginnt das Kamerafeld frisch; zurück zur eigenen Einladung
    // schaltet die Kamera sofort aus.
    'qr-toggle': (data) => {
      // data.richtung sagt, WAS gesetzt wird — nicht mehr „das Gegenteil von jetzt".
      // Runde 8 (R8-36): Es gibt nur noch die Richtung „scan" (die Kachel „Scannen"); mein Code
      // steht immer daneben. Ein fremder Aufruf mit „code" hält die Kamera an.
      const ziel = data.richtung === 'scan' ? 'scan' : 'qr';
      // R8-38: Wartet das Kamerabild auf einen Tipp, startet GENAU dieser Tipp es — play() direkt im
      // Tipp, dort erlaubt es jeder Browser. Läuft gar kein Scanner mehr, setzt der Tipp neu an.
      if (ziel === 'scan' && ui.qrView === 'scan' && ui.qrKamera === 'antippen') {
        const vorher = qrScan;
        qrScanAntippen().then((laeuft) => {
          if (ui.qrKamera !== 'antippen') return;
          if (laeuft || !vorher) { ui.qrKamera = ''; ctx.render(); }
        });
        return;
      }
      // Idempotent: Gilt die Richtung schon, passiert nichts — und es wird auch nichts neu
      // gezeichnet (sonst risse ein laufendes Kamerabild grundlos ab).
      //
      // Runde 7, Welle 3 (Prüfer, K1-Rest): Genau diese frühe Rückkehr machte den Knopf
      // „Scannen" wirkungslos, solange ein Kamera-Hinweis stand (ui.qrKamera = 'verweigert').
      // Wer die Kamera nachträglich freigab und auf das Wort tippte, sah NICHTS passieren und
      // musste den Umweg über „Mein Code" und zurück nehmen. Ein Hinweis bedeutet aber: es
      // läuft gerade KEINE Kamera — es geht also nichts kaputt, wenn hier neu angesetzt wird.
      // Der Tipp auf „Scannen" tut damit immer das, was auf dem Knopf steht (Hausregel 9).
      if ((ui.qrView === 'scan' ? 'scan' : 'qr') === ziel) {
        // Nur ein stehender HINWEIS wird zurückgesetzt. 'erkannt' ist der kurze Moment, in dem
        // ein gefundener Code gerade eingelöst wird — den unterbricht hier niemand.
        if (ziel !== 'scan' || !KAMERA_HINWEIS[ui.qrKamera]) return;
        ui.qrKamera = '';
        ui.qrAbgelehnt = '';
        ctx.render();
        return;
      ...
      ui.qrKamera = '';
      ui.qrAbgelehnt = '';
      if (ziel !== 'scan') qrScanStoppen();
      ctx.render();
    },
    'qr-copy': () => { copyToClipboard(code); markCopied(); },
    // N3/N5 (R8-36): „Teilen" über die ehrliche Tabelle der Hülle — wie in Crew.
    'qr-share': () => { einladungTeilen(ctx, code); },
...
// Welche Navigations-Einträge ihren modus= schon gesetzt haben (siehe renderFriendAdd).
const MODUS_GESETZT = new WeakSet();

// v3.1 §1: Profil-Inhaltskörper mit der Einladung als oberster Overlay-Ebene.
function renderFriendAdd(ctx) {
  // K1: Wer „Freund hinzufügen" wählt, will scannen; wer „Jemanden einladen" wählt, will
  // seinen Code zeigen. Der Parameter sagt, welche der beiden gleich sichtbaren Richtungen
  // zuerst gilt — ohne ihn bleibt es beim eigenen Code.
  //
  // Runde 7, Welle 2 (Prüfer, N4/K1 — MITTEL): Vorher hing das an ctx.ui.qrModus. ctx.ui gehört
  // zur Stack-Ebene UND zur Routen-Signatur (app.js, uiFor) — bei zweimal derselben Route mit
  // denselben Parametern ist es DASSELBE Objekt. Wer die Richtung einmal von Hand wechselte und
  // später über dieselbe Kachel zurückkam, landete deshalb beim zuletzt gesehenen Stand statt
  // beim angeforderten (gemessen: 1. Mal scan=true, 2. Mal scan=false).
  // Jetzt hängt es an der NAVIGATION, nicht am UI-Zustand: gemerkt wird das params-Objekt, das
  // der Router für genau diesen Eintritt angelegt hat (router.js legt { id, params } als neues
  // Objekt ab). Jeder neue Eintritt ist ein neues Objekt und setzt die Richtung erneut; jeder
  // Neuaufbau desselben Eintritts lässt die Wahl des Menschen stehen.
  // Eine WeakSet statt einer Markierung IM params-Objekt, weil app.js aus genau diesem Objekt
  // die UI-Signatur baut (JSON.stringify) — ein zusätzliches Feld würde den UI-Zustand des
  // Blattes bei jedem Render wegwerfen.
  const modus = ctx.params?.modus || '';
  const zuDenAnfragen = Boolean(ctx.params?.anfragen);
  if ((modus || zuDenAnfragen) && ctx.params && !MODUS_GESETZT.has(ctx.params)) {
    MODUS_GESETZT.add(ctx.params);
    if (modus) ctx.ui.qrView = modus === 'scan' ? 'scan' : 'qr';
    // Runde 8 (R8-11): Wer vom Anfrage-Hinweis kommt, will antworten — das Blatt geht gleich ganz
    // auf, die erhaltenen Anfragen stehen im Blick statt unter dem Rand.
    if (zuDenAnfragen) ctx.ui.qrExpanded = true;
  }
...
function requestActs(ctx) {
  ...
  return {
    ...
    // R8-36 „Anfragen-Muster wie in Crew": dieselben Acts wie im Blatt hinter dem „+" in Crew
    // (anfrage-annehmen / anfrage-ignorieren) — dieselbe Zeile tut überall dasselbe. „Ignorieren"
    // (R8-11: Anfragen sind ignorierbar) nimmt die Anfrage still aus der Liste; wer sie geschickt
    // hat, erfährt davon nichts.
    'anfrage-annehmen': (data) => {
      const anfrage = (repo.getFriendRequests() || []).find((entry) => entry.id === data.anfrage);
      if (!anfrage) return;
...
function bindQrZug(root, ctx) {
  ...
  // Rahmen und am Fenster, nicht an der Karte.
  //
  // Runde 6 (A5): Jeder Bind bringt einen frischen ctx mit, die Ereignisse hängen aber dauerhaft
  // am Rahmen. Deshalb steht der aktuelle ctx AM RAHMEN (statt im Abschluss des ersten Aufbaus),
  // und alle beweglichen Teile der Geste liegen in EINEM Objekt daneben — sonst arbeiteten
  // Ereignisse und Griff-Tipp mit verschiedenen Kopien desselben Zustands.
  rahmen.__qrCtx = ctx;
  const jetzt = () => rahmen.__qrCtx || ctx;
  const S = rahmen.__qrStand || (rahmen.__qrStand = { bild: 0, stand: null, start: null, rad: null, masse: null, versatz: null, deckkraft: null });

  // R8-36: Kompakt ist das Blatt so hoch wie sein Inhalt (die Liste höchstens LISTE_KOMPAKT). Das
  // wird gemessen, nicht angenommen: alles außer der Liste, plus der Inhalt der Liste bis zu ihrer
  // kompakten Höhe — in jedem Zustand dieselbe Zahl, auch wenn das Blatt gerade offen ist.
  const kompaktVon = (k, max) => {
    const liste = k?.querySelector('[data-scroll-keep="invite-requests"]');
    if (!k || !liste) return max;
    const ohneListe = k.offsetHeight - liste.offsetHeight;
    const inhalt = [...liste.children].reduce((summe, kind) => summe + kind.offsetHeight, 0) + QR_LISTE_UNTEN;
    return Math.min(max, Math.round(ohneListe + Math.min(inhalt, LISTE_KOMPAKT)));
  };
  // Runde 9 (Jonathan): Das Blatt geht nie weiter auf, als sein Inhalt reicht — offen ist es so
  // hoch wie alles darin (höchstens bis 48 px unter den Rand).
  const vollVon = (k, max) => {
    const liste = k?.querySelector('[data-scroll-keep="invite-requests"]');
    if (!k || !liste) return max;
    const inhalt = [...liste.children].reduce((summe, kind) => summe + kind.offsetHeight, 0) + QR_LISTE_UNTEN;
    return Math.min(max, Math.round(k.offsetHeight - liste.offsetHeight + inhalt));
  };
  const messen = () => {
    const r = rahmen.getBoundingClientRect();
    const skala = rahmen.offsetWidth ? r.width / rahmen.offsetWidth : 1;
    const rand = Math.max(160, rahmen.offsetHeight - 48);
    const kompakt = kompaktVon(karteJetzt(), rand);
    const max = Math.max(kompakt, vollVon(karteJetzt(), rand));
    return { max, kompakt, skala: skala || 1 };
  };
  // Runde 6 (A5, gemessen): masse() las bei JEDER Bewegung das Layout (getBoundingClientRect,
  // offsetWidth, offsetHeight) — unmittelbar nachdem das letzte Bild `translate` geschrieben
  // hatte. Jedes Lesen nach einem Schreiben erzwingt eine Neuberechnung der Stile: 31 Stück je
  // Zug, ~2 ms je Bewegung mit sechsfacher Bremse gegenüber 0,5 ms, sobald sich nichts mehr
  // ändert. Der Rahmen ändert seine Größe während eines Zugs nicht — einmal messen reicht.
  const masse = () => {
    if (!S.masse) S.masse = messen();
    return S.masse;
  };
  ...
  // Runde 6 (A5): Geschrieben wird nur, was sich WIRKLICH ändert. Der Schleier trägt einen
  // Weichzeichner über den ganzen Schirm; jede Berührung seines Stils lässt ihn neu rechnen —
  // auch wenn derselbe Wert erneut hineingeschrieben wird. Beim Aufziehen bleibt seine
  // Deckkraft konstant 1,000: dort wird jetzt gar nichts mehr geschrieben.
  const malen = () => {
    S.bild = 0;
    const z = S.stand;
    if (!z) return;
    const versatz = `0 ${z.versatz.toFixed(1)}px`;
    if (versatz !== S.versatz) { z.k.style.translate = versatz; S.versatz = versatz; }
    if (z.deckkraft !== S.deckkraft) {
      const schleier = schleierVon(z.k);
      if (schleier) schleier.style.opacity = z.deckkraft;
      S.deckkraft = z.deckkraft;
    }
  };
  const vorbereiten = (k, hoehe) => {
    ...
    // R8-36: Während des Zugs steht die Karte in voller Höhe — die Liste darf dann nachrücken.
    const liste = k.querySelector('[data-scroll-keep="invite-requests"]');
    if (liste) liste.style.maxHeight = 'none';
    S.versatz = `0 ${(max - hoehe).toFixed(1)}px`;
    S.deckkraft = null;
    k.style.translate = S.versatz;
  };
  const zeigen = (k, hoehe) => {
    const { max, kompakt } = masse();
    const unter = Math.max(0, kompakt - hoehe);
    S.stand = { k, versatz: max - hoehe, deckkraft: Math.max(0.08, 1 - unter / kompakt).toFixed(3) };
    if (!S.bild) S.bild = requestAnimationFrame(malen);
  };
  const aufraeumen = (k, offen) => {
    if (S.bild) { cancelAnimationFrame(S.bild); S.bild = 0; }
    S.stand = null;
    S.versatz = null;
    S.deckkraft = null;
    k.removeAttribute('data-sheet-zug');
    for (const eigenschaft of ['translate', 'transition', 'top']) k.style.removeProperty(eigenschaft);
    // Die Höhe stand vor dem Zug als Inline-Stil im Markup der Karte. Sie einfach zu entfernen
    // ließe die Karte auf Inhaltshöhe zusammenfallen — sie bekommt deshalb genau den Wert
    // ihres Zustands zurück, denselben, den ein Neuzeichnen schreiben würde.
    const zustand = offen === undefined ? Boolean(jetzt().ui.qrExpanded) : offen;
    k.style.height = qrHoehe(zustand);
    const listeZustand = k.querySelector('[data-scroll-keep="invite-requests"]');
    if (listeZustand) listeZustand.style.maxHeight = qrListeHoehe(zustand);
  ...
  // Runde 6 (A5, gemessen): Am Ende jedes Zugs wechselte der Zustand über ctx.render() — ein
  // vollständiger Neuaufbau des ganzen Profil-Bildschirms samt QR-Grafik, genau in dem Moment,
  // in dem die Karte zur Ruhe kommt (gemessen: 7 ms hier, 53 ms mit sechsfacher Bremse — auf
  // dem Handy mehrere verlorene Bilder). Der Zustand ist aber reine Geometrie: Höhe, Kennzeichen,
  // Aktion des Griffs, Scrollen der Liste. Genau das wird hier gesetzt, ohne Neuzeichnen. Das
  // nächste ohnehin fällige Neuzeichnen baut daraus dasselbe Markup.
  const einrasten = (k, offen) => {
    jetzt().ui.qrExpanded = offen;
    aufraeumen(k, offen);
    if (!k.isConnected) return;
    k.setAttribute('data-qr-state', offen ? 'offen' : 'kompakt');
    k.querySelector('[data-qr-grab]')?.setAttribute('data-act', offen ? 'qr-collapse' : 'qr-expand');
    const anfragen = k.querySelector('[data-scroll-keep="invite-requests"]');
    if (anfragen) anfragen.style.overflowY = offen ? 'auto' : 'hidden';
  };
  ...
  const gleiten = (k, hoehe, danach) => {
    ...
    requestAnimationFrame(() => {
      ...
      S.versatz = `0 ${(max - hoehe).toFixed(1)}px`;
      k.style.translate = S.versatz;
      if (schleier) { schleier.style.transition = `opacity 260ms ${QR_KURVE}`; schleier.style.opacity = '1'; S.deckkraft = '1'; }
      window.setTimeout(() => { k.__qrGleitet = false; if (k.isConnected) danach(); }, 290);
    });
  };

  // Der Griff-Tipp nimmt denselben Weg wie der Zug: aufziehen und einklappen gleiten, statt zu
  // springen — und zwar ohne Neuzeichnen. app.js/qrSheetActs rufen das über den Rahmen auf.
  rahmen.__qrSchalten = (offen) => {
    const k = karteJetzt();
    if (!k) { jetzt().ui.qrExpanded = offen; jetzt().render(); return; }
    if (k.__qrGleitet) return;
    S.masse = null;
    const { max, kompakt } = masse();
    vorbereiten(k, k.offsetHeight);
    gleiten(k, offen ? max : kompakt, () => einrasten(k, offen));
  };
  ...
  const zuschieben = (k) => {
    ...
    schleier?.click();
    if (!k.isConnected) return;
    if (!k.closest('[data-exit]')) { gleiten(k, kompakt, () => aufraeumen(k, false)); return; }
    requestAnimationFrame(() => {
  ...
  const beginn = (x, y, ziel, maus = false) => {
    const k = karteJetzt();
    if (!k || !ziel || !ziel.closest || !k.contains(ziel) || k.__qrGleitet) { S.start = null; return; }
    if (maus && ziel.closest('input, textarea')) { S.start = null; return; }
    const anfragen = k.querySelector('[data-scroll-keep="invite-requests"]');
    const ausListe = !ziel.closest('[data-qr-grab]') && Boolean(anfragen && anfragen.contains(ziel));
    // Runde 6 (A5): Hier — einmal je Geste — wird gemessen. Danach fasst kein Bewegungsschritt
    // mehr das Layout an.
    S.masse = null;
    S.start = { y, hoehe: k.offsetHeight, bewegt: false, ausListe, liste: anfragen, letzte: null, spur: [], ueber: false };
    masse();
  };
  ...
  const zug = (y, zeit) => {
    ...
    if (!start.bewegt) {
      ...
      if (start.ausListe) {
        ...
        if (dy > 0 ? ganzOben : scroll > 0) { S.start = null; return false; }
      }
      start.bewegt = true;
  ...
  const ende = () => {
    const k = karteJetzt();
    const start = S.start;
    if (!start || !k) { S.start = null; return; }
  ...
  // Die Ereignisse werden genau EINMAL je Rahmen angemeldet; alles darüber arbeitet mit dem
  // Zustand am Rahmen und liest den jeweils aktuellen ctx.
  ...
  rahmen.addEventListener('wheel', (e) => {
    ...
    let rad = S.rad;
    if (!rad) {
      const warOffen = Boolean(jetzt().ui.qrExpanded);
      const inListe = Boolean(anfragen && anfragen.contains(e.target));
      ...
      const besitzer = warOffen && (schritt > 0 || (inListe && anfragen.scrollTop > 0)) ? 'liste' : 'sheet';
      S.masse = null;
    ...
    if (rad.besitzer === 'liste') {
      ...
      if (anfragen && !anfragen.contains(e.target)) {
        anfragen.scrollTop += schritt;
...
// R8-38: Das Bild wartet auf einen Tipp (iOS im Stromsparmodus u. a.) — play() läuft IM Tipp.
// → Promise<boolean>: läuft das Bild jetzt?
function qrScanAntippen() {
  return qrScan?.stop?.antippen ? qrScan.stop.antippen() : Promise.resolve(false);
}
...
function bindQrScan(root, ctx) {
  ...
  const stop = starteQrScan(feld, {
    ...
    // R8-38: Die Kamera läuft, aber das Bild wartet auf einen Tipp — oder es läuft (wieder).
    onZustand: (zustand) => {
      if (!eigenerScan()) return;
      const neu = zustand === 'antippen' ? 'antippen' : '';
      if ((ui.qrKamera || '') === neu || ui.qrKamera === 'erkannt') return;
      ui.qrKamera = neu;
      ctx.render();
    },
  });
...
function qrSheetActs(root, ctx, schliessen) {
  ...
  return {
    ...
    // Runde 6 (A5): Der Tipp auf den Griff nimmt denselben Weg wie der Zug — die Karte gleitet
    // in ihre neue Lage, statt zu springen, und kommt ohne Neuzeichnen aus (bindQrZug). Fehlt
    // der Zug (z. B. ohne Rahmen), bleibt es beim alten Weg.
    'qr-expand': () => qrSchalten(root, ctx, true),
    'qr-collapse': () => qrSchalten(root, ctx, false),
  };
}

function qrSchalten(root, ctx, offen) {
  const rahmen = root.querySelector('.runtime-phone') || root;
  if (typeof rahmen.__qrSchalten === 'function') { rahmen.__qrSchalten(offen); return; }
  ctx.ui.qrExpanded = offen;
  ctx.render();
}

// Welche Eintritte (params-Objekte) die Anfragenliste schon ans Ende gerollt haben.
const ANFRAGEN_GEROLLT = new WeakSet();

function bindFriendAdd(root, ctx) {
  // Runde 8 (R8-11): Hier stehen alle erhaltenen Anfragen — groß muss danach keine mehr erscheinen.
  anfrageHinweisGesehen(ctx, (ctx.repo.getFriendRequests() || []).map((anfrage) => anfrage.id));
  // Runde 9: Hier stehen die Anfragen ohnehin groß — die Karte im Kopf ist damit erledigt.
  anfrageHinweisBeenden(ctx);
  // Vom Hinweis aus: Die neueste Anfrage — die, auf die man eben getippt hat — kommt als letzte
  // an. Die Liste rollt deshalb einmal je Eintritt an ihr Ende, damit genau sie im Blick steht.
  if (ctx.params?.anfragen && !ANFRAGEN_GEROLLT.has(ctx.params)) {
    ANFRAGEN_GEROLLT.add(ctx.params);
    const liste = root.querySelector('[data-scroll-keep="invite-requests"]');
    if (liste) requestAnimationFrame(() => { liste.scrollTop = liste.scrollHeight; });
  }
...
${navRow({ act: 'open', data: { route: 'profile.location' }, label: tx('Standort'), valueHtml: value(locationValue(settings)) })}
${/* Runde 7 (H2): Zuhause war eine Rechnung ohne Eingabefeld — hier ist der Weg dorthin. */''}
${navRow({ act: 'open', data: { route: 'profile.zuhause' }, label: tx('Zuhause'), valueHtml: value(zuhauseWert(repo)) })}
${/* Routen speichern (0047): die freiwillige Abholadresse — nur für den Fahrer, bei dem man mitfährt. */''}
${navRow({ act: 'open', data: { route: 'profile.abholadresse' }, label: tx('Abholadresse'), valueHtml: value(abholWert(repo)), last: true })}`)}
</div>
<div style="display:flex;flex-direction:column;gap:6px">
${groupLabel(tx('App'))}
...
${navRow({ act: 'open', data: { route: 'profile.freiReset' }, label: tx('Frei zurücksetzen um'), valueHtml: numberValue(settings.freeResetTime || '00:00') })}
${navRow({ act: 'guide-wiederholen', label: tx('Kurzer Einstieg'), valueHtml: value(tx('ansehen')), last: true })}`)}
</div>
</div>`;
}
  ...
  return {
    ...
    bind: (root, innerCtx) => {
      ...
      bindActions(root, {
        ...
        // Runde 6 (B1): „im Profil wiederholbar" — dieselben vier Karten wie beim ersten Start.
        'guide-wiederholen': () => { guideStarten(innerCtx, { wiederholung: true }); },
      });
    },
  };
}
...
// Runde 11 (A5): Die Karte „Standort für Vorschläge" (mit „Vergessen") ist aus den Freigaben raus —
// der Standort für Vorschläge ist immer an, und „Vergessen" gibt es nur noch unter „Was Crew gelernt hat".

function renderVisibility(ctx) {
  const settings = ctx.repo.getSettings();
  // v7 A33a: verbindlicher Standardwert (vollständiges Profil). Runde 11 (A13): Einen Aktivitätsstatus
  // für Freunde gibt es nicht mehr — nur „frei" zählt.
...
// Runde 11 (C4, D8 — Jonathan: „App zu: nur die native App … die App sagt das ehrlich"): Was mit dem
// Standort geschieht, wenn Crew ZU ist, hängt am Gerät, nicht an einer Einstellung:
//   · Browser / Web-App: nichts — kein Browser läuft im Hintergrund weiter (core/native.js WEB.standortHintergrund).
//   · native App, Wache läuft: das Handy meldet sich selbst, sparsam, bei Ortswechsel (iOS) bzw. auch etwa alle
//     30 Minuten (Android) — und seit 0044 erreicht das den Server sofort (CrewStandort › anServer / StandortServer).
//   · native App ohne „Immer"-Erlaubnis: ein Knopf, der genau diese Erlaubnis anfragt (core/native.js wacheAnfragen).
function standortZuStand() {
  if (kann('standort-hintergrund').wie !== 'nativ') return { art: 'web', text: tx('Im Hintergrund kann das nur die Crew-App fürs Handy, nicht der Browser.') };
  if (huelleLage().wege.wache) return { art: 'laeuft', text: tx('Auch wenn Crew zu ist, meldet dein Handy deinen Standort — sparsam, sobald du woanders bist.') };
  return { art: 'erlauben', text: tx('Ist Crew zu, meldet dein Handy deinen Standort erst, wenn du es erlaubst.') };
}
...
<span style="display:flex;transform:rotate(${offen ? 90 : 0}deg);transition:transform .16s ease-out;pointer-events:none">${svgRowChevron}</span></button>`
  ...
  const html = screenScaffold({
    ...
    scrollKey: 'location',
    body: `<div style="display:flex;flex-direction:column;gap:14px;padding:2px 20px 0">
${groupCard(`<div style="display:flex;justify-content:space-between;align-items:center;padding:13px 16px"><div style="display:flex;flex-direction:column;gap:2px"><span style="font-size:14px;font-weight:600">${tx('Standort verwenden')}</span></div>${toggleSwitch(Boolean(location.use), 'loc-use')}</div>`)}
${location.use ? (() => {
    const zu = standortZuStand();
    const vorher = zu.art === 'laeuft' ? '' : `${tx('Solange Crew offen ist, bleibt dein Standort frisch. Ist die App zu, sehen deine Freunde nur, wann du zuletzt dort warst.')} `;
    const knopf = zu.art === 'erlauben'
      ? `<button data-act="loc-hintergrund" data-role="standort-hintergrund" style="margin:6px 0 0;padding:9px 14px;border-radius:999px;border:1.5px solid var(--ink-a14);background:var(--surface);color:var(--ink);font:650 13px/1 ${FONT};cursor:pointer;appearance:none"><span style="pointer-events:none">${tx('Auch erlauben, wenn Crew zu ist')}</span></button>`
      : '';
    return `<div data-role="standort-ehrlich" data-art="${zu.art}" style="display:flex;flex-direction:column;align-items:flex-start;font-size:11.5px;color:var(--muted);line-height:1.45;padding:0 4px"><span>${vorher}${zu.text}</span>${knopf}</div>`;
  })() : ''}
    ...
    bind: (root, innerCtx) => {
      ...
      bindActions(root, {
        'loc-use': () => {
          ...
          if (!next) {
            innerCtx.ui.locPickOpen = false;
            repo2.updateSettings({ location: { ...current(), use: false } });
            return;
          }
          // Runde 9 (Hausregel 9): „an" nur, wenn das System den Standort erlaubt. Der Tipp fragt
          // das System (dessen eigenes Fenster); sagt es nein, bleibt der Schalter aus.
          erlaubnisFragen('standort').then((wort) => {
            if (wort === 'erteilt') {
              repo2.updateSettings({ location: { ...current(), use: true } });
              repo2.standortSenden?.({ sofort: true });
            } else {
              innerCtx.toast?.(tx('Standort ist in den Einstellungen des Geräts aus'));
            }
          });
        },
        ...
        // Runde 11 (C4): „Immer erlauben" — nur hier, nur auf diesen Tipp (der Klick geht mit, core/native.js
        // istKnopfDruck). Was danach gilt, sagt das Telefon; steht es weiter auf „nur beim Benutzen", führt der
        // Weg über die Einstellungen des Handys, und genau das steht dann da.
        'loc-hintergrund': (_data, _el, ereignis) => {
          wacheAnfragen(ereignis).then((lage) => {
            innerCtx.render();
            if (!lage?.hintergrund) innerCtx.toast?.(tx('Stell in den Einstellungen des Handys den Standort für Crew auf „Immer".'));
          }).catch(() => {});
        },
        'loc-person': (data) => {
          if (!current().use) return;
...
// R8-38: Hier landen Anfragen zu den eigenen Daten (dieselbe Adresse wie auf web/konto-loeschen.html).
const KONTAKT_ADRESSE = 'development@desteny.at';
...
<button data-act="acc-password-save" aria-disabled="true" style="${GREEN_PILL};margin-top:14px;opacity:.4">${tx('Speichern')}</button>`, { closeAct: 'acc-close' });
  ...
  // R8-38: Während des Abmeldens zeigt der Knopf, dass es läuft — ein zweiter Tipp tut nichts, und
  // der Schleier schließt das Blatt nicht (sonst stünde man wieder im Konto, während es abmeldet).
  if (ui.sheet === 'abmelden') {
    const laeuft = Boolean(ui.abmeldenLaeuft);
...
<button data-act="acc-signout" data-role="abmelden-knopf"${laeuft ? ' aria-disabled="true" aria-busy="true"' : ''} style="${GREEN_PILL};margin-top:12px;${laeuft ? 'opacity:.6;cursor:default;' : ''}">${laeuft ? tx('Wird abgemeldet …') : tx('Abmelden')}</button>
${laeuft ? '' : `<button data-act="acc-close" style="border:1.5px solid var(--ink-a14);background:var(--surface);color:var(--ink);font:650 14px/1 ${FONT};padding:13px 0;border-radius:999px;width:100%;cursor:pointer;appearance:none;margin-top:9px">${tx('Abbrechen')}</button>`}`, { closeAct: laeuft ? 'acc-abmelden-laeuft' : 'acc-close' });
  }
  // Runde 6 (E2, Jonathan: „mach konto löschung versteckter") → Runde 8 (R8-38, Jonathan: „Konto:
  // erst Infos; ‚Konto verwalten' → Passwort/E-Mail; ganz unten Daten anfordern / Konto löschen."):
  // Auf der Seite stehen nur noch die Angaben zum Konto. Was man daran ÄNDERN kann, liegt hier —
  // oben, was man manchmal braucht (E-Mail, Passwort), und abgesetzt ganz unten, was man fast nie
  // braucht und bewusst suchen soll: Daten anfordern und das Konto löschen. Beide als leise
  // Textzeilen ohne Kasten — findbar für den, der sie sucht (auch Pflicht in den App-Läden), aber
  // nicht im Weg. Die Löschung selbst fragt danach wie bisher noch einmal nach.
  if (ui.sheet === 'verwalten') {
    const aendern = konto.echt
...
${konto.kannPasswort ? navRow({ act: 'acc-sheet', data: { sheet: 'passwort' }, label: tx('Passwort ändern'), last: true }) : ''}`)
      : `<span style="display:block;font-size:12.5px;color:var(--ink-soft);line-height:1.5">${tx('Die Demo läuft nur auf diesem Gerät.')}</span>`;
    const leise = (ziel, text, rot, rolle) => `<button data-act="acc-sheet" data-sheet="${ziel}" data-role="${rolle}" style="display:block;min-height:44px;border:0;background:transparent;padding:0 12px;margin:0 auto;cursor:pointer;appearance:none;font:600 13px/1.2 ${FONT};color:${rot ? 'var(--red)' : 'var(--muted)'}"><span style="pointer-events:none">${esc(text)}</span></button>`;
    return sheet(`<span style="font-family:${BRICOLAGE};font-size:19px;font-weight:650;display:block;padding-bottom:12px">${tx('Konto verwalten')}</span>
<div data-role="konto-aendern">${aendern}</div>
<div data-role="konto-verwalten" style="display:flex;flex-direction:column;align-items:center;margin-top:48px;padding-top:6px;border-top:1px solid var(--ink-a06)">
${konto.echt ? leise('daten', tx('Daten anfordern'), false, 'konto-daten') : ''}
${leise('delete', tx('Desteny-Konto löschen'), true, 'konto-loeschen')}
</div>`, { closeAct: 'acc-close' });
  }
  // R8-38: „Daten anfordern" — eine Kopie aller Daten zu diesem Konto. Die Anfrage geht dorthin, wo
  // sie bearbeitet wird: per Mail an Desteny Development (dieselbe Adresse wie auf
  // konto-loeschen.html). Die Mail schreibt und schickt man selbst — die App schickt nichts in
  // fremdem Namen, und in der Adresse der Mail steht nichts über das Konto: erkannt wird es am
  // Absender. Geht auf einem Gerät keine Mail-App auf, steht die Adresse im Satz darüber.
  if (ui.sheet === 'daten') {
    const mail = `mailto:${KONTAKT_ADRESSE}?subject=${encodeURIComponent(tx('Crew – Daten anfordern'))}&body=${encodeURIComponent(tx('Hallo, bitte schickt mir eine Kopie aller Daten, die Crew zu meinem Konto speichert.'))}`;
    return sheet(`<span style="font-family:${BRICOLAGE};font-size:19px;font-weight:650;display:block;padding-bottom:8px">${tx('Daten anfordern')}</span>
<span data-role="daten-satz" style="display:block;font-size:12.5px;color:var(--ink-soft);line-height:1.5">${tx('Du bekommst eine Kopie aller Daten, die Crew zu deinem Konto speichert. Die Anfrage geht per Mail an {adresse}.', { adresse: `<strong style="color:var(--ink);white-space:nowrap">${KONTAKT_ADRESSE}</strong>` })}</span>
<a href="${esc(mail)}" data-role="daten-mail" style="${GREEN_PILL};display:block;box-sizing:border-box;margin-top:14px;text-decoration:none">${tx('Mail schreiben')}</a>`, { closeAct: 'acc-close' });
  }
  if (ui.sheet === 'delete') {
...
// Desteny-Apps. Ruhig, in Tinte — keine Werbung.
// Runde 8 (R8-38): Die Seite zeigt nur noch, WAS das Konto ist (Karte, Apps), dann „Abmelden" und
// ganz unten „Konto verwalten". Ändern, Daten anfordern und Löschen liegen eine Ebene tiefer.
...
${konto.echt
  ...
  // Runde 8 (R8-38): „erst Infos“. Was man am Konto ändern kann (E-Mail, Passwort), steht nicht mehr
  // auf der Seite, sondern unter „Konto verwalten“ — zusammen mit „Daten anfordern“ und dem Löschen.
  ...
  const html = screenScaffold({
    ...
    // Runde 6 (E2): Ganz unten steht nicht mehr die rote Löschkarte, sondern die ruhige Zeile
    // „Konto verwalten" — ohne zweite Scrollfläche: der Inhaltskörper ist mindestens so hoch
    // wie die Scrollfläche, die letzte Karte schiebt sich per margin-top:auto ans Ende.
...
${konto.echt
      ...
      : ''}
<div data-role="konto-verwalten-zeile" style="margin-top:auto;flex:none">
${groupCard(navRow({ act: 'acc-sheet', data: { sheet: 'verwalten' }, label: tx('Konto verwalten'), last: true }))}
</div>
</div>`,
  });
...
function bindAccount(root, ctx) {
  ...
  bindActions(root, {
    ...
    // R8-38 (Jonathan: „Abmelden muss beim ERSTEN Mal klappen — heute springt man 2–3 Mal zurück").
    // Ursache: Hier stand `repo.signOut(); setTimeout(() => location.reload(), 120)`. Das Abmelden am
    // Server läuft aber in der Schreibschlange der Datenschicht und braucht mehrere Anfragen
    // hintereinander (Push-Abo dieses Geräts löschen, Echtzeit-Kanal schließen, Sitzung beenden) —
    // auf dem Handy weit mehr als 120 ms. Das Neuladen riss es mittendrin ab: Die gespeicherte
    // Sitzung lag noch im Gerät, die App startete wieder angemeldet, man war „zurück". Nur wenn das
    // Netz gerade schnell genug war, klappte es — daher „2–3 Mal".
    // Jetzt: Der Knopf zeigt „Wird abgemeldet …" (ein zweiter Tipp tut nichts), gewartet wird, bis
    // die Datenschicht fertig ist (settled), und neu geladen wird erst DANACH. Der Server-Weg lädt
    // nach getaner Arbeit selbst neu (zurAnmeldung); steht die Seite 1,5 s danach noch, lädt sie hier.
    'acc-signout': async () => {
      if (ui.abmeldenLaeuft) return;
      ui.abmeldenLaeuft = true;
      ctx.render();
      try { await repo.signOut(); } catch { /* die Datenschicht meldet Fehler selbst */ }
      try { if (typeof repo.settled === 'function') await repo.settled(20000); } catch { /* weiter */ }
...
// Runde 9 (Jonathan: „Erklärungstexte entfernen … ‚Ton und Vibration testen' als kleinen
// Nischen-Eintrag ganz nach unten"): Die Karte des Geräts bleibt, wie sie ist — ohne die Probe-Karte
// darunter; die Probe steht als kleine Zeile am Ende der Seite (tonProbeZeile).
function pushKarte(ctx) {
  return pushSeitenKarte(ctx, { schalter: (an) => toggleSwitch(an, 'push-toggle'), tonProbe: false });
}
...
<span style="font-size:14px;font-weight:600">${tx('Töne')}</span>
${toggleSwitch(notifications.toene !== false, 'notif-toene')}</div>`);
}

// Die Probe: dieselbe Aktion wie bisher (ui/mitteilungen.js › tonProbeAktionen), nur klein und unten.
function tonProbeZeile() {
  return `<button data-act="ton-probe" data-role="ton-probe-zeile" data-treffer style="align-self:center;display:flex;align-items:center;gap:6px;min-height:40px;padding:0 12px;border:0;background:transparent;color:var(--muted);font:600 12.5px/1 ${FONT};cursor:pointer;appearance:none"><span style="pointer-events:none;display:flex">${symbol('wellen', 'var(--muted)', 14)}</span><span style="pointer-events:none">${tx('Ton und Vibration testen')}</span></button>`;
}
...
<span style="font-size:14px;font-weight:600">${tx('Wenn dich jemand anstupst')}</span>
${toggleSwitch(notifications.anstupser !== false, 'notif-anstupser')}</div>`)}</div>
${choiceCard(tx('Bescheid, wenn jemand frei ist'), `
...
<span style="font-family:${BRICOLAGE};font-size:19px;font-weight:650;flex:1">${tx('Bescheid bei · Individuell')}</span>
...
function renderFreiReset(ctx) {
  ...
  // Runde 7 (H1): Bis Runde 6 stand hier eine Zusage ohne Funktion — die Uhrzeit wurde
  // gespeichert, angezeigt und von niemandem gelesen; der Satz versprach obendrein ein
  // Zurücksetzen „durch dein eigenes aktives Meet", das es im Code nirgends gab. Jetzt wirkt
  // die Einstellung wirklich (repo.freiZuruecksetzenAm), und der Bildschirm sagt beides
  // ehrlich: WAS passiert und WANN es das nächste Mal passiert.
  //
  // Die Vorschau rechnet mit dem Rad, nicht mit dem gespeicherten Wert: Wer dreht, will sehen,
  // was seine Wahl bedeutet, BEVOR er sie übernimmt. Gerechnet wird mit derselben Regel wie im
  // Gateway (projections.naechsteFreiGrenze) — eine zweite Rechnung hier wäre eine zweite
  // Wahrheit über denselben Zeitpunkt.
  const gewaehlteZeit = `${pad2(ui.hour)}:${pad2(ui.minute)}`;
  const free = repo.getSettings().free || {};
  const anker = free.pending && Number(free.fromAt) > 0 ? Number(free.fromAt) : Number(free.setAt) || 0;
  const grenze = free.active ? naechsteFreiGrenze(gewaehlteZeit, anker) : null;
  const heuteNoch = grenze ? new Date(grenze).toDateString() === new Date().toDateString() : false;
  const wirkung = grenze
    ? tx('Du bist gerade frei. Mit dieser Uhrzeit nimmt Crew das {wann} von selbst zurück.', {
      wann: heuteNoch ? tx('heute um {zeit}', { zeit: gewaehlteZeit }) : tx('morgen um {zeit}', { zeit: gewaehlteZeit }),
    })
    : tx('Du bist gerade nicht frei. Die Uhrzeit gilt ab dem nächsten Mal, wenn du FREE drückst.');

  const content = `<span style="font-family:${BRICOLAGE};font-size:19px;font-weight:650;display:block">${tx('Frei zurücksetzen um')}</span>
<span style="display:block;font-size:12.5px;color:var(--ink-soft);line-height:1.45;padding:8px 0 2px">${tx('Bist du frei, nimmt Crew das zu dieser Uhrzeit von selbst zurück — auch dann, wenn die App inzwischen zu war. „Busy“ und deine Meets bleiben davon unberührt.')}</span>
<span data-role="frei-reset-wirkung" style="display:flex;align-items:center;gap:8px;margin-top:10px;padding:10px 12px;border-radius:14px;background:${grenze ? 'var(--green-tint)' : 'var(--field)'};font-size:12.5px;font-weight:600;color:${grenze ? 'var(--green-dark)' : 'var(--ink-soft)'};line-height:1.4">${esc(wirkung)}</span>
...
// ============================================================================
// 06.13 Zuhause (profile.zuhause) — Runde 7 (H2)
// ============================================================================
//
// settings.homeAddress gab es seit v6 nur noch im Beispielbestand: FESTLEGEN konnte man es
// nirgends, im echten Konto war es immer null. Gleichzeitig rechnen Vorschläge, Karte und
// Mitfahrten damit (repo.getVorschlagsMitte, getMyLocation mit quelle 'zuhause') — eine
// Rechnung ohne Eingabefeld. Hier ist das Feld: suchen, aus dem Standort übernehmen, ändern,
// entfernen.
//
// Hausregel 8 gilt hier doppelt, weil es schon einmal eine erfundene Adresse als „Zuhause" gab:
// Fehlt der Ort, sagt der Bildschirm das und zeigt KEIN Beispiel. Aus dem Gerätestandort
// entsteht ein Punkt ohne Straßennamen — Crew dichtet keinen dazu, sondern schreibt ehrlich
// „Aktueller Standort" und die Koordinaten.

function zuhauseTitel(ort) {
  return (ort?.name || '').trim() || tx('Aktueller Standort');
}

// Der Wert in der Einstellungszeile. Ohne Ort steht dort „Fehlt" — nicht „—" und nicht leer:
// Ein Strich sieht aus wie eine Einstellung, die es nicht gibt.
function zuhauseWert(repo) {
  const ort = repo.getHomeAddress?.() || null;
  return ort ? zuhauseTitel(ort) : tx('Fehlt');
}

function zuhauseGrundText(grund) {
  if (grund === 'verweigert') return tx('Der Standort ist für Crew nicht erlaubt.');
  if (grund === 'geraet') return tx('Dieses Gerät gibt keinen Standort her.');
  return tx('Der Standort war gerade nicht zu finden.');
}

// Die Trefferliste vereint die Orte, die die App schon kennt, mit der echten Adresssuche
// (ui/map.js ortSuchen) — derselbe Weg wie beim Ort eines Meets. Ohne Netz bleiben die
// bekannten Orte übrig, statt dass die Liste leer behauptet, es gäbe nichts.
let zuhauseNummer = 0;
let zuhauseTimer = null;

function zuhauseTreffer(ctx, p = 'zh') {
  const { repo, ui } = ctx;
  const frage = (ui[`${p}Query`] || '').trim();
  if (frage.length < 2) { ui[`${p}Liste`] = []; return []; }
  const bekannt = (repo.searchPlaces(frage) || []).filter((ort) => typeof ort.lat === 'number' && typeof ort.lon === 'number');
  const liste = [...bekannt];
  for (const treffer of (ui[`${p}Treffer`] || [])) {
    if (!liste.some((ort) => ort.name.toLowerCase() === String(treffer.name || '').toLowerCase())) liste.push(treffer);
  }
  ui[`${p}Liste`] = liste.slice(0, 6);
  return ui[`${p}Liste`];
}

function zuhauseSuche(ctx, p = 'zh') {
  clearTimeout(zuhauseTimer);
  const frage = (ctx.ui[`${p}Query`] || '').trim();
  if (frage.length < 2) {
    ctx.ui[`${p}Treffer`] = [];
    ctx.ui[`${p}Sucht`] = false;
    ctx.render();
    return;
  }
  const meine = (zuhauseNummer += 1);
  ctx.ui[`${p}Sucht`] = true;
  ctx.render();
  zuhauseTimer = setTimeout(async () => {
    let gefunden = [];
    try {
      const { ortSuchen } = await import('../ui/map.js');
      gefunden = await ortSuchen(frage, { limit: 6 });
    } catch { /* ohne Netz bleiben die bekannten Orte */ }
    if (meine !== zuhauseNummer) return;
    ctx.ui[`${p}Treffer`] = (gefunden || []).filter((ort) => typeof ort.lat === 'number' && typeof ort.lon === 'number');
    ctx.ui[`${p}Sucht`] = false;
    ctx.render();
  }, 280);
}

function renderZuhause(ctx) {
  const { repo, ui } = ctx;
  const ort = repo.getHomeAddress?.() || null;
  const treffer = zuhauseTreffer(ctx);
  const frage = (ui.zhQuery || '').trim();

  const koordinaten = ort ? `${Number(ort.lat).toFixed(4)}, ${Number(ort.lon).toFixed(4)}` : '';
  const herkunft = ort ? (ort.quelle === 'standort' ? tx('aus deinem Standort') : tx('aus der Adresssuche')) : '';
  const karte = ort
    ? groupCard(`<div data-role="zuhause-karte" style="display:flex;align-items:flex-start;gap:12px;padding:14px 16px">
<span style="width:34px;height:34px;border-radius:11px;background:var(--green-tint);display:flex;align-items:center;justify-content:center;flex:none">${svgLockSmall('var(--green-dark)')}</span>
<span style="display:flex;flex-direction:column;gap:3px;flex:1;min-width:0">
<span data-role="zuhause-name" style="font-size:15px;font-weight:650;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(zuhauseTitel(ort))}</span>
${ort.city ? `<span style="font-size:12.5px;color:var(--muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(ort.city)}</span>` : ''}
<span style="font-size:11.5px;color:var(--muted-light);font-variant-numeric:tabular-nums">${esc(koordinaten)} · ${esc(herkunft)}</span>
</span></div>`)
    : groupCard(`<div data-role="zuhause-fehlt" style="display:flex;flex-direction:column;gap:4px;padding:14px 16px">
<span style="font-size:15px;font-weight:650">${tx('Noch kein Zuhause hinterlegt')}</span>
<span style="font-size:12.5px;color:var(--muted);line-height:1.45;text-wrap:pretty">${tx('Crew erfindet keine Adresse. Ohne Zuhause kann es nicht sagen, wie weit ein Vorschlag von dir weg ist.')}</span>
</div>`);

  const zeile = (text) => `<span style="display:block;font-size:12.5px;color:var(--ink-soft);line-height:1.45;text-wrap:pretty">${esc(text)}</span>`;
  const wozu = choiceCard(tx('Wozu'), `<div style="display:flex;flex-direction:column;gap:8px;padding:4px 0 12px">
${zeile(tx('Crew rechnet damit, wie weit Vorschläge und Treffpunkte von dir weg sind — und wie du zu einem Meet kommst.'))}
${zeile(tx('Es bleibt in deinen Einstellungen. Freunde sehen deine Adresse nicht — außer du sagst selbst Ja, wenn jemand fragt, ob ihr euch bei dir treffen könnt.'))}
</div>`);

  const trefferHtml = frage.length < 2
    ? ''
    : (treffer.length
      ? treffer.map((eintrag, index) => `<button data-act="zh-waehlen" data-index="${index}" data-role="zuhause-treffer" style="display:flex;align-items:center;gap:11px;padding:11px 0;min-height:44px;width:100%;border:0;border-top:1px solid var(--ink-a05);background:transparent;text-align:left;cursor:pointer;appearance:none;font-family:${FONT};color:var(--ink)">
<span style="display:flex;flex-direction:column;gap:1px;flex:1;min-width:0;pointer-events:none">
<span style="font-size:14px;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(eintrag.name)}</span>
<span style="font-size:11.5px;color:var(--muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(eintrag.address || eintrag.city || '')}</span>
</span>${svgRowChevron}</button>`).join('')
      : `<span style="display:block;font-size:12.5px;color:var(--muted);padding:12px 0">${ui.zhSucht ? tx('Suche läuft …') : tx('Kein Ort gefunden')}</span>`);

  const suche = choiceCard(tx('Adresse suchen'), `<div style="padding:6px 0 10px">
<input id="zuhause-suche" type="search" inputmode="search" autocomplete="off" placeholder="${esc(tx('Straße, Ort'))}" aria-label="${esc(tx('Adresse suchen'))}" style="${INPUT_STYLE}">
</div>${trefferHtml}`);

  const standort = `<div style="display:flex;flex-direction:column;gap:7px">
<button data-act="zh-standort" data-role="zuhause-standort" style="display:flex;align-items:center;justify-content:center;gap:8px;min-height:48px;width:100%;border:1.5px solid var(--ink-a14);border-radius:999px;background:var(--surface);cursor:pointer;appearance:none;font:650 14px/1 ${FONT};color:var(--ink);${ui.zhLaeuft ? 'opacity:.5' : ''}"><span style="pointer-events:none">${ui.zhLaeuft ? tx('Standort wird geholt …') : tx('Aktuellen Standort übernehmen')}</span></button>
<span style="display:block;font-size:11.5px;color:var(--muted);line-height:1.4;padding:0 4px;text-wrap:pretty">${tx('Daraus wird ein Punkt, keine Straße — Crew erfindet keinen Namen dazu.')}</span>
</div>`;

  const entfernen = ort
    ? `<button data-act="zh-entfernen" data-role="zuhause-entfernen" style="align-self:center;border:0;background:transparent;padding:12px 8px;min-height:44px;font:650 13.5px/1 ${FONT};color:var(--red);cursor:pointer;appearance:none"><span style="pointer-events:none">${tx('Zuhause entfernen')}</span></button>`
    : '';

  const html = screenScaffold({
    header: subHeader(tx('Zuhause')),
    bottomInset: 28,
    scrollKey: 'zuhause',
    body: `<div style="display:flex;flex-direction:column;gap:14px;padding:2px 20px 0">
${karte}
...
${entfernen}
  ...
  return {
    ...
    bind: (root, innerCtx) => {
      const repo2 = innerCtx.repo;
      const feld = root.querySelector('#zuhause-suche');
      if (feld) {
        // Kein value-Attribut im Markup: Der In-Place-Abgleich fasst den Knoten dadurch nie an,
        // der Fokus bleibt beim Tippen stehen und die Trefferliste darf sich frei neu zeichnen.
        if (innerCtx.ui.zhQuery && feld.value !== innerCtx.ui.zhQuery) feld.value = innerCtx.ui.zhQuery;
        feld.addEventListener('input', () => {
          innerCtx.ui.zhQuery = feld.value;
          zuhauseSuche(innerCtx);
        });
      }
      bindActions(root, {
        'zh-waehlen': (data) => {
          const eintrag = (innerCtx.ui.zhListe || [])[Number(data.index)];
          if (!eintrag) return;
          const ergebnis = repo2.setHomeAddress({
            name: eintrag.name,
            city: eintrag.address || eintrag.city || '',
            lat: eintrag.lat,
            lon: eintrag.lon,
            quelle: 'suche',
          });
          if (!ergebnis?.ok) {
            innerCtx.toast(ergebnis?.grund === 'name' ? tx('Dieser Treffer hat keinen Namen.') : tx('Dieser Treffer hat keine Koordinaten.'));
            return;
          }
          innerCtx.ui.zhQuery = '';
          innerCtx.ui.zhTreffer = [];
          innerCtx.ui.zhListe = [];
          if (feld) feld.value = '';
          haptik('erfolg');
          innerCtx.render();
          innerCtx.toast(tx('Zuhause gespeichert'));
        },
        'zh-standort': () => {
          if (innerCtx.ui.zhLaeuft) return;
          innerCtx.ui.zhLaeuft = true;
          innerCtx.render();
          Promise.resolve(repo2.setHomeAddressAusStandort()).then((ergebnis) => {
            innerCtx.ui.zhLaeuft = false;
            innerCtx.render();
            if (ergebnis?.ok) {
              haptik('erfolg');
              innerCtx.toast(tx('Zuhause auf deinen Standort gesetzt'));
              return;
            }
            innerCtx.toast(zuhauseGrundText(ergebnis?.grund));
          }).catch(() => {
            innerCtx.ui.zhLaeuft = false;
            innerCtx.render();
            innerCtx.toast(zuhauseGrundText('unbekannt'));
          });
        },
        'zh-entfernen': () => {
          repo2.clearHomeAddress();
          innerCtx.ui.zhQuery = '';
          innerCtx.ui.zhTreffer = [];
          if (feld) feld.value = '';
          innerCtx.render();
          innerCtx.toast(tx('Zuhause entfernt'));
        },
      });
    },
  };
...
// ============================================================================
// 06.13b Abholadresse (profile.abholadresse) — Routen speichern (0047)
// ============================================================================
//
// Freiwillig. Wer bei jemandem mitfährt, wird an dieser Adresse abgeholt — das ist der Standard. Nur wer bei einer
// Mitfahrt ausdrücklich „Von meinem aktuellen Standort“ wählt, wird am Live-Standort abgeholt. Sie ist NICHT das
// Zuhause: Das Zuhause bleibt privat. Die Abholadresse sieht nur der Fahrer, bei dem man mitfährt (Server:
// abholadressen_meines_autos). Es zählt nur, was die Person selbst sucht oder auswählt — Crew leitet nie selbst
// einen Wohnort ab und erfindet keine Adresse (Hausregel 8): Fehlt sie, sagt die Seite das.

function abholTitel(ort) {
  return [(ort?.name || '').trim(), (ort?.city || '').trim()].filter(Boolean).join(', ');
}

function abholWert(repo) {
  const ort = repo.getAbholadresse?.() || null;
  return ort ? ((ort.name || '').trim() || tx('Gesetzt')) : tx('Fehlt');
}

function renderAbholadresse(ctx) {
  const { repo, ui } = ctx;
  const ort = repo.getAbholadresse?.() || null;
  const treffer = zuhauseTreffer(ctx, 'ah');
  const frage = (ui.ahQuery || '').trim();
  const zuhause = repo.getHomeAddress?.() || null;
  // Das Zuhause lässt sich nur übernehmen, wenn es eine echte, benannte Adresse ist (aus der Suche) — kein Punkt aus dem Standort.
  const zuhauseNutzbar = zuhause && zuhause.quelle !== 'standort' && (zuhause.name || '').trim() && !(ort && ort.lat === zuhause.lat && ort.lon === zuhause.lon);

  const pin = '<svg width="17" height="17" viewBox="0 0 24 24" fill="none" style="display:block"><path d="M12 21.5s7-6.1 7-11a7 7 0 1 0-14 0c0 4.9 7 11 7 11Z" stroke="var(--green-dark)" stroke-width="2" stroke-linejoin="round"></path><circle cx="12" cy="10.5" r="2.4" fill="var(--green-dark)"></circle></svg>';
  const karte = ort
    ? groupCard(`<div data-role="abholadresse-karte" style="display:flex;align-items:flex-start;gap:12px;padding:14px 16px">
<span style="width:34px;height:34px;border-radius:11px;background:var(--green-tint);display:flex;align-items:center;justify-content:center;flex:none">${pin}</span>
<span style="display:flex;flex-direction:column;gap:3px;flex:1;min-width:0">
<span data-role="abholadresse-name" style="font-size:15px;font-weight:650;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(ort.name || tx('Gesetzt'))}</span>
${ort.city ? `<span style="font-size:12.5px;color:var(--muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(ort.city)}</span>` : ''}
</span></div>`)
    : groupCard(`<div data-role="abholadresse-fehlt" style="display:flex;flex-direction:column;gap:4px;padding:14px 16px">
<span style="font-size:15px;font-weight:650">${tx('Noch keine Abholadresse hinterlegt')}</span>
<span style="font-size:12.5px;color:var(--muted);line-height:1.45;text-wrap:pretty">${tx('Freiwillig. Ohne Adresse weiß dein Fahrer nicht, wo er dich abholen soll — außer du wählst bei einer Mitfahrt „Von meinem aktuellen Standort“.')}</span>
</div>`);

  const zeile = (text) => `<span style="display:block;font-size:12.5px;color:var(--ink-soft);line-height:1.45;text-wrap:pretty">${esc(text)}</span>`;
  const wozu = choiceCard(tx('Wozu'), `<div style="display:flex;flex-direction:column;gap:8px;padding:4px 0 12px">
${zeile(tx('Wenn du bei jemandem mitfährst, holt er dich an dieser Adresse ab.'))}
${zeile(tx('Nur der Fahrer, bei dem du mitfährst, sieht sie — sonst niemand. Löschst du dein Konto, ist sie mit allen gespeicherten Routen weg.'))}
</div>`);

  const trefferHtml = frage.length < 2
    ? ''
    : (treffer.length
      ? treffer.map((eintrag, index) => `<button data-act="ah-waehlen" data-index="${index}" data-role="abholadresse-treffer" style="display:flex;align-items:center;gap:11px;padding:11px 0;min-height:44px;width:100%;border:0;border-top:1px solid var(--ink-a05);background:transparent;text-align:left;cursor:pointer;appearance:none;font-family:${FONT};color:var(--ink)">
<span style="display:flex;flex-direction:column;gap:1px;flex:1;min-width:0;pointer-events:none">
<span style="font-size:14px;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(eintrag.name)}</span>
<span style="font-size:11.5px;color:var(--muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(eintrag.address || eintrag.city || '')}</span>
</span>${svgRowChevron}</button>`).join('')
      : `<span style="display:block;font-size:12.5px;color:var(--muted);padding:12px 0">${ui.ahSucht ? tx('Suche läuft …') : tx('Kein Ort gefunden')}</span>`);

  const suche = choiceCard(tx('Adresse suchen'), `<div style="padding:6px 0 10px">
<input id="abholadresse-suche" type="search" inputmode="search" autocomplete="off" placeholder="${esc(tx('Straße, Ort'))}" aria-label="${esc(tx('Adresse suchen'))}" style="${INPUT_STYLE}">
</div>${trefferHtml}`);

  const vomZuhause = zuhauseNutzbar
    ? `<div style="display:flex;flex-direction:column;gap:7px">
<button data-act="ah-zuhause" data-role="abholadresse-zuhause" style="display:flex;align-items:center;justify-content:center;gap:8px;min-height:48px;width:100%;border:1.5px solid var(--ink-a14);border-radius:999px;background:var(--surface);cursor:pointer;appearance:none;font:650 14px/1 ${FONT};color:var(--ink)"><span style="pointer-events:none">${tx('Mein Zuhause verwenden')}</span></button>
<span style="display:block;font-size:11.5px;color:var(--muted);line-height:1.4;padding:0 4px;text-wrap:pretty">${tx('Kopiert die Adresse — dein Zuhause selbst bleibt privat.')}</span>
</div>`
    : '';

  const entfernen = ort
    ? `<button data-act="ah-entfernen" data-role="abholadresse-entfernen" style="align-self:center;border:0;background:transparent;padding:12px 8px;min-height:44px;font:650 13.5px/1 ${FONT};color:var(--red);cursor:pointer;appearance:none"><span style="pointer-events:none">${tx('Abholadresse entfernen')}</span></button>`
    : '';

  const html = screenScaffold({
    header: subHeader(tx('Abholadresse')),
    bottomInset: 28,
    scrollKey: 'abholadresse',
    body: `<div style="display:flex;flex-direction:column;gap:14px;padding:2px 20px 0">
${karte}
${wozu}
${suche}
...
${entfernen}
  ...
  return {
    ...
    bind: (root, innerCtx) => {
      const repo2 = innerCtx.repo;
      const feld = root.querySelector('#abholadresse-suche');
      if (feld) {
        // Wie beim Zuhause: kein value-Attribut im Markup, damit der Fokus beim Tippen stehen bleibt.
        if (innerCtx.ui.ahQuery && feld.value !== innerCtx.ui.ahQuery) feld.value = innerCtx.ui.ahQuery;
        feld.addEventListener('input', () => {
          innerCtx.ui.ahQuery = feld.value;
          zuhauseSuche(innerCtx, 'ah');
        });
      }
      const gespeichert = (ergebnis) => {
        if (!ergebnis?.ok) {
          innerCtx.toast(ergebnis?.grund === 'name' ? tx('Dieser Treffer hat keinen Namen.') : tx('Dieser Treffer hat keine Koordinaten.'));
          return false;
        }
        innerCtx.ui.ahQuery = '';
        innerCtx.ui.ahTreffer = [];
        innerCtx.ui.ahListe = [];
        if (feld) feld.value = '';
        haptik('erfolg');
        innerCtx.render();
        innerCtx.toast(tx('Abholadresse gespeichert'));
        return true;
      };
      bindActions(root, {
        'ah-waehlen': (data) => {
          const eintrag = (innerCtx.ui.ahListe || [])[Number(data.index)];
          if (!eintrag) return;
          gespeichert(repo2.setAbholadresse({ name: eintrag.name, city: eintrag.address || eintrag.city || '', lat: eintrag.lat, lon: eintrag.lon }));
        },
        'ah-zuhause': () => {
          const heim = repo2.getHomeAddress?.() || null;
          if (!heim) return;
          gespeichert(repo2.setAbholadresse({ name: heim.name, city: heim.city || '', lat: heim.lat, lon: heim.lon }));
        },
        'ah-entfernen': () => {
          repo2.clearAbholadresse();
          innerCtx.ui.ahQuery = '';
          innerCtx.ui.ahTreffer = [];
          if (feld) feld.value = '';
          innerCtx.render();
          innerCtx.toast(tx('Abholadresse entfernt'));
        },
      });
    },
  };
...
<span style="font-size:11px;color:var(--muted);padding-left:2px">${tx('Eigenes Emoji: das rechte Feld antippen und aus der Tastatur wählen. Am Bild steht dann das Emoji statt des Symbols, auf hellem Grund — die Farbe gilt für das Label.')}</span>
</div>
...
// Dieselbe Plakette wie avatarMark (volle Farbe, Zeichen weiß) — hier für die Vorschau im Sheet und
// die kleine Anzeige in der Freundesliste.
// Runde 8 (R8-7): Die Vorschau rechnet mit DERSELBEN Regel wie die Marke am Bild (MASS/avatarMasse).
// Runde 8 (R8-61, Schnittstelle des Pakets form: „JEDER Punkt an einem Kreis benutzt kreisPunkt"):
// Am Bild ist die Plakette jetzt ein kreisPunkt unten links (Mitte auf dem Rand unter 45°), und die
// Lücke um sie ist wirklich durchsichtig — das Bild darunter bekommt dafür sein Loch (kreisMaske),
// genau wie an jedem echten besten Freund. Der frühere Überstand über die Bildkante (MASS.versatz)
// ist damit auch hier abgelöst. Allein stehend (frei: neben „Zeichen" in der Freundesliste) liegt sie
// auf keinem Kreis — dort bleibt der Ring der alleinstehenden Plakette (MASS.ring).
function bfZeichen(marke, plate) {
  const px = Math.round(plate * MASS.symbol * 100) / 100;
  return SPECIAL_SYMBOLS[specialSymbolKey(marke.symbol)]('var(--on-accent)').replace(/width="[\d.]+" height="[\d.]+"/, `width="${px}" height="${px}"`);
}

function bfPlakette(marke, avatarGroesse, ring = 'var(--surface)') {
  const plate = avatarMasse(avatarGroesse).zeichen;
  const ringBreite = Math.round(plate * MASS.ring * 100) / 100;
  return `<span data-role="bf-vorschau-zeichen" style="position:relative;width:${plate}px;height:${plate}px;border-radius:50%;background:${marke.color};box-shadow:0 0 0 ${ringBreite}px ${ring};box-sizing:border-box;display:flex;align-items:center;justify-content:center;flex:none;pointer-events:none">${bfZeichen(marke, plate)}</span>`;
}

// Die Vorschau AM BILD: das Bild (oder eine leere Scheibe) mit dem Loch, darauf der Punkt.
function bfAmBild(marke, groesse, bildHtml) {
  const loch = typeof bausteine.kreisMaske === 'function' ? bausteine.kreisMaske(groesse, ['ul']) : '';
  const plate = avatarMasse(groesse).zeichen;
  return `<span style="position:relative;display:block;width:${groesse}px;height:${groesse}px;flex:none;pointer-events:none"><span style="display:block;width:${groesse}px;height:${groesse}px;border-radius:50%;${loch}">${bildHtml}</span>${punktAmKreis(groesse, 'ul', { farbe: marke.color, inhalt: bfZeichen(marke, plate), attribute: 'data-role="bf-vorschau-zeichen"' })}</span>`;
}
...
${bfAmBild(marke, 48, personAvatar(person, { size: 48, free: false, active: false }))}
<span style="font-size:12px;font-weight:600;color:var(--ink-soft);max-width:100%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc((person.name || '').split(' ')[0])}</span></div>`).join('')
    : `<div style="display:flex;align-items:center;gap:12px">${bfAmBild(marke, 48, '<span style="display:block;width:48px;height:48px;border-radius:50%;background:var(--field)"></span>')}<span style="font-size:12.5px;color:var(--muted);line-height:1.4">${tx('Noch keine besten Freunde — das Zeichen gilt, sobald du jemanden markierst.')}</span></div>`;
...
// Runde 7 (J5): Der Ring IST der Zustand — grün und gefüllt heißt „du bekommst Bescheid".
// Die Schalter-Pille daneben sagte dasselbe ein zweites Mal und ist entfallen.
...
// Runde 7 (J6): Der Stift am besten Freund braucht seinen Platz — bei 182 px brach „Beste
// Freunde" ab, und „Aus der Crew entfernen" tat es schon vorher. 206 px sind 57 % der engsten
// Breite (360 px) und damit weit von den 74 % entfernt, die Runde 6 zu Recht gerügt hat.
...
function menuRow({ act, icon, label, valueHtml = '', last = false, tone = 'var(--ink)', wrap = false }) {
  ...
  // Runde 8 (R8-12): Jede Zeile beider Menues ist mindestens 44 px hoch — dieselbe Zeile fuer
  // Person und Gruppe, und fuer den Daumen gross genug.
  return `<button data-act="${act}" role="menuitem" style="display:flex;align-items:center;gap:9px;min-height:44px;padding:8px 12px;border:0;${last ? '' : 'border-bottom:1px solid var(--ink-a06);'}background:transparent;width:100%;box-sizing:border-box;text-align:left;cursor:pointer;appearance:none;font-family:${FONT}">
...
function menuCard(rows, style, attrs = '') {
  return `<div role="menu"${attrs} style="position:absolute;${style};background:var(--surface);border-radius:14px;box-shadow:0 8px 24px var(--shadow-16);border:1px solid var(--ink-a07);overflow:hidden;box-sizing:border-box;z-index:13">${rows}</div>`;
}
...
// Runde 8 (R8-12): Zeichen der Zeilen, die Person und Gruppe gemeinsam haben oder die nur die
// Gruppe braucht. Gleiche Groesse und Strichstaerke wie Stern, Herz und Ring.
const svgGemeinsameMeets = `<svg width="15" height="15" viewBox="0 0 24 24" fill="none" style="flex:none;pointer-events:none"><rect x="3.8" y="5.2" width="16.4" height="15" rx="3" stroke="var(--muted-light)" stroke-width="1.8"></rect><path d="M3.8 9.8h16.4M8.4 3v3.4M15.6 3v3.4" stroke="var(--muted-light)" stroke-width="1.8" stroke-linecap="round"></path></svg>`;
// Angeheftet: die Nadel wird gruen gefuellt — der Zustand steht im Zeichen (J5), nicht in einem Schalter.
const svgAnheften = (an) => `<svg width="15" height="15" viewBox="0 0 24 24" fill="none" style="flex:none;pointer-events:none"><path d="M9 3.8h6l-.9 5.3 3.4 3.4v1.8H6.5v-1.8l3.4-3.4L9 3.8Z" ${an ? 'fill="var(--green)" stroke="var(--green-dark)"' : 'stroke="var(--muted-light)"'} stroke-width="1.7" stroke-linejoin="round"></path><path d="M12 14.3v6" stroke="${an ? 'var(--green-dark)' : 'var(--muted-light)'}" stroke-width="1.8" stroke-linecap="round"></path></svg>`;
const svgGruppeVerlassen = `<svg width="15" height="15" viewBox="0 0 24 24" fill="none" style="flex:none;pointer-events:none"><path d="M14 4.5H6.5A1.5 1.5 0 0 0 5 6v12a1.5 1.5 0 0 0 1.5 1.5H14" stroke="var(--danger)" stroke-width="1.8" stroke-linecap="round"></path><path d="M16 8.5 19.5 12 16 15.5M19 12H10" stroke="var(--danger)" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"></path></svg>`;

// Wo ein Menue aufgeht: direkt unter dem Knopf (bzw. unter seinem sichtbaren Kreis) und
// rechtsbuendig mit ihm — in Koordinaten des Telefonrahmens, eine Skalierung der Vorschau
// herausgerechnet. Reicht der Platz darunter nicht (langes Druecken auf eine Zeile weit unten),
// rueckt die Karte so weit hoch, dass sie ganz im Bild bleibt.
const MENU_ZEILE = 44;
function menueLage(el, zeilen) {
  const ziel = el?.querySelector?.('[data-kreis]') || el;
  const rahmen = ziel?.closest?.('.runtime-phone') || document.querySelector('.runtime-phone');
  if (!rahmen || !ziel?.getBoundingClientRect) return { top: 140, right: MENU_RIGHT };
  const f = rahmen.getBoundingClientRect();
  const skala = rahmen.offsetWidth ? f.width / rahmen.offsetWidth : 1;
  const r = ziel.getBoundingClientRect();
  const hoehe = zeilen * MENU_ZEILE + 2;
  const unten = (r.bottom - f.top) / skala + 6;
  const top = Math.max(12, Math.min(unten, rahmen.offsetHeight - hoehe - 16));
  const right = Math.max(8, Math.round((f.right - r.right) / skala));
  return { top: Math.round(top), right };
}
// Dieselbe Grenze beim Zeichnen: auch eine gemerkte Lage aus einer anderen Fenstergroesse
// laeuft nie unten aus dem Bild.
const menueOben = (top, zeilen) => `top:min(${Math.round(top)}px, calc(100% - ${zeilen * MENU_ZEILE + 18}px))`;
...
// ‚Alle Meets' heißt ‚Gemeinsame Meets'."): Die erste Zeile ist in BEIDEN Menues dieselbe —
// „Gemeinsame Meets" fuehrt in die Meets, die man mit dieser Person bzw. Gruppe hat. Danach
// folgt, was nur hierher gehoert, und zuletzt, rot, der Weg hinaus. Das Gruppenmenue
// (crewMenu) ist dieselbe Karte mit denselben Zeilen-Bausteinen.
export function friendMenu(ctx, personId) {
  ...
  const anchorTop = Math.max(12, Math.round(ui.menuTop || 140));
  ...
  // Runde 6 (Jonathan): EINE Ebene, vier kurze Zeilen. Vorher lagen hier zwei Ebenen mit bis zu
  // sieben Zeilen, drei davon begannen mit „Als … markieren" — mehr Worte als Bedeutung.
  // Jede Zeile schaltet jetzt selbst um: Wer schon markiert ist, nimmt die Markierung mit
  // demselben Tipp wieder weg.
  //
  // Runde 7 (Jonathan, J5): „Schalter im ···-Menü raus. Der Zustand steht schon im Zeichen:
  // Symbol links wird eingefärbt, wenn aktiv; bei ‚Besondere' wird das x/y grün. Das reicht —
  // kein Schalter, kein ‚an'." Umgesetzt: Das Zeichen LINKS trägt den Zustand (es ist bei
  // Markierung ohnehin schon das echte Zeichen in seiner Farbe, beim Frei-Hinweis der grüne
  // Ring), die Zählung rechts wird grün, sobald diese Person dazugehört. Die Wörter „an"/„aus"
  // und die Schalter-Pille sind ersatzlos entfallen.
  //
  // Runde 7 (Jonathan, J6): „‚Bester Freund bearbeiten' fehlt — bei ‚Besondere Person' gibt es
  // das Bearbeiten, beim besten Freund nicht." Das gemeinsame Zeichen der besten Freunde war
  // nur über die Freundesliste erreichbar. Ist diese Person ein bester Freund, steht am Ende
  // der Zeile jetzt ein Stift: Tipp auf die Zeile nimmt die Markierung weg, Tipp auf den Stift
  // öffnet das Zeichen — genau die Symmetrie zur besonderen Person.
  const zaehler = (an) => `<span style="pointer-events:none;font-size:10.5px;font-weight:650;color:${an ? 'var(--green-dark)' : 'var(--muted-light)'};font-variant-numeric:tabular-nums">${bestIds.length}/3</span>`;
  const bearbeitenStift = `<button data-act="menu-bf-marke" data-treffer aria-label="${esc(tx('Zeichen für beste Freunde'))}" style="width:30px;height:30px;margin-right:8px;border:0;border-radius:50%;background:var(--paper);display:flex;align-items:center;justify-content:center;flex:none;cursor:pointer;appearance:none;padding:0"><span style="pointer-events:none;display:flex">${svgPencilSmall(13)}</span></button>`;
  const besteZeile = isBest
    ? `<div style="display:flex;align-items:center;border-bottom:1px solid var(--ink-a06)"><span style="flex:1;min-width:0;display:flex">${menuRow({ act: 'menu-best', icon: markSymbolGlyph(bf.symbol, bf.color, 1), label: tx('Beste Freunde'), valueHtml: zaehler(true), last: true })}</span>${bearbeitenStift}</div>`
    : menuRow({ act: 'menu-best', icon: svgStarOutline, label: tx('Beste Freunde'), valueHtml: zaehler(false) });
  const rows = menuRow({ act: 'menu-meets', icon: svgGemeinsameMeets, label: tx('Gemeinsame Meets') })
    + besteZeile
    + menuRow({ act: 'menu-mark', icon: special ? markSymbolGlyph(special.symbol, special.color, 1) : svgHeartOutline, label: tx('Besondere Person'), valueHtml: '' })
    + menuRow({ act: 'menu-frei', icon: svgFreiHint(freiAn), label: tx('Bescheid, wenn frei'), valueHtml: '' })
    + menuRow({ act: 'menu-remove-friend', icon: svgTrashRed, label: tx('Aus der Crew entfernen'), tone: 'var(--danger)', last: true });

  return `<div data-act="menu-close" style="position:absolute;inset:0;z-index:12"></div>
${menuCard(rows, `right:${MENU_RIGHT}px;${menueOben(anchorTop, 5)};width:${MENU_WIDTH}px`)}`;
}
...
export function friendMenuActs(ctx, umgebung = {}) {
  ...
  return {
    ...
    'menu-open': (data, el) => {
      const top = el ? menueLage(el, 5).top : 140;
    ...
    // Runde 8 (R8-12): dieselbe Seite wie „Alle Meets" im Raum — die Meets mit dieser Person.
    'menu-meets': () => {
      const personId = ui.menuFor;
      ui.menuFor = null;
      ui.menuMark = false;
      if (!personId) { ctx.render(); return; }
      nav.go('room.allMeets', { roomId: roomIdForPerson(personId) });
    },
    ...
    'menu-best': () => {
      ...
      // Runde 6: Derselbe Tipp nimmt die Markierung wieder weg — dafür braucht es keine
      // eigene Zeile „Markierung entfernen" mehr.
      if (isBest) {
        ui.menuFor = null;
        ui.menuMark = false;
        applyBest(personId, false);
        ctx.render();
        ctx.toast(tx('Markierung entfernt'));
        return;
      }
    ...
    'menu-mark': () => {
      ...
      ui.menuFor = null;
      ui.menuMark = false;
      // Runde 6 (Chef, beim Nachprüfen berichtigt): Bei einer schon besonderen Person öffnet der
      // Tipp das Sheet zum BEARBEITEN — dort stehen Zeichen, Farbe, Label und „Entfernen".
      // Kurz hatte ich hier das Entfernen eingebaut; dann ließe sich eine Markierung nie mehr
      // ändern, nur noch wegwerfen und neu machen. „Weniger Zeilen" darf keine Funktion kosten.
      if (isBest && !isSpecial) {
    ...
    'menu-frei': () => {
      ...
      ctx.render();
      const vorname = (person?.name || '').split(' ')[0];
      ctx.toast(an ? tx('Bescheid, wenn {name} frei ist', { name: vorname }) : tx('Kein Hinweis mehr, wenn {name} frei ist', { name: vorname }));
    },
...
// =====================================================================================
// Runde 8 (R8-12) — das EINE Gruppenmenue
// =====================================================================================
//
// Vorher gab es fuer eine Gruppe zwei Menues mit verschiedenen Zeilen: im Raum „Gruppe
// verlassen" (room.js), in der Chatliste „Anheften" (crew.js) — und „Alle Meets" als eigenen
// Knopf daneben. Jetzt baut dieses Modul EIN Menue, und Chatliste (langes Druecken), Raumkopf
// und Gruppenseite zeigen es gleich:
//
//   Gemeinsame Meets · Anheften · Gruppe verlassen
//
// Dieselbe Karte, dieselbe Zeile und dieselbe erste Zeile wie beim Personenmenue.
//
// Einbinden (jede Seite, die eine Gruppe zeigt):
//   Overlays:  ${crewMenu(ctx, crewId)}   — oder crewMenu(ctx) auf einer Seite mit vielen Gruppen.
//              Zeichnet das offene Menue UND die Bestaetigung „Gruppe verlassen"; sonst ''.
//   Aktionen:  ...crewMenuActs(ctx, { crewId })   (crewId optional: Standard fuer den Oeffner)
//   Oeffner:   data-act="crew-menu" data-crew="<id>" auf dem Knopf — oder beim langen Druecken
//              crewMenuActs(ctx)['crew-menu']({ crew: id }, zeile).
//   Zustand:   ui.crewMenuFor (offen fuer), ui.menuTop (Oberkante — dieselbe wie beim
//              Personenmenue, das lange Druecken in crew.js setzt genau sie), ui.crewMenuRight
//              (rechter Abstand, Standard wie beim Personenmenue), ui.crewVerlassen
//              ({ crewId, step }) — eigene Namen, damit nichts mit room.js zusammenstoesst.
// Runde 11 (A14): Für Admins kommt „Mitglieder hinzufügen" als vierte Zeile dazu (für die Lage des
// Menüs wird mit der größeren Zahl gerechnet, gezeichnet mit der echten).
const GRUPPEN_ZEILEN = 4;
const crewDarfHinzufuegen = (crew) => (crew?.adminIds?.length ? crew.adminIds : [crew?.creatorId]).includes(ME);

export function crewMenu(ctx, crewId) {
  const { repo, ui } = ctx;
  const offen = ui.crewMenuFor || null;
  const verlassen = ui.crewVerlassen || null;
  const id = crewId || offen || verlassen?.crewId || null;
  if (!id) return '';
  const crew = repo.getCrew(id);
  if (!crew) return '';
  let html = '';
  // Solange die Bestaetigung offen ist, bleibt ui.crewMenuFor gesetzt (ein Wirt darf crewMenu nur
  // bei gesetztem ui.crewMenuFor aufrufen) — die Karte selbst ist dann zu.
  const fragt = Boolean(verlassen && verlassen.crewId === id);
  if (offen === id && !fragt) {
    const angeheftet = (repo.getSettings().pinnedIds || []).includes(id);
    const darfNeue = crewDarfHinzufuegen(crew);
    const rows = menuRow({ act: 'crew-meets', icon: svgGemeinsameMeets, label: tx('Gemeinsame Meets') })
      + (darfNeue ? menuRow({ act: 'crew-add-members', icon: plus('var(--muted)', 15), label: tx('Mitglieder hinzufügen') }) : '')
      + menuRow({ act: 'crew-pin', icon: svgAnheften(angeheftet), label: angeheftet ? tx('Angeheftet') : tx('Anheften') })
      + menuRow({ act: 'crew-leave', icon: svgGruppeVerlassen, label: tx('Gruppe verlassen'), tone: 'var(--danger)', last: true });
    const rechts = Math.max(8, Math.round(ui.crewMenuRight ?? MENU_RIGHT));
    html += `<div data-act="crew-menu-close" style="position:absolute;inset:0;z-index:12"></div>
${menuCard(rows, `right:${rechts}px;${menueOben(Math.max(12, ui.menuTop ?? 140), darfNeue ? GRUPPEN_ZEILEN : GRUPPEN_ZEILEN - 1)};width:${MENU_WIDTH}px;max-width:calc(100% - ${rechts + 12}px)`, ` data-role="crew-menu" data-crew="${esc(id)}" data-angeheftet="${angeheftet}"`)}`;
  }
  if (fragt) html += gruppeVerlassenSheet(ctx, crew, verlassen.step);
  return html;
}

// Bestaetigung in drei Stufen: 'confirm' | 'admin' | 'aufloesen'. Genau drei Ausgaenge, damit nie
// eine herrenlose Gruppe zurueckbleibt: verlassen · Adminrolle uebergeben und dann verlassen ·
// als letztes Mitglied aufloesen. Wortlaut wie bisher im Raum.
function gruppeVerlassenSheet(ctx, crew, step) {
  const others = crew.memberIds.filter((id) => id !== ME).map((id) => ctx.repo.getPerson(id)).filter(Boolean);
  const settings = ctx.repo.getSettings();
  const abbrechen = `<button data-act="crew-leave-cancel" style="flex:1;border:1.5px solid var(--ink-a14);background:var(--surface);color:var(--ink-soft);font:650 14px/1 ${FONT};padding:13px 0;border-radius:999px;cursor:pointer;appearance:none"><span style="pointer-events:none">${tx('Abbrechen')}</span></button>`;
  const rot = (act, text) => `<button data-act="${act}" style="flex:1;background:var(--danger);border:0;color:var(--on-accent);font:650 14px/1 ${FONT};padding:14px 0;border-radius:999px;cursor:pointer;appearance:none"><span style="pointer-events:none">${text}</span></button>`;
  let title;
  let text;
  let body = '';
  let actions;
  if (step === 'admin') {
    title = tx('Erst die Gruppe übergeben');
    text = tx('Du bist die einzige Person, die {name} verwaltet. Wähle, wer das übernimmt — danach verlässt du die Gruppe.', { name: esc(crew.name) });
    body = `<div style="display:flex;flex-direction:column;max-height:230px;overflow-y:auto">${others.map((person, index) => `<button data-act="crew-admin-pick" data-person="${esc(person.id)}" style="display:flex;align-items:center;gap:11px;padding:10px 0;border:0;${index ? 'border-top:1px solid var(--ink-a05);' : ''}background:transparent;cursor:pointer;appearance:none;font-family:${FONT};text-align:left;width:100%">
<span style="display:flex;flex:none;pointer-events:none">${personAvatar(person, { size: 34, fontSize: 12.5, marker: personMarker(person.id, settings) })}</span>
<span style="font-size:14px;font-weight:650;flex:1;min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;pointer-events:none">${esc(person.name)}</span>
<span style="pointer-events:none;display:flex">${chevronRight('var(--line-strong)', 15)}</span>
</button>`).join('')}</div>`;
    actions = abbrechen;
  } else if (step === 'aufloesen') {
    title = tx('Gruppe auflösen?');
    text = tx('Du bist das letzte Mitglied von {name}. Die Gruppe und ihr Chat verschwinden; vergangene eigene Meets bleiben in deinem Verlauf.', { name: esc(crew.name) });
    actions = `${abbrechen}\n${rot('crew-dissolve-confirm', tx('Auflösen'))}`;
  } else {
    title = tx('{name} verlassen?', { name: crew.name });
    text = tx('Du siehst danach weder den Chat noch neue Meets dieser Gruppe. Vergangene eigene Meets bleiben in deinem Verlauf.');
    actions = `${abbrechen}\n${rot('crew-leave-confirm', tx('Verlassen'))}`;
  }
  return `<div data-role="crew-leave" data-step="${esc(step || 'confirm')}" style="position:absolute;inset:0;z-index:22;display:flex;align-items:flex-end">
<div data-act="crew-leave-cancel" style="position:absolute;inset:0;background:var(--scrim);-webkit-backdrop-filter:var(--scrim-filter);backdrop-filter:var(--scrim-filter)"></div>
<div style="position:relative;width:100%;padding:16px 20px 26px;border-radius:26px 26px 0 0;background:var(--surface);box-shadow:0 -10px 30px var(--shadow-15);font-family:${FONT};display:flex;flex-direction:column;gap:11px">
<div style="width:38px;height:4px;margin:0 auto 2px;border-radius:999px;background:var(--handle)"></div>
<span style="font-family:${BRICOLAGE};font-size:19px;font-weight:650">${esc(title)}</span>
<span style="font-size:13px;color:var(--ink-soft);line-height:1.45;text-wrap:pretty">${text}</span>
${body}
<div style="display:flex;gap:8px;padding-top:2px">${actions}</div>
</div></div>`;
}

// umgebung: { crewId } — die Gruppe, die ein Oeffner ohne data-crew meint (Raum, Gruppenseite);
//           { nachVerlassen } — was nach dem Austritt passiert (Standard: zurueck zur Chatliste).
export function crewMenuActs(ctx, umgebung = {}) {
  const { repo, ui, nav } = ctx;
  const nachVerlassen = umgebung.nachVerlassen || (() => {
    if (nav.current?.()?.id === 'crew.home') ctx.render();
    else nav.resetTo('crew.home');
  });
  const verlassen = () => {
    const crewId = ui.crewVerlassen?.crewId;
    const crew = crewId ? repo.getCrew(crewId) : null;
    if (!crew) { ui.crewVerlassen = null; ui.crewMenuFor = null; ctx.render(); return; }
    const result = repo.leaveCrew(crew.id);
    if (result?.ok) {
      ui.crewVerlassen = null;
      ui.crewMenuFor = null;
      ctx.toast(tx('{name} verlassen', { name: crew.name }));
      nachVerlassen(crew);
      return;
    }
    if (result?.reason === 'adminUebergabe') { ui.crewVerlassen = { crewId: crew.id, step: 'admin' }; ctx.render(); return; }
    if (result?.reason === 'aufloesen') { ui.crewVerlassen = { crewId: crew.id, step: 'aufloesen' }; ctx.render(); return; }
    ui.crewVerlassen = null;
    ui.crewMenuFor = null;
    ctx.render();
    ctx.toast(result?.reason || tx('Das hat nicht geklappt'));
  };
  return {
    'crew-menu': (data, el) => {
      const id = data?.crew || umgebung.crewId || null;
      if (!id) return;
      if (ui.crewMenuFor === id && !ui.crewVerlassen) { ui.crewMenuFor = null; ctx.render(); return; }
      ui.crewVerlassen = null;
      const lage = el ? menueLage(el, GRUPPEN_ZEILEN) : { top: 140, right: MENU_RIGHT };
      ui.crewMenuFor = id;
      ui.menuTop = lage.top;
      ui.crewMenuRight = lage.right;
      // Nie zwei Menues zugleich.
      ui.menuFor = null;
      ui.menuMark = false;
      haptik('tipp');
      ctx.render();
    },
    'crew-menu-close': () => { ui.crewMenuFor = null; ctx.render(); },
    'crew-meets': () => {
      const id = ui.crewMenuFor;
      ui.crewMenuFor = null;
      if (!id) { ctx.render(); return; }
      nav.go('room.allMeets', { roomId: roomIdForCrew(id) });
    },
    // Runde 11 (A14): „Mitglieder hinzufügen" öffnet die vorhandene Personen-Auswahl der Gruppenseite
    // (room.js › personenWahlSheet) — auf der Gruppenseite selbst sofort, sonst über die Route.
    'crew-add-members': () => {
      const id = ui.crewMenuFor;
      ui.crewMenuFor = null;
      if (!id) { ctx.render(); return; }
      if (nav.current?.()?.id === 'room.crewDetails') {
        ui.personenWahl = { crewId: id, auswahl: [], suche: '', zurueck: false };
        ctx.render();
        return;
      }
      nav.go('room.crewDetails', { crewId: id, personenHinzu: true });
    },
    // Anheften: Die Gruppe steht oben in der Chatliste. Derselbe Tipp loest sie wieder.
    'crew-pin': () => {
      const id = ui.crewMenuFor;
      if (!id) return;
      const jetzt = [...(repo.getSettings().pinnedIds || [])];
      const drin = jetzt.includes(id);
      repo.updateSettings({ pinnedIds: drin ? jetzt.filter((eintrag) => eintrag !== id) : [...jetzt, id] });
      haptik(drin ? 'zurueck' : 'erfolg');
      ui.crewMenuFor = null;
      ctx.render();
      ctx.toast(drin ? tx('Nicht mehr angeheftet') : tx('Angeheftet'));
    },
    'crew-leave': () => {
      const id = ui.crewMenuFor;
      if (!id) { ctx.render(); return; }
      ui.crewVerlassen = { crewId: id, step: 'confirm' };
      ctx.render();
    },
    'crew-leave-cancel': () => { ui.crewVerlassen = null; ui.crewMenuFor = null; ctx.render(); },
    'crew-leave-confirm': verlassen,
    // Uebergabe und Austritt sind EIN Vorgang: wer uebergibt, wollte gehen.
    'crew-admin-pick': (data) => {
      const crewId = ui.crewVerlassen?.crewId;
      if (!crewId) return;
      const result = repo.transferCrewAdmin(crewId, data.person);
      if (!result?.ok) { ctx.toast(result?.reason || tx('Das hat nicht geklappt')); return; }
      verlassen();
    },
    'crew-dissolve-confirm': () => {
      const crewId = ui.crewVerlassen?.crewId;
      const crew = crewId ? repo.getCrew(crewId) : null;
      ui.crewVerlassen = null;
      ui.crewMenuFor = null;
      if (!crew) { ctx.render(); return; }
      const result = repo.dissolveCrew(crew.id);
      if (!result?.ok) { ctx.render(); ctx.toast(result?.reason || tx('Das hat nicht geklappt')); return; }
      ctx.toast(tx('{name} aufgelöst', { name: crew.name }));
      nachVerlassen(crew);
    },
  };
}
...
<button data-act="menu-open" data-person="${person.id}" data-treffer aria-label="${esc(tx('Optionen'))}" style="width:34px;height:34px;border-radius:50%;background:var(--paper);border:0;display:flex;align-items:center;justify-content:center;flex:none;cursor:pointer;appearance:none;padding:0"><span style="pointer-events:none;color:var(--ink-soft);letter-spacing:1px;font-weight:650;font-size:13px;line-height:1">···</span></button>
  ...
  const bfKnopf = `<button data-act="bf-marke-open" aria-label="${esc(tx('Zeichen für beste Freunde'))}" style="display:flex;align-items:center;gap:6px;height:44px;margin:-6px -6px -10px 0;padding:6px 6px 0;border:0;background:transparent;cursor:pointer;appearance:none;font:650 11.5px ${FONT};color:var(--green-dark);flex:none"><span style="pointer-events:none;display:flex">${bfPlakette(besteFreundeMarke(settings), 40, 'var(--surface)')}</span><span style="pointer-events:none">${tx('Zeichen')}</span></button>`;

  // Runde 8 (R8-11, Jonathan: „Gesendete Anfragen nur in den Einstellungen."): Hier — und nur
  // hier — stehen die Anfragen, die ich geschickt habe, jede mit „Zurückziehen". Erhaltene
  // Anfragen beantwortet man unter „Freunde" (Profil) bzw. hinter dem „+" (Crew).
  const outgoing = settings.outgoingRequests || [];
  const gesendet = outgoing.length
    ? `<div data-role="gesendete-anfragen" style="margin:12px 20px 0;background:var(--surface);border-radius:20px;border:1px solid var(--ink-a07);padding:2px 16px 4px;display:flex;flex-direction:column">
<span style="font-size:10.5px;font-weight:650;letter-spacing:.09em;text-transform:uppercase;color:var(--muted-light);padding:11px 0 3px">${tx('Gesendet · {n}', { n: outgoing.length })}</span>
...
<span style="margin-left:auto;font-size:12.5px;color:var(--muted);font-variant-numeric:tabular-nums">${people.length}</span>
    ...
    overlays: `${friendsSheets(ctx)}${discardSheet(ctx)}${ctx.ui.qrOffen ? qrOverlay(ctx) : ''}${bildGrossHtml(ctx)}`,
...
function bindFriends(root, ctx) {
  ...
  bindActions(root, {
    ...discardActions(ctx),
    // Runde 8 (R8-11): Zurückziehen der gesendeten Anfragen (und dieselben Antworten wie überall).
    ...requestActs(ctx),
  ...
  // Runde 7 (C3): Tipp auf das Profilbild einer Zeile zeigt das Bild gross — dieselbe Ansicht wie
  // im Raum, kein zweiter Weg. Der Rest der Zeile fuehrt wie bisher ins Profil der Person.
  bildGrossBinden(root, ctx);
  bildGrossInZeilen(root, ctx, '[data-act="open-friend"]');
}
...
${receivedRows}
    ...
    : '';
  // Runde 8 (R8-11): Gesendete Anfragen stehen nur noch unter Einstellungen → Freunde.
  const emptyCard = requests.length === 0
...
// Runde 11 (A9, Jonathan D6): Es gibt genau SECHS Hauptgruppen — Zuhause · Essen · Ausgehen · Sport ·
// Draußen · Kultur. Keine erweiterte Auswahl, kein „Weitere Interessen": alles Weitere lernt Crew aus
// dem, was man wählt und übergeht. Die Namen sind gespeicherte Interessen (settings.interests) und
// werden verglichen — sie bleiben deutsch; schluessel macht sie zu Übersetzungs-Schlüsseln, angezeigt
// werden sie über wort() (web/core/sprache.js). Ihr Zeichen kommt aus dem EINEN Katalog
// (ui/activity-icons.js) über einen festen Schlüssel.
const HAUPTGRUPPEN = [
  { label: schluessel('Zuhause'), zeichen: 'sofa' },
  { label: schluessel('Essen'), zeichen: 'gabel' },
  { label: schluessel('Ausgehen'), zeichen: 'ticket' },
  { label: schluessel('Sport'), zeichen: 'fussball' },
  { label: schluessel('Draußen'), zeichen: 'wandern' },
  { label: schluessel('Kultur'), zeichen: 'museum' },
];
const ONBOARDING_FIELDS = HAUPTGRUPPEN.map((gruppe) => ({ ...gruppe, icon: activityIconSvg({ icon: gruppe.zeichen }, 'var(--green-dark)', 20) }));
...
function interestIcon(label, color = 'var(--green-dark)', size = 18) {
  const gruppe = HAUPTGRUPPEN.find((g) => g.label === label);
  return activityIconSvg(gruppe ? { icon: gruppe.zeichen } : { title: label }, color, size);
}

// Die Auswahl der sechs Hauptgruppen als flache Liste (Runde 11, A9) — für das Sheet „Neues Interesse"
// im Profil.
//   istGewaehlt(label) → markiert die Zeile
//   act               → data-act jeder Zeile (data-name trägt das Label)
function interestCatalogHtml({ act, istGewaehlt }) {
  return HAUPTGRUPPEN.map((eintrag) => {
...
function renderOnboardingName(ctx) {
  ...
  const html = screenScaffold({
    ...
    scrollKey: 'onb-name',
    // J3: Auch hier gilt die eine Verwerfen-Entscheidung — der Baukasten beim Einrichten ist
    // dasselbe Blatt mit demselben Entwurf wie im Profil.
    overlays: `${photoSheet}${discardSheet(ctx)}`,
    bottomInset: 116,
...
function bindOnboardingName(root, ctx) {
  ...
  // J3: Dieselbe Verwerfen-Entscheidung wie im Profil — ein Blatt mit Entwurf, ein Wächter.
  const fotoWaechter = dirtyGuard(ctx, 'onb-foto');
  const fotoZu = () => { ui.pb = null; ctx.render(); };
  bindActions(root, {
    ...discardActions(ctx),
    ...
    'onb-photo': () => {
      ...
      ui.pbFest = fest;
      ui.pb = { ...fest, z: { ...fest.z } };
      fotoWaechter.reset(pbErgebnis(ui.pb));
      ctx.render();
    },
    ...
    // Abbrechen und der Tipp daneben: unverändert schließt sofort, verändert fragt nach.
    'onb-photo-cancel': () => {
      if (!ui.pb) { fotoZu(); return; }
      if (fotoWaechter.confirm(pbErgebnis(ui.pb), fotoZu)) fotoZu();
    },
    'onb-next': goNext,
  });
}
...
<span style="font-size:13px;font-weight:${selected ? 650 : 600};color:${selected ? 'var(--green-dark)' : 'var(--ink-soft)'};flex:1;min-width:0;line-height:1.25;text-wrap:pretty">${esc(wort(feld.label))}</span>
  ...
  // Runde 11 (A9): Es gibt nur die sechs Hauptgruppen — keine zweite Ebene, kein eigenes Interesse.
...
function bindOnboardingInterests(root, ctx) {
  ...
  bindActions(root, {
    ...
    'onb-int-next': () => {
      ...
      // N2: Der Einrichten-Weg endet auf dem Einladungs-Bildschirm, sobald es EINE Einladung
      // gibt — aus dem Beispielbestand (pendingInvite) oder aus dem abgelegten Code (N1).
      const target = (repo.getSettings().pendingInvite || sichtbareEinladungen(repo).length) ? 'onboarding.invite' : 'onboarding.start';
...
// Runde 7 (N4): „Der Einstieg verspricht zwei Wege und zeigt einen." Der Satz „Beides dauert
// unter einer Minute" stand über GENAU EINER Kachel — die zweite war irgendwann entfallen, der
// Text nicht. Sie ist zurück, und zwar als das, was hier wirklich fehlt: jemanden EINLADEN, der
// Crew noch nicht hat. Ohne diesen Weg setzt der ganze Einstieg voraus, dass die andere Person
// die App schon hat und danebensteht (N5).
// Runde 7 (C2): Kein Titel ist eine Frage — „Womit fängst du an?" sagte nichts, was die zwei
// Kacheln nicht schon sagen.
function onbStartBody() {
  const option = (act, daten, iconHtml, bg, title, sub) => `<button data-act="${act}"${daten} style="background:var(--surface);border:1px solid var(--ink-a09);border-radius:22px;padding:18px;display:flex;align-items:center;gap:14px;box-shadow:0 2px 8px var(--shadow-05);flex:none;width:100%;text-align:left;cursor:pointer;appearance:none;font-family:${FONT};color:var(--ink)">
...
${onbTitle(tx('Hol deine Leute dazu'))}
${'' /* Beide Wege führen auf dasselbe Blatt — nur von verschiedenen Seiten (K1). */}
<span style="font-size:14px;color:var(--ink-soft);line-height:1.5">${tx('Beides dauert unter einer Minute.')}</span>
</div>
${option('onb-add-friend', '', svgFriendPlus, 'var(--paper)', tx('Freund hinzufügen'), tx('Den Code von jemandem scannen'))}
${option('onb-invite-friend', '', svgCrewPair, 'var(--green-tint)', tx('Jemanden einladen'), tx('Deinen Code zeigen oder schicken'))}
</div>`;
}
...
// Runde 7 (N2): Der Bildschirm war fertig gebaut und lief nie — er hing allein an
// settings.pendingInvite, das ausschließlich der Beispielbestand setzt (der Server-Weg trägt
// dort hart null ein). Jetzt liest er zusätzlich den abgelegten Einladungscode (einladungLesen).
// Über den Code allein weiß diese App NICHTS über die einladende Person: fremde Codes sind für
// den Client absichtlich unlesbar. Deshalb steht hier kein erfundener Name, sondern der Code
// selbst und ehrlich, was das Annehmen bewirkt (Hausregel 8).
// Welle 5 (N2b): Lagen zwei Links, stand hier nur der aktive Code — „Einladung annehmen" löste
// aber beide ein. Jetzt steht jeder Code da, den der Knopf einlöst (codes = sichtbareEinladungen).
function inviteCodeCard(codes) {
  const mehrere = codes.length > 1;
  return onbCard(`<div style="display:flex;align-items:center;gap:12px">
<span style="width:44px;height:44px;border-radius:14px;background:var(--green-tint);display:flex;align-items:center;justify-content:center;flex:none">${svgCrewPair}</span>
<div style="display:flex;flex-direction:column;gap:2px;flex:1;min-width:0">
${codes.map((code) => `<span data-role="onb-invite-code" style="font:650 16px/1.2 ui-monospace,monospace;letter-spacing:.1em;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(code)}</span>`).join('')}
<span style="font-size:12.5px;color:var(--muted);line-height:1.35">${mehrere ? tx('Aus den Links, mit denen du hergekommen bist') : tx('Aus dem Link, mit dem du hergekommen bist')}</span></div>
</div>
<span style="font-size:12.5px;color:var(--ink-soft);line-height:1.45">${mehrere ? tx('Annehmen schickt je eine Freundschaftsanfrage an die Personen, denen diese Codes gehören. Sie entscheiden dann.') : tx('Annehmen schickt eine Freundschaftsanfrage an die Person, der dieser Code gehört. Sie entscheidet dann.')}</span>
<button data-act="onb-invite-accept" style="border:0;background:var(--green);color:var(--on-accent);font:650 14.5px/1 ${FONT};padding:15px 0;border-radius:999px;width:100%;text-align:center;cursor:pointer;appearance:none">${tx('Einladung annehmen')}</button>`);
}
...
function renderOnboardingInvite(ctx) {
  ...
  // Welle 5 (Punkt 4): Ein Code, der einem ANDEREN Konto gehört, wird hier nicht mehr als
  // annehmbar gezeigt — app.js löst ihn für mich nicht ein, der Knopf hätte nichts getan.
  const codes = sichtbareEinladungen(repo);
  const offen = codes.length ? codes : null;
  ...
  if (!crew && !meet) {
    const nurCode = `<div data-role="onb-invite" style="display:flex;flex-direction:column;padding:24px 24px 0;gap:18px">
<div style="display:flex;flex-direction:column;gap:6px;flex:none">
${onbTitle(tx('Du bist eingeladen'))}
<span style="font-size:14px;color:var(--ink-soft);line-height:1.5">${tx('Jemand hat dich zu Crew geholt.')}</span>
</div>
${inviteCodeCard(offen)}
</div>`;
    return { html: onbTargetScaffold(nurCode, 'onb-invite-decline', tx('Ohne Einladung weiter'), 'onb-invite'), bind: bindOnboardingTarget };
  }

  const body = `<div data-role="onb-invite" style="display:flex;flex-direction:column;padding:24px 24px 0;gap:18px">
<div style="display:flex;flex-direction:column;gap:6px;flex:none">
${onbTitle(tx('Du bist eingeladen'))}
...
${offen ? inviteCodeCard(offen) : ''}
</div>`;

  // Runde 7, Welle 3 (Prüfer, N2b — SCHWER): Liegt NEBEN der Crew-/Meet-Einladung auch noch
  // ein Code, zeigte dieser Bildschirm den Code GAR NICHT. Es gab hier kein „Einladung
  // annehmen", und der grüne Hauptweg „Zur Crew" nahm die Einladung trotzdem an: app.js löste
  // den liegenden Eintrag nach dem Einrichten ein, die Freundschaftsanfrage ging ungefragt
  // raus (gemessen: outgoingRequests 0 → 1, Toast „Anfrage an Nina gesendet"). Genau der
  // Mangel, den N2 beheben sollte — nur an der Crew-Variante desselben Bildschirms.
  //
  // Zwei Dinge sind deshalb jetzt wahr:
  //   * Der Code steht als eigene Karte MIT „Einladung annehmen" unter der Crew-/Meet-Karte.
  //     Wer ihn annehmen will, sieht ihn und tippt darauf (Hausregel 9).
  //   * Jeder andere grüne Weg von hier („Zur Crew", „Dabei", „Später") räumt den Eintrag weg
  //     und sagt es — statt still eine Anfrage an eine Person zu schicken, die auf diesem
  //     Bildschirm nie zu sehen war (Hausregel 8).
  return { html: onbTargetScaffold(body, offen ? 'onb-invite-decline' : 'onb-skip', offen ? tx('Ohne Einladung weiter') : tx('Erstmal umsehen'), 'onb-invite'), bind: bindOnboardingTarget };
}
...
function bindOnboardingTarget(root, ctx) {
  const { repo, nav } = ctx;
  // N2b: Jeder Weg von diesem Bildschirm, der NICHT „Einladung annehmen" heißt, muss den
  // liegenden Code wegräumen — sonst löst app.js ihn nach dem Einrichten ein und schickt die
  // Freundschaftsanfrage, um die niemand gebeten hat. Und weil Wegräumen etwas wegnimmt, sagt
  // die App es (Hausregel 8): verworfen ist verworfen, aber nicht für immer.
  const codeNichtAnnehmen = () => {
    if (!sichtbareEinladungen(repo).length) return;
    einladungWeg();
    ctx.toast(tx('Einladung verworfen — über den Einladungslink kommst du jederzeit wieder her.'));
  };
  bindActions(root, {
    'onb-to-crew': (data) => {
      codeNichtAnnehmen();
    ...
    // K1/N4: Zwei Richtungen, zwei Eingänge — dasselbe Blatt, einmal auf „Scannen",
    // einmal auf „Mein Code".
    'onb-add-friend': () => { finishOnboarding(ctx); nav.resetTo('profile.friendAdd', { modus: 'scan' }); },
    'onb-invite-friend': () => { finishOnboarding(ctx); nav.resetTo('profile.friendAdd', { modus: 'code' }); },
    // N1/N2: „Einrichten fertig" ist genau die Bedingung, auf die die andere Hälfte des
    // Vertrags wartet — app.js löst den liegenden Code ein, sobald jemand angemeldet UND
    // eingerichtet ist (einladungEinloesen), meldet das Ergebnis und räumt den Eintrag weg.
    // Hier wird deshalb NICHT ein zweites Mal eingelöst: zwei Wege für dieselbe Sache hätten
    // zwei Anfragen an dieselbe Person geschickt (Regel 3).
    //
    // Runde 7, Welle 2 (Prüfer, N2 — SCHWER): Genau deshalb war „Einladung annehmen" bis eben
    // zeichengleich mit dem Weg daneben. Beide riefen finishOnboarding + setTab('crew'), und
    // app.js löste den liegenden Code danach so oder so ein — gemessen: mit „Erstmal umsehen"
    // ging die Freundschaftsanfrage trotzdem raus (outgoingRequests 0 → 1, Toast „Anfrage an
    // Nina gesendet"). Es gab auf diesem Bildschirm keinen Weg, NICHT anzunehmen; der grüne
    // Knopf war Verzierung (Hausregel 9).
    // Die Bedingung, auf die app.js wartet, ist der liegende Eintrag. Also entscheidet dieser
    // Bildschirm genau darüber, statt einen zweiten Einlöseweg zu bauen:
    //   annehmen   → der Eintrag bleibt liegen, app.js löst ihn ein und sagt mit der echten
    //                Antwort des Servers, was passiert ist („Anfrage an … gesendet").
    //   weitergehen→ der Eintrag wird weggeräumt, es geht nichts raus, und die App sagt es.
    'onb-invite-accept': () => { finishOnboarding(ctx); nav.setTab('crew'); },
    'onb-invite-decline': () => {
      einladungWeg();
      finishOnboarding(ctx);
      nav.setTab('crew');
      // Ehrlich: verworfen ist verworfen — aber nicht für immer (Hausregel 8/9).
      ctx.toast(tx('Einladung verworfen — über den Einladungslink kommst du jederzeit wieder her.'));
    },
...
export const profileScreens = {
  ...
  // Runde 7: H2 (Zuhause festlegen). Runde 8 (R8-39): die Sammelseite der Wochen ist entfallen —
  // die Woche einer Person steht in ihrem Profil (busyKarte).
  'profile.zuhause': renderZuhause,
  'profile.abholadresse': renderAbholadresse,
