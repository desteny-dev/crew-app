...
import { istHuelle, kann, huellePlugin } from '../core/native.js';
import {
...
import { backArrow, plus, cross, pencil, chevronRight } from '../ui/icons.js';
...
import { anfrageHinweisHtml, anfrageHinweisGesehen, ANFRAGE_HINWEIS_MS } from '../ui/anfragen-hinweis.js';
...
import { naechsteFreiGrenze } from '../data/projections.js';
...
const BUSY_FARBE_DECKEND = 'color-mix(in srgb, var(--ink) 16%, var(--field))';
...
export function busyKalender(tage, optionen = {}) {
  const { act = null, eigen = false, heute = new Date().getDay(), rolle = 'wochenstreifen-lesend', hoehe = BUSY_HOEHE } = optionen;
  ...
  const spalten = (tage || []).map((eintrag, index) => {
    ...
    const bloecke = (eintrag.streifen || []).map((block) => busyBlockHtml(block, eigen, hoehe)).join('');
    const satz = busyTagSatz(eintrag, eigen);
...
<span data-role="streifen-flaeche" style="position:relative;display:block;height:${hoehe}px;margin-top:${BUSY_KOPF_ABSTAND}px;border-radius:7px;background:var(--field);overflow:hidden;pointer-events:none">${linien}${bloecke}</span>`;
...
// R8-39 „Dasselbe in den Profilen anderer": die Karte für das Profil EINER ANDEREN Person —
// dieselbe Überschrift, derselbe Kalender, nur lesend. Die Blöcke kommen aus dem Vertrag
// (repo.getBelegtWoche(personId), mit titel/privat); private stehen dort als „Busy". Hat die Person
// nichts eingetragen, steht die Karte gar nicht da — eine leere Woche sagt nichts.
// Einbau (room.js › renderPersonDetails, fremde Datei): `${busyKarte(ctx, person.id)}`.
export function busyKarte(ctx, personId) {
  let sicht = null;
  try { sicht = ctx?.repo?.getBelegtWoche?.(personId) || null; } catch { sicht = null; }
  if (!sicht || sicht.sichtbar === false) return '';
  const tage = sicht.tage || [];
  if (!tage.some((tag) => (tag.streifen || []).length)) return '';
  return `<div data-role="busy-karte" data-person="${esc(personId)}" style="flex:none;background:var(--surface);border-radius:20px;border:1px solid var(--ink-a07);padding:6px 16px 6px">
<div style="display:flex;align-items:center;padding:10px 0 4px"><span data-role="busy-titel" style="font-size:11px;font-weight:650;letter-spacing:.09em;text-transform:uppercase;color:var(--muted-light)">${tx('Busy')}</span></div>
<div style="padding-top:8px;border-top:1px solid var(--ink-a05)">${busyKalender(tage, { eigen: false, rolle: 'wochenstreifen-lesend' })}</div>
</div>`;
}
...
function homeHeader(ctx) {
  ...
  const rundKnopf = (act, aria, inner, punkt = false) => {
    const loch = punkt && typeof bausteine.kreisMaske === 'function' ? bausteine.kreisMaske(KOPF_KNOPF, ['or']) : '';
    const dot = punkt
      ? `<span data-role="kopf-punkt" aria-hidden="true" style="position:absolute;inset:0;pointer-events:none;z-index:2;${hinweisLaeuft ? `animation:profilPunktEin .3s cubic-bezier(.34,1.56,.64,1) ${ANFRAGE_HINWEIS_MS + 380}ms both` : ''}">${punktAmKreis(KOPF_KNOPF, 'or', { farbe: 'var(--orange)', luecke: true })}</span>`
      : '';
    return `<button data-act="${act}" data-treffer aria-label="${aria}" style="position:relative;flex:none;width:${KOPF_KNOPF}px;height:${KOPF_KNOPF}px;border:0;border-radius:50%;background:transparent;display:flex;align-items:center;justify-content:center;padding:0;cursor:pointer;appearance:none"><span data-role="kopf-kreis" aria-hidden="true" style="position:absolute;inset:0;border-radius:50%;border:1px solid var(--ink-a12);background:var(--surface);box-sizing:border-box;pointer-events:none;${loch}${loch && hinweisLaeuft ? `animation:profilLochEin 1ms step-end ${ANFRAGE_HINWEIS_MS + 380}ms both;` : ''}"></span><span style="position:relative;pointer-events:none;display:flex">${inner}</span>${dot}</button>`;
  };
...
function renderProfileHome(ctx) {
  ...
  const hinweis = anfrageHinweisHtml(ctx, { act: 'go-anfragen' });
  ctx.ui.anfrageHinweisDa = Boolean(hinweis);
  const html = screenScaffold({
    page: true,
    header: homeHeader(ctx),
    body: homeBody(ctx, { hinweisHtml: hinweis }),
...
function bindProfileHome(root, ctx) {
  ...
  if (root.querySelector('[data-role="anfrage-hinweis"]')) anfrageHinweisGesehen(ctx);
...
const QR_LUECKE = 12;        // Abstand zwischen den beiden Kacheln
...
const ANFRAGE_PILLE = `flex:none;height:34px;padding:0 15px;border-radius:999px;font:650 13px/1 ${FONT};cursor:pointer;appearance:none;white-space:nowrap`;
...
<button data-act="anfrage-ignorieren" data-anfrage="${id}" data-treffer style="${ANFRAGE_PILLE};border:1px solid var(--ink-a14);background:var(--surface);color:var(--ink-soft)"><span style="pointer-events:none">${esc(tx('Ignorieren'))}</span></button>
</span></span></div>`;
}
...
const qrHoehe = (offen) => (offen ? 'calc(100% - 48px)' : 'auto');
...
<div style="display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:${QR_LUECKE}px">
<div data-role="weg-code" style="display:flex;flex-direction:column;gap:8px;min-width:0">${codeKachel(code)}${inviteCodeRow(code, ui.copied)}</div>
<div data-role="weg-scan" style="display:flex;flex-direction:column;gap:8px;min-width:0">${scanKachel(scan, ui.qrKamera || '')}${codeEingabe()}</div>
</div>
${teilenKnopf()}
</div>
...
function bindQrZug(root, ctx) {
  ...
  const messen = () => {
    ...
    return { max, kompakt: kompaktVon(karteJetzt(), max), skala: skala || 1 };
  };
...
${navRow({ act: 'open', data: { route: 'profile.zuhause' }, label: tx('Zuhause'), valueHtml: value(zuhauseWert(repo)), last: true })}`)}
</div>
<div style="display:flex;flex-direction:column;gap:6px">
${groupLabel(tx('App'))}
...
${location.use ? `<span data-role="standort-ehrlich" style="display:block;font-size:11.5px;color:var(--muted);line-height:1.45;padding:0 4px">${tx('Solange Crew offen ist, bleibt dein Standort frisch. Ist die App zu, sehen deine Freunde nur, wann du zuletzt dort warst.')}</span>` : ''}
...
function zuhauseSuche(ctx) {
  ...
  zuhauseTimer = setTimeout(async () => {
    ...
    ctx.ui.zhTreffer = (gefunden || []).filter((ort) => typeof ort.lat === 'number' && typeof ort.lon === 'number');
    ctx.ui.zhSucht = false;
    ctx.render();
  }, 280);
}
...
${menuCard(rows, `right:${rechts}px;${menueOben(Math.max(12, ui.menuTop ?? 140), GRUPPEN_ZEILEN)};width:${MENU_WIDTH}px;max-width:calc(100% - ${rechts + 12}px)`, ` data-role="crew-menu" data-crew="${esc(id)}" data-angeheftet="${angeheftet}"`)}`;
  }
