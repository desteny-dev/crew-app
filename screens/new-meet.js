...
// Runde 11 (A15): Ein Schalter (CREW_CONFIG.vorschlaege, web/config.js) für Vorschläge beim Erstellen.
import { vorschlaegeAn } from '../core/chat-erkennung.js';
...
// Runde 8 (R8-6): Sind GENAU die Leute einer Gruppe eingeladen, fragt die Seite nach dem Senden
// „Welcher Chat?". Vorgemerkt wird hier (publishDraft → gruppeWaehlbar), gestellt wird die Frage
// dort, wo man landet: meet-details.js (Route meet.details).
import { chatWahlVormerken } from './crew.js';
import {
...
// Runde 6 (P4, C5): Ein Tipp auf die kleine Vorschlagskarte öffnet die GROSSE Karte in der App;
// die Anbieterwahl (Apple/Google) kommt erst dort hinter „Route" bzw. hier über den Knopf „Route".
...
// Runde 6 (C5): vier Pfeile nach außen — „diese Karte geht groß auf" (nicht „hier geht es hinaus").
const aufziehenIcon = (c = 'var(--ink)') => `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" style="pointer-events:none"><path d="M9.5 4.5H4.5V9.5M14.5 4.5h5v5M9.5 19.5h-5v-5M14.5 19.5h5v-5" stroke="${c}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path></svg>`;
...
// --- Runde 7 (E7): Zusammenhänge übernehmen, wo immer ein Meet entsteht ----------------------
//
// Bis hierher musste JEDER Einstieg selbst einen Entwurf anlegen UND ihn füllen: der Raum
// (room.js draftFuerRaum/planAlsMeet), die Crew-Startseite (spontanTreffen), der Meet-Browser
// (mb-new-meet), „Als Vorlage nutzen". Wer das vergaß — und der Kalendertag und die neue Karte
// vergessen es heute —, landete in einem leeren Entwurf, und der Mensch tippte alles noch
// einmal. Ab jetzt nimmt dieser Bereich den Zusammenhang auch direkt aus der ROUTE entgegen:
//
//   crewId                          mit welcher Gruppe
//   personIds  ['p-a','p-b'] | 'p-a,p-b'   mit wem
//   date ('YYYY-MM-DD'), time ('HH:MM')    aus einem Kalendertag
//   placeName (+ placeAddress, lat, lon)   aus der Karte oder einem Ort
//   ideaTitle (+ ideaCategory, ideaIcon, ideaIconKey)   aus einem Vorschlag oder einem Meet
//     — ideaIcon ist das SINNBILD („🌊"), ideaIconKey der KATALOGSCHLÜSSEL („wellen"),
//       genau wie an jedem Meet und jedem Vorschlag im Bestand. Wer nur eins von beiden hat,
//       schickt nur eins; wer ein Meet als Vorlage weiterreicht, hängt beide an.
//   note                                   eine Notiz, die weiter gilt („Turnbeutel")
//
// Damit genügt anderen Bereichen EIN nav.go('newMeet.discover', { … }) — sie brauchen weder
// createDraft noch updateDraft zu kennen. Was fehlt, bleibt leer: erfunden wird nichts.
//
// Runde 7, Welle 2 (Prüferfund): Gebaut war der Weg, benutzt hat ihn niemand — alle fünf
// Aufrufer legen ihren Entwurf selbst an und übergeben NUR die draftId. In genau diesem Fall
// hat ensureDraft den Zusammenhang aus der Route stillschweigend weggeworfen: Wer
// `{ draftId, date }` schickte, verlor den Tag. Ein Aufrufer, der beides mitgibt — und die
// drei fremden Bildschirme tun das, bis ihre Pakete umstellen —, war damit schlechter dran
// als einer, der gar nichts schickt.
//
// Ab jetzt gilt der Zusammenhang IMMER, auch über einem fertigen Entwurf. Die Regel dabei ist
// eng und in beide Richtungen ehrlich:
//   • Gefüllt wird ausschließlich, was im Entwurf LEER ist. Was der Aufrufer schon
//     hineingeschrieben hat, ist die genauere Wahrheit und bleibt unangetastet.
//   • Genau einmal je Besuch dieses Bereichs (ui.kontextAus), damit ein Rerender nach dem
//     Löschen eines Feldes es nicht wieder herzaubert.
// Damit greift die Übernahme ohne Mitwirkung der fremden Dateien: sie müssen ihren Entwurf
// nicht aufgeben, es genügt, den Zusammenhang ZUSÄTZLICH an die Route zu hängen.
function personIdsAusRoute(wert) {
  if (Array.isArray(wert)) return wert.filter(Boolean).map(String);
  return String(wert || '').split(',').map((id) => id.trim()).filter(Boolean);
}

// Runde 7, Welle 3 (Prüferfund, Hausregel 3): EIN Feld, EINE Form.
//
// Der Fix aus Welle 3 hat das Zeichen gerettet, aber in der falschen Schublade abgelegt: In
// `idea.icon` stand danach ein KATALOGSCHLÜSSEL („waves", „boots"), während jeder andere
// Datensatz im Bestand dort ein Sinnbild trägt („🌊", „⛰️") und den Schlüssel daneben in
// `iconKey` (seed/meets.js, seed/discover.js). Sichtbar war das nicht, weil activityIconKey
// beides versteht — aber es waren zwei Wahrheiten über dasselbe Feld, und sie gingen sofort
// auseinander: supabase-gateway.publishDraft liest `draft.idea.iconKey` (und bekam undefined),
// new-meet.js vergleicht `idea.icon === '🍕'`, und eine Vorlage erzeugte damit buchstäblich
// ein anderes Feld als ihr Vorbild.
//
// Ab jetzt gilt überall dasselbe: `icon` ist das SINNBILD, `iconKey` der KATALOGSCHLÜSSEL.
// Beide reisen über die Route (ideaIcon / ideaIconKey) und beide landen im Entwurf.
//
// Welle 5 (Prüferfund): Hier wurde einmal am INHALT geraten, in welche Schublade ein Wert
// gehört (reines ASCII = Schlüssel). Trugen beide Felder ASCII-Wörter (m-seebuehne
// 'film'/'ticket', m-jazz 'note'/'note'), gewann das erste, das zweite fiel weg und die
// Vorlage verlor ihr Zeichen. Mit EINER Form gibt es nichts zu raten: ideaIcon landet in
// `icon`, ideaIconKey in `iconKey` — genau so, wie es am Vorbild steht. Erfunden wird nichts:
// fehlt eine der beiden Formen, bleibt sie null und die Anzeige leitet ab.
function zeichenAusRoute(ideaIcon, ideaIconKey) {
  const icon = String(ideaIcon || '').trim() || null;
  const iconKey = String(ideaIconKey || '').trim() || null;
  return { icon, iconKey };
}

function kontextAusRoute(params = {}) {
  const personIds = personIdsAusRoute(params.personIds);
  const start = {};
  if (params.crewId) start.withCrewId = params.crewId;
  else if (personIds.length) start.withPersonIds = personIds;

  const patch = {};
  if (params.date) patch.when = { date: params.date, time: params.time || '', open: !params.time };
  if (params.placeName) {
    const lat = Number(params.lat);
    const lon = Number(params.lon);
    patch.place = {
      mode: 'search',
      name: String(params.placeName),
      address: String(params.placeAddress || ''),
      ...(Number.isFinite(lat) && Number.isFinite(lon) ? { lat, lon } : {}),
    };
  }
  if (params.ideaTitle) {
    const title = String(params.ideaTitle);
    // Hier stand einmal hart `icon: null` — und damit verlor eine Vorlage genau das Zeichen,
    // das am alten Meet handverlesen dranstand („Freitagsrunde 🍕" wurde beim nächsten Mal zum
    // Funkeln, weil activityIconKey ohne Stichwort im Titel auf die Kategorie zurückfällt und
    // demo-gateway publishDraft dann '✨' festschreibt). Ein mitgegebenes Zeichen ist keine
    // Erfindung, sondern die genauere Wahrheit — wer keins schickt, bekommt weiter null und
    // damit die Ableitung aus Titel und Kategorie. In welche Schublade es gehört, entscheidet
    // zeichenAusRoute (siehe dort).
    const zeichen = zeichenAusRoute(params.ideaIcon, params.ideaIconKey);
    patch.idea = {
      title,
      icon: zeichen.icon,
      iconKey: zeichen.iconKey,
      category: params.ideaCategory || eigeneKategorie(title),
      details: '',
      source: params.ideaSource || 'own',
      suggestionId: params.suggestionId || null,
      ownId: null,
      place: patch.place ? { ...patch.place } : null,
    };
  }
  if (params.note) patch.note = String(params.note);
  return { start, patch };
}

// Ist dieses Feld im Entwurf wirklich leer? „Leer" heißt: es steht nichts drin, was ein Mensch
// gewählt hätte. Ein `when` ohne Datum und ein `place` ohne Namen sind Gerüste, keine Angaben.
function leerImEntwurf(draft, feld, wert) {
  const da = draft?.[feld];
  if (feld === 'when') return !da?.date;
  if (feld === 'place') return !da?.name && typeof da?.lat !== 'number';
  if (feld === 'idea') return !da?.title;
  if (feld === 'note') return !String(da || '').trim();
  return da == null || da === '' ? true : !wert;
}

// Den Zusammenhang aus der Route über einen BESTEHENDEN Entwurf legen — nur in die Lücken.
// Rückgabe: der (ggf. frische) Entwurf.
function kontextUebernehmen(ctx, draft) {
  const { repo, params, ui } = ctx;
  if (!draft || ui.kontextAus === draft.id) return draft;
  ui.kontextAus = draft.id;
  const { start, patch } = kontextAusRoute(params);
  const offen = {};
  for (const [feld, wert] of Object.entries(patch)) {
    if (leerImEntwurf(draft, feld, wert)) offen[feld] = wert;
  }
  // Mit wem: eine Gruppe oder Personen stehen im Entwurf selbst und werden nie überschrieben —
  // wer schon eine Runde gewählt hat, bekommt keine zweite untergeschoben.
  if (start.withCrewId && !draft.withCrewId && !(draft.withPersonIds || []).length) offen.withCrewId = start.withCrewId;
  if (start.withPersonIds?.length && !draft.withCrewId && !(draft.withPersonIds || []).length) offen.withPersonIds = start.withPersonIds;
  if (!Object.keys(offen).length) return draft;
  repo.updateDraft(draft.id, offen);
  return repo.getDraft(draft.id) || draft;
}

function ensureDraft(ctx) {
  const { repo, params, ui } = ctx;
  const existing = params.draftId ? repo.getDraft(params.draftId) : null;
  // Auch ein mitgebrachter Entwurf bekommt den Zusammenhang der Route — in seine Lücken.
  if (existing) return { draft: kontextUebernehmen(ctx, existing), replaced: false };
  if (!ui.autoDraftId || !repo.getDraft(ui.autoDraftId)) {
    const { start, patch } = kontextAusRoute(params);
    ui.autoDraftId = repo.createDraft(start).id;
    if (Object.keys(patch).length) repo.updateDraft(ui.autoDraftId, patch);
    ui.kontextAus = ui.autoDraftId;
  }
...
function iconBeschreibung(entry = {}) {
  ...
  // Runde 7, Welle 3: Hier standen zwei getrennte Zweige, und der für `icon` kam zuerst. Damit
  // GEWANN das Sinnbild über den Schlüssel — umgekehrt als in activityIconKey, das den
  // Schlüssel als die genauere Angabe behandelt. An „Spikeball im Park" (icon '🏐', iconKey
  // 'ball') zeichnete diese Fläche deshalb einen anderen Ball als jede andere. Jetzt reichen
  // beide Felder weiter, und die Rangfolge steht an genau EINER Stelle: in activityIconKey.
  if (entry.icon || entry.iconKey) {
    return { icon: entry.icon || null, iconKey: entry.iconKey || null, title, category };
  }
...
function applySelection(ctx, draftId, entries) {
  ...
  const patch = {
    ...
    // Runde 7, Welle 3 (Hausregel 3): `iconKey` fiel hier heraus. Ein Vorschlag trägt BEIDE
    // Formen (seed/discover.js: icon '🏐', iconKey 'ball'), und der Schlüssel ist die genauere
    // — wer ihn wegwirft, zeichnet am Entwurf ein anderes Zeichen als an der Karte, von der
    // der Mensch es ausgewählt hat. Beide gehen mit.
    idea: main
      ? {
        title: main.title, icon: main.icon || null, iconKey: main.iconKey || null,
    ...
    ideaAlternatives: entries.slice(1).map((e) => ({
      title: e.title, icon: e.icon || null, iconKey: e.iconKey || null, category: e.category || null,
...
function whatRow(draft) {
  ...
  const sub = idea
    ...
    : (vorschlaegeAn() ? tx('Entdecken oder eigene Idee') : tx('Eigene Idee'));
...
function coreUi(ctx, opts) {
  ...
  // Runde 11 (A15): Sind die Vorschläge aus, gibt es nur „Neue Idee" (eigene Idee, Gespeichertes,
  // Spontan) — Meet erstellen geht ohne jeden Vorschlag voll. Der Code für die Vorschläge bleibt.
  if (!vorschlaegeAn()) ctx.ui[key].tab = 'own';
  return ctx.ui[key];
}
...
// =====================================================================
// Runde 7 (E8): Interessen und Ressourcen wachsen aus den TREFFEN
// =====================================================================
// Bisher wusste die App über eine Gruppe nur, was jemand EINMAL in eine Liste getippt hat
// (settings.interests/resources) — ein Formular, das niemand pflegt, und das über eine Runde
// von fünf Leuten ohnehin nichts aussagt. Was diese Leute TATSÄCHLICH miteinander machen,
// stand ungenutzt in ihren eigenen vergangenen Meets.
//
// Der ehrliche kleine Anfang — und ausdrücklich nicht mehr:
//   WAS ZÄHLT   Die vergangenen gemeinsamen Meets der Eingeladenen (bzw. der Crew): nicht
//               abgesagt, nicht als „hat nicht stattgefunden" bewertet, höchstens ein halbes
//               Jahr her, jedes Meet genau einmal. Gezählt wird die Kategorie — und aus
//               `bring` das, was ihr wiederholt mitgebracht habt (die einzige Ressourcenspur,
//               die es in echten Daten gibt).
//   WO SICHTBAR (1) An der Vorschlagskarte steht der GRUND: „Macht ihr öfter: Sport" bzw.
//               „Wart ihr schon zusammen: Grillen am See" — statt der bisherigen Auskunft aus
//               einer Interessenliste, die niemand pflegt.
//               (2) In der REIHENFOLGE: innerhalb jeder Stufe steht vorn, was ihr wirklich
//               macht (mitGemeinsamemVorrang). Die Stufen selbst bleiben der Engine überlassen.
//   WAS NICHT   Nichts wird still in settings.interests geschrieben, nichts wird vorhergesagt,
//               es gibt keinen Punktestand und keine Bewertung von Menschen. Gibt es keine
//               gemeinsame Vergangenheit, steht dort nichts — statt einer erfundenen Vorliebe.
const E8_TAGE = 182;
const E8_LEER = { kategorien: [], nachKategorie: new Map(), nachArt: new Map(), dinge: [], meets: 0 };

// Die gemeinsamen, vergangenen Meets. Ohne Crew zählen die Meets mit den eingeladenen
// Personen — jedes nur einmal, auch wenn mehrere von ihnen dabei waren.
function gemeinsameMeets(ctx, opts) {
  const repo = ctx.repo;
  if (typeof repo.getMeets !== 'function') return [];
  const { crewId, personIds } = teilnehmendeVon(ctx, opts);
  const kontexte = crewId ? [{ crewId }] : personIds.slice(0, 6).map((id) => ({ personId: id }));
  if (!kontexte.length) return [];
  const grenze = toISODate(new Date(now().getTime() - E8_TAGE * 86400000));
  const gesehen = new Set();
  const raus = [];
  for (const context of kontexte) {
    let liste = [];
    try { liste = repo.getMeets({ context, direction: 'history' }) || []; } catch { liste = []; }
    for (const meet of liste) {
      if (!meet?.id || gesehen.has(meet.id)) continue;
      gesehen.add(meet.id);
      if (meet.status === 'draft' || meet.cancelled) continue;
      // Was nicht stattgefunden hat, sagt nichts darüber, was ihr macht.
      if (meet.review?.verdict === 'fand-nicht-statt') continue;
      if ((meet.date || '') < grenze) continue;
      raus.push(meet);
    }
  }
  return raus;
...
// Das Bild, das daraus entsteht. Reine Zählung, keine Deutung.
function gemeinsamesBild(ctx, opts) {
  const meets = gemeinsameMeets(ctx, opts);
  if (!meets.length) return E8_LEER;
  const nachKategorie = new Map();
  const arten = new Map();
  const dinge = new Map();
  for (const meet of meets) {
    const kategorie = CATEGORIES.some((c) => c.id === meet.category && c.id !== 'egal')
      ? meet.category
      : eigeneKategorie(meet.title);
    const eintrag = nachKategorie.get(kategorie) || { kategorie, anzahl: 0, zuletzt: '', titel: '' };
    eintrag.anzahl += 1;
    if ((meet.date || '') >= eintrag.zuletzt) { eintrag.zuletzt = meet.date || ''; eintrag.titel = meet.title || ''; }
    nachKategorie.set(kategorie, eintrag);
    // Runde 7, Welle 2 (E8, Prüferfund): Der Satz „Habt ihr zusammen gemacht: <Titel>" nennt ein
    // KONKRETES vergangenes Meet — verglichen wurde aber nur die Kategorie. An der Karte
    // „Strandbad Bregenz" stand deshalb „Habt ihr zusammen gemacht: Spieleabend bei Noah". Ein
    // Mensch liest das als „wir waren zusammen im Strandbad". Das ist eine Lüge, keine Näherung.
    // Damit der Satz wahr werden KANN, wird jetzt auch die feinere ART mitgezählt (dasselbe
    // Aktivitätszeichen wie an der Vorschlagskarte, ui/activity-icons.js) — und der Titel wird
    // nur dort genannt, wo wirklich dieselbe Art gemeint ist.
    // Streng aus dem TITEL — ohne den Rückfall auf die Kategorie (und ohne 'funkeln'), den
    // artVon() für die Vorschlagsliste hat. Ein Rückfall würde hier wieder Ungleiches gleich
    // machen und genau die Lüge erzeugen, die dieser Abschnitt beseitigt.
    const art = iconKeyForText(meet.title);
    if (art) {
      const nachArt = arten.get(art) || { art, anzahl: 0, zuletzt: '', titel: '' };
      nachArt.anzahl += 1;
      if ((meet.date || '') >= nachArt.zuletzt) { nachArt.zuletzt = meet.date || ''; nachArt.titel = meet.title || ''; }
      arten.set(art, nachArt);
    }
    // Ressourcen: was jemand wirklich übernommen hat („Ben bringt den Grill"), nicht was
    // irgendwo als Besitz eingetragen ist. Offene Punkte zählen nicht — sie sind ein Wunsch.
    for (const punkt of meet.bring || []) {
      const label = String(punkt?.label || '').trim();
      if (!label || !(punkt.takenBy || []).length) continue;
      const schluessel = label.toLowerCase();
      const ding = dinge.get(schluessel) || { label, anzahl: 0 };
      ding.anzahl += 1;
      dinge.set(schluessel, ding);
    }
  }
  const kategorien = [...nachKategorie.values()]
  ...
  return {
    ...
    nachKategorie,
    nachArt: arten,
    // Einmal mitgebracht ist Zufall; zweimal ist eine Gewohnheit.
    dinge: [...dinge.values()].filter((d) => d.anzahl >= 2).sort((a, b) => b.anzahl - a.anzahl),
    meets: meets.length,
  };
}

// Wirkung auf die REIHENFOLGE: Innerhalb einer Stufe steht vorn, was diese Runde wirklich
// macht. Die Stufen selbst bleiben unangetastet — was „besonders" ist, entscheidet weiter die
// Engine; hier wird nur innerhalb der Stufe umsortiert, stabil (gleicher Rang = alte Ordnung).
//
// Warum keine eigene Zeile „Was ihr zusammen macht" über der Liste (erst gebaut, dann wieder
// entfernt): Sie schob die Vorschlagsliste um 82 px nach unten, und die dritte Karte lag damit
// hinter der unteren Bedienebene (gemessen: Ebene beginnt bei y = 686, die Karte landete bei
// 726). Ein Mensch scrollt zwar, aber eine Zeile, die Inhalt unter die Bedienung schiebt, ist
// den Platz nicht wert — und die Kategorie-Knöpfe hätte es im Filter ohnehin schon gegeben.
// Sichtbar wird E8 deshalb dort, wo es hingehört: an der Vorschlagskarte selbst (grundFuer).
function mitGemeinsamemVorrang(items, bild) {
  if (!bild.kategorien.length || items.length < 2) return items;
  const stufeVon = (s) => (STUFEN_REIHE.includes(s.stufe) ? s.stufe : 'immer');
  const vorrang = (s) => (s.category && bild.nachKategorie.has(s.category) ? 0 : 1);
  const gruppen = { besonders: [], passend: [], immer: [] };
  items.forEach((s, index) => gruppen[stufeVon(s)].push({ s, index }));
  const sortiert = (liste) => liste
    .sort((a, b) => (vorrang(a.s) - vorrang(b.s)) || (a.index - b.index))
    .map((eintrag) => eintrag.s);
  return [...sortiert(gruppen.besonders), ...sortiert(gruppen.passend), ...sortiert(gruppen.immer)];
}
...
// Die Art eines Vorschlags OHNE den Rückfall auf Kategorie und Funkeln: Sie zählt nur, wenn
// der Vorschlag ein Zeichen wirklich trägt (icon/iconKey) — alles andere wäre geraten. Wird
// dort gebraucht, wo ein Satz etwas über die gemeinsame Vergangenheit BEHAUPTET.
function ohneKategorieRueckfall(s) {
  const key = activityIconKey({ iconKey: s?.iconKey, icon: s?.icon, title: s?.title });
  return key === 'funkeln' ? null : key;
}
...
// Runde 7 (E8): Der erste Grund kommt jetzt aus den TREFFEN dieser Runde, nicht aus einer
// gepflegten Liste — was ihr wirklich gemacht habt, wiegt schwerer als ein Formulareintrag.
function grundFuer(ctx, s, bild = E8_LEER) {
  const settings = ctx.repo.getSettings() || {};
  const art = s.art || artVon(s);
  const text = `${s.title || ''} ${s.blurb || ''}`.toLowerCase();
  // Wortlaut bewusst in derselben Familie wie die vorhandenen Gründe („Passt zu: …",
  // „Ihr habt: …", „Habt ihr schon gewählt") — die Karte behält EINE Sprache.
  // Runde 7, Welle 2 (E8): Wortlaut und Vergleich decken sich jetzt — das war der Prüferfund.
  // Zuerst die feinere ART: nur wenn wirklich DIESELBE Sache schon einmal war, darf der Satz
  // ein konkretes Meet beim Namen nennen („Habt ihr zusammen gemacht: Bouldern in der K1").
  const artStreng = iconKeyForText(s.title);
  const gleicheArt = artStreng ? bild.nachArt?.get(artStreng) : null;
  if (gleicheArt?.anzahl >= 2) return tx('Habt ihr öfter gemacht: {was}', { was: gleicheArt.titel });
  if (gleicheArt?.anzahl === 1) return tx('Habt ihr zusammen gemacht: {was}', { was: gleicheArt.titel });
  // Sonst bleibt nur die KATEGORIE — und dann nennt der Satz auch nur sie. „Habt ihr öfter
  // gemacht: Sport" ist wahr; „… : Spieleabend bei Noah" an einer Strandbad-Karte wäre es nicht.
  const gemeinsam = s.category ? bild.nachKategorie.get(s.category) : null;
  if (gemeinsam?.anzahl >= 2) {
    return tx('Habt ihr öfter gemacht: {was}', { was: categoryAnzeige(gemeinsam.kategorie) || gemeinsam.kategorie });
  }
  if (gemeinsam?.anzahl === 1) return tx('Habt ihr zusammen gemacht: {was}', { was: categoryAnzeige(gemeinsam.kategorie) || gemeinsam.kategorie });
  // Ressourcen aus den Treffen: was ihr wiederholt selbst mitgebracht habt.
  // Runde 7, Welle 3 (Prüferfund): Auch dieser Satz behauptet etwas über EURE Vergangenheit —
  // verglichen wurde aber weiter gegen `art = s.art || artVon(s)`, also MIT dem Kategorie-
  // Rückfall (und dem 'funkeln'), den die beiden Zweige darüber gerade deshalb vermeiden, weil
  // er Ungleiches gleich macht. Gemessen (r7b-entwurf, E8c): An einer Karte „Bei Mia" ohne
  // Stichwort im Titel und ohne eigenes Zeichen fällt art auf die Kategorie zurück (chillen →
  // 'sofa') — und dann stand dort „Habt ihr oft dabei: Sofakissen", obwohl nichts an dieser
  // Karte mit dem Ding zu tun hat. Verglichen wird jetzt die Art OHNE Rückfall: das Stichwort
  // im TITEL, sonst ein Zeichen, das der Vorschlag selbst trägt (icon/iconKey) — sonst gar
  // nichts. Der Zweig bleibt lebendig, wo er die Wahrheit sagt („Sofaabend" → Sofakissen), und
  // schweigt, wo er nur über die Kategorie geraten hätte.
  const artOhneRueckfall = artStreng || ohneKategorieRueckfall(s);
  for (const ding of bild.dinge) {
    if (artOhneRueckfall && iconKeyForText(ding.label) === artOhneRueckfall) return tx('Habt ihr oft dabei: {ding}', { ding: ding.label });
  }
...
function listInnerHtml(ctx, opts, filter) {
  ...
  const mitte = vorschlagsMitte(ctx, opts);
  // E8: EINMAL je Zeichnung gezählt — Reihenfolge und Karten lesen dasselbe Bild.
  const bild = gemeinsamesBild(ctx, opts);
  const roh = mitGemeinsamemVorrang(suggestionsFor(ctx, filter, query, { opts }), bild);
  ...
  for (const eintrag of reihe) {
    ...
    if (!query && eintrag.stufe === 'besonders') {
      teile.push(vorschlagZugHtml(s, besondersKarteHtml(s, { ...karte, grund: s.reason || grundFuer(ctx, s, bild), fotoOffen: ui.fotoInfo === `karte:${s.id}` }), 18));
    } else if (kompakt) {
      gruppe.push(vorschlagZugHtml(s, kurzZeileHtml({ title: s.title, ...karte }), 0, gruppe.length > 0));
    } else {
      const grund = !query && eintrag.stufe === 'passend' ? (grundFuer(ctx, s, bild) || s.reason || '') : '';
...
export function renderDiscoverCore(ctx, opts = {}) {
  ...
  const toggleRow = !vorschlaegeAn() ? '' : `<div style="position:relative;background:var(--field);border-radius:16px;padding:4px;display:flex">
...
export function bindDiscoverCore(root, ctx, opts = {}) {
  ...
  bindActions(root, {
    ...
    'sug-karte': (data) => vorschlagKarteGross(ctx, data.suggestion),
...
// Runde 4 (D8): Tipp auf die kleine Karte → Route.
// Runde 6 (C5, Jonathan: „ich will jede karte in der app öffnen können"): Der Tipp öffnet jetzt die
// GROSSE Karte in der App — der Ort in der Mitte, Titel und Adresse oben, „Route" und „Schließen"
// unten. Die Anbieterwahl erscheint nicht mehr beim Tipp auf die kleine Karte.
function vorschlagKarteGross(c, suggestionId) {
  const s = c.repo.getSuggestion(suggestionId);
  if (!s?.place || !hatLage(s.place)) return;
  karteGrossOeffnen({
    titel: s.place.name || s.shortTitle || s.title || '',
    adresse: s.place.address || '',
    lat: s.place.lat,
    lon: s.place.lon,
    zeichen: activityIconSvg({ icon: s.icon, iconKey: s.iconKey, title: s.title, category: s.category }, 'currentColor', 20),
    unter: s.distanceKm != null ? `${fmtKm(s.distanceKm)} km` : '',
  });
}
...
function suggestionSheetHtml(s, options = {}) {
  ...
  const miniMap = hatLage(s.place)
    ? `<div data-act="sug-karte" data-suggestion="${esc(s.id)}" data-role="vorschlag-ortkarte" role="button" aria-label="${esc(tx('Karte öffnen'))}" style="border-radius:14px;overflow:hidden;clip-path:inset(0 round 14px);position:relative;height:132px;background:var(--field);flex:none;cursor:pointer">
...
<span data-role="vorschlag-ortkarte-route" style="position:absolute;left:8px;bottom:8px;z-index:2;display:flex;align-items:center;gap:5px;height:26px;padding:0 10px 0 8px;border-radius:13px;background:var(--glass);-webkit-backdrop-filter:blur(10px);backdrop-filter:blur(10px);box-shadow:0 1px 4px var(--shadow-10);font:650 11px/1 ${FONT};color:var(--ink);pointer-events:none">${aufziehenIcon('var(--ink)')}${tx('Karte öffnen')}</span>
</div>`
    : '';
  ...
  return {
    ...
    bind(root, c) {
      ...
      bindActions(root, {
        ...
        'sug-karte': (data) => vorschlagKarteGross(c, data.suggestion),
      });
      bindVorschlagOrtkarte(root);
      bindLoopActions(root, c, draft);
      bindActions(root, discardActions(c));
...
// --- Runde 7 (H2): Zuhause — es FEHLT ehrlich -----------------------------------------------
//
// Bis hierher stand hier `|| { name: 'Rathausstraße 12', city: 'Bregenz' }`. Wer nie ein
// Zuhause hinterlegt hatte — also JEDER echte Mensch, denn festlegen konnte man es nirgends —,
// bekam unter „Bei mir" eine fremde Bregenzer Adresse vorgesetzt und hätte sie mit „Ort
// übernehmen" sogar als Treffpunkt ins Meet geschrieben. Das ist genau der Fall, den Hausregel 8
// verbietet. Ab jetzt gilt der Datenvertrag: getHomeAddress() → Ort | null, und null heißt null.
function zuhause(ctx) {
  if (typeof ctx.repo.getHomeAddress === 'function') return ctx.repo.getHomeAddress() || null;
  const ort = ctx.repo.getSettings().homeAddress;
  return ort && typeof ort.lat === 'number' && typeof ort.lon === 'number' ? ort : null;
}

// Die Anschrift des Zuhauses als EIN Satz — leer, wenn es keine gibt. Aus dem Gerätestandort
// kommt kein Straßenname (Vertrag: quelle 'standort', name ''); dann wird auch keiner behauptet.
function zuhauseAnschrift(ort) {
  return [ort?.name, ort?.city].map((teil) => String(teil || '').trim()).filter(Boolean).join(', ');
}

// Was ein Mensch unter „Bei mir" liest. Ohne Zuhause: die ehrliche Auskunft, nicht eine Adresse.
function zuhauseText(ort) {
  if (!ort) return tx('Noch kein Zuhause hinterlegt');
  return zuhauseAnschrift(ort) || tx('Aktueller Standort');
}
...
function placeDaheim(ctx, draft, ui) {
  const settings = ctx.repo.getSettings();
  // H2: Ohne hinterlegtes Zuhause steht hier keine Adresse, sondern der Weg, eines festzulegen.
  const meinZuhause = zuhause(ctx);
  const meinText = zuhauseText(meinZuhause);
...
${gewaehlt ? `<button data-act="place-person-weg" aria-label="${esc(tx('Auswahl aufheben'))}" style="width:44px;height:44px;border:0;border-left:1px solid var(--ink-a07);background:transparent;padding:0;display:flex;align-items:center;justify-content:center;cursor:pointer;appearance:none;flex:none">${crossSmallIcon('var(--muted)', 13)}</button>` : ''}
  ...
  // „Bei mir" trägt zusätzlich einen leisen Weg, das eigene Zuhause genau zu setzen. H2: Fehlt es
  // ganz, heißt derselbe Knopf „festlegen" — der Satz daneben sagt vorher ehrlich, dass nichts da ist.
  const ichZeile = `<div style="display:flex;align-items:center;gap:2px">
<div style="flex:1;min-width:0">${zeile(`<span style="width:32px;height:32px;border-radius:50%;overflow:hidden;flex:none;pointer-events:none">${avatarFlaeche({ photo: settings.photo, color: settings.color, initials: settings.initials }, 32, 12)}</span>${texte(tx('Bei mir'), meinText, meinZuhause ? 'var(--muted)' : 'var(--orange-dark)')}`, ME, ui.personId === ME)}</div>
<button data-act="mine-map" data-role="zuhause-setzen" style="flex:none;border:0;background:transparent;padding:14px 8px;font:650 11.5px ${FONT};color:var(--green-dark);cursor:pointer;appearance:none;white-space:nowrap"><span style="pointer-events:none">${meinZuhause ? tx('genauer') : tx('festlegen')}</span></button>
</div>`;
...
function placeDaheimCta(ctx, draft, ui) {
  if (ui.personId === ME) {
    // H2: Ohne Zuhause gibt es nichts zu übernehmen. Statt einer erfundenen Adresse führt der
    // Knopf dorthin, wo man es in einem Schritt festlegt — und hält damit sein Wort (Hausregel 9).
    if (!zuhause(ctx)) {
      return `<button data-act="mine-map" data-role="zuhause-cta" style="${PLACE_KNOPF};background:var(--green);color:var(--on-accent)">${tx('Zuhause festlegen')}</button>`;
    }
    return placeApplyBtn(true);
  }
...
function renderPlace(ctx) {
  ...
  if (daheim) {
    ...
    if (ui.personId === ME) {
      // H2: Kein Zuhause → kein Pin und keine Adresse, sondern der ehrliche Satz samt Weg dorthin.
      const meinOrt = zuhause(ctx);
      if (meinOrt) {
        pinLabel = tx('Bei mir');
        ort = { lat: meinOrt.lat, lon: meinOrt.lon };
      } else {
        hinweis = tx('Du hast noch kein Zuhause hinterlegt — leg es unten fest, dann steht es hier.');
      }
    } else if (ui.personId) {
      ...
      bindActions(root, {
        ...
        'place-apply': () => {
          if (ui.mode === 'home' && ui.personId === ME) {
            ...
            // H2: Ohne Zuhause wird nichts übernommen — der Weg dorthin steht einen Tipp entfernt.
            const meinOrt = zuhause(c);
            if (!meinOrt) {
              c.toast(tx('Leg zuerst fest, wo du zu Hause bist'));
              c.nav.go('newMeet.mapPick', { draftId: draft.id, target: 'mine' });
              return;
            }
            // T5/H2: Länge und Breite wandern mit — daran hängt die Karte im Meet und die Route.
            c.repo.updateDraft(draft.id, {
              place: {
                mode: 'mine', name: 'Bei mir', address: zuhauseAnschrift(meinOrt),
                lat: meinOrt.lat, lon: meinOrt.lon,
              },
            });
            c.nav.back();
...
// Runde 7, Welle 2 (H2, vom Prüfer gemessen): Diese Leiste hatte ZWEI Zustände zu wenig.
//
//   (a) WARTEN war kein Zustand. Das frühere `ui.pickEcht` wurde erst gesetzt, wenn das Nachschlagen
//       geantwortet hat — bis dahin ist es false, und false hieß im Code „es gibt hier keine
//       Anschrift". Wer während „Wird nachgeschlagen …" auf „Diesen Ort wählen" tippte, bekam
//       deshalb die Meldung „Zu dieser Stelle gibt es keine Anschrift", obwohl niemand das
//       wusste — und sein Zuhause wurde nicht gesetzt, obwohl die Leiste es zugesagt hatte.
//       Eine Zusage, die nicht hält, plus eine Begründung, die nicht stimmt.
//   (b) AUSFALL war kein Zustand. Ohne Karte (kein Netz, kein WebGL) bleibt pickPunkt null;
//       die Speicher-Bedingung griff gar nicht, es kam KEIN Hinweis — und der Entwurf bekam
//       trotzdem einen Ort „Bei mir · Diese Stelle". Ein Knopf, der unter einer ausdrücklichen
//       Zusage stumm etwas anderes tut.
//
// Jetzt gibt es vier benannte Zustände, und jeder sagt dasselbe an der Leiste, im Hinweis und
// im Knopf. Der Mensch drückt EINMAL: läuft das Nachschlagen noch, wartet der Knopf es ab,
// statt ihn mit einer erfundenen Begründung wegzuschicken.
const PICK_WARTE_MS = 6000;

function pickZuhauseText(zustand) {
  if (zustand === 'anschrift') return tx('Wird als dein Zuhause gespeichert');
  if (zustand === 'ohne') return tx('Ohne Anschrift kein Zuhause — die Stelle gilt nur für dieses Meet');
  if (zustand === 'aus') return tx('Ohne Karte lässt sich kein Zuhause festlegen');
  return tx('Die Anschrift wird noch nachgeschlagen');
}
...
function renderMapPick(ctx) {
  ...
  if (!ui.pickZustand) ui.pickZustand = 'wartet';
  const zustand = ui.pickZustand;
  const addr = zustand === 'aus'
    ? { name: tx('Karte nicht verfügbar'), city: tx('Ohne Netz lässt sich kein Ort wählen') }
    : ui.pickAdresse || { name: tx('Diese Stelle'), city: tx('Wird nachgeschlagen …') };
...
<span data-role="pick-center" style="position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);width:9px;height:9px;border-radius:50%;background:var(--ink-a35);pointer-events:none;z-index:2"></span>
  ...
  // Der Knopf ist nur dann tot, wenn es wirklich nichts zu wählen gibt (Ausfall). Beim Warten
  // bleibt er drückbar — er wartet dann selbst, statt den Menschen warten zu lassen.
  const knopfAus = zustand === 'aus';
...
${/* H2: Wer hier für „Bei mir" wählt, setzt sein Zuhause. Was davon JETZT gilt, steht da, bevor er drückt. */ ''}${ctx.params.target === 'mine' ? `<span data-role="pick-zuhause" data-zustand="${zustand}" style="display:flex;align-items:center;gap:8px;font:600 11.5px/1.4 ${FONT};color:var(--ink-soft)">${houseIcon('var(--ink-soft)', 14)}<span data-role="pick-zuhause-text">${esc(pickZuhauseText(zustand))}</span></span>` : ''}
<button data-act="pick-confirm" data-zustand="${zustand}"${knopfAus ? ' disabled aria-disabled="true"' : ''} style="display:block;width:100%;background:${knopfAus ? 'var(--field)' : 'var(--green)'};color:${knopfAus ? 'var(--muted)' : 'var(--on-accent)'};font:650 15px/1 ${FONT};padding:16px 0;border-radius:999px;text-align:center;border:0;cursor:${knopfAus ? 'default' : 'pointer'};appearance:none">${tx('Diesen Ort wählen')}</button>
</div>`);
  ...
  return {
    ...
    bind(root, c) {
      ...
      const zuhausEl = root.querySelector('[data-role="pick-zuhause"]');
      const knopfEl = root.querySelector('[data-act="pick-confirm"]');

      // EINE Stelle, an der ein Zustandswechsel sichtbar wird — Hinweiszeile und Knopf können
      // deshalb nicht auseinanderlaufen.
      const zustandSetzen = (neu) => {
        ui.pickZustand = neu;
        if (zuhausEl) {
          zuhausEl.dataset.zustand = neu;
          const textEl = zuhausEl.querySelector('[data-role="pick-zuhause-text"]');
          if (textEl) textEl.textContent = pickZuhauseText(neu);
        }
        if (!knopfEl) return;
        knopfEl.dataset.zustand = neu;
        const aus = neu === 'aus';
        knopfEl.disabled = aus;
        if (aus) knopfEl.setAttribute('aria-disabled', 'true');
        else knopfEl.removeAttribute('aria-disabled');
        knopfEl.style.background = aus ? 'var(--field)' : 'var(--green)';
        knopfEl.style.color = aus ? 'var(--muted)' : 'var(--on-accent)';
        knopfEl.style.cursor = aus ? 'default' : 'pointer';
      };
      ...
      if (kartenKnoten) {
        import('../ui/map.js').then(async ({ karteHalten, ortAnPunkt, START_MITTE, kartenLageQuelle }) => {
          ...
          if (!karte) {
            ...
            // sagt, dass er die Stelle nicht nachschlagen kann — UND der Knopf hört auf, einen
            // Ort zu versprechen, den es nicht gibt (H2, Welle 2b).
          ...
          const nachschlagen = () => {
            ...
            // WARTEN ist ein eigener Zustand — und er ist von außen sichtbar: der laufende
            // Versuch liegt in ui.pickLauf, damit „Diesen Ort wählen" ihn abwarten kann,
            // statt seine eigene (falsche) Begründung zu erfinden.
            zustandSetzen('wartet');
            ui.pickLauf = (async () => {
              let treffer = null;
              try { treffer = await ortAnPunkt(punkt.lat, punkt.lng); } catch { treffer = null; }
              ...
              // H2: Ob die Stelle wirklich eine Anschrift hat, entscheidet, ob daraus ein
              // Zuhause werden darf — ein übersetztes „Diese Stelle" wäre kein Name, sondern eine Lücke.
              const echt = Boolean(treffer && (treffer.name || treffer.address));
              ...
              zustandSetzen(echt ? 'anschrift' : 'ohne');
            })();
            return ui.pickLauf;
          };
          if (!karte.__crewPick) {
          ...
          if (!ui.pickAdresse) nachschlagen();
            ...
            if (nameEl) nameEl.textContent = ui.pickAdresse.name;
            if (cityEl) cityEl.textContent = ui.pickAdresse.city;
            // Die gemerkte Anschrift entscheidet selbst, in welchem Zustand die Leiste steht —
            // „Diese Stelle" ist der Name einer Lücke, kein Name.
            zustandSetzen(ui.pickAdresse.name && ui.pickAdresse.name !== tx('Diese Stelle') ? 'anschrift' : 'ohne');
          }
        }).catch(() => {
          // Auch ein gescheiterter Import ist ein Ausfall — und muss genauso dastehen.
          if (nameEl) nameEl.textContent = tx('Karte nicht verfügbar');
          if (cityEl) cityEl.textContent = tx('Ohne Netz lässt sich kein Ort wählen');
          zustandSetzen('aus');
        });
      ...
      // Die eigentliche Übernahme — sie läuft erst, wenn feststeht, WAS da ist.
      const uebernehmen = () => {
        const punkt = ui.pickPunkt;
        const zustandJetzt = ui.pickZustand;
        // Ausfall: es gibt keinen Punkt und keine Anschrift. Dann wird auch nichts in den
        // Entwurf geschrieben — lieber kein Ort als ein behaupteter.
        if (zustandJetzt === 'aus' || !punkt) {
          c.toast(tx('Ohne Karte lässt sich kein Ort wählen'));
          return;
        }
        const echt = zustandJetzt === 'anschrift';
        const gewaehlt = ui.pickAdresse || { name: tx('Diese Stelle'), city: '' };
        const koordinaten = { lat: punkt.lat, lon: punkt.lon };
        // Die ausführliche Zeile enthält den Namen oft schon („Am Brand" · „Am Brand 3,
        // 6900 Bregenz"). Dann wird er nicht noch einmal davorgesetzt. Ohne Anschrift steht
        // dort nur, was wirklich bekannt ist: die Koordinaten.
        const anschrift = !echt
          ? (gewaehlt.city || '')
          : gewaehlt.city && gewaehlt.city.includes(gewaehlt.name)
            ? gewaehlt.city
            : [gewaehlt.name, gewaehlt.city].filter(Boolean).join(', ');
        if (c.params.target === 'mine') {
          // Runde 7 (H2): Was hier gewählt wird, IST das Zuhause — einmal gesetzt, gilt es
          // überall (Vorschlagsmitte, Karte, nächstes Meet), statt in diesem einen Entwurf zu
          // versanden. Ohne nachgeschlagene Anschrift wird kein Name erfunden: dann bleibt es
          // beim Ort dieses Meets, und die App sagt den WAHREN Grund (Hausregel 8).
          if (typeof c.repo.setHomeAddress === 'function') {
            const ergebnis = echt
              ? c.repo.setHomeAddress({
                name: gewaehlt.name, city: gewaehlt.city, lat: punkt.lat, lon: punkt.lon, quelle: 'suche',
              })
              : null;
            c.toast(ergebnis?.ok
              ? tx('Zuhause gespeichert')
              : zustandJetzt === 'wartet'
                ? tx('Die Anschrift kam nicht — die Stelle gilt nur für dieses Meet.')
                : tx('Zu dieser Stelle gibt es keine Anschrift — sie gilt nur für dieses Meet.'));
          }
          c.repo.updateDraft(draft.id, {
      ...
      bindActions(root, {
        // EIN Tipp genügt, auch mitten im Nachschlagen: der Knopf wartet die Antwort ab (höchstens
        // PICK_WARTE_MS) und handelt dann nach dem, was WIRKLICH da ist. Vorher hat er in genau
        // dieser Sekunde eine falsche Begründung ausgegeben und das Zuhause nicht gesetzt.
        'pick-confirm': async () => {
          if (ui.pickZustand === 'aus') { uebernehmen(); return; }
          if (ui.pickZustand === 'wartet' && ui.pickLauf) {
            const vorher = knopfEl ? knopfEl.textContent : '';
            if (knopfEl) { knopfEl.disabled = true; knopfEl.textContent = tx('Moment …'); }
            await Promise.race([ui.pickLauf, new Promise((fertig) => window.setTimeout(fertig, PICK_WARTE_MS))]);
            if (!root.isConnected) return;
            if (knopfEl) { knopfEl.disabled = false; knopfEl.textContent = vorher; }
          }
          uebernehmen();
        },
      });
