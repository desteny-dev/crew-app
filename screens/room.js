...
//   • §5: Anstupsen ist ein Impuls (repo.canNudge/nudge → Toast + Cooldown).
...
import {
  ...
  anstupsen, anstupsZustand, besteFreundeMarkeAus,
} from '../ui/components.js';
// Runde 4 (P4): Anstupsen-Ablauf (senden · zurücknehmen · Sperre) und die einstellbare
// Beste-Freunde-Markierung (anstupsen, anstupsZustand, besteFreundeMarkeAus oben) kommen aus
// derselben Quelle wie überall.
...
import { friendMenu, friendMenuActs, markSheetActs, bindMarkSheetInputs, markSwitchSheet, removeFriendSheet as gemeinsamesEntfernenSheet, markSheet as gemeinsamesMarkSheet, blockSheet, reportSheet, blockActs } from './profile.js';
import { fromISODate, now, weekdayShort, toISODate, terminText, jetztTermin, istJetzt } from '../core/dates.js';
...
// Runde 5 (P4, E1): Mitfahren — Chat-Erkennung schreibt in die Fahrten des Meets, das Raum-Panel zeigt sie.
import {
  mitfahrenMoeglich, mitfahrStand, mitfahrenKachel, mitfahrenInhalt, mitfahrtAusChat, mitfahrtSchonErfasst, mitfahrMeldung,
} from '../ui/mitfahren.js';

// --- Bereichseigene SVGs (1:1 aus reference/current/source/07-Raum.dc.html) ---

const meetsButtonSvg = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none"><rect x="3.8" y="5.2" width="16.4" height="15" rx="3" stroke="var(--ink)" stroke-width="1.8"></rect><path d="M3.8 9.8h16.4M8.4 3v3.4M15.6 3v3.4" stroke="var(--ink)" stroke-width="1.8" stroke-linecap="round"></path><circle cx="12" cy="14.8" r="1.6" fill="var(--ink)"></circle></svg>`;
const nudgeIconSvg = `<svg width="24" height="24" viewBox="0 0 24 24" fill="none"><circle cx="9" cy="12" r="2.8" fill="var(--ink)"></circle><path d="M14 8a5.6 5.6 0 0 1 0 8M17.5 5a10.4 10.4 0 0 1 0 14" stroke="var(--ink)" stroke-width="1.9" stroke-linecap="round"></path></svg>`;
// Fable 3: dasselbe Zeichen in currentColor mit benannten Teilen — der Punkt springt, die
// Bögen laufen nach außen (nudgeImpuls). Farbe kommt vom Kreis (.nudge-kreis).
const nudgeIconImpuls = (size) => `<svg class="nudge-icon" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" style="display:block"><circle class="nudge-punkt" cx="9" cy="12" r="2.8" fill="currentColor"></circle><path class="nudge-bogen" d="M14 8a5.6 5.6 0 0 1 0 8" stroke="currentColor" stroke-width="1.9" stroke-linecap="round"></path><path class="nudge-bogen nudge-bogen-2" d="M17.5 5a10.4 10.4 0 0 1 0 14" stroke="currentColor" stroke-width="1.9" stroke-linecap="round"></path></svg>`;

const nudgeIconGreen = `<svg width="17" height="17" viewBox="0 0 24 24" fill="none"><circle cx="9" cy="12" r="2.6" fill="var(--green-dark)"></circle><path d="M14 8.2a5.4 5.4 0 0 1 0 7.6M17.4 5.2a10 10 0 0 1 0 13.6" stroke="var(--green-dark)" stroke-width="1.9" stroke-linecap="round"></path></svg>`;
...
function avatarStack(people, options = {}) {
  ...
  const cell = (inner) => `<span style="position:relative;flex:none;width:${size}px;height:${size}px;margin-left:-${overlap}px;isolation:isolate">${inner}</span>`;
  ...
  const faces = shown.map((person) => cell(gesicht(person))).join('');
  const rest = extra > 0 ? cell(disc('var(--field)', 'var(--muted)', `+${extra}`)) : '';
...
//   Uhrzeit (Meet noch offen)  → „19:30 vorschlagen" — ein Zeitvorschlag am Meet
...
function raumHinweise(ctx, info, panelMeet) {
  ...
  for (const message of texte.slice(-HINWEIS_FENSTER).reverse()) {
    ...
    // Ein laufendes Meet hat seine Zeit — „bin um 19:10 da" ist dann eine Ankunft.
    if (zielPanel && zielPanel.status !== 'active') {
      const zeit = uhrzeitIn(message.text);
      if (zeit) {
        const datum = tagIn(message.text, jetzt)?.datum || zielPanel.date;
        const tag = datum !== zielPanel.date ? meetDateLabel(datum) : '';
        kandidaten.push({ art: 'zeit', ziel: zielPanel, zeit, datum, knopf: tag ? t('{tag} · {zeit} vorschlagen', { tag, zeit }) : t('{zeit} vorschlagen', { zeit }) });
      } else if (plan?.jetzt) {
        // Runde 4 (F2): „wer hat jetzt Zeit?" schlägt „Jetzt" vor — keine Uhrzeit, die in einer
        // Minute Vergangenheit ist.
        kandidaten.push({ art: 'zeit', ziel: zielPanel, jetzt: true, zeit: t('Jetzt'), datum: toISODate(jetzt), knopf: t('Jetzt vorschlagen') });
      }
    }
    ...
    for (const kandidat of kandidaten) {
      ...
      else if (kandidat.art === 'zeit' && kandidat.jetzt
        && (istJetzt(ziel) || (ziel.variants || []).some((variant) => variant.kind === 'time' && istJetzt(variant)))) continue;
      else if (kandidat.art === 'zeit' && !kandidat.jetzt
        && ((ziel.time === kandidat.zeit && ziel.date === kandidat.datum)
          || (ziel.variants || []).some((variant) => variant.kind === 'time' && variant.time === kandidat.zeit
            && (variant.date || ziel.date) === kandidat.datum))) continue;
...
<button data-act="hinweis-weg" ${daten} data-treffer aria-label="${esc(t('Vorschlag ausblenden'))}" title="${esc(t('Vorschlag ausblenden'))}" style="width:30px;height:30px;border:0;background:transparent;display:flex;align-items:center;justify-content:center;cursor:pointer;appearance:none;padding:0;flex:none"><span style="pointer-events:none;display:flex">${crossSmallSvg}</span></button>
  ...
  const quittung = hinweis.art === 'zeit' ? t('Zeit vorgeschlagen') : t('Im Meet vermerkt');
...
function timeAgoLabel(message) {
  const diffMin = Math.max(0, Math.round((now().getTime() - messageDateValue(message).getTime()) / 60000)); // Fable 3 (nudge-11): nie negativ
  if (diffMin < 2) return t('gerade eben');
  if (diffMin < 60) return tn(diffMin, 'vor {n} Minute', 'vor {n} Minuten');
  if (diffMin < 60 * 24) {
    const hours = Math.round(diffMin / 60);
    return tn(hours, 'vor {n} Stunde', 'vor {n} Stunden');
  }
  return message.date ? weekdayShort(message.date) : '';
}
...
function renderRoomView(ctx) {
  ...
  // Bottom-Ebene: über dem Composer steht nur noch der Catch-up bzw. — wenn kein Panel
  // und kein Catch-up da ist — der neutrale Anstupsen-Knopf.
  // v7 spec/08 §4: Der empfangene Impuls ist KEINE schwebende Karte mehr über dem
  // Composer, sondern ein strukturierter Eintrag an seiner Zeitposition in der Timeline.
  const catchUp = catchupArea(ctx);
  let extra = '';
  let extraHeight = 0;
  // v7 spec/04 §3: Anstupsen ist immer genau EINMAL erreichbar — als grosser runder
  // Knopf im freien Raum oder (sonst) als kleiner Knopf neben „Alle Meets" im Kopf.
  const grosserAnstupser = !panelMeet && !catchUp;
  if (catchUp) {
    extra = catchUp;
    extraHeight = ui.catchOpen ? 92 + 44 * (ui.catchUp?.length || 0) : 60;
  } else if (grosserAnstupser) {
    extra = nudgeArea(ctx, params.roomId);
    extraHeight = 116;
  }
  ...
  const overlayTop = panelHtml || hintHtml
    ? `<div data-role="room-panel-layer" style="padding-top:${versatz}px">${panelHtml}${hintHtml}</div>`
    : '';
  ...
  const html = screenScaffold({
    header: `${ROOM_STYLE}${roomHeader(ctx, info, Boolean(activeMeet), { anstupsen: !grosserAnstupser })}`,
    body,
    overlayTop,
    overlayTopInset: overlayTop ? panelLageFuer(ctx, params.roomId).inset(Boolean(panelHtml), Boolean(hintHtml)) : 0,
...
function panelLageFuer(ctx, roomId) {
  ...
  return {
    ...
    inset: (mitPanel, mitHinweis) => {
      if (passt && gemerkt.inset) return gemerkt.inset;
      return (mitPanel ? 196 : 0) + (mitHinweis ? 64 : 0) || 64;
    },
  };
}
...
<span style="font-size:16px;font-weight:650;letter-spacing:-.01em">${esc(title)}</span></div>`,
    body: '',
    scrollKey: 'room-empty',
  });
}

function roomHeader(ctx, info, active, options = {}) {
  ...
  if (info.kind === 'crew') {
    ...
    identity = `<span style="display:flex;align-items:center;gap:6px;flex:none;pointer-events:none">${gruppenKreis(info.crew, 34, 13)}<span style="opacity:.85">${avatarStack(reihe, { size: 21, font: 8, border: 'var(--paper)', max: 3, more: false, settings })}</span></span>`;
...
${options.anstupsen ? kopfAnstupsKnopf(ctx) : ''}
<button data-act="all-meets" aria-label="${esc(t('Alle Meets'))}" style="position:relative;z-index:1;width:34px;height:34px;border-radius:50%;background:var(--surface);border:1.5px solid var(--ink-a10);display:flex;align-items:center;justify-content:center;flex:none;cursor:pointer;padding:0"><span style="pointer-events:none;display:flex">${meetsButtonSvg}</span></button>
</div>`;
}
...
export function gruppenKreis(crew, size = 34, fontSize = 13) {
  ...
  if (bild) {
    return `<span style="width:${size}px;height:${size}px;border-radius:50%;overflow:hidden;flex:none;display:block"><img src="${esc(bild)}" alt="" aria-hidden="true" style="width:100%;height:100%;object-fit:cover;display:block"></span>`;
  }
  return `<span style="width:${size}px;height:${size}px;border-radius:50%;background:linear-gradient(150deg,${base},${shade(base, -0.18)});color:var(--on-accent);display:flex;align-items:center;justify-content:center;flex:none;font:650 ${fontSize}px 'Bricolage Grotesque',sans-serif">${esc(crewInitials(crew?.name))}</span>`;
}
...
function gruppenVorschau(crew, size, fontSize) {
  ...
  const base = crew?.color || 'var(--blue)';
  return `<span style="width:${size}px;height:${size}px;border-radius:50%;background:linear-gradient(150deg,${base},${shade(base, -0.18)});display:flex;align-items:center;justify-content:center;flex:none">${groupIcon('var(--on-accent)', Math.round(size * 0.42))}</span>`;
}
...
<button data-act="${act}" aria-label="${esc(text)}" style="position:relative;display:block;width:84px;height:84px;border:0;background:transparent;padding:0;cursor:pointer;appearance:none;flex:none;border-radius:50%">
<span data-role="gruppenbild-vorschau" style="position:relative;display:block;width:84px;height:84px;border-radius:50%;overflow:hidden;pointer-events:none">${gruppenVorschau(crew, 84, 28)}</span>
...
// v7 spec/04 §3 / A19b: Anstupsen bleibt im Raum mit Panel erreichbar — als kleiner
// runder Knopf links neben „Alle Meets". Im Cooldown ist er ruhig gedimmt statt weg.
// Fable 3 (Entscheidung Jonathan): Der Kopf-Knopf BLEIBT oben rechts — er ändert sich, nicht
// sein Ort. Dieselbe Mechanik wie der große Knopf: Druckzustand, Impuls, Zustand „angestupst".
function kopfAnstupsKnopf(ctx) {
  const z = anstupsZustand(ctx.repo, ctx.params.roomId);
  if (z.self) return '';
  return `<button data-act="nudge" data-role="nudge-knopf" class="nudge-knopf nudge-knopf--klein" data-state="${z.state}" aria-label="${esc(z.label)}" title="${esc(z.label)}" style="position:relative;z-index:1;flex:none;border:0;background:transparent;padding:0;cursor:pointer;appearance:none"><span class="nudge-kreis" style="width:34px;height:34px;pointer-events:none">${nudgeIconImpuls(17)}</span><span data-role="nudge-hinweis" class="nudge-hinweis nudge-hinweis--klein"></span></button>`;
}
...
const punkteKreis = (offen) => `<span data-kreis="1" style="pointer-events:none;width:40px;height:40px;border-radius:50%;background:${offen ? 'var(--field)' : 'var(--surface)'};border:1.5px solid ${offen ? 'var(--ink-a22)' : 'var(--ink-a10)'};box-sizing:border-box;display:flex;align-items:center;justify-content:center">${punkteSvg}</span>`;

function crewDotsButton(open) {
  return `<button data-act="crew-menu" aria-label="${esc(t('Gruppenoptionen'))}" aria-expanded="${open}" style="position:relative;z-index:1;width:44px;height:44px;margin:-5px -2px -5px 0;border:0;background:transparent;display:flex;align-items:center;justify-content:center;flex:none;cursor:pointer;padding:0;appearance:none">${punkteKreis(open)}</button>`;
}

// Runde 4 (C3): „Das Dropdown ‚Gruppe verlassen' ist irgendwie versetzt." Es stand auf einer festen
// Höhe (92 px) — ohne Statuszeile (als App) oder in anderen Größen lag es neben dem Knopf. Jetzt
// wird der sichtbare Kreis gemessen: Das Menü beginnt 6 px darunter, rechtsbündig mit ihm.
function menueAnker(knopf) {
  const lage = imRahmen(knopf?.querySelector?.('[data-kreis]') || knopf);
  return lage ? { top: lage.unten + 6, right: lage.rechts } : null;
}

// Menü und Bestätigungswege einer Gruppe — dieselbe Bauart wie das Personen-Menü.
// Es gibt genau drei Ausgänge, damit nie eine herrenlose Gruppe zurückbleibt:
// verlassen · Adminrolle übergeben und dann verlassen · als letztes Mitglied auflösen.
function crewMenu(lage) {
  const oben = Math.round(lage?.top ?? 92);
  const rechts = Math.max(8, Math.round(lage?.right ?? 20));
  return `<div data-act="crew-menu" style="position:absolute;inset:0;z-index:12"></div>
<div role="menu" data-role="crew-menu" style="position:absolute;right:${rechts}px;top:${oben}px;background:var(--surface);border-radius:16px;box-shadow:0 8px 24px var(--shadow-16);border:1px solid var(--ink-a07);overflow:hidden;width:250px;max-width:calc(100% - ${rechts + 12}px);box-sizing:border-box;z-index:13;transform-origin:top right;animation:raumMenueRein .16s ease-out">
<button data-act="crew-leave" role="menuitem" style="display:flex;align-items:center;gap:10px;min-height:48px;padding:0 16px;border:0;background:transparent;width:100%;text-align:left;cursor:pointer;appearance:none;font-family:'Instrument Sans',sans-serif">
<span style="pointer-events:none;display:flex;width:15px;justify-content:center">${leaveGlyph}</span>
<span style="pointer-events:none;font-size:13.5px;font-weight:650;color:var(--danger);flex:1">${t('Gruppe verlassen')}</span>
</button>
</div>`;
}

const leaveGlyph = `<svg width="15" height="15" viewBox="0 0 24 24" fill="none" style="flex:none"><path d="M14 4.5H6.5A1.5 1.5 0 0 0 5 6v12a1.5 1.5 0 0 0 1.5 1.5H14" stroke="var(--danger)" stroke-width="1.8" stroke-linecap="round"></path><path d="M16 8.5 19.5 12 16 15.5M19 12H10" stroke="var(--danger)" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"></path></svg>`;

// Bestätigung in drei Stufen (ui.crewLeave.step): 'confirm' | 'admin' | 'aufloesen'.
function crewLeaveSheet(ctx, crew) {
  const state = ctx.ui.crewLeave;
  ...
  let actions;
  if (state.step === 'admin') {
    title = t('Erst die Gruppe übergeben');
    text = t('Du bist die einzige Person, die {name} verwaltet. Wähle, wer das übernimmt — danach verlässt du die Gruppe.', { name: esc(crew.name) });
    body = `<div style="display:flex;flex-direction:column;max-height:230px;overflow-y:auto">${others.map((person, index) => `<button data-act="crew-admin-pick" data-person="${esc(person.id)}" style="display:flex;align-items:center;gap:11px;padding:10px 0;border:0;${index ? 'border-top:1px solid var(--ink-a05);' : ''}background:transparent;cursor:pointer;appearance:none;font-family:'Instrument Sans',sans-serif;text-align:left;width:100%">
...
<span style="pointer-events:none;display:flex">${rowChevron}</span>
    ...
    actions = `<button data-act="crew-leave-cancel" style="flex:1;border:1.5px solid var(--ink-a14);background:var(--surface);color:var(--ink-soft);font:650 14px/1 'Instrument Sans',sans-serif;padding:13px 0;border-radius:999px;cursor:pointer;appearance:none"><span style="pointer-events:none">${t('Abbrechen')}</span></button>`;
  } else if (state.step === 'aufloesen') {
    title = t('Gruppe auflösen?');
    text = t('Du bist das letzte Mitglied von {name}. Die Gruppe und ihr Chat verschwinden; vergangene eigene Meets bleiben in deinem Verlauf.', { name: esc(crew.name) });
    actions = `<button data-act="crew-leave-cancel" style="flex:1;border:1.5px solid var(--ink-a14);background:var(--surface);color:var(--ink-soft);font:650 14px/1 'Instrument Sans',sans-serif;padding:13px 0;border-radius:999px;cursor:pointer;appearance:none"><span style="pointer-events:none">${t('Abbrechen')}</span></button>
<button data-act="crew-dissolve-confirm" style="flex:1;background:var(--danger);border:0;color:var(--on-accent);font:650 14px/1 'Instrument Sans',sans-serif;padding:14px 0;border-radius:999px;cursor:pointer;appearance:none"><span style="pointer-events:none">${t('Auflösen')}</span></button>`;
  } else {
    title = t('{name} verlassen?', { name: crew.name });
    text = t('Du siehst danach weder den Chat noch neue Meets dieser Gruppe. Vergangene eigene Meets bleiben in deinem Verlauf.');
    actions = `<button data-act="crew-leave-cancel" style="flex:1;border:1.5px solid var(--ink-a14);background:var(--surface);color:var(--ink-soft);font:650 14px/1 'Instrument Sans',sans-serif;padding:13px 0;border-radius:999px;cursor:pointer;appearance:none"><span style="pointer-events:none">${t('Abbrechen')}</span></button>
<button data-act="crew-leave-confirm" style="flex:1;background:var(--danger);border:0;color:var(--on-accent);font:650 14px/1 'Instrument Sans',sans-serif;padding:14px 0;border-radius:999px;cursor:pointer;appearance:none"><span style="pointer-events:none">${t('Verlassen')}</span></button>`;
  }
...
<span style="font-family:'Bricolage Grotesque',sans-serif;font-size:19px;font-weight:650">${esc(title)}</span>
...
// Gemeinsame Handler für Raumkopf UND Crew-Details — eine Logik, zwei Bedienstellen.
function crewLeaveActions(ctx, crew) {
  ...
  const verlassen = () => {
    ...
    if (result?.ok) {
      ui.crewLeave = null;
      ui.crewMenu = false;
      ctx.toast(t('{name} verlassen', { name: crew.name }));
      nav.resetTo('crew.home');
      return;
    }
    if (result?.reason === 'adminUebergabe') { ui.crewLeave = { step: 'admin' }; ctx.render(); return; }
    if (result?.reason === 'aufloesen') { ui.crewLeave = { step: 'aufloesen' }; ctx.render(); return; }
    ui.crewLeave = null;
    ctx.render();
    ctx.toast(result?.reason || t('Das hat nicht geklappt'));
  };
  return {
    'crew-menu': (data, el) => {
      if (!ui.crewMenu && el?.tagName === 'BUTTON') ui.crewMenuLage = menueAnker(el);
      ui.crewMenu = !ui.crewMenu;
      ctx.render();
    },
    'crew-leave': () => { ui.crewMenu = false; ui.crewLeave = { step: 'confirm' }; ctx.render(); },
    'crew-leave-cancel': () => { ui.crewLeave = null; ctx.render(); },
    'crew-leave-confirm': verlassen,
    // Übergabe und Austritt sind EIN Vorgang: wer übergibt, wollte gehen.
    'crew-admin-pick': (data) => {
      const result = repo.transferCrewAdmin(crew.id, data.person);
      if (!result?.ok) { ctx.toast(result?.reason || t('Das hat nicht geklappt')); return; }
      verlassen();
    },
    'crew-dissolve-confirm': () => {
      ...
      ui.crewLeave = null;
      ui.crewMenu = false;
      if (!result?.ok) { ctx.render(); ctx.toast(result?.reason || t('Das hat nicht geklappt')); return; }
      ctx.toast(t('{name} aufgelöst', { name: crew.name }));
      nav.resetTo('crew.home');
    },
  };
}

// --- Anstupsen-Button (07.1): kein relevantes Meet → runder neutraler Button mittig ---
// v3.1 §5: ein Impuls, kein Objekt. Nach dem Senden bleibt nur der Cooldown sichtbar
// (ruhig gedimmt) — kein „Angestupst"-Zustand, kein Abbrechen-Ablauf.

// v3.2 B5: Der Impuls hinterlässt beim Sender KEIN Objekt im Chat. Sichtbar bleiben nur
// die kurze Toast-Rückmeldung (im Handler) und dieser ruhige Zustand: frisch gesendet
// steht kurz „Angestupst", danach dimmt der Cooldown den Knopf, bis er wieder frei ist.
// v4 P0-5: Der Träger ist durchsichtig und lässt Eingaben durch — deckend und bedienbar
// ist nur der runde Knopf selbst.
// Fable 3 (J1, nudge-01…05, nudge-09): EIN Zustand am Ort der Handlung. Kein Toast, keine
// Opacity 0,5, kein aria-disabled: Der Knopf morpht zu „Angestupst · vor 3 min" und zählt
// die Zeit selbst (30-s-Takt, nudgeTick). Ein Tipp im Cooldown wackelt und nennt die Restzeit.
function nudgeArea(ctx, roomId) {
  const z = anstupsZustand(ctx.repo, roomId);
  if (z.self) return '';
  const kontext = nudgeKontext(ctx, roomId);
  return bottomBar(`<div style="display:flex;justify-content:center;padding:6px 0 10px">
<button data-act="nudge" data-role="nudge-knopf" class="nudge-knopf" data-state="${z.state}" style="pointer-events:auto;border:0;background:transparent;padding:0;cursor:pointer;display:flex;flex-direction:column;align-items:center;gap:9px;font-family:'Instrument Sans',sans-serif">
<span class="nudge-kreis" style="width:64px;height:64px;box-shadow:0 2px 8px var(--shadow-07);pointer-events:none">${nudgeIconImpuls(24)}</span>
<span data-role="nudge-label" class="nudge-label" style="font-size:13px;font-weight:650;color:var(--ink-soft);pointer-events:none;background:var(--paper-a92);border-radius:8px;padding:1px 8px">${esc(z.label)}</span>
<span data-role="nudge-hinweis" class="nudge-hinweis" style="pointer-events:none"></span>
${kontext ? `<span style="font-size:11.5px;color:var(--muted);pointer-events:none">${esc(kontext)}</span>` : ''}
</button></div>`, 'pointer-events:none');
}

// Zustand beider Knöpfe: Runde 4 (E3, P4) aus anstupsZustand (ui/components.js) — idle · sent ·
// zurueck („Zurückgenommen · wieder in 5 min"). Die frühere eigene Kopie ist entfallen.
const restLabel = (ms) => { const sek = Math.max(0, Math.ceil(ms / 1000)); return `${Math.floor(sek / 60)}:${String(sek % 60).padStart(2, '0')}`; };

// nudge-09: Man weiß, was man auslöst — Zustand der Person unter dem Knopf.
function nudgeKontext(ctx, roomId) {
  const id = String(roomId || '').replace(/^r:/, '');
  if (id.startsWith('c-')) {
    const crew = ctx.repo.getCrew?.(id);
    const n = crew ? Math.max(0, (crew.memberIds || []).length - 1) : 0;
    return n ? tn(n, 'an {n} Person · Wer hat Lust?', 'an {n} Personen · Wer hat Lust?') : '';
  }
  const person = ctx.repo.getPerson(id);
  if (!person) return '';
  if (person.activeMeetId) { const meet = ctx.repo.getMeet(person.activeMeetId); return meet ? t('{name}: im Meet · {titel}', { name: person.name, titel: meet.title }) : t('{name}: im Meet', { name: person.name }); }
  if (person.free?.active) return t('{name} ist gerade frei', { name: person.name });
  if (person.free?.from) return t('{name}: frei ab {zeit}', { name: person.name, zeit: person.free.from });
  return '';
}

// Impuls am Knoten (Web Animations, kein render): Kreis färbt sich 600 ms grün, der Punkt
// springt und federt zurück, die Bögen laufen einmal nach außen. Danach Zustand „sent".
function nudgeImpuls(el) {
  if (!el || el.dataset.impuls) return;
  el.dataset.impuls = '1';
  el.dataset.state = 'sent';
  const kreis = el.querySelector('.nudge-kreis');
  const punkt = el.querySelector('.nudge-punkt');
  const boegen = el.querySelectorAll('.nudge-bogen');
  try {
    kreis?.animate([{ background: 'var(--green)', color: 'var(--on-accent)', borderColor: 'var(--green)' }, { background: 'var(--green)', color: 'var(--on-accent)', borderColor: 'var(--green)', offset: 0.6 }, { background: 'var(--green-tint)', color: 'var(--green-dark)', borderColor: 'var(--green-a28)' }], { duration: 600, easing: 'ease-out' });
    punkt?.animate([{ transform: 'translateX(0)' }, { transform: 'translateX(6px)', offset: 0.35 }, { transform: 'translateX(-2px)', offset: 0.7 }, { transform: 'translateX(0)' }], { duration: 380, easing: 'cubic-bezier(.34,1.56,.64,1)' });
    boegen.forEach((bogen, index) => bogen.animate([{ transform: 'scale(1)', opacity: 1 }, { transform: 'scale(1.35)', opacity: 0 }, { transform: 'scale(1)', opacity: 1 }], { duration: 420, delay: 60 * index, easing: 'ease-out' }));
  } catch { /* ältere Engines ohne WAAPI: Zustand reicht */ }
}

// Tipp im Cooldown (nudge-04): Label wackelt 2× 4 px, darunter 2 s die Restzeit.
function nudgeAbgelehnt(el, gate) {
  if (!el) return;
  const ziel = el.querySelector('[data-role="nudge-label"]') || el.querySelector('.nudge-kreis');
  try { ziel?.animate([{ transform: 'translateX(0)' }, { transform: 'translateX(-4px)' }, { transform: 'translateX(4px)' }, { transform: 'translateX(-4px)' }, { transform: 'translateX(0)' }], { duration: 240 }); } catch { /* egal */ }
  const hinweis = el.querySelector('[data-role="nudge-hinweis"]');
  if (!hinweis) return;
  hinweis.textContent = gate.self ? t('Das bist du selbst') : t('Wieder möglich in {zeit}', { zeit: restLabel(gate.remainingMs) });
  hinweis.dataset.on = '1';
  window.clearTimeout(hinweis.__t);
  hinweis.__t = window.setTimeout(() => { hinweis.textContent = ''; delete hinweis.dataset.on; }, 2000);
}

// 30-s-Takt: Label „vor N min" und Rückkehr zu „Anstupsen" nach dem Cooldown — ohne render.
let nudgeTicker = 0;
function nudgeTick(root, ctx, roomId) {
  const knopf = root.querySelector('[data-role="nudge-knopf"]');
  if (!knopf || !knopf.isConnected) return;
  const z = anstupsZustand(ctx.repo, roomId);
  knopf.dataset.state = z.state;
  if (z.state === 'idle') delete knopf.dataset.impuls;
  knopf.setAttribute('title', z.label);
  const label = knopf.querySelector('[data-role="nudge-label"]');
  if (label) label.textContent = z.label; else knopf.setAttribute('aria-label', z.label);
}

// --- Anstupsen als strukturierter Timeline-Eintrag (v7 spec/08 §4) ---
//
// Der Eintrag steht an seiner CHRONOLOGISCHEN Stelle in derselben Timeline wie
// Nachrichten und Standortkarten — nicht als schwebende Karte über dem Composer und
// niemals als Textblase im Namen der anstupsenden Person. Empfänger sehen genau zwei
// Aktionen (Frei · + Meet), der Absender denselben Ereignistyp neutral als „gesendet".
const ANSTUPS_FRIST_MS = 12 * 60 * 60 * 1000;

function nudgeAbgelaufen(message) {
  return now().getTime() - messageDateValue(message).getTime() > ANSTUPS_FRIST_MS;
}

// Gemeinsame Hülle aller strukturierten Ereignisse: zentrierte, vollbreite und bewusst
// neutrale Funktionskarte — erkennbar KEINE Blasengeometrie (16 16 16 5).
function ereignisKarte(inhalt, options = {}) {
  const rand = options.rahmen || 'var(--ink-a10)';
  return `<div data-role="${options.rolle || 'raum-ereignis'}" style="align-self:center;width:100%;max-width:330px;background:var(--surface);border:1px solid ${rand};border-radius:18px;padding:${options.schmal ? '9px 13px' : '13px 14px'};display:flex;flex-direction:column;gap:10px;box-shadow:0 3px 10px var(--shadow-05)">${inhalt}</div>`;
}

function nudgeEintrag(ctx, message) {
  const eigen = message.authorId === ME;
  const author = ctx.repo.getPerson(message.authorId);
  const zeit = `<span style="font-size:11px;color:var(--muted-light);flex:none">${esc(timeAgoLabel(message))}</span>`;
  const zeichen = `<span style="width:30px;height:30px;border-radius:50%;background:var(--green-tint);display:flex;align-items:center;justify-content:center;flex:none">${nudgeIconGreen}</span>`;

  if (eigen) {
    const gegenueber = (() => { const id = String(ctx.params?.roomId || '').replace(/^r:/, ''); return id.startsWith('p-') ? (ctx.repo.getPerson(id)?.name || '') : ''; })();
    // Absenderzustand: derselbe Ereignistyp, neutral. Keine ausgehende Blase, kein Text.
    return ereignisKarte(`<div style="display:flex;align-items:center;gap:10px">
${zeichen}
<span style="font-size:13px;font-weight:650;color:var(--ink-soft);flex:1;min-width:0">${message.zurueck ? t('Du hast zurückgestupst') : (gegenueber ? t('Du hast {name} angestupst', { name: esc(gegenueber) }) : t('Du hast angestupst'))}</span>
${zeit}
</div>`, { schmal: true, rolle: 'nudge-gesendet' });
  }

  const name = author?.name || t('Jemand');
  const kopf = `<div style="display:flex;align-items:center;gap:10px">
<span style="display:flex;flex:none;pointer-events:none">${personAvatar(author || { color: 'var(--field)', initials: '' }, { size: 30, fontSize: 11, marker: author ? personMarker(author.id, ctx.repo.getSettings()) : null, dotBorder: 'var(--surface)' })}</span>
<span style="display:flex;flex-direction:column;gap:1px;flex:1;min-width:0">
<span style="font-size:13.5px;font-weight:650;letter-spacing:-.01em;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${t('{name} hat dich angestupst · Lust auf etwas?', { name: esc(name) })}</span>
<span style="font-size:11px;color:var(--muted-light)">${esc(timeAgoLabel(message))}</span>
</span>
</div>`;

  if (message.response) {
    // Ruhiger erledigter Zustand — die Aktion ist gelaufen, der Verlauf bleibt lesbar.
    const wort = message.response === 'frei' ? t('Du hast dich frei gemeldet') : message.response === 'zurueck' ? t('Du hast zurückgestupst') : t('Du hast ein Meet gestartet');
    return ereignisKarte(`<div style="display:flex;align-items:center;gap:10px">
${zeichen}
<span style="font-size:12.5px;font-weight:600;color:var(--ink-soft);flex:1;min-width:0">${t('{name} angestupst · {wort}', { name: esc(name), wort: esc(wort) })}</span>
${zeit}
</div>`, { schmal: true, rolle: 'nudge-erledigt' });
  }

  if (nudgeAbgelaufen(message)) {
    // spec/08 §4: Unbeantwortete Anstupser dürfen nach 12 Stunden einklappen, damit die
    // Timeline nicht dauerhaft überfrachtet.
    return ereignisKarte(`<div style="display:flex;align-items:center;gap:10px">
${zeichen}
<span style="font-size:12.5px;font-weight:600;color:var(--muted);flex:1;min-width:0">${t('{name} hatte dich angestupst', { name: esc(name) })}</span>
${zeit}
</div>`, { schmal: true, rolle: 'nudge-abgelaufen' });
  }

  return ereignisKarte(`${kopf}
<div style="display:flex;gap:8px">
<button data-act="nudge-frei" data-msg="${esc(message.id)}" style="flex:1;background:var(--green);border:0;color:var(--on-accent);font:650 13.5px/1 'Instrument Sans',sans-serif;padding:12px 0;border-radius:999px;text-align:center;cursor:pointer;appearance:none"><span style="pointer-events:none">FREE</span></button>
<button data-act="nudge-meet" data-msg="${esc(message.id)}" style="flex:1;border:1.5px solid var(--ink-a14);background:transparent;color:var(--ink);font:650 13.5px/1 'Instrument Sans',sans-serif;padding:11px 0;border-radius:999px;text-align:center;cursor:pointer;appearance:none"><span style="pointer-events:none">+ Meet</span></button>
<button data-act="nudge-zurueck" data-msg="${esc(message.id)}" class="nudge-knopf nudge-knopf--klein" aria-label="${esc(t('Zurückstupsen'))}" title="${esc(t('Zurückstupsen'))}" style="border:0;background:transparent;padding:0;cursor:pointer;appearance:none;flex:none;display:flex"><span class="nudge-kreis" style="width:40px;height:40px;pointer-events:none">${nudgeIconImpuls(18)}</span></button>
</div>`, { rolle: 'nudge-empfangen' });
}
...
<span style="font-size:10.5px;color:var(--muted-light);flex:none;align-self:flex-end;font-variant-numeric:tabular-nums">${esc(message.time || '')}</span>
  ...
  const author = ctx.repo.getPerson(message.authorId);
  const avatar = author
    ? `<span style="display:flex;flex:none;pointer-events:none">${personAvatar(author, { size: 26, fontSize: 10, marker: personMarker(author.id, ctx.repo.getSettings()) })}</span>`
    : '';
...
// Runde 5 (P4 → P2, E1): Mitfahren im Raum-Panel — die Leiste steht direkt über „Liste | Umfragen",
// die Handler kommen über bindMeetPanels. Gemessen wuchs das Panel aufgeklappt von 176 auf 575 px und
// lag bei 360 × 740 über Zusammenfassung und Eingabezeile. Der offene Inhalt hat deshalb eine eigene
// Scrollfläche, deren Höhe am echten Baum gemessen wird (mitfahrFlaecheMessen): Zusammenfassung und
// Eingabe bleiben frei, und unter dem Panel bleibt immer ein Stück Verlauf zu sehen. Der gemessene Wert
// steht beim nächsten Aufbau schon im Markup — nichts springt.
const MITFAHR_CHAT_LUFT = 96;
const MITFAHR_MIN_HOEHE = 132;
const MITFAHR_START_HOEHE = 220;

function raumMitfahren(ctx, info, meet) {
  if (!mitfahrenMoeglich(meet)) return '';
  const stand = mitfahrStand(ctx.repo, meet);
  const offen = ctx.ui.mitfahren === meet.id;
  const kachel = mitfahrenKachel(ctx, meet, { kompakt: true, offen, stand });
  if (!offen) return kachel;
  const gemerkt = ctx.ui.mitfahrHoehe;
  const hoehe = gemerkt && gemerkt.roomId === info.room.id ? gemerkt.hoehe : MITFAHR_START_HOEHE;
  return `${kachel}<div data-role="raum-mitfahren-flaeche" data-scroll-keep="raum-mitfahren-${esc(meet.id)}" style="max-height:${hoehe}px;overflow-y:auto;overscroll-behavior:contain;border-radius:12px;flex:none;scrollbar-width:none">${mitfahrenInhalt(ctx, meet, { kompakt: true, stand })}</div>`;
}

// Liefert true, wenn sich die Höhe geändert hat (dann muss die Panel-Ebene neu vermessen werden).
function mitfahrFlaecheMessen(root, ctx, roomId) {
  const flaeche = root.querySelector('[data-role="raum-mitfahren-flaeche"]');
  if (!flaeche) return false;
  const karte = flaeche.closest('[data-role="raum-panel-karte"]');
  const rahmen = flaeche.closest('.runtime-phone');
  const skala = rahmen?.offsetWidth ? (rahmen.getBoundingClientRect().width / rahmen.offsetWidth) || 1 : 1;
  const grenzen = ['#room-extra', '#room-composer']
    .map((wahl) => root.querySelector(wahl))
    .filter((el) => el && el.getBoundingClientRect().height > 0)
    .map((el) => el.getBoundingClientRect().top);
  const unten = grenzen.length ? Math.min(...grenzen) : (rahmen ? rahmen.getBoundingClientRect().bottom - 80 * skala : window.innerHeight - 80);
  const f = flaeche.getBoundingClientRect();
  const nachFlaeche = karte ? Math.max(0, karte.getBoundingClientRect().bottom - f.bottom) : 0;
  const hoehe = Math.max(MITFAHR_MIN_HOEHE, Math.floor((unten - f.top - nachFlaeche) / skala - MITFAHR_CHAT_LUFT));
  const vorher = Math.round(parseFloat(flaeche.style.maxHeight) || 0);
  if (vorher !== hoehe) flaeche.style.maxHeight = `${hoehe}px`;
  ctx.ui.mitfahrHoehe = { roomId, hoehe };
  // Unten weich ausblenden, solange darunter noch etwas kommt.
  const mehr = () => { flaeche.dataset.mehr = flaeche.scrollHeight - flaeche.clientHeight - flaeche.scrollTop > 4 ? '1' : '0'; };
  mehr();
  if (!flaeche.__mehrGebunden) {
    flaeche.__mehrGebunden = true;
    flaeche.addEventListener('scroll', mehr, { passive: true });
  }
  return vorher !== hoehe;
}
...
function chatBody(ctx, info, panelMeet) {
  ...
  const messages = info.room.messages.map((message) => {
    ...
    if (message.kind === 'nudge') return nudgeEintrag(ctx, message);
...
function bindRoomView(root, ctx) {
  ...
  const panelMeet = panelMeetFor(ctx, info);
  const openNudges = info.room.messages.filter(
    (message) => message.kind === 'nudge' && message.authorId !== ME && !message.response,
  );
  // Der Eintrag in der Timeline nennt seine Nachricht ausdrücklich; ohne data-msg (alte
  // Aufrufe) gilt der jüngste offene Impuls.
  const nudgeZiel = (data) => (data?.msg
    ? openNudges.filter((message) => message.id === data.msg)
    : openNudges.slice(-1));
  ...
  bindActions(root, {
    ...
    'all-meets': () => nav.go('room.allMeets', { roomId: params.roomId }),
    ...
    // §5/B5: Anstupsen bleibt ein Impuls mit Cooldown und kurzer Rückmeldung — und
    // hinterlässt weiterhin KEINE Chatnachricht und keinen erfundenen Text.
    // v7 spec/08 §4: Der Absender sieht denselben Ereignistyp in der Timeline neutral als
    // „Anstupsen gesendet". Deshalb entsteht jetzt ein echter strukturierter Datensatz.
    // Fable 3: Rückmeldung am Ort der Handlung. Kein Toast; der Datensatz entsteht in
    // repo.nudge() (der Raum zeichnet ihn als Ereigniskarte), der Impuls läuft am Knopf.
    // Runde 4 (E3, P4): senden → zweiter Tipp nimmt zurück („Zurückgenommen · wieder in 5 min") →
    // ein Tipp in der Sperre wird abgelehnt. Daten und Vibration macht anstupsen(); hier nur das Bild.
    nudge: (data, el) => {
      const knopf = el?.closest?.('[data-role="nudge-knopf"]') || el;
      anstupsen(ctx, params.roomId, {
        gesendet: () => {
          nudgeImpuls(knopf);
          window.setTimeout(() => nudgeImpuls(root.querySelector('[data-role="nudge-knopf"]')), 40);
        },
        zurueckgenommen: () => nudgeTick(root, ctx, params.roomId),
        abgelehnt: (gate) => nudgeAbgelehnt(knopf, gate),
      });
    },
    'nudge-zurueck': (data, el) => {
      rueckmeldung('tipp');
      nudgeImpuls(el?.closest?.('[data-act="nudge-zurueck"]') || el);
      nudgeZiel(data).forEach((message) => repo.respondNudge(params.roomId, message.id, 'zurueck'));
      window.setTimeout(() => nudgeImpuls(root.querySelector('[data-role="nudge-knopf"]')), 40);
    },
    'nudge-frei': (data) => {
      nudgeZiel(data).forEach((message) => repo.respondNudge(params.roomId, message.id, 'frei'));
      ctx.toast(t('Du bist jetzt frei'));
    },
    'nudge-meet': (data) => {
      nudgeZiel(data).forEach((message) => repo.respondNudge(params.roomId, message.id, 'meet'));
      startDraft();
    },
    ...
    'hinweis-los': (data) => {
      ...
      if (hinweis.art === 'mitfahrt') {
        ...
        const { zeit, datum, jetzt } = hinweis;
        ausfuehren = () => {
          const ergebnis = repo.addVariant(meetId, jetzt ? { kind: 'time', ...jetztTermin() } : { kind: 'time', date: datum, time: zeit });
          if (ergebnis?.ok) return true;
  ...
  // Fable 3: 30-s-Takt für „Angestupst · vor N min" und die Rückkehr zu „Anstupsen".
  window.clearInterval(nudgeTicker);
  nudgeTicker = window.setInterval(() => nudgeTick(root, ctx, params.roomId), 30000);
...
<span style="font-size:13px;font-weight:${index === 0 ? 650 : 600};${index === 0 ? '' : 'color:var(--ink-soft);'}pointer-events:none;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(person.name + suffix)}</span>
  ...
  if (scroll && panelEbene && kopfEbene) {
    panelLageMessen();
    // Runde 5 (E1): Höhe des offenen Mitfahren-Bereichs; ändert sie sich, die Panel-Ebene nachmessen.
    if (mitfahrFlaecheMessen(root, ctx, params.roomId)) panelLageMessen();
...
function renderAllMeets(ctx) {
  const { params } = ctx;
  const info = resolveRoom(ctx, params.roomId);
  if (!info) return { html: emptyScreen(t('Alle Meets')), bind() {} };
...
<button data-act="back" style="border:0;background:transparent;padding:0;display:flex;cursor:pointer"><span style="pointer-events:none;display:flex">${backArrow('var(--ink)', 22)}</span></button>
<div style="display:flex;flex-direction:column;min-width:0"><span style="font-family:'Bricolage Grotesque',sans-serif;font-size:22px;font-weight:650">${t('Alle Meets')}</span><span style="font-size:11.5px;color:var(--muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(info.title)}</span></div>
</div>`;
  ...
  const html = screenScaffold({
    ...
    body: parts.body,
    bottom: parts.view === 'map' ? '' : parts.cta,
...
function profileBox(title, rowsHtml, headerRight = '') {
  return `<div style="background:var(--surface);border-radius:20px;border:1px solid var(--ink-a07);padding:6px 16px;flex:none">
...
// =====================================================================================
// room.crewDetails — 07.5 (Interessen mit Icons + Punkten, Ressourcen im Profil-Stil)
// =====================================================================================
...
function meetHistoryRow(ctx, meet, index) {
  const settings = ctx.repo.getSettings();
  return `<div data-act="dt-meet" data-meet="${esc(meet.id)}" data-treffer style="display:flex;justify-content:space-between;align-items:center;padding:9px 0;${index ? 'border-top:1px solid var(--ink-a05);' : ''}cursor:pointer">
<div style="display:flex;flex-direction:column;gap:1px;min-width:0;pointer-events:none"><span style="font-size:14px;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(meet.title)}</span><span style="font-size:11.5px;color:var(--muted)">${esc(meetDateLabel(meet.date))}</span></div>
...
function renderCrewDetails(ctx) {
  ...
  if (!crew) return { html: emptyScreen(''), bind() {} };
  const settings = repo.getSettings();
  const roomId = roomIdForCrew(crew.id);
  const others = crew.memberIds.filter((id) => id !== ME).map((id) => repo.getPerson(id)).filter(Boolean);

  // Runde 4 (E6): „Dass man in den Gruppen wirklich sieht, dass sie frei sind, und es nicht so ein
  // grauer Text ist." Wer gerade frei ist, steht oben unter einer grünen Überschrift (und die Zahl
  // steht grün schon an der zugeklappten Zeile). Wer laut seinen festen Zeiten sicher keine Zeit
  // hat, steht ruhig am Ende — mit „Keine Zeit bis …", nie mit dem Grund. Frei gewinnt immer.
  const frei = others.filter(istGeradeFrei);
  const keineZeit = (person) => (!istGeradeFrei(person) && !person.free?.from ? belegtHinweis(person.belegt) : '');
  const rest = others.filter((person) => !istGeradeFrei(person))
  ...
  const gruppenTitel = (text, farbe, punkt) => `<div data-role="${punkt ? 'gerade-frei' : 'mitglieder-rest'}" style="display:flex;align-items:center;gap:6px;padding:12px 0 2px;font-size:11px;font-weight:650;letter-spacing:.09em;text-transform:uppercase;color:${farbe}">${punkt ? '<span style="width:7px;height:7px;border-radius:50%;background:var(--green);flex:none"></span>' : ''}${esc(text)}</div>`;
  const membersRows = ui.members
    ? `<div style="padding:0 16px 8px;border-top:1px solid var(--ink-a05)">
${personRow(repo.getMe(), settings, { status: t('Du'), action: 'cd-me' })}
${frei.length ? `${gruppenTitel(t('Gerade frei · {n}', { n: frei.length }), 'var(--green-dark)', true)}${frei.map((person) => personRow(person, settings, { action: 'cd-person', statusArt: 'frei', status: person.status ? t('Frei · {status}', { status: person.status }) : t('Gerade frei') })).join('')}` : ''}
${frei.length && rest.length ? gruppenTitel(t('Alle anderen'), 'var(--muted-light)', false) : ''}
${rest.map(({ person, belegt }) => personRow(person, settings, { action: 'cd-person', ...(belegt ? { status: belegt, statusArt: 'belegt' } : {}) })).join('')}
${nurAdminsHinweis(ctx, crew)}
</div>`
    : '';

  // Interessen: Icon + Punkte (wie oft gemeinsam — abgeleitet aus geteilten Interessen).
  const interests = crewSharedInterests(ctx, crew);
  const interestRows = interests.map((entry) => `<div style="display:flex;align-items:center;gap:10px;padding:10px 0;border-top:1px solid var(--ink-a05)">
...
${interestDots(Math.min(3, entry.people.length - 1), 'var(--green)')}
</div>`).join('');

  const resources = crewResourceEntries(ctx, crew);
  const resourceRows = resources.map((entry, index) => resourceRow(ctx, entry, index)).join('');

  const upcoming = repo.getMeets({ context: { crewId: crew.id }, direction: 'upcoming' });
  const activeMeet = upcoming.find((meet) => meet.status === 'active');
  const history = repo.getMeets({ context: { crewId: crew.id }, direction: 'history' }).slice(0, 3);
  const historyRows = history.map((meet, index) => meetHistoryRow(ctx, meet, activeMeet || index > 0)).join('');
  const meetsRows = `${activeMeet ? activeMeetRow(ctx, activeMeet) : ''}${historyRows}`
    || `<div style="padding:9px 0;font-size:12.5px;color:var(--muted-light)">${t('Noch nichts gemeinsam erlebt')}</div>`;

  // Eine Scrollfläche (§1): Identität, Mitglieder, Interessen, Ressourcen und
  // „Gemeinsam erlebt" scrollen zusammen; nur die Zurück-Zeile bleibt stehen.
  const html = screenScaffold({
...
<span style="flex:1"></span>${crewDotsButton(Boolean(ui.crewMenu))}</div>`,
    body: `
...
<button data-act="cd-bild-gross" aria-label="${esc(t('Gruppenbild groß ansehen'))}" style="display:block;width:92px;height:92px;border:0;background:transparent;padding:0;cursor:pointer;appearance:none;border-radius:50%"><span style="display:block;pointer-events:none">${gruppenKreis(crew, 92, 26)}</span></button>
...
<div style="background:var(--surface);border-radius:20px;border:1px solid var(--ink-a07);overflow:hidden">
<div data-act="cd-members" style="padding:13px 16px;display:flex;align-items:center;gap:12px;cursor:pointer">
${avatarStack([repo.getMe(), ...others], { size: 30, font: 11, max: 3, settings })}
<span style="font-size:14.5px;font-weight:650;flex:1;min-width:0;pointer-events:none;display:flex;align-items:center;flex-wrap:wrap;gap:4px 8px">${tn(crew.memberIds.length, '{n} Mitglied', '{n} Mitglieder')}${frei.length ? freiAnzahlChip(frei.length) : ''}</span>
<span style="color:var(--line-strong);pointer-events:none">${ui.members ? '⌄' : '›'}</span>
</div>
${darfHinzufuegen(crew) ? personenHinzuZeile() : ''}
${membersRows}
</div>
${interestRows ? profileBox(t('Gemeinsame Interessen'), interestRows, `<span style="font-size:10.5px;color:var(--line-solid)">${t('wie oft gemeinsam')}</span>`) : ''}
...
${profileBox(t('Gemeinsam erlebt'), meetsRows, `<button data-act="cd-all" style="border:0;background:transparent;padding:0;cursor:pointer;font-family:'Instrument Sans',sans-serif"><span style="pointer-events:none;font-size:12px;font-weight:650;color:var(--green-dark)">${t('Alle ›')}</span></button>`)}
</div>`,
    bottomInset: 26,
    ...
    overlays: `${ui.editSheet && !ui.gruppenPb && !ui.personenWahl ? crewEditSheet(ctx, crew) : ''}${gruppenBaukastenSheet(ctx)}${personenWahlSheet(ctx, crew)}${ui.crewMenu ? crewMenu(ui.crewMenuLage) : ''}${crewLeaveSheet(ctx, crew)}${ctx.freundHinzufuegen?.overlay(ctx) || ''}${discardSheet(ctx)}${bildGrossHtml(ctx)}`,
  ...
  return {
    ...
    bind(root) {
      bindActions(root, {
        ...
        'cd-members': () => { ui.members = !ui.members; ctx.render(); },
...
function renderPersonDetails(ctx) {
  ...
  if (!person) return { html: emptyScreen(''), bind() {} };
  const settings = repo.getSettings();
  ...
  const crewRows = crews.map((crew, index) => {
    const others = crew.memberIds.filter((id) => id !== ME).map((id) => repo.getPerson(id)).filter(Boolean);
    return `<div data-act="pd-crew" data-crew="${crew.id}" style="display:flex;align-items:center;gap:11px;padding:9px 0;${index ? 'border-top:1px solid var(--ink-a05);' : ''}cursor:pointer">
${avatarStack(others, { size: 26, font: 10, max: 2, settings })}
<span style="font-size:14px;font-weight:600;flex:1;pointer-events:none;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(crew.name)}</span>
...
<span style="font-family:'Bricolage Grotesque',sans-serif;font-size:22px;font-weight:650">${t('Neue Crew')}</span>
</div>`,
