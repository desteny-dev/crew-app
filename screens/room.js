...
// Runde 11 (C1, D3): Nachricht wischen = antworten — die Regel, wem ein Zug gehört, steht dort.
...
import {
  ...
  zusageReihe, teilnehmendeKarte, moduleDa, modulDazu,
...
import {
  ...
  ortIn, klingtNachVorschlag, vorschlaegeAn,
...
// v3.1 §3: In einem Raum steht HÖCHSTENS EIN Panel gleichzeitig — aktives Meet zuerst,
// sonst das nächste kommende. Vergangene Meets bleiben im Verlauf (getMeets 'upcoming' liefert
// sie gar nicht), nie als ewige Chat-Karte. Im 1:1-Raum zählen nur direkte 1:1-Meets sowie aktive
// gemeinsame Meets — geplante Crew-Meets bleiben im Crew-Raum (reference 07.10: Raum ohne Panel
// trotz Crew-Planung).
// Runde 11 Nachtrag (Wunsch 1, Jonathan: „nicht nur das nächste Meet sehen — nach links wischen,
// um das nächste im Chat zu sehen"): Das Panel zeigt nicht mehr zwingend NUR das erste — ui.panelIndex
// merkt sich die per Wisch gewählte Stelle in derselben Liste. Ohne eigene Wahl bleibt es wie bisher
// das aktive bzw. das erste kommende Meet.
function panelMeetIndex(ctx, relevant) {
  if (!relevant.length) return 0;
  const basis = relevant.findIndex((meet) => meet.status === 'active');
  const start = basis >= 0 ? basis : 0;
  const { panelIndex } = ctx.ui;
  return Number.isInteger(panelIndex) && panelIndex >= 0 && panelIndex < relevant.length ? panelIndex : start;
}

function panelMeetFor(ctx, info) {
  const relevant = roomMeetOptions(ctx, info);
  if (!relevant.length) return null;
  return relevant[panelMeetIndex(ctx, relevant)];
}
...
// Runde 11 (A15): Ohne den Schalter (vorschlaegeAn) erkennt Crew im Chat nichts von selbst.
function bringAdoptionLabel(text) {
  return vorschlaegeAn() ? mitbringWort(text) : '';
}
...
function raumHinweise(ctx, info, panelMeet) {
  ...
  if (!vorschlaegeAn()) return hinweise;
...
// --- Verpasst (Catch-up 07.8/07.9) — Runde 11 (B8) --------------------------------------------------
//
// Jonathan: „Zusammenfassung endlich verlässlich, OHNE KI." Deshalb ist es Regelwerk, kein Raten: Es
// stehen NUR Wörter aus dem Chat da (ein kurzes Zitat der Nachricht) — Crew formuliert nichts dazu, was
// niemand geschrieben hat (Hausregel 8). Höchstens fünf Stichpunkte, die wichtigsten zuerst gewählt:
//   1. Fragen an mich und @mich          („Frage" · „Erwähnt")
//   2. Zeit- und Ortsvorschläge          („Zeit" · „Ort") — die Erkennung aus core/chat-erkennung.js, DIREKT
//      aufgerufen: Der Schalter für die Vorschläge (vorschlaegeAn, A15) betrifft nur die Vorschläge unter den
//      Nachrichten, nicht diese Zusammenfassung.
//   3. Zu- und Absagen                   („Zusage" · „Absage")
//   4. neue Meets und Umfragen im Chat   („Neues Meet" · „Neue Umfrage") — was seit dem letzten Öffnen dieses
//      Raums dazugekommen ist (Stand auf dem Gerät, beim allerersten Öffnen wird nichts behauptet)
//   5. Rest: Lust auf etwas, Mitfahrt, erwähnte Ressource (die zwei ersten nur mit dem Vorschlags-Schalter)
// Erscheint ab CATCHUP_AB ungelesenen Nachrichten.
const CATCHUP_AB = 5;
const CATCHUP_PUNKTE = 5;
const GESEHEN_MERKER = 'crew.raum.gesehen';

const keywordBold = (word) => `<b style="font-weight:700;color:var(--ink)">${esc(word)}</b>`;
const labelBold = (text) => `<b style="font-weight:700;color:var(--ink)">${esc(text)}</b>`;

const ZUSAGE_WORTE = /(?<![\p{L}])(?:bin (?:mit )?dabei|komme mit|ich komme(?!\s+(?:leider\s+)?nicht)|count me in|i'?m in|i'?m coming|i'?ll be there|i'?ll come|je viens(?!\s+pas)|je suis (?:partant|partante|dispo)|j'en suis|me apunto|cuenta conmigo|estoy dentro)(?![\p{L}])/iu;
const ABSAGE_WORTE = /(?<![\p{L}])(?:kann (?:leider )?nicht (?:kommen|mitkommen|mit|dabei)|komme (?:leider )?nicht|schaffe (?:es )?(?:leider )?nicht|bin raus|sage ab|muss absagen|can'?t make it|i can'?t come|i'?m out|cannot come|je ne peux pas venir|je ne viens pas|no puedo ir|no voy|no llego)(?![\p{L}])/iu;

function stichwort(text, max = 32) {
  const glatt = String(text || '').replace(/\s+/g, ' ').trim();
  return glatt.length > max ? `${glatt.slice(0, max - 1).trimEnd()}…` : glatt;
}

function gesehenLesen() {
  try { return JSON.parse(globalThis.localStorage?.getItem(GESEHEN_MERKER) || '{}') || {}; } catch { return {}; }
}

// Was in diesem Raum an Meets und Umfragen steht — als Kennungen, nur um „neu" zu erkennen.
function raumInhalt(ctx, info) {
  const { repo } = ctx;
  let meets = [];
  let umfragen = [];
  try { meets = repo.getMeets({ context: info.context, direction: 'upcoming' }) || []; } catch { meets = []; }
  try { umfragen = (typeof repo.getRoomPolls === 'function' ? repo.getRoomPolls(info.room.id) : []) || []; } catch { umfragen = []; }
  return { meets, umfragen };
}

// Wird bei JEDEM Aufbau des Raums nachgezogen — auch was ich selbst hier anlege, gilt damit als gesehen
// und taucht beim nächsten Öffnen nicht als „neu" auf.
function gesehenMerken(ctx, info) {
  const { meets, umfragen } = raumInhalt(ctx, info);
  const jetzt = { m: meets.map((meet) => meet.id), p: umfragen.map((umfrage) => umfrage.id) };
  const schluessel = JSON.stringify(jetzt);
  if (ctx.ui.gesehenStand === schluessel) return;
  ctx.ui.gesehenStand = schluessel;
  try {
    const alle = gesehenLesen();
    delete alle[info.room.id];
    alle[info.room.id] = jetzt;
    const ids = Object.keys(alle);
    while (ids.length > 60) delete alle[ids.shift()];
    globalThis.localStorage?.setItem(GESEHEN_MERKER, JSON.stringify(alle));
  } catch { /* ohne Gerätespeicher gibt es eben kein „neu" */ }
}
...
function catchUpEntries(ctx, info, unseen) {
  ...
  for (const resource of repo.getSettings().resources || []) knownResources.add(String(resource));

  const jetzt = now();
  const orte = bekannteOrte(repo);
  const ich = repo.getMe?.() || null;
  const vorname = String(ich?.name || '').trim().split(/\s+/)[0] || '';
  const maskiert = vorname.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const erwaehntMuster = maskiert ? new RegExp(`@${maskiert}(?![\\p{L}\\p{N}-])`, 'iu') : null;
  const nameMuster = maskiert ? new RegExp(`(?<![\\p{L}\\p{N}@])${maskiert}(?![\\p{L}\\p{N}])`, 'iu') : null;
  const zitat = (text) => t('„{text}“', { text: esc(stichwort(text)) });
  const zeile = (label, person, text) => `${labelBold(label)} · ${esc(person.name)}: ${zitat(text)}`;

  const kandidaten = [];
  const gesehen = new Set();
  const merke = (schluessel) => {
    if (gesehen.has(schluessel)) return false;
    gesehen.add(schluessel);
    return true;
  };

  // Vom neuesten zum ältesten: je Person und Art zählt die jüngste Nachricht.
  for (let index = unseen.length - 1; index >= 0; index -= 1) {
    const message = unseen[index];
    const text = message.text || '';
    const person = repo.getPerson(message.authorId);
    if (!person) continue;

    // Zeit oder Ort vorgeschlagen? Dieselbe Erkennung wie unter den Nachrichten (chat-erkennung.js) — hier
    // direkt aufgerufen, damit die Zusammenfassung auch dann verlässlich ist, wenn die Vorschläge aus sind.
    const zeit = uhrzeitIn(text);
    const tag = tagIn(text, jetzt);
    const ort = ortIn(text, orte);
    const zeitVorschlag = Boolean(zeit || (tag?.datum && klingtNachVorschlag(text)));
    const frage = /[?¿]/.test(text);
    const genannt = Boolean(nameMuster?.test(text));
    const erwaehnt = Boolean(erwaehntMuster?.test(text));
    // Mit meinem Namen angesprochen (oder @ich) — oder im 1:1-Chat jede Frage des Gegenübers.
    const direkt = erwaehnt || (genannt && frage);
    const anMich = direkt || (frage && info.kind === 'person');

    let art = null;
    let label = '';
    if (direkt) { art = frage ? 'frage' : 'erwaehnt'; label = frage ? t('Frage') : t('Erwähnt'); }
    else if (zeitVorschlag) { art = 'zeit'; label = t('Zeit'); }
    else if (ort) { art = 'ort'; label = t('Ort'); }
    else if (anMich) { art = 'frage'; label = t('Frage'); }
    else if (genannt) { art = 'erwaehnt'; label = t('Erwähnt'); }
    else if (ABSAGE_WORTE.test(text)) { art = 'absage'; label = t('Absage'); }
    else if (ZUSAGE_WORTE.test(text)) { art = 'zusage'; label = t('Zusage'); }
    if (art) {
      // An mich gerichtet (Frage oder @) geht vor, auch wenn zugleich eine Zeit oder ein Ort drinsteht.
      const rang = (anMich || erwaehnt || genannt) ? 0 : (art === 'zeit' || art === 'ort' ? 1 : 2);
      if (merke(`${art}:${person.id}`)) kandidaten.push({ index, rang, person, html: zeile(label, person, text) });
      continue;
    }
    ...
    if (vorschlaegeAn()) {
      ...
      if (keyword && merke(`lust:${person.id}`)) {
        kandidaten.push({ index, rang: 4, person, html: t('{name} hätte Lust auf {wort}', { name: esc(person.name), wort: keywordBold(keyword) }) });
        continue;
      }
      // Runde 3: Wer eine Mitfahrt ANBIETET, steht auch hier.
      if (bietetMitfahrt(text) && merke(`fahrt:${person.id}`)) {
        kandidaten.push({ index, rang: 4, person, html: t('{name} bietet eine {an}Mitfahrt{aus} an', { name: esc(person.name), an: '<b style="font-weight:700;color:var(--ink)">', aus: '</b>' }) });
        continue;
      }
      if (suchtMitfahrt(text) && merke(`fahrt:${person.id}`)) {
        kandidaten.push({ index, rang: 4, person, html: t('{name} sucht eine {an}Mitfahrgelegenheit{aus}', { name: esc(person.name), an: '<b style="font-weight:700;color:var(--ink)">', aus: '</b>' }) });
        continue;
      }
    }
    for (const resource of knownResources) {
      const pattern = new RegExp(`(?:^|[\\s@])${resource.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'i');
      if (pattern.test(text) && merke(`res:${person.id}:${resource.toLowerCase()}`)) {
        kandidaten.push({ index, rang: 4, person, html: t('{name} erwähnt {wort}', { name: esc(person.name), wort: keywordBold(resource) }) });
        break;
      }
    }
  }

  // Neue Meets und Umfragen seit dem letzten Öffnen dieses Raums (Stand auf dem Gerät).
  const vorher = gesehenLesen()[info.room.id];
  if (vorher) {
    const { meets, umfragen } = raumInhalt(ctx, info);
    for (const meet of meets.filter((eintrag) => !(vorher.m || []).includes(eintrag.id) && eintrag.creatorId !== ME && eintrag.status !== 'draft')) {
      kandidaten.push({ index: -1, rang: 3, person: repo.getPerson(meet.creatorId) || null, html: `${labelBold(t('Neues Meet'))} · ${esc(meet.title)}` });
    }
    for (const umfrage of umfragen.filter((eintrag) => !(vorher.p || []).includes(eintrag.id) && eintrag.createdBy !== ME)) {
      kandidaten.push({ index: -1, rang: 3, person: repo.getPerson(umfrage.createdBy) || null, html: `${labelBold(t('Neue Umfrage'))} · ${esc(umfrage.question)}` });
    }
  }

  // Die wichtigsten fünf (nach Rang, dann die jüngsten) — angezeigt in der Reihenfolge des Chats.
  return kandidaten
    .sort((a, b) => a.rang - b.rang || b.index - a.index)
    .slice(0, CATCHUP_PUNKTE)
...
function renderRoomView(ctx) {
  ...
  gesehenMerken(ctx, info);

  // §3: höchstens EIN relevantes Panel — aktiv zuerst, sonst das nächste kommende.
  const panelMeet = panelMeetFor(ctx, info);
  const activeMeet = panelMeet && panelMeet.status === 'active' ? panelMeet : null;

  // Bottom-Ebene: über dem Composer steht der Catch-up („Verpasst"). Runde 11 (B9, D9): Der fette Knopf
  // „Meet vorschlagen" ist weg — im LEEREN Chat steht stattdessen eine leise Pille „Meet" über dem „+"
  // der Eingabezeile (composerRow).
  const catchUp = catchupArea(ctx);
  const leererChat = !info.room.messages.some((message) => message.kind === 'text' || message.kind === 'location');
  let extra = '';
  let extraHeight = 0;
  if (catchUp) {
    extra = catchUp;
  ...
  // Runde 11 (C1): Das Zitat einer Antwort steht direkt über der Eingabe — die letzte Nachricht
  // rückt um seine Höhe nach oben, damit es nichts verdeckt.
  const antwort = antwortLeiste(ctx, info);
  if (antwort) extraHeight += ANTWORT_LEISTE_HOEHE;
  ...
  const panelHtml = panelMeet ? panelArea(ctx, info, panelMeet, { liste: roomMeetOptions(ctx, info) }) : '';
  ...
  const html = screenScaffold({
    ...
    bottom: `${bottomFade(8)}${extra ? `<div id="room-extra" style="pointer-events:none">${extra}</div>` : ''}${antwort}${bottomBar(composerRow(ctx, leererChat), 'pointer-events:none')}`,
...
function roomHeader(ctx, info, active) {
  ...
  // Runde 9 (Jonathan): kein pulsierender grüner Kopf mehr für „Meet ist aktiv".
  void active;
  const fade = '';
  let identity;
  let nameHtml;
...
// Kleine Punktreihe wie ein Karussell — nur wenn es wirklich etwas zum Wischen gibt. Runde 11
// Nachtrag (Wunsch 2): Sie steht AUSSERHALB der Box, ruhig darunter — keine Aktion, kein Gewicht.
function panelPunkte(liste, index) {
  if (liste.length <= 1) return '';
  const punkte = liste.map((_, i) => `<span aria-hidden="true" style="width:${i === index ? 14 : 5}px;height:5px;border-radius:3px;background:${i === index ? 'var(--green)' : 'var(--ink-a14)'};flex:none"></span>`).join('');
  return `<div data-role="raum-panel-punkte" aria-hidden="true" style="display:flex;justify-content:center;gap:5px;padding:8px 0 0;pointer-events:none">${punkte}</div>`;
}

function panelArea(ctx, info, meet, options = {}) {
  const { ui } = ctx;
  const liste = options.liste || [meet];
  const index = Math.max(0, liste.findIndex((eintrag) => eintrag.id === meet.id));
  ...
  // Runde 11 (B2, D2): Der Kopf hat in Details und Chat dieselben drei Zeilen — Titel, Zeit, Ort.
  const ortText = meet.placePending ? t('Ort — Zustimmung steht aus') : (meet.place?.name || t('Ort offen'));
  ...
  // Runde 11 (B2, D2): Module (Anreise · Liste · Umfrage) stehen NUR da, wenn sie Inhalt haben oder über das
  // „+" der Eingabezeile hinzugefügt wurden — in fester Reihenfolge, ohne eigene Pille (das „+" ist der Weg).
  const da = moduleDa(ctx, meet);
  const zeigeListe = da.bring || listCount > 0;
...
<span style="pointer-events:none;display:flex">${tileChevron(open, open)}</span>
  ...
  const kacheln = [
    zeigeListe ? tile('bring', listCheckSvg, t('Liste · {n}', { n: listCount }), listOpen, false) : '',
    da.polls ? tile('polls', pollBarsSvg(), t('Umfragen · {n}', { n: pollCount }), pollsOpen, false) : '',
  ].filter(Boolean).join('');
  const modulTeile = `${da.anreise ? anreiseKnopf(ctx, meet, { kompakt: true }) : ''}${kacheln ? `<div style="display:flex;gap:8px">${kacheln}</div>` : ''}${zeigeListe && listOpen ? listBox(ctx, listItems) : ''}${da.polls && pollsOpen ? pollBox(ctx, meet) : ''}`;
...
<div data-role="raum-panel-buehne" style="position:relative">
<div data-role="raum-panel-karte" data-meet="${esc(meet.id)}" aria-label="${liste.length > 1 ? esc(t('Meet {n} von {gesamt}', { n: index + 1, gesamt: liste.length })) : ''}" style="position:relative;z-index:10;pointer-events:auto;background:var(--surface);border:1px solid var(--ink-a10);border-radius:18px;box-shadow:0 8px 20px var(--shadow-12);padding:12px;display:flex;flex-direction:column;gap:10px;touch-action:pan-y">
...
<span data-role="panel-ort" style="font-size:12px;color:var(--muted);max-width:100%;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;pointer-events:none">${esc(ortText)}</span>
</span>
...
${zusageReihe(ctx, meet, { compact: true, bare: true })}
${teilnehmendeKarte(ctx, meet, { compact: true, bare: true })}
${modulTeile ? `<div data-role="panel-module" style="display:flex;flex-direction:column;gap:8px">${modulTeile}</div>` : ''}
</div>
</div>
${panelPunkte(liste, index)}
</div>`;
}
...
// trägt links das Zeichen der Aktivität und daneben Titel, Tag · Ort und die leise Pille „Wie war's?“
// aus dem gemeinsamen Baustein (Runde 11, D10: keine Gesichter, kein ✕). Sie kommt nur, wenn hier
// nichts zu planen ist — so konkurriert sie nie mit einem laufenden oder kommenden Meet.
...
function chatBody(ctx, info, panelMeet) {
  ...
  const messages = info.room.messages.map((message) => {
    ...
    // Runde 11 (C1): Eine Antwort zeigt die zitierte Nachricht — gelesen aus DEMSELBEN Raum (null: weg).
    const zitat = message.replyTo
      ? { id: message.replyTo, nachricht: info.room.messages.find((entry) => entry.id === message.replyTo && !loeschenVorgemerkt(info.room.id, entry.id)) || null }
      : null;
    return renderMessage(ctx, message, { adoption, hinweis, zitat });
  }).join('');
...
// =====================================================================================
// Antworten (Runde 11, C1 — Jonathan D3: „Nachrichten im Chat wischen = Antworten: Zitat über der
// Eingabe, gesendete Nachricht zeigt das Zitat.")
//
// Eine Nachricht nach RECHTS ziehen (wie in jedem Messenger) — ab ANTWORT_WEG zeigt ein Pfeil und ein
// kurzes Tippen der Haptik, dass Loslassen jetzt antwortet. Dann steht über der Eingabe ein Zitat
// (mit ✕ zum Verwerfen), und die gesendete Nachricht trägt `replyTo` (beide Gateways, Migration 0042).
// In der Blase steht das Zitat oben; ein Tipp darauf holt die zitierte Nachricht in den Blick. Ist sie
// nicht mehr da (gelöscht, älter als 14 Tage), sagt das Zitat genau das.
// Dieselbe Antwort gibt es im Menü nach langem Drücken — für alle, die das Wischen nicht kennen.
// Konflikte: Der linke Rand gehört dem Zurück-Wischen, die Meet-Box oben ihrem eigenen Wisch
// (bindPanelWisch) — beide liegen außerhalb dessen, was hier beginnt (ui/gesten.js).
// =====================================================================================
const ANTWORT_WEG = 56;
const ANTWORT_MAX = 76;
const antwortSvg = (farbe, groesse) => `<svg width="${groesse}" height="${groesse}" viewBox="0 0 24 24" fill="none" style="pointer-events:none;flex:none"><path d="M9.5 6.5 4 12l5.5 5.5" stroke="${farbe}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path><path d="M4.5 12h9a6 6 0 0 1 6 6v.5" stroke="${farbe}" stroke-width="2" stroke-linecap="round"></path></svg>`;

function zitatWer(ctx, nachricht) {
  if (nachricht.authorId === ME) return t('Du');
  return ctx.repo.getPerson(nachricht.authorId)?.name || t('Unbekannt');
}

function zitatText(nachricht) {
  if (nachricht.kind === 'location') return t('Standort');
  return String(nachricht.text || '').replace(/\s+/g, ' ').trim();
}

function zitatInBlase(ctx, zitat, eigen) {
  const grund = eigen ? 'var(--green-a10)' : 'var(--ink-a05)';
  const rahmen = 'display:block;box-sizing:border-box;width:100%;margin:0 0 6px;padding:5px 9px 6px;border:0;border-left:3px solid var(--green);border-radius:8px;text-align:left;font-family:inherit';
  if (!zitat.nachricht) {
    return `<span data-role="nachricht-zitat" data-fehlt="1" style="${rahmen};background:${grund};font-size:12px;font-style:italic;color:var(--muted)">${esc(t('Nachricht nicht mehr da'))}</span>`;
  }
  const n = zitat.nachricht;
  return `<button type="button" data-act="zitat-zeigen" data-ziel="${esc(n.id)}" data-role="nachricht-zitat" style="${rahmen};background:${grund};cursor:pointer;appearance:none;color:inherit"><span style="pointer-events:none;display:block;font-size:11.5px;font-weight:700;color:var(--green-dark);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(zitatWer(ctx, n))}</span><span style="pointer-events:none;display:block;font-size:12.5px;line-height:1.3;color:var(--ink-soft);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(zitatText(n))}</span></button>`;
}

// Das Zitat über der Eingabe. Es gehört zu genau einem Raum — ein anderer Chat übernimmt es nicht.
function antwortZiel(ctx, info) {
  const a = ctx.ui.antwortAuf;
  if (!a || a.roomId !== info.room.id) return null;
  return info.room.messages.find((entry) => entry.id === a.id && !loeschenVorgemerkt(info.room.id, entry.id)) || null;
}

const ANTWORT_LEISTE_HOEHE = 58;
function antwortLeiste(ctx, info) {
  const n = antwortZiel(ctx, info);
  if (!n) return '';
  const an = n.authorId === ME ? t('Antwort auf deine Nachricht') : t('Antwort an {name}', { name: zitatWer(ctx, n) });
  return `<div data-role="antwort-leiste" style="pointer-events:none;padding:0 20px 0 66px">
<div style="pointer-events:auto;display:flex;align-items:center;gap:8px;min-height:48px;box-sizing:border-box;padding:6px 4px 6px 12px;background:var(--surface);border:1.5px solid var(--ink-a10);border-left:3px solid var(--green);border-radius:14px;box-shadow:0 1px 3px var(--shadow-08);font-family:'Instrument Sans',sans-serif">
<span style="pointer-events:none;display:flex;flex:none">${antwortSvg('var(--green-dark)', 16)}</span>
<span style="pointer-events:none;flex:1;min-width:0;display:flex;flex-direction:column;gap:1px">
<span data-role="antwort-an" style="font-size:11.5px;font-weight:700;color:var(--green-dark);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(an)}</span>
<span data-role="antwort-text" style="font-size:12.5px;color:var(--ink-soft);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(zitatText(n))}</span>
</span>
<button type="button" data-act="antwort-weg" data-role="antwort-weg" data-treffer aria-label="${esc(t('Antwort verwerfen'))}" style="flex:none;width:32px;height:32px;border:0;border-radius:50%;background:transparent;display:flex;align-items:center;justify-content:center;padding:0;cursor:pointer;appearance:none"><span style="pointer-events:none;font-size:13px;color:var(--muted)">✕</span></button>
</div></div>`;
}

function antwortSetzen(ctx, info, id) {
  ctx.ui.antwortAuf = { roomId: info.room.id, id };
  ctx.ui.refocusComposer = true;
  ctx.render();
}

function bindAntwortWisch(root, ctx, info) {
  const flaeche = root.querySelector('.screen-scroll');
  if (!flaeche) return;
  const pfeilWeg = (d) => { d.pfeil?.remove(); d.pfeil = null; };
  const zurueck = (d) => {
    const zeile = d.zeile;
    zeile.style.transition = 'transform .2s cubic-bezier(.22,.61,.36,1)';
    zeile.style.transform = '';
    window.setTimeout(() => { zeile.style.transition = ''; zeile.style.position = ''; pfeilWeg(d); }, 210);
  };
  waagrechtZiehen(flaeche, 'antwort', {
    beginnt(ziel) {
      const zeile = ziel.closest?.('[data-nachricht]');
      if (!zeile || !flaeche.contains(zeile) || ctx.ui.nachrichtMenue) return null;
      return { zeile, id: zeile.dataset.nachricht, gefuehlt: false, pfeil: null, weg: 0 };
    },
    // Nur nach rechts — nach links gibt es hier nichts (und der Zug bleibt frei).
    darf: (d, dx) => dx > 0,
    bewegt(d, dx) {
      const weg = Math.max(0, Math.min(ANTWORT_MAX, dx * 0.6));
      d.weg = weg;
      if (!d.pfeil) {
        d.pfeil = document.createElement('span');
        d.pfeil.dataset.role = 'antwort-pfeil';
        d.pfeil.__morphFrei = true;
        d.pfeil.style.cssText = 'position:absolute;left:-8px;top:50%;width:30px;height:30px;margin-top:-15px;transform:translateX(-100%);border-radius:50%;background:var(--ink-a07);display:flex;align-items:center;justify-content:center;pointer-events:none;opacity:0';
        d.pfeil.innerHTML = antwortSvg('var(--ink-soft)', 16);
        d.zeile.style.position = 'relative';
        d.zeile.appendChild(d.pfeil);
      }
      d.zeile.style.transition = 'none';
      d.zeile.style.transform = `translateX(${weg.toFixed(1)}px)`;
      const anteil = Math.min(1, weg / ANTWORT_WEG);
      d.pfeil.style.opacity = String(anteil);
      d.pfeil.style.background = anteil >= 1 ? 'var(--green-tint)' : 'var(--ink-a07)';
      if (!d.gefuehlt && anteil >= 1) { d.gefuehlt = true; rueckmeldung('tipp'); }
      if (anteil < 1) d.gefuehlt = false;
    },
    endet(d) {
      if (d.weg < ANTWORT_WEG) { zurueck(d); return; }
      // Der Render gleich danach setzt die Zeile ohnehin auf ihr Markup zurück — Pfeil und Versatz
      // gehen deshalb jetzt, nicht erst nach der Rückfahrt.
      pfeilWeg(d);
      d.zeile.style.transition = '';
      d.zeile.style.transform = '';
      d.zeile.style.position = '';
      antwortSetzen(ctx, info, d.id);
    },
    abbruch(d) { zurueck(d); },
  });
}
...
// --- Catch-up (07.8 geschlossen / 07.9 offen) — Inhalt: catchUpEntries („Verpasst") ---

function catchupArea(ctx) {
  const { ui } = ctx;
  if (!ui.unreadCount || ui.unreadCount < CATCHUP_AB || !ui.catchUp?.length || ui.catchDismissed) return '';
  if (!ui.catchOpen) {
...
<button data-act="catch-open" data-role="verpasst-oeffnen" style="border:0;background:transparent;padding:0;cursor:pointer;font-family:'Instrument Sans',sans-serif"><span style="pointer-events:none;font-size:12.5px;font-weight:650;color:var(--green-dark)">${t('Zusammenfassen')}</span></button>
</div>`, 'pointer-events:none;padding:2px 0 8px');
  }
  const cards = ui.catchUp.map((entry) => `<div data-role="verpasst-punkt" style="background:var(--paper);border-radius:12px;padding:10px 12px;display:flex;align-items:center;gap:9px">
<span style="display:flex;flex:none;pointer-events:none">${entry.person
    ? personAvatar(entry.person, { size: 22, fontSize: 8.5, marker: personMarker(entry.person.id, ctx.repo.getSettings()), compact: true, dotBorder: 'var(--paper)' })
    : '<span style="width:22px;height:22px;border-radius:50%;background:var(--field);display:block"></span>'}</span>
<span style="font-size:12.5px;font-weight:550;color:var(--ink-soft);flex:1;min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${entry.html}</span>
</div>`).join('');
  return bottomBar(`<div data-role="verpasst" style="pointer-events:auto;margin:0 20px;background:var(--surface);border:1.5px solid var(--ink-a12);border-radius:16px;padding:13px 14px;display:flex;flex-direction:column;gap:9px;box-shadow:0 6px 16px var(--shadow-10)">
<div style="display:flex;justify-content:space-between;align-items:center;padding:0 2px"><span style="font-size:11px;font-weight:650;letter-spacing:.09em;text-transform:uppercase;color:var(--muted-light)">${t('Verpasst')} · ${tn(ui.unreadCount, '{n} neue Nachricht', '{n} neue Nachrichten')}</span><button data-act="catch-close" aria-label="${esc(t('Schließen'))}" style="border:0;background:transparent;padding:0 2px;cursor:pointer;font-family:'Instrument Sans',sans-serif"><span style="pointer-events:none;font-size:13px;color:var(--muted-light)">✕</span></button></div>
${cards}
</div>`, 'pointer-events:none;padding:2px 0 8px');
}
...
// Runde 11 (B9, D9): Im LEEREN Chat steht über dem „+" eine kleine, leise Pille „Meet" mit einem kurzen
// Verbindungsstrich — sie wächst aus dem „+" heraus und sagt: hinter dem „+" stecken die Aktionen (Meet,
// Liste, Umfrage, Mitfahrt). Ein Tipp öffnet dasselbe wie das „+", mit Meet vorgewählt. Mit der ersten
// Nachricht ist sie weg.
function meetPille() {
  const live = 'pointer-events:auto';
  return `<button data-act="plus-meet" data-role="meet-pille" data-treffer aria-label="${esc(`Meet · ${t('Treffen planen')}`)}" style="${live};position:absolute;left:20px;top:-32px;height:28px;padding:0 12px 0 10px;display:flex;align-items:center;gap:6px;border-radius:999px;background:var(--paper-soft);border:1.5px solid var(--ink-a10);color:var(--muted);font:650 12px 'Instrument Sans',sans-serif;cursor:pointer;appearance:none;box-shadow:0 1px 3px var(--shadow-08);box-sizing:border-box"><span style="pointer-events:none;display:flex">${menuMeetSvg.replace('width="17" height="17"', 'width="13" height="13"').replace(/var\(--ink\)/g, 'currentColor')}</span><span style="pointer-events:none">Meet</span></button>
<span data-role="meet-pille-strich" aria-hidden="true" style="position:absolute;left:38px;top:-4px;width:2px;height:11px;border-radius:1px;background:var(--ink-a14);pointer-events:none"></span>`;
}

function composerRow(ctx, leer = false) {
  const { ui } = ctx;
...
function plusSheet(vorwahl = '') {
  const zeile = (act, svg, label, sub, last, an = false) => `<button data-act="${act}"${an ? ' data-vorgewaehlt="1"' : ''} style="display:flex;align-items:center;gap:11px;padding:11px ${an ? 8 : 2}px;border:0;${last || an ? '' : 'border-bottom:1px solid var(--ink-a06);'}${an ? 'border-radius:14px;background:var(--green-tint)' : 'background:transparent'};width:100%;text-align:left;cursor:pointer;appearance:none;font-family:'Instrument Sans',sans-serif">
<span style="width:34px;height:34px;border-radius:11px;background:var(--paper);display:flex;align-items:center;justify-content:center;flex:none;pointer-events:none">${svg}</span>
<span style="display:flex;flex-direction:column;gap:1px;pointer-events:none;min-width:0;flex:1">
<span style="font-size:14px;font-weight:650;color:var(--ink)">${esc(label)}</span>
<span style="font-size:11.5px;color:var(--muted)">${esc(sub)}</span></span>
...
${kapitel(t('Planen'))}
<div style="display:flex;flex-direction:column">
${zeile('menu-meet', menuMeetSvg, 'Meet', t('Treffen planen'), false, vorwahl === 'meet')}
...
${zeile('menu-list', menuListSvg, t('Liste'), t('wer bringt was mit'))}
${zeile('menu-anreise', menuAutoSvg, t('Mitfahrt'), t('wer fährt, wer fährt mit'), true)}
</div>
${kapitel(t('Teilen'))}
<div style="display:flex;flex-direction:column">
...
// Runde 11 (B2): „Mitfahrt" im Plus öffnet die Seite Anreise dieses Meets (dieselbe Auswahl wie „+ Hinzufügen" in den Details).
const menuAutoSvg = `<svg width="17" height="17" viewBox="0 0 24 24" fill="none" style="flex:none;pointer-events:none"><path d="M4.5 15.5v-3.3l1.9-4.5a1.9 1.9 0 0 1 1.7-1.2h7.8a1.9 1.9 0 0 1 1.7 1.2l1.9 4.5v3.3M3.6 15.5h16.8M5 12.2h14" stroke="var(--ink)" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"></path><circle cx="7.7" cy="17.4" r="1.9" fill="var(--ink)"></circle><circle cx="16.3" cy="17.4" r="1.9" fill="var(--ink)"></circle></svg>`;
...
function nachrichtMenue(ctx, info) {
  ...
  const eigen = message.authorId === ME;
  const eintraege = [];
  // Runde 11 (C1): Antworten geht auch ohne das Wischen zu kennen.
  eintraege.push(['nachricht-antworten', antwortSvg('var(--ink)', 18), t('Antworten'), 'var(--ink)']);
...
// Runde 11 Nachtrag (Wunsch 1): Waagrechtes Wischen auf der Meet-Box blaettert durch alle
// kommenden Meets dieses Chats (roomMeetOptions) — nur wenn es mehr als eines gibt. Die Karte
// folgt dabei dem Finger (nur transform, kein Layout-Sprung); beim Loslassen gleitet sie weiter
// zum Nachbar-Meet — der kommt sichtbar von der Seite herein — oder federt unter der Schwelle
// zurueck. Am Anfang/Ende (kein Nachbar in diese Richtung) gibt die Karte nur gedaempft nach.
// Ein Tipp bleibt unberuehrt: erst eine wirklich horizontale Bewegung zaehlt als Wisch
// (waagrechtZiehen, ui/gesten.js) — ein senkrechter Scroll-/Tippversuch bricht sofort ab.
// prefers-reduced-motion verkuerzt die Übergaenge auf 1 ms; die Bewegung MIT dem Finger ist
// direkte Bedienung und bleibt davon unberuehrt.
const PANEL_WISCH_SCHWELLE = 40;
const PANEL_RAND_MAX = 36;

function panelWischDauer() { return bewegungGedaempft() ? 1 : 260; }

// Leichte Vorschau des Nachbar-Meets waehrend des Wischs — nur Kopfzeile, ohne Bedienelemente.
// Sie ersetzt sich durch die echte Karte, sobald der Wechsel steht (ctx.render() in bindPanelWisch).
function panelVorschauHtml(meet) {
  const active = meet.status === 'active';
  const zeitText = active
    ? `${esc(meetDateLabel(meet.date))} · ${t('seit {zeit}', { zeit: esc(meet.time) })}`
    : esc(terminText(meet, { datum: meetDateLabel }));
  const ortText = meet.placePending ? t('Ort — Zustimmung steht aus') : (meet.place?.name || t('Ort offen'));
  return `<div data-role="raum-panel-vorschau" aria-hidden="true" style="position:absolute;inset:0;background:var(--surface);border:1px solid var(--ink-a10);border-radius:18px;box-shadow:0 8px 20px var(--shadow-12);padding:12px;display:flex;flex-direction:column">
<div style="display:flex;align-items:center;gap:10px;width:100%">
<span style="width:34px;height:34px;border-radius:11px;background:var(--paper);display:flex;align-items:center;justify-content:center;flex:none">${activityIconSvg(meet, 'var(--ink)', 17)}</span>
<span style="display:flex;flex-direction:column;gap:2px;flex:1;min-width:0">
<span style="font-size:14.5px;font-weight:650;letter-spacing:-.01em;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(meet.title)}</span>
<span style="font-size:12px;color:${active ? 'var(--green-dark)' : 'var(--ink-soft)'};font-weight:${active ? 600 : 400};font-variant-numeric:tabular-nums">${zeitText}</span>
<span style="font-size:12px;color:var(--muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(ortText)}</span>
</span>
</div>
</div>`;
}

function bindPanelWisch(root, ctx, info) {
  const karte = root.querySelector('[data-role="raum-panel-karte"]');
  if (!karte) return;
  karte.__wisch = { ctx, info };
  if (karte.__wischGebunden) return;
  karte.__wischGebunden = true;

  const vorschauEntfernen = (zug) => { zug.vorschau?.remove(); zug.vorschau = null; zug.richtung = 0; };

  const absetzen = (zug, zuKarte, zuVorschau, dann) => {
    const dauer = panelWischDauer();
    const kurve = `transform ${dauer}ms cubic-bezier(.22,.8,.24,1)`;
    karte.style.transition = kurve;
    karte.style.transform = zuKarte;
    if (zug.vorschau) { zug.vorschau.style.transition = kurve; zug.vorschau.style.transform = zuVorschau; }
    window.setTimeout(() => {
      karte.style.transition = '';
      karte.style.transform = '';
      vorschauEntfernen(zug);
      dann?.();
    }, dauer + 20);
  };

  waagrechtZiehen(karte, 'panel', {
    beginnt() {
      const { ctx: aktCtx, info: aktInfo } = karte.__wisch;
      const liste = roomMeetOptions(aktCtx, aktInfo);
      if (liste.length < 2) return null;
      const von = Math.max(0, liste.findIndex((eintrag) => eintrag.id === karte.dataset.meet));
      return { ctx: aktCtx, liste, von, breite: karte.getBoundingClientRect().width || 300, richtung: 0, vorschau: null };
    },
    bewegt(zug, dx) {
      karte.style.transition = 'none';
      const richtung = dx < 0 ? 1 : -1; // 1: naechstes Meet (links ziehen), -1: voriges
      const zielIndex = zug.von + richtung;
      const zielGueltig = zielIndex >= 0 && zielIndex < zug.liste.length;
      if (!zielGueltig) {
        if (zug.vorschau) vorschauEntfernen(zug);
        const gedaempft = dx * (PANEL_RAND_MAX / (PANEL_RAND_MAX + Math.abs(dx)));
        karte.style.transform = `translateX(${gedaempft.toFixed(1)}px)`;
        return;
      }
      const bewegung = Math.sign(dx) * Math.min(Math.abs(dx), zug.breite);
      karte.style.transform = `translateX(${bewegung.toFixed(1)}px)`;
      if (zug.richtung !== richtung) {
        vorschauEntfernen(zug);
        zug.richtung = richtung;
        const huelle = document.createElement('div');
        huelle.innerHTML = panelVorschauHtml(zug.liste[zielIndex]);
        zug.vorschau = huelle.firstElementChild;
        zug.vorschau.style.transition = 'none';
        karte.parentElement.insertBefore(zug.vorschau, karte);
      }
      const start = richtung > 0 ? zug.breite : -zug.breite;
      zug.vorschau.style.transform = `translateX(${(start + bewegung).toFixed(1)}px)`;
    },
    endet(zug, dx, schwung) {
      const richtung = dx < 0 ? 1 : -1;
      const zielIndex = zug.von + richtung;
      const zielGueltig = zielIndex >= 0 && zielIndex < zug.liste.length;
      const weit = Math.abs(dx) >= PANEL_WISCH_SCHWELLE || Math.abs(schwung) > 0.5;
      if (zielGueltig && weit) {
        rueckmeldung('tipp');
        const raus = richtung > 0 ? -zug.breite : zug.breite;
        const { ctx: aktCtx } = zug;
        absetzen(zug, `translateX(${raus}px)`, 'translateX(0px)', () => {
          aktCtx.ui.panelIndex = zielIndex;
          aktCtx.render();
        });
        return;
      }
      const start = zug.richtung > 0 ? zug.breite : (zug.richtung < 0 ? -zug.breite : 0);
      absetzen(zug, 'translateX(0px)', `translateX(${start}px)`);
    },
    abbruch(zug) {
      const start = zug.richtung > 0 ? zug.breite : (zug.richtung < 0 ? -zug.breite : 0);
      absetzen(zug, 'translateX(0px)', `translateX(${start}px)`);
...
function bindRoomView(root, ctx) {
  ...
  bindAntwortWisch(root, ctx, info);
  const panelMeet = panelMeetFor(ctx, info);
  ...
  const send = () => {
    ...
    // Runde 11 (C1): Steht ein Zitat über der Eingabe, ist diese Nachricht die Antwort darauf.
    const antwortAuf = antwortZiel(ctx, info)?.id || null;
    ui.antwortAuf = null;
    repo.sendMessage(params.roomId, text, antwortAuf ? { replyTo: antwortAuf } : undefined);
  };
  ...
  bindActions(root, {
    ...
    'menu-list': () => {
      ...
      ui.plusMenu = false;
      modulDazu(ui, panelMeet.id, 'bring');
      ui.panel = 'bring';
      ui.pollForm = false;
      ctx.render();
    },
    // Runde 11 (B2): Mitfahrt = das Modul Anreise dieses Meets — es steht danach im Panel.
    'menu-anreise': () => {
      ui.plusMenu = false;
      if (!panelMeet) { ctx.toast(t('Dafür braucht es zuerst ein Meet')); ctx.render(); return; }
      modulDazu(ui, panelMeet.id, 'anreise');
      nav.go('meet.anreise', { meetId: panelMeet.id });
    },
    // Runde 11 (B9): die leise Pille im leeren Chat — dasselbe wie das „+", Meet vorgewählt.
    'plus-meet': () => { ui.plusMenu = 'meet'; ctx.render(); },
    ...
    // Runde 11 (C1): Antworten — aus dem Menü, das Zitat verwerfen, die zitierte Nachricht zeigen.
    'nachricht-antworten': (data) => {
      ui.nachrichtMenue = null;
      if (info.room.messages.some((entry) => entry.id === data.msg)) antwortSetzen(ctx, info, data.msg);
      else ctx.render();
    },
    'antwort-weg': () => { ui.antwortAuf = null; ui.refocusComposer = true; ctx.render(); },
    'zitat-zeigen': (data) => {
      const ziel = [...root.querySelectorAll('.screen-scroll [data-nachricht]')].find((el) => el.dataset.nachricht === data.ziel);
      const blase = ziel?.querySelector('[data-blase]');
      if (!ziel || !blase) { ctx.toast(t('Nachricht nicht mehr da')); return; }
      ziel.scrollIntoView({ block: 'center', behavior: 'smooth' });
      blase.animate?.([{ boxShadow: '0 0 0 3px var(--green-a42)' }, { boxShadow: '0 0 0 0 transparent' }], { duration: 1200, easing: 'ease-out' });
    },
...
${ressourcen ? profileBox(t('Ressourcen'), ressourcen, '', 'ressourcen') : ''}
  ...
  return {
    html,
    bind(root) {
      // Runde 11 (A14): Kam man über „Mitglieder hinzufügen" im ···-Menü hierher, steht die Auswahl gleich offen.
      if (params.personenHinzu && !ui.personenHinzuGezeigt && darfHinzufuegen(crew)) {
        ui.personenHinzuGezeigt = true;
        ui.personenWahl = { crewId: crew.id, auswahl: [], suche: '', zurueck: false };
        ctx.render();
      }
      bindActions(root, {
        ...discardActions(ctx),
