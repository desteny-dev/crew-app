...
import { erlaubnisStand } from '../core/native.js';
...
// Runde 7, dritte Welle: Gefragt wird core/native.js und nicht mehr `navigator.permissions`
// selbst. In der Hülle IST navigator.permissions zwar auf das Plugin gelegt — aber nur, solange
// sich das Ersetzen des Zugangs überhaupt durchsetzen ließ. `erlaubnisStand('standort')` fragt
// das Plugin direkt und gibt im Browser dieselbe Antwort wie vorher. Eine Wahrheit, ein Weg.
let frischtAuf = false;
export function vorschlagsStandortAuffrischen(nachher) {
  const wert = gelesenerStandort();
  if (!wert || frischtAuf || Date.now() - (wert.am || 0) < STANDORT_AUFFRISCHEN_MS) return;
  frischtAuf = true;
  erlaubnisStand('standort')
    .then((stand) => (stand === 'erteilt' ? standortFuerVorschlaegeHolen() : null))
    .then((ergebnis) => {
