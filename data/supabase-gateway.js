...
import {
  ...
  naechsteFreiGrenze, freiAbgelaufen, freiAnker,
  busyBereinigen, busyVollstaendig, busyFuerFreunde, busyWoche, crewAdminGrund,
  ...
  standortGenauigkeitNorm, standortGenauigkeitFuer, lageGroeber, lageAusPosition, lageWeitGenugAnders,
...
export class SupabaseGateway extends CrewRepository {
  ...
  async _hydrate() {
    ...
    const [
      ...
      belegteZeiten, fragen, ankuenfte, findEintraege, findAdmin,
    ] = await Promise.all([
    ...
    for (const p of rows(profiles)) {
      ...
      state.people.push({
        ...
        // Runde 8 (R8-39): Busy mit Titel — außer hinter einem Schloss. Nur Freunde haben hier
        // überhaupt etwas stehen; die Sicht gibt von allen anderen nichts heraus.
  ...
  // Eine Zeile aus free_status in die Form des Datenvertrags. Runde 7 (H1): Die Spalte
  // `gilt_bis` sagt, wann dieses „frei" von selbst endet. Sie steht auf dem SERVER, damit ein
  // „frei" vom Freitagabend am Sonntag bei niemandem mehr leuchtet — auch wenn die Person ihr
  // Gerät seit Tagen nicht angefasst hat; bei fremden Menschen ist das die EINZIGE Möglichkeit,
  // es zu wissen (ihre Uhrzeit-Einstellung ist privat und bleibt es).
  ...
  freiAusZeile(row) {
    const giltBis = row.gilt_bis ? new Date(row.gilt_bis).getTime() : null;
    const abgelaufen = Boolean(row.active && giltBis && Date.now() >= giltBis);
    return {
      active: Boolean(row.active) && !abgelaufen,
      pending: Boolean(row.pending) && !abgelaufen,
      from: abgelaufen ? null : (row.from_time || null),
      fromAt: !abgelaufen && row.from_at ? new Date(row.from_at).getTime() : null,
      setAt: !abgelaufen && row.set_at ? new Date(row.set_at).getTime() : null,
      lust: abgelaufen ? null : (row.lust || null),
    };
  }

  applyFreeStatus(row) {
    const frei = this.freiAusZeile(row);
  ...
  // Block mit `indizes`; für Freunde die Blöcke mit Titel — hinter einem Schloss ohne; für alle
  // anderen nichts. Gerechnet in projections.js busyWoche, für beide Gateways gleich.
  getBelegtWoche(personId = ME) {
    ...
    if (eigen) {
      ...
      return {
        ...
        tage: busyWoche(fenster, { eigen: true }), fenster, jetzt: belegtJetzt(fenster),
      };
    }
    const person = this.state.people.find((entry) => entry.id === personId);
    if (!person) return { ok: false, eigen: false, sichtbar: false, tage: [], fenster: null, jetzt: null };
    if (!this.busySichtbarVon(person)) {
      return { ok: true, eigen: false, sichtbar: false, tage: [], fenster: null, jetzt: null };
    }
  ...
  standortAusPosition(position) {
    ...
    const jetzt = lage.at;
    if (!lageWeitGenugAnders(alt, lage) && jetzt - (this.standortGesendetAm || 0) < 10 * 60000) return;
  ...
  getFindEintraege() {
    const s = this.state.settings;
    return findListe(this.state.findEintraege || [], {
      interessen: s.interests || [],
      lernen: s.lernen || null,
      mitte: this.getVorschlagsMitte(),
      jetzt: Date.now(),
    });
  }
  ...
  addFindEintrag(roh) {
    ...
    const sicht = findListe([eintrag], {
      ...
      mitte: this.getVorschlagsMitte(), jetzt: Date.now(),
    })[0] || { ...eintrag, score: 0 };
  ...
  rideZuordnenLokal(meetId, mitfahrerId, fahrerId) {
    ...
    zeilen[mitfahrerId] = { art: 'mitfahrt', plaetze: null, fahrerId: fahrerId || null, am: Date.now(), rolle: 'mitfahrer' };
  }
