...
import { merkeLinkFehler, holeLinkFehler } from '../core/auth-texte.js';
...
// Runde 7 (J1), GEMESSEN: Wer auf diesem Browser je die Demo benutzt hatte, verlor jeden Link
// aus einer Crew-Mail — der Demo-Merker wurde VOR allem anderen geprüft, und createRepository
// kehrte im Demo-Modus zurück, bevor der Anhang der Adresse überhaupt gelesen wurde. Der
// Bestätigungslink führte damit in die Demo, und das Konto blieb für immer unbestätigt.
//
// Hier steht die Wurzel dieses Fehlers gerade: Ein Link aus einer Mail ist ein Beweis, dass
// es um ein ECHTES Konto geht. Er schlägt jede gemerkte Wahl — in beide Richtungen, denn der
// Merker wird gleich mit umgestellt (sonst käme man nach dem Bestätigen wieder in der Demo an).
export function linkAusMailDa(ort = globalThis.location) {
  if (!ort) return false;
  const anhang = new URLSearchParams(String(ort.hash || '').replace(/^#/, ''));
  const suche = new URLSearchParams(ort.search || '');
  const hat = (name) => Boolean(anhang.get(name) || suche.get(name));
  const code = anhang.get('code') || suche.get('code') || '';
  const typ = anhang.get('type') || suche.get('type') || '';
  return hat('access_token') || hat('refresh_token') || hat('token_hash') || hat('error_code')
    || (hat('error') && hat('error_description'))
    // Der PKCE-Weg bringt nur `?code=` mit — eine lange Kennung, kein Wort aus der App.
    || /^[0-9a-f][0-9a-f-]{19,}$/i.test(code)
    || /^(signup|recovery|invite|magiclink|email_change|email)$/i.test(typ);
}
...
export function gatewayMode() {
  try {
    ...
    // Ein Link aus einer Crew-Mail schlägt die gemerkte Demo-Wahl (siehe linkAusMailDa).
    if (linkAusMailDa()) {
...
async function einloeseLinkAusAdresse(client) {
  ...
  // Runde 7 (J1), GEMESSEN: Ein Link, der nicht mehr gilt, kommt genau hier an — als
  // #error=access_denied&error_code=otp_expired&error_description=… Bisher wurde dieser Anhang
  // stumm per replaceState weggeworfen, und der Mensch stand vor der nackten Anmeldemaske ohne
  // ein einziges Wort. Jetzt wird er gelesen und an die Anmeldemaske weitergereicht; dort steht
  // dann, was los ist — und daneben der Knopf, der eine neue Mail schickt.
  const fehler = anhang.get('error') || suche.get('error') || '';
  const fehlerCode = anhang.get('error_code') || suche.get('error_code') || '';
  const beschreibung = anhang.get('error_description') || suche.get('error_description') || '';
  if (fehler || fehlerCode) merkeLinkFehler({ fehler, code: fehlerCode, beschreibung });

  try {
    if (zugang && erneuerung) {
      const { error } = await client.auth.setSession({ access_token: zugang, refresh_token: erneuerung });
      eingeloest = !error;
      // Ein Link, den der Server ablehnt, ist kein Nichts: er ist abgelaufen oder verbraucht.
      if (error) merkeLinkFehler({ fehler: 'link', code: '', beschreibung: error.message || '' });
    } else if (code) {
    ...
    // Ein kaputter Link führt zur normalen Anmeldung, nicht zu einem Absturz — aber er führt
    // nicht mehr ins Schweigen. Das leere catch war der Grund, warum niemand je erfuhr, warum
    // der Knopf aus der Mail nichts tat.
    merkeLinkFehler({ fehler: 'link', code: '', beschreibung: ausnahme?.message || String(ausnahme) });
  }

  if (zugang || code || fehler || fehlerCode) {
    suche.delete('code');
    suche.delete('type');
    suche.delete('error');
    suche.delete('error_code');
    suche.delete('error_description');
    const rest = suche.toString();
...
export async function createRepository(engine, hooks = {}) {
  ...
  if (mode !== 'supabase') {
    // Runde 10: Auch das Geräte-Repository wird gestartet, nicht nur angelegt — sonst
    // liefe im Demo-Modus die Standort-Wache nie (DemoDataGateway.ready).
    const repo = new DemoDataGateway(engine);
    await repo.ready();
    return { mode: 'demo', repo };
  }
  ...
  // Wer drin ist, braucht die Nachricht über den kaputten Link nicht mehr — sonst stünde sie
  // irgendwann später vor einer Anmeldung, zu der sie längst nicht mehr gehört.
  holeLinkFehler();
