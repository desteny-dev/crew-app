...
export class CrewRepository {
  ...
  // Die Woche einer Person als BLÖCKE MIT TITEL (projections.js busyWoche):
  //   eigen  → { ok:true, eigen:true, sichtbar:true,
  //              tage:[{ tag, streifen:[{ vonMin, bisMin, titel, privat, indizes:[fensterIndex…] }] }],
  //              fenster:[…eigene Fenster…], jetzt:{ belegt, bis, ende } }
  //   Freund → { ok:true, eigen:false, sichtbar:true,
  //              tage:[{ tag, streifen:[{ vonMin, bisMin, titel:string|null, privat:boolean }] }],
  //              fenster:null, jetzt }         titel ist null, wenn das Schloss zu ist
  //   kein Freund / blockiert → { ok:true, eigen:false, sichtbar:false, tage:[], fenster:null, jetzt:null }
  //   unbekannt              → { ok:false, eigen:false, sichtbar:false, tage:[], fenster:null, jetzt:null }
  ...
  // Reihenfolge: besondere Person → was läuft → letzte Aktivität → frei → beste Freunde →
  // Rest (Name). Ungelesen verändert die Reihenfolge ABSICHTLICH nicht.
