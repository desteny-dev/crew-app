...
export function eintragLesen(roh, kontext = {}) {
  ...
  return {
    ...
    // Runde 11 (C2): WARUM der Score so ist — die Teile der Rechnung (projections.js › findScoreTeile).
    scoreGruende: Array.isArray(roh.scoreGruende) ? roh.scoreGruende.filter((teil) => teil && Number.isFinite(Number(teil.wert))) : [],
    tester,
...
// Runde 10 (Jonathan): „Ideen fuer Zuhause" — privat, ohne Ausgehen. Erkannt am Stichwort
// `zuhause` (seed/find.js, und ein Admin kann es an einen eigenen Eintrag schreiben). Eine neue
// `art` waere ein zweiter Datenvertrag gewesen; ein Stichwort reicht und gilt in beiden Gateways.
export function istZuhause(eintrag) {
  return (eintrag?.tags || []).some((tag) => normText(tag) === 'zuhause');
}
...
export function besteNachArt(eintraege, art, jetzt = Date.now()) {
  ...
  return anzeigePlatzieren(aktuell(eintraege, jetzt)
    ...
    // Ideen für Zuhause haben eine eigene Reihe (zuhauseIdeen) und stehen nicht unter Erlebnissen.
    .filter((eintrag) => eintrag.art === art && !istZuhause(eintrag) && !liegtIn(eintrag, raum))
    .sort(nachScore))
    .slice(0, REIHE_MAX);
}

// Ideen für Zuhause: kein Ort auf der Karte, keine Uhrzeit — deshalb nur nach Score geordnet.
export function zuhauseIdeen(eintraege, jetzt = Date.now()) {
  return anzeigePlatzieren(aktuell(eintraege, jetzt)
    .filter((eintrag) => istZuhause(eintrag))
    .sort(nachScore))
    .slice(0, REIHE_MAX);
}
...
// Runde 11 (C2): Eine der sechs Hauptgruppen („Kultur") passt über ihre Gruppe, nicht über das Wort —
// „Kultur" steht fast nie im Titel eines Museums.
export function interessePasst(interesse, eintrag) {
  const gruppe = gruppeVonInteresse(interesse);
  if (gruppe) return hauptgruppenVon(eintrag).has(gruppe);
...
// --- Runde 11 (C2): die sechs Hauptgruppen -------------------------------------------------------
// Seit Runde 11 (A9, D6) wählt man Interessen nur noch aus sechs Hauptgruppen: Zuhause · Essen ·
// Ausgehen · Sport · Draußen · Kultur. Sie stehen als deutsche Wörter in settings.interests. Ein
// Wortabgleich griff dafür fast nie („Kultur" steht nicht im Titel eines Museums) — „Für dich" blieb
// leer. Deshalb sagt HIER EINE Regel, zu welchen Gruppen ein Eintrag gehört; der Abgleich der
// Interessen (interessePasst) und der Score (projections.js › findScoreTeile) lesen beide nur sie.
// Ein Eintrag darf zu mehreren Gruppen gehören (ein Open-Air-Konzert ist Ausgehen, Draußen, Kultur).
export const HAUPTGRUPPEN = Object.freeze(['zuhause', 'essen', 'ausgehen', 'sport', 'draussen', 'kultur']);

const GRUPPE_KULTUR = /\b(kultur\w*|museum|museen|theater\w*|kunst\w*|kunsthaus|ausstellung\w*|fuehrung\w*|konzert\w*|musik\w*|slam\w*|lesung\w*|kino\w*|film\w*|oper\w*|geschichte|historisch\w*|vernissage)\b/;

// „Draußen" → 'draussen', „Kultur" → 'kultur'; alles andere (ein älteres, eigenes Interesse) → null.
export function gruppeVonInteresse(interesse) {
  const text = normText(typeof interesse === 'string' ? interesse : interesse?.name);
  return HAUPTGRUPPEN.includes(text) ? text : null;
}

// Nimmt einen gelesenen Eintrag (mit `text`) oder einen rohen aus dem Datenvertrag (titel + tags).
export function hauptgruppenVon(eintrag) {
  const gruppen = new Set();
  if (!eintrag) return gruppen;
  const text = eintrag.text || normText([eintrag.titel, ...(eintrag.tags || [])].join(' '));
  const zuhause = istZuhause(eintrag);
  if (zuhause) gruppen.add('zuhause');
  if (eintrag.art === 'restaurant' || PASSEND_ESSEN.test(text)) gruppen.add('essen');
  if (!zuhause && (eintrag.art === 'event' || PASSEND_PARTY.test(text) || PASSEND_LEUTE.test(text))) gruppen.add('ausgehen');
  if (PASSEND_AKTIV.test(text)) gruppen.add('sport');
  if (!NIE_DRAUSSEN.test(text) && PASSEND_DRAUSSEN.test(text)) gruppen.add('draussen');
  if (GRUPPE_KULTUR.test(text)) gruppen.add('kultur');
  return gruppen;
}

// Die eigenen Interessen, die zu einem Eintrag passen — so, wie sie gespeichert sind (für den Grund
// unter dem Titel: „Kultur"). Höchstens zwei, damit die Zeile eine Zeile bleibt.
export function interessenGruende(eintrag, interessen = []) {
  return (Array.isArray(interessen) ? interessen : [])
    .map((wert) => String((typeof wert === 'string' ? wert : wert?.name) ?? '').trim())
    .filter((name) => name && interessePasst(name, eintrag))
    .slice(0, 2);
}

// --- Runde 11 (C2, D7): die Seite ------------------------------------------------------------------
// Jonathan: „Hero oben, höchstens 1,5 Bildschirme. Hero (bester Tipp) → Dieses Wochenende (Events,
// Clubs, Festivals; Restaurants nur, wenn besonders) → Beste Restaurants (weit oben) → Erlebnisse →
// Zuhause. Nicht alles als Reihe."
//   · bester: der beste Tipp nach Score — etwas zum Hingehen, also keine Idee für Zuhause (außer es
//     gibt nichts anderes). Er steht in keiner Reihe darunter noch einmal.
//   · Dieses Wochenende: Termine dieses Wochenendes. Ein Restaurant steht nur da, wenn es einen
//     Termin hat (ein besonderer Abend) — ein Lokal ohne Termin ist kein Wochenend-Ereignis.
//   · „Für dich" ist keine eigene Reihe mehr: Die Interessen wirken im Score (und damit auf den
//     besten Tipp und jede Reihenfolge), und wo eines passt, steht es als Grund unter dem Titel.
//   · „Demnächst" fällt weg (höchstens 1,5 Bildschirme). Spätere Termine findet man auf der Karte
//     und im Erlebnis „Worauf hast du Lust?".
// Jeder Eintrag steht höchstens EINMAL auf der Seite — in der ersten Stelle, die ihn nimmt.
export const ZEILEN_KURZ = 3;
export const KACHELN_KURZ = 4;

export function besterTipp(eintraege, jetzt = Date.now()) {
  const alle = aktuell(eintraege, jetzt).sort(nachScore);
  return alle.find((eintrag) => !istZuhause(eintrag)) || alle[0] || null;
}

export function findUebersicht(eintraege, { gemerktIds = [], jetzt = Date.now() } = {}) {
  const bester = besterTipp(eintraege, jetzt);
  const genommen = new Set(bester ? [bester.id] : []);
  const nimm = (liste) => {
    const rest = liste.filter((eintrag) => !genommen.has(eintrag.id));
    for (const eintrag of rest) genommen.add(eintrag.id);
    return anzeigePlatzieren(rest).slice(0, REIHE_MAX);
  };
  const abschnitte = [
    { id: 'wochenende', liste: nimm(diesesWochenende(eintraege, jetzt)) },
    { id: 'restaurants', liste: nimm(besteNachArt(eintraege, 'restaurant', jetzt)) },
    { id: 'erlebnisse', liste: nimm(besteNachArt(eintraege, 'erlebnis', jetzt)) },
    { id: 'zuhause', liste: nimm(zuhauseIdeen(eintraege, jetzt)) },
  ];
  return { bester, abschnitte, gemerkt: gemerktListe(eintraege, gemerktIds, jetzt) };
}
...
export function finderStand(eintraege, antworten = [], jetzt = Date.now(), { max = FINDER_MAX } = {}) {
  ...
  } else if (kandidaten.length > FINDER_GENUG && gefragt.size < max) {
