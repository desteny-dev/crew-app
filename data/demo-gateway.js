...
import {
  ...
  busyBereinigen, busyVollstaendig, busyFuerFreunde, busyWoche, crewAdminGrund, findListe, gemerktIdsBereinigen,
...
export class DemoDataGateway extends CrewRepository {
  ...
  migrate(state) {
    ...
    if (state.settings?.free?.active && !state.settings.free.setAt) state.settings.free.setAt = Date.now();
  ...
  personWithDerived(person) {
    ...
    const standort = person.sharesLocation && hatLage(lage)
      ? {
        lat: lage.lat,
        lon: lage.lon,
        at: Date.now() - (Number(lage.vorMin) || 0) * 60000,
        genauigkeitM: genauigkeitVon(lage, 'geteilt'),
      }
      : null;
    // Runde 8 (R8-39): Busy sehen Freunde immer — mit Titel, außer hinter einem Schloss.
    const belegt = this.busySichtbarVon(person) ? busyFuerFreunde(person.belegt) : [];
    return { ...person, standort, belegt, activeMeetId: person.showsActivity && activeMeet ? activeMeet.id : null };
  }
  ...
  standortAusPosition(position) {
    ...
    if (!lageWeitGenugAnders(alt, lage) && jetzt - (this.standortGesetztAm || 0) < 10 * 60000) return;
