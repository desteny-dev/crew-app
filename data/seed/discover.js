...
// Runde 6 (C2, Jonathan: „ich will wissen ob freunde jetzt an der gezeigten location sind oder vor x
// minuten"): `vorMin` sagt, wie alt die Lage ist. In der Demo absichtlich gemischt — frisch, ein paar
// Minuten alt, Stunden alt —, damit die Karte alle drei Zustände zeigen kann.
// Runde 7 (F9): `genauigkeitM` ist der Halbmesser in Metern, in dem die Person wirklich ist.
// Runde 7, Welle 2 (M1): Bis hierher stand hier 100 m — nicht, weil ein Gerät so schlecht misst,
// sondern weil die App selbst auf 0,001° rundete. Diese Rundung ist zurückgenommen (Begründung
// in projections.js, Abschnitt F9/M1). Jetzt stehen hier Zahlen, wie ein echtes Handy sie meldet:
// ein paar Meter im Freien, deutlich gröber in einer Bahnhofshalle oder zwischen hohen Häusern.
// Zwei Werte sind absichtlich groß — sonst sähe die Karte aus, als wäre jede Lage gleich scharf.
// Die Koordinaten tragen entsprechend fünf Nachkommastellen: auf 100 m gerundet ließe sich nicht
// sagen, in WELCHEM Lokal jemand sitzt — und genau das ist die Frage dieser App.
// Routen speichern (0047): Die Abholadressen der Demo-Leute. Sie sind bewusst BENANNTE ORTE (dieselben, an denen ihr
// Standort oben liegt), keine erfundenen Hausnummern (Hausregel 8). Sie sieht nur, wer sie als Fahrer mitnimmt — wie am Server.
export const seedAbholadressen = {
  [PEOPLE.MIRA]: { name: 'Strandbad Bregenz', city: 'Bregenz', lat: 47.50617, lon: 9.73382 },
  [PEOPLE.SAM]: { name: 'Kornmarkt', city: 'Bregenz', lat: 47.50284, lon: 9.74713 },
  [PEOPLE.TOBI]: { name: 'Marktplatz', city: 'Dornbirn', lat: 47.41372, lon: 9.74186 },
  [PEOPLE.JONAS]: { name: 'Festspielhaus', city: 'Bregenz', lat: 47.50529, lon: 9.73876 },
  [PEOPLE.KIRA]: { name: 'Bahnhof Dornbirn', city: 'Dornbirn', lat: 47.41738, lon: 9.73941 },
};

export const seedStandorte = {
  [PEOPLE.MIRA]: { lat: 47.50617, lon: 9.73382, vorMin: 1, genauigkeitM: 9 }, // am Strandbad Bregenz (aktives Meet), gerade eben
  [PEOPLE.SAM]: { lat: 47.50284, lon: 9.74713, vorMin: 12, genauigkeitM: 22 }, // Bregenz, Kornmarkt
  [PEOPLE.TOBI]: { lat: 47.41372, lon: 9.74186, vorMin: 95, genauigkeitM: 350 }, // Dornbirn, Marktplatz (enge Gasse)
  [PEOPLE.JONAS]: { lat: 47.50529, lon: 9.73876, vorMin: 3, genauigkeitM: 14 }, // Bregenz, Festspielhaus
  [PEOPLE.KIRA]: { lat: 47.41738, lon: 9.73941, vorMin: 240, genauigkeitM: 900 }, // Dornbirn, Bahnhof (Halle, grober Empfang)
};
