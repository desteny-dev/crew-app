...
// Runde 6: Die fühlbare Rückmeldung (rueckmeldung) fragt die EINE Brücke zur nativen Hülle —
// core/native.js hängt selbst von nichts ab, es entsteht also kein Ringschluss.
import { huellePlugin } from './native.js';
...
// Runde 8 (R8-65, Jonathan: „Tab-Wiedereintritt: Crew wieder ganz oben"): Wer einen Tab BETRITT,
// landet an seinem Anfang. Die gemerkten Lagen der Flaechen in `bereich` werden vergessen und auf
// null gesetzt — ein folgendes restoreScroll findet dann nichts mehr, wohin es zurueckspringen
// koennte. Innerhalb eines Tabs (Liste → Chat → zurueck) bleibt alles, wie es war: dort ruft
// niemand diese Funktion.
export function scrollVergessen(bereich) {
  if (!bereich) return;
  bereich.querySelectorAll('[data-scroll-keep]').forEach((element) => {
    scrollMemory.delete(element.dataset.scrollKeep);
...
// Runde 7: Eine fremde Bibliothek setzt an ihrem data-fremd-Knoten eigene Klassen (MapLibre:
// „maplibregl-map", die overflow:hidden und die Lage der Bedienelemente trägt). Seit die Karte
// einen Tabwechsel im Depot überlebt, gleicht der erste Abgleich danach den Knoten gegen eine
// Vorlage ab, in der diese Klassen nie standen — und warf sie weg. Regel: Eine Klasse, die
// weder in der alten noch in der neuen Vorlage steht, hat die Bibliothek gesetzt und bleibt.
function fremdeKlassen(alt, neu) {
  const teile = (text) => (text || '').split(/\s+/).filter(Boolean);
  const vorher = new Set(teile(alt.__vorlage && alt.__vorlage.getAttribute('class')));
  const soll = teile(neu.getAttribute('class'));
  const eigene = [...alt.classList].filter((k) => !vorher.has(k) && !soll.includes(k));
  return [...soll, ...eigene].join(' ');
}

// Runde 8 (R8-37, Jonathan: „das Blatt bleibt exakt am Finger"): Ein Blatt, das gerade gezogen wird,
// gehoert dem Finger — nicht dem Abgleich. Wer zieht (sheetWischen unten, das „Freund hinzufügen"-
// Blatt in profile.js), setzt data-sheet-zug an die Karte und data-sheet-schleier an den Schleier und
// schreibt Lage, Hoehe und Deckkraft als Inline-Stil. Ein Render mitten im Zug (ein Toast, eine neue
// Nachricht, der Anfrage-Hinweis, der nach zehn Sekunden schrumpft) setzte das style-Attribut auf das
// Markup zurueck. Gemessen: das QR-Blatt sprang dabei um 240 px weg — seine Zug-Hoehe fiel auf die
// Markup-Hoehe zurueck, der Versatz blieb. Diese Eigenschaften ueberleben jetzt jeden Abgleich, solange
// das Merkmal steht; abgeraeumt werden sie wie bisher vom Zieher selbst, wenn der Zug vorbei ist.
const ZUG_STIL = [
  ['data-sheet-zug', ['translate', 'height', 'top', 'transition', 'will-change']],
  ['data-sheet-schleier', ['opacity', 'transition']],
];

function zugStilRetten(alt) {
  const eintrag = ZUG_STIL.find(([merkmal]) => alt.hasAttribute(merkmal));
  if (!eintrag) return null;
  return eintrag[1]
    .map((name) => [name, alt.style.getPropertyValue(name), alt.style.getPropertyPriority(name)])
    .filter(([, wert]) => wert !== '');
}

function attributeAbgleichen(alt, neu) {
  const aktiv = document.activeElement;
  const fremd = alt.hasAttribute('data-fremd');
  const zugStil = zugStilRetten(alt);
  for (const attr of [...alt.attributes]) {
    ...
    if (fremd && attr.name === 'class') continue;
    if (!neu.hasAttribute(attr.name)) alt.removeAttribute(attr.name);
  }
  if (fremd) {
    const klassen = fremdeKlassen(alt, neu);
    if ((alt.getAttribute('class') || '') !== klassen) alt.setAttribute('class', klassen);
  }
  ...
  // R8-37: Was der Zug gesetzt hat, steht nach dem Abgleich wieder da (siehe ZUG_STIL oben).
  if (zugStil) {
    for (const [name, wert, vorrang] of zugStil) {
      if (alt.style.getPropertyValue(name) !== wert) alt.style.setProperty(name, wert, vorrang);
    }
  }
}
...
// die Rückmeldung deshalb über das Haptics-Plugin der Hülle (falls eingebaut), sonst über
...
// Genau die Methoden, die rueckmeldung() unten benutzt — mehr holt die Bruecke nicht.
...
export function rueckmeldung(art = 'tipp') {
  ...
  let weg = '';
  try {
    // Runde 6: Das Plugin kommt aus der EINEN Brücke (core/native.js), nicht aus einem eigenen
    // Zugriff auf Capacitor — sonst hat jede Datei ihre eigene Vorstellung von der Hülle.
    const haptik = huellePlugin('Haptics', HAPTIK_METHODEN);
...
//   · Runde 8 (R8-37, Jonathan: „bleibt exakt am Finger — springt heute davon und zurück"): Der
//     Weg des Blatts ist in jedem Bild der Weg des Fingers SEIT DEM AUFSETZEN. Vorher zählte er ab
//     der Stelle, an der der Zug erkannt wurde; das Blatt hing dadurch den ganzen Zug lang 6 px
//     (Chrome: gemessen 16 px) hinter dem Finger. Und schon vor der Entscheidung geht es mit
//     (vorlaeufig), nach unten, wenn der Inhalt oben steht; wird es kein Zug, legt es sich weich
//     zurück. Das „Freund hinzufügen"-Blatt hat seinen eigenen Zug (profile.js); dort kam der
//     Sprung von der 86-%-Klemme in styles.css — siehe dort.
...
// Runde 6 (Chef, zu D1): Das oberste OFFENE Sheet — unabhängig davon, wo gerade getippt wird.
// Gebraucht für die Zurück-Geste: Solange ein Sheet über der Seite liegt, gehört „zurück" dem
// Sheet, nicht der Seite. Gesucht wird vom Schleier aus (volle Fläche, trägt data-act zum
// Schließen) zur Karte dahinter — genau die Paarung, die auch sheetUnter() erkennt.
export function offenesSheet(wurzel = document.body) {
  const schleier = wurzel.querySelectorAll('[data-act]');
  for (let i = schleier.length - 1; i >= 0; i -= 1) {
    const el = schleier[i];
    if (el.firstElementChild || !vollflaechig(el, el.parentElement)) continue;
    for (let karte = el.nextElementSibling; karte; karte = karte.nextElementSibling) {
      if (!untenAngedockt(karte)) continue;
      if (karte.matches(SHEET_KARTE) || sheetForm(karte)) return { karte, schleier: el };
    }
  }
  return null;
}
...
export function sheetWischen(wurzel) {
  ...
  const beginn = (x, y, ziel, quelle, id) => {
    ...
    zug = {
      ...
      scroller: scrollflaecheIn(ziel, treffer.karte), hoehe: treffer.karte.offsetHeight || 1, spur: [], bild: 0, vorlaeufig: false,
    };
  };

  // R8-37: Solange nicht entschieden ist, ob gezogen, getippt oder quer gewischt wird, geht das
  // Blatt trotzdem schon mit dem Finger — nach unten, und nur, wenn der Inhalt oben steht. Ein
  // Blatt, das gerade noch zurueckfedert, bleibt dabei in Ruhe (seine Bewegung gehoert der Feder).
  const vorlaeufig = (weg) => {
    if (!zug || zug.karte.hasAttribute('data-sheet-zug')) return;
    if (weg > 0) {
      // Ohne Uebergang: ein Rest vom letzten Zuruecklegen darf den Finger nicht nachziehen lassen.
      if (!zug.vorlaeufig) zug.karte.style.transition = 'none';
      zug.weg = weg;
      zug.vorlaeufig = true;
      if (!zug.bild) zug.bild = requestAnimationFrame(malen);
      return;
    }
    if (zug.vorlaeufig) zuruecklegen(zug);
  };

  // Was vorlaeufig mitging (hoechstens ZUG_START_PX), legt sich mit der Kurve des Oeffnens zurueck
  // an seinen Platz — kein Sprung, auch nicht um diese paar Pixel.
  const zuruecklegen = (z) => {
    z.vorlaeufig = false;
    z.weg = 0;
    if (z.bild) { cancelAnimationFrame(z.bild); z.bild = 0; }
    const { karte, schleier } = z;
    if (!karte.isConnected) return;
    karte.style.transition = `translate 160ms ${SHEET_KURVE}`;
    karte.style.translate = '0px 0px';
    schleier.style.removeProperty('opacity');
    window.setTimeout(() => {
      if ((zug && zug.karte === karte) || karte.hasAttribute('data-sheet-zug')) return;
  ...
  const bewegung = (x, y, zeit) => {
    ...
    if (zug.modus === null) {
      const dx = x - zug.x0;
      const dy = y - zug.y0;
      const obenAn = !zug.scroller || zug.scroller.scrollTop <= 0;
      if (Math.max(Math.abs(dx), Math.abs(dy)) < ZUG_START_PX) {
        // Noch unentschieden — das Blatt geht trotzdem schon mit (R8-37, vorlaeufig oben). Das
        // native Rollen bleibt dabei unberuehrt: preventDefault gibt es erst nach der Entscheidung.
        vorlaeufig(obenAn && dy > 0 && dy >= Math.abs(dx) ? dy : 0);
        return false;
      }
      if (Math.abs(dx) > Math.abs(dy) || dy < 0) { zug.modus = 'weg'; vorlaeufig(0); return false; }
      if (!obenAn) {
        vorlaeufig(0);
        ...
        // R8-37 (Jonathan: „springt davon und zurück"): Gemessen wird ab dem AUFSETZEN des Fingers,
        // nicht ab der Stelle, an der der Zug erkannt wurde. Vorher war genau diese Stelle die
        // Basis — das Blatt hing dann den ganzen Zug lang um den Erkennungsweg hinter dem Finger:
        // 6 px, und auf Geraeten, die die ersten Pixel gar nicht melden (Chrome haelt Bewegungen
        // innerhalb seiner Toleranz zurueck), gemessen 16 px. Jetzt ist der Weg des Blatts in
        // jedem Bild der Weg des Fingers.
        uebernehmen(zug.y0);
      }
    }
    if (zug.modus === 'warten') {
      if (zug.scroller.scrollTop > 0) return false;
      // Hier hat der INHALT den Finger bis an seinen Anfang gefuehrt; erst jetzt faengt das Blatt
      // an. Die kurze Strecke ohne Bewegung bleibt Absicht: ein schneller Wisch an den Anfang
      // der Liste soll das Blatt nicht mitreissen. Einen Sprung gibt es dabei nicht — der Weg
      // beginnt bei null und folgt dem Finger dann Pixel fuer Pixel.
      uebernehmen(y + ZUG_UEBERGABE_PX);
      zug.uebergabe = true;
    }
    let weg = y - zug.basis;
    // Runde 9 (Jonathan): Ein Blatt geht nicht weiter hoch, als sein Inhalt reicht — auch nicht ein
    // paar Pixel zum Federn (darunter sah man sonst den Schleier). Nach oben bleibt es stehen.
    if (weg < 0) weg = 0;
    zug.weg = weg;
    zug.vorlaeufig = false;
  ...
  const loslassen = () => {
    ...
    zug = null;
    if (z.bild) { cancelAnimationFrame(z.bild); z.bild = 0; }
    // R8-37: Ein Tipp (oder ein Zug, der nie einer fuers Blatt wurde) — was vorlaeufig mitging,
    // legt sich zurueck. Der Klick selbst laeuft normal weiter.
    if (z.modus !== 'zug') { if (z.vorlaeufig) zuruecklegen(z); return; }
  ...
  return {
    ...
    nachRender() {
      if (!zug || (zug.modus !== 'zug' && !zug.vorlaeufig)) return;
