...
// --- Runde 11 (C4, D8): der Ort aus der Zeit „App zu" erreicht den Server --------------------------
//
// Bis hierher legte die Wache (CrewStandort) einen Ort, den das Telefon bei geschlossener App meldete,
// nur im Telefon ab — die Freunde sahen ihn erst, wenn jemand Crew wieder öffnete. Jetzt schickt die
// Hülle ihn selbst an den Server (iOS CrewStandort.swift › anServer, Android StandortServer.java), ohne
// JavaScript und ohne Anmeldung: mit einem eigenen, engen Telefon-Schlüssel (Migration 0044). Diese
// drei Zugänge geben ihn der Hülle bzw. nehmen ihn wieder weg. Im Browser (und in einer älteren Fassung
// der App ohne diese Methoden) antworten sie ehrlich mit „kann nicht".

/** { kann: boolean, nutzer: string } — kann die Hülle Orte selbst schicken, und für wen tut sie es gerade? */
export async function telefonServerLage() {
  const wache = faehigkeitPlugin('standort-hintergrund');
  if (typeof wache?.serverSetzen !== 'function') return { kann: false, nutzer: '' };
  try {
    const antwort = await wache.serverZustand?.();
    return { kann: true, nutzer: String(antwort?.nutzer || '') };
  } catch {
    return { kann: true, nutzer: '' };
  }
}

/** Gibt der Hülle Adresse, öffentlichen Schlüssel des Projekts und den Telefon-Schlüssel. → true = angenommen. */
export async function telefonServerSetzen({ adresse, oeffentlich, geraet, nutzer }) {
  const wache = faehigkeitPlugin('standort-hintergrund');
  if (typeof wache?.serverSetzen !== 'function') return false;
  try {
    const antwort = await wache.serverSetzen({ adresse: adresse || '', oeffentlich: oeffentlich || '', geraet: geraet || '', nutzer: nutzer || '' });
    return Boolean(geraet) && String(antwort?.nutzer || '') === String(nutzer || '');
  } catch {
    return false;
  }
}

/** Die Hülle schickt ab jetzt nichts mehr (Standort aus, anderes Konto). */
export function telefonServerVergessen() {
  return telefonServerSetzen({});
}
...
async function webErlaubnisStand(art, fragen = false) {
  ...
  let stand = 'prompt';
  if (typeof erlaubnis?.query === 'function') {
    try {
      const antwort = await erlaubnis.query({ name: 'geolocation' });
      stand = antwort?.state === 'granted' ? 'granted' : antwort?.state === 'denied' ? 'denied' : 'prompt';
    } catch {
      stand = 'prompt';   // Firefox wirft für unbekannte Namen — das ist kein Nein
    }
  }
  if (!fragen || stand !== 'prompt') return stand;
  // Runde 9: Im Browser gibt es für den Standort kein „requestPermission" — die Systemfrage kommt
  // mit der ersten Messung. Anders als bei Mitteilungen braucht sie keine Berührung.
  return new Promise((fertig) => {
    try {
      globalThis.navigator.geolocation.getCurrentPosition(
        () => fertig('granted'),
        (fehler) => fertig(fehler?.code === 1 ? 'denied' : 'prompt'),
