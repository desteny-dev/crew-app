...
//
// Runde 8 (R8-26, Jonathan: „‚wie wäre 16 Uhr' o. ä. → Vorschlag mit einem Tipp ins Meet übernehmen
// (auf dem bestehenden Ergebnis aufbauen, nicht am Algorithmus)"): Uhrzeit und Tag erkennt diese
// Datei schon (uhrzeitIn, tagIn). Dazu kommen nur zwei kleine Nachschlagewerke, keine neue Erkennung:
//   ortIn               ein Ort, den die App schon kennt (Vorschläge, Ortsbestand), steht im Satz
//   klingtNachVorschlag der Satz schlägt etwas vor („wie wäre …", „lieber …", eine Frage) — nur damit
//                       ein bloßes „heute" in „ich war heute beim Arzt" keinen Tag vorschlägt.
...
// --- Runde 8 (R8-26): Orte und Vorschlagston ------------------------------------------------
// Kein Ortsverständnis: nur der Abgleich mit Orten, die die App schon kennt. Ein Name zählt, wenn
// er als Ganzes im Satz steht (ohne Akzente, ohne Groß/Klein) — oder sein erstes Wort, wenn es
// mindestens fünf Buchstaben hat, kein Allerweltswort ist und genau EINEM bekannten Ort gehört
// („Molo" allein ist zu kurz, „Strandbad" trifft, solange es nur ein Strandbad gibt).
const ALLERWELTSWORT = new Set([
  'cafe', 'kaffee', 'restaurant', 'gasthaus', 'gasthof', 'hotel', 'pizzeria', 'bistro', 'kino', 'museum',
  'park', 'stadtpark', 'badesee', 'strand', 'bahnhof', 'schule', 'kirche', 'markt', 'platz', 'halle',
  'the', 'beach', 'lake', 'park', 'plage', 'musee', 'parque', 'playa', 'museo',
]);
const alsMuster = (text) => text.replace(/[.*+?^${}()|[\]\\]/g, '\\$&').replace(/\s+/g, '\\s+');

/** Ein bekannter Ort im Satz? → der Ort aus `orte` ({ name, … }) oder null. */
export function ortIn(text, orte = []) {
  const satz = ohneAkzent(norm(text));
  if (!satz.trim()) return null;
  const bekannte = (Array.isArray(orte) ? orte : [])
    .filter((ort) => ort && typeof ort.name === 'string' && ohneAkzent(ort.name).trim().length >= 4);
  const steht = (name) => wortMuster(alsMuster(ohneAkzent(name).trim())).test(satz);
  // Längere Namen zuerst: „Strandbad Lochau" vor „Lochau".
  const ganz = [...bekannte].sort((a, b) => b.name.length - a.name.length).find((ort) => steht(ort.name));
  if (ganz) return ganz;
  const nachErstemWort = new Map();
  for (const ort of bekannte) {
    const wort = ohneAkzent(ort.name).split(/[^\p{L}\p{N}]+/u).find(Boolean) || '';
    if (wort.length < 5 || ALLERWELTSWORT.has(wort) || wort === ohneAkzent(ort.name).trim()) continue;
    nachErstemWort.set(wort, [...(nachErstemWort.get(wort) || []), ort]);
  }
  for (const [wort, liste] of nachErstemWort) {
    if (liste.length === 1 && steht(wort)) return liste[0];
  }
  return null;
}

const VORSCHLAG_MUSTER = wortMuster([
  "wie w(?:ä|ae|a)r(?:e|s|'s)?", 'lieber', 'oder doch', 'besser', 'stattdessen', 'statt',
  'geht (?:auch|es|das|dir|euch)', 'passt (?:dir|euch|es|das)', 'können wir', 'koennen wir', 'sollen wir',
  'what about', 'how about', 'instead', 'rather', 'works for',
  'et si', 'plutôt', 'plutot', 'ça vous va', 'ça te va', 'ca vous va', 'ca te va',
  'qué tal', 'que tal', 'mejor', 'en vez de', 'os va bien', 'te va bien',
].join('|'));

/** Schlägt der Satz etwas vor? Eine Frage oder ein Vorschlagswort („wie wäre", „lieber", „how about"). */
export function klingtNachVorschlag(text) {
  const satz = norm(text);
  return /[?¿]/.test(satz) || VORSCHLAG_MUSTER.test(satz);
}
...
// Runde 11 (A15, Jonathan: „funktioniert noch nicht gut, später zusammen mit dem Algorithmus"):
// EIN Schalter für alles Automatische aus dieser Datei — die Erkennung im Chat (room.js) und die
// Vorschläge beim Erstellen eines Meets (new-meet.js). Er steht in web/config.js
// (`CREW_CONFIG.vorschlaege`) und ist AUS, solange dort nicht ausdrücklich `true` steht. Der Code
// bleibt, damit er später wieder angeschaltet werden kann; Find und „Für dich" hängen nicht daran.
export function vorschlaegeAn() {
  return globalThis.CREW_CONFIG?.vorschlaege === true;
}
