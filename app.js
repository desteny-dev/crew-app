...
import { registriereServiceWorker, mitteilungenBeimStart, standortBeimStart } from './core/pwa.js';
...
// Runde 11 (C1, D3): WEM ein waagrechter Zug gehört, steht einmal in ui/gesten.js.
import { randSeite, eigeneWaagrechteGeste } from './ui/gesten.js';

// Runde 10: Einmal je Konto — Mitteilungen und „Standort verwenden" auf an. `standardAn` ist die
// Markierung, dass das geschehen ist; sie steht in den Einstellungen und reist mit dem Konto.
const STANDARD_AN = 1;
function standardEinstellungen(repo) {
  const stand = repo.getSettings?.();
  if (!stand || typeof stand !== 'object') return false;
  if (Number(stand.standardAn) >= STANDARD_AN) return false;
  // Geschrieben wird nur, was sich WIRKLICH ändert: Steht ohnehin alles auf an, ist die Markierung
  // das Einzige, was dazukommt. (Ein Schreiben auf denselben Wert wäre nicht folgenlos — es würde
  // etwa die Standort-Wache neu starten und das Gerät ohne Anlass nach seiner Lage fragen.)
  const patch = { standardAn: STANDARD_AN };
  const m = stand.notifications || {};
  const aus = ['invitations', 'mentions', 'chat', 'updates', 'reminder'].filter((name) => m[name] !== true);
  if (aus.length) patch.notifications = { ...m, ...Object.fromEntries(aus.map((name) => [name, true])) };
  if (stand.location?.use !== true) patch.location = { ...(stand.location || {}), use: true };
...
// T5: Eine echte Karte gehört MapLibre, eine rollbare Reihe sich selbst — wer dort zieht, meint
// sie. Runde 11 (C1): Die Regel steht jetzt in ui/gesten.js, weil auch die Seiten sie lesen.
function ownsHorizontalGesture(target, root) {
  return eigeneWaagrechteGeste(target, root);
}
...
// Der Rand ist WISCH_RAND (ui/gesten.js) — derselbe wie für die Tab-Bahn.
const ZURUECK_ANTEIL = 0.3;
let zurueckZug = null;

// Runde 11 (C1, Jonathan D3): „Zurück-Wischen zieht die Ebene weg und zeigt die vorherige Seite
// darunter (nie Schwarz)." Vorher schob der Zug die Seite beiseite, und darunter lag der leere
// Grund des #app. Jetzt merkt sich jeder Schritt NACH VORN ein Standbild der Seite, die er verlässt
// (dieselbe Bauart wie beim Gleiten, abbildMitScroll) — eines je Ebene des Stapels. Beim Zurück-
// Wischen liegt es unter der wegziehenden Seite. Kommt der Zug an, zeichnet die echte Seite an genau
// dieser Stelle — ohne zweite Bewegung (zurueckGewischt schaltet den Eintritt ab).
const unterlagen = [];
let zurueckGewischt = false;

function unterlageMerken(alteWurzel) {
  const abbild = abbildMitScroll(alteWurzel);
  // Nur die sichtbare Seite: Nachbarn in der Bahn, das Karten-Depot und ein gerade stehender
  // Hinweis (Toast) gehören nicht ins Bild — sie wären beim Zurückwischen längst weg.
  abbild.querySelectorAll('.tab-depot, .tab-page:not([data-role="active"]), .gleit-ebene, #crew-toast').forEach((knoten) => knoten.remove());
  unterlagen[nav.depth() - 2] = abbild;
}

function unterlageEinsetzen(seite) {
  const abbild = unterlagen[nav.depth() - 2];
  if (!abbild || !seite) return null;
  ...
  abbild.dataset.gleitEbene = 'unten';
  abbild.dataset.role = 'zurueck-unterlage';
  seite.after(abbild);
  // Ein frisch eingehängter Knoten steht oben — die gemerkten Rollstände kommen erst jetzt zurück.
...
function bindeZurueckWisch(wurzel) {
  ...
  // Runde 11 (C1): Die wegziehende Seite liegt ÜBER dem Standbild der vorherigen und wirft an
  // ihrer linken Kante einen Schatten — wie ein Blatt, das man beiseiteschiebt.
  const ebeneLoesen = (s, unterlage) => {
    if (s) { s.style.zIndex = ''; s.style.boxShadow = ''; }
    unterlage?.remove();
  };
  const ruhe = (sanft) => {
    const s = seite();
    const unterlage = zurueckZug?.unterlage || null;
    if (s) {
      s.style.transition = sanft ? 'translate .22s cubic-bezier(.22,.61,.36,1)' : '';
      s.style.translate = '';
      if (sanft) setTimeout(() => { s.style.transition = ''; s.style.willChange = ''; ebeneLoesen(s, unterlage); }, 240);
      else { s.style.willChange = ''; ebeneLoesen(s, unterlage); }
    }
    zurueckZug = null;
    hoerAuf();
  };
  ...
  wurzel.addEventListener('touchstart', (ereignis) => {
    ...
    if (randSeite(t.clientX, wurzel) !== 'links' || !zurueckMoeglich()) { hoerAuf(); return; }
    ...
    if (!zurueckZug.aktiv) {
      ...
      if (!zurueckZug.sheet) {
        ...
        if (s) {
          ...
          s.style.willChange = 'translate';
          // Runde 11 (C1): Darunter liegt die Seite, zu der es zurückgeht — nie der leere Grund.
          // (Eine große Bildansicht ist keine Ebene des Stapels; dort bleibt es beim Schieben.)
          if (!globalThis.history?.state?.crewBildGross) {
            zurueckZug.unterlage = unterlageEinsetzen(s);
            if (zurueckZug.unterlage) { s.style.zIndex = '2'; s.style.boxShadow = '-14px 0 32px var(--shadow-16)'; }
          }
        }
      }
    }
  ...
  const schluss = () => {
    ...
    setTimeout(() => {
      ...
      // Die Seite ist schon weggezogen — der Render danach zeichnet die vorherige ohne Eintritt,
      // genau dort, wo ihr Standbild liegt; erst dann geht das Standbild (im selben Bild).
      zurueckGewischt = Boolean(unterlage);
      zurueckGehen();
      zurueckGewischt = false;
      ebeneLoesen(s, unterlage);
    }, 150);
  };
...
function bindTabTrack(root, startTab) {
  ...
  function begin(x, y, target) {
    ...
    // Runde 11 (C1, Jonathan D3): Den Tab wechselt nur ein Zug vom äußeren Rand. Die Mitte gehört
    // der Seite (Zeilen, Liste↔Kalender, Karte) — dort beginnt hier nichts (ui/gesten.js).
    if (!randSeite(x, track)) { mode = 'reject'; start = null; return false; }
  ...
  track.addEventListener('wheel', (event) => {
    ...
    // Wie beim Finger: Tab-Wechsel nur, wenn die Geste am äußeren Rand beginnt (ui/gesten.js).
    // Läuft schon eine Geste (wheelSum != 0), gehört sie weiter der Bahn.
    // Ein waagrechter Wisch in der Mitte wechselt nichts — und soll auch nicht den Browser zurückblättern.
    if (wheelSum === 0 && !randSeite(event.clientX, track)) { event.preventDefault(); return; }
    event.preventDefault();
...
function render() {
  ...
  // Runde 11 (C1): Wie gewechselt wurde, bevor die Bewegung darüber entscheidet — für die Standbilder
  // des Zurück-Wischens. Kam das Zurück aus dem Wisch, ist die Seite schon weg: kein zweiter Eintritt.
  const urWechsel = move;
  if (zurueckGewischt && move === 'back') move = 'none';
  ...
  // Runde 11 (C1): Jeder Schritt nach vorn merkt sich die Seite, die er verlässt; was über dem
  // Stapel liegt, verfällt (zurück, Tabwechsel, Neubeginn).
  if (urWechsel === 'push' && alteWurzel && alteRoute) unterlageMerken(alteWurzel);
  unterlagen.length = Math.max(0, nav.depth() - 1);
...
async function starteApp() {
  ...
  // Runde 10 (Jonathan): „Mitteilungen und Standort verwenden stehen standardmäßig auf an — für
  // ALLE Konten, neue wie bestehende." Neue Konten beginnen so (seed/profile.js, beide Gateways);
  // bestehende bekommen es EINMAL beim nächsten Start. Gemerkt wird das in den Einstellungen
  // selbst (`standardAn`), nicht im Gerät: Wer danach etwas umstellt, behält seine Wahl — auch auf
  // dem zweiten Gerät. Verbietet das System etwas, zeigt die App gleich darauf ehrlich „aus"
  // (standortBeimStart, direkt unten — Hausregel 9).
  standardEinstellungen(repo);
  // Runde 9: „Standort verwenden" folgt der Entscheidung des Systems (core/pwa.js › standortBeimStart).
  standortBeimStart({
    istAn: () => Boolean(repo.getSettings()?.location?.use),
    an: () => { repo.updateSettings({ location: { ...(repo.getSettings().location || {}), use: true } }); repo.standortSenden?.({ sofort: true }); },
    aus: () => repo.updateSettings({ location: { ...(repo.getSettings().location || {}), use: false } }),
  }).catch(() => {});
  // Runde 9: Und wer Mitteilungen noch nie entschieden hat (altes wie neues Konto), wird EINMAL
  // gefragt — core/pwa.js › mitteilungenBeimStart; bei „erteilt" bleibt es beim Abgleich. Laufen
  // sie nicht, steht bei jedem Start ein ruhiger, wegtippbarer Hinweis.
  if (gestartet.client) {
    Promise.all([import('./data/push.js'), import('./ui/mitteilungen.js'), import('./ui/guide.js')]).then(([push, mitteilungen, guide]) => {
      const pushCtx = { repo, render: () => render(), toast: showToast };
      return mitteilungenBeimStart({
        abgleichen: () => push.pushAbgleichen(gestartet.client),
        fragbar: () => mitteilungen.pushLage(repo) === 'bereit',
        konto: repo.getMe?.()?.id || '',
        anschalten: () => mitteilungen.pushEinschalten(pushCtx).then((ergebnis) => {
          guide.erlaubnisMerken?.('mitteilungen', ergebnis === 'an' ? 'erteilt' : 'abgelehnt');
          return ergebnis;
        }),
      });
    }).catch(() => {});
